...
def validate_overlap_for(doc, doctype, fieldname, value=None):
	...
	if existing:
		dataent.throw(_("This {0} conflicts with {1} for {2} {3}").format(doc.doctype, existing.name,
...
def get_overlap_for(doc, doctype, fieldname, value=None):
	...
	existing = dataent.db.sql("""select name, from_time, to_time from `tab{0}`
...
def validate_duplicate_student(students):
	...
	for stud in students:
		if stud.student in unique_students:
			dataent.throw(_("Student {0} - {1} appears Multiple times in row {2} & {3}")
