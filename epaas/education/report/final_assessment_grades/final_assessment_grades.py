...
from epaas.education.report.course_wise_assessment_report.course_wise_assessment_report import get_chart_data
...
def execute(filters=None):
	...
	args.students = dataent.db.sql_list("select student from `tabStudent Group Student` where parent=%s", (student_group))
	...
	for student in args.students:
		if student_details.get(student):
			...
			for course in course_dict:
				scrub_course = dataent.scrub(course)
...
def get_column(course_dict):
	...
	for course in course_dict:
		columns.append({
			"fieldname": "grade_" + dataent.scrub(course),
			"label": course,
			"fieldtype": "Data",
			"width": 110
		})
		columns.append({
			"fieldname": "score_" + dataent.scrub(course),
