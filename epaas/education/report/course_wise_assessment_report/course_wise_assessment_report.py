...
def execute(filters=None):
	...
	for student in result_dict:
		...
		for criteria in assessment_criteria_dict:
			scrub_criteria = dataent.scrub(criteria)
...
def get_formatted_result(args, get_assessment_criteria=False, get_course=False, get_all_assessment_groups=False):
	...
	assessment_result = dataent.db.sql('''
		SELECT
	...
	def add_total_score(result, assessment_group):
		if "Total Score" not in formatted_assessment_result[result.student][result.course][assessment_group]:
			formatted_assessment_result[result.student][result.course][assessment_group]["Total Score"] = dataent._dict({
	...
	for result in assessment_result:
		...
		assessment_criteria_details = dataent._dict({"assessment_criteria": result.assessment_criteria,
...
def get_column(assessment_criteria):
	...
	for d in assessment_criteria:
		...
		columns.append({
			"fieldname": dataent.scrub(d) +"_score",
...
def get_chart_data(grades, criteria_list, kounter):
	...
	for grade in grades:
		tmp = dataent._dict({"values":[], "title": grade})
