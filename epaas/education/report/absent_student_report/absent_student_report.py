...
def execute(filters=None):
	...
	for student in absent_students:
		if not student.student in leave_applicants:
			...
			stud_details = dataent.db.get_value("Student", student.student, ['student_email_id', 'student_mobile_number'], as_dict=True)
...
def get_absent_students(date):
	absent_students = dataent.db.sql("""select student, student_name, student_group from `tabStudent Attendance` 
		where status="Absent" and date = %s order by student_group, student_name""", date, as_dict=1)
	return absent_students

def get_leave_applications(date):
	leave_applicants = []
	for student in dataent.db.sql("""select student from `tabStudent Leave Application` 
...
def get_transportation_details(date, student_list):
	academic_year = dataent.get_all("Academic Year", filters=[["year_start_date", "<=", date],["year_end_date", ">=", date]])
	if academic_year:
		academic_year = academic_year[0].name
	elif dataent.defaults.get_defaults().academic_year:
		academic_year = dataent.defaults.get_defaults().academic_year
	else:
		return {}

	transportation_details = dataent.get_all("Program Enrollment", fields=["student", "mode_of_transportation", "vehicle_no"],
