...
class StudentApplicant(Document):
	def autoname(self):
		...
		if self.student_admission:
			...
			if self.program:
				...
				dataent.throw(_("Select the program first"))
	...
	def on_update_after_submit(self):
		student = dataent.get_list("Student",  filters= {"student_applicant": self.name})
		if student:
			dataent.throw(_("Cannot change status as student {0} is linked with student application {1}").format(student[0].name, self.name))

	def on_submit(self):
		if self.paid and not self.student_admission:
			dataent.throw(_("Please select Student Admission which is mandatory for the paid student applicant"))
	...
	def validation_from_student_admission(self):
		...
		if student_admission and student_admission.minimum_age and \
			getdate(student_admission.minimum_age) < getdate(self.date_of_birth):
				dataent.throw(_("Not eligible for the admission in this program as per DOB"))
...
def get_student_admission_data(student_admission, program):
	student_admission = dataent.db.sql("""select sa.admission_start_date, sa.admission_end_date,
