...
QUnit.test('test student applicant', function(assert){
	...
	dataent.run_serially([
		...
		() => dataent.timeout(0.5),
		() => dataent.tests.click_button('Submit'),
		() => dataent.tests.click_button('Yes'), // Submitting again after amend
		() => {
			testing_status = $('span.indicator.orange').text();
		...
		() => dataent.tests.click_button('Update'),
		() => {
			testing_status = $('h1.editable-title').text();
		...
		() => dataent.timeout(0.5),
		() => dataent.tests.click_button('Enroll'),
		() => dataent.timeout(0.5),
		() => {
			assert.ok(dataent.get_route()[0] == 'Form'); // Checking if the current page is Program Enrollment page or not
			assert.ok(dataent.get_route()[1] == 'Program Enrollment');
		},

		// Routing to Student List to check if the Applicant's entry has been made or not
		() => dataent.timeout(0.5),
		() => dataent.set_route('List', 'Student'),
		() => dataent.timeout(0.5),
		() => {$(`a:contains("Fname Mname Lname"):visible`)[0].click();},
		() => dataent.timeout(0.5),
