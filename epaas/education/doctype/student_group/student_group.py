...
from epaas.education.utils import validate_duplicate_student
...
class StudentGroup(Document):
	...
	def validate_mandatory_fields(self):
		if self.group_based_on == "Course" and not self.course:
			dataent.throw(_("Please select Course"))
		if self.group_based_on == "Course" and (not self.program and self.batch):
			dataent.throw(_("Please select Program"))
		if self.group_based_on == "Batch" and not self.program:
			dataent.throw(_("Please select Program"))

	def validate_strength(self):
		if self.max_strength and len(self.students) > self.max_strength:
			dataent.throw(_("""Cannot enroll more than {0} students for this student group.""").format(self.max_strength))
	...
	def validate_and_set_child_table_fields(self):
		...
		for d in self.students:
			if not d.student_name:
				d.student_name = dataent.db.get_value("Student", d.student, "title")
			if not d.group_roll_number:
				max_roll_no += 1
				d.group_roll_number = max_roll_no
			if d.group_roll_number in roll_no_list:
				dataent.throw(_("Duplicate roll number for student {0}".format(d.student_name)))
			else:
...
def get_students(academic_year, group_based_on, academic_term=None, program=None, batch=None, course=None):
	...
	if enrolled_students:
		...
		for s in enrolled_students:
			if dataent.db.get_value("Student", s.student, "enabled"):
				s.update({"active": 1})
			else:
				s.update({"active": 0})
		...
		dataent.msgprint(_("No students found"))
		return []
...
def fetch_students(doctype, txt, searchfield, start, page_len, filters):
	if filters.get("group_based_on") != "Activity":
		...
		student_group_student = dataent.db.sql_list('''select student from `tabStudent Group Student` where parent=%s''',
		...
		return dataent.db.sql("""select name, title from tabStudent
