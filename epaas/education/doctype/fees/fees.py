...
class Fees(AccountsController):
	...
	def set_missing_accounts_and_fields(self):
		...
		if not (self.receivable_account and self.income_account and self.cost_center):
			accounts_details = dataent.get_all("Company",
	...
	def get_student_emails(self):
		student_emails = dataent.db.sql_list("""
		...
		student_email_id = dataent.db.get_value("Student", self.student, "student_email_id")
		if student_email_id:
	...
	def on_submit(self):
		...
		if self.send_payment_request and self.student_email:
			...
			dataent.msgprint(_("Payment request {0} created").format(getlink("Payment Request", pr.name)))
...
def get_fee_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	student = dataent.db.sql("select name from `tabStudent` where student_email_id= %s", user)
	if student:
