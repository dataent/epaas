...
dataent.ui.form.on('Fee Schedule', {
	setup: function(frm) {
	...
	onload: function(frm) {
		...
		dataent.realtime.on("fee_schedule_progress", function(data) {
	...
	fee_structure: function(frm) {
		if (frm.doc.fee_structure) {
			dataent.call({
				method: "epaas.education.doctype.fee_schedule.fee_schedule.get_fee_structure",
				args: {
...
dataent.ui.form.on("Fee Schedule Student Group", {
	student_group: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.student_group && frm.doc.academic_year) {
			dataent.call({
				method: "epaas.education.doctype.fee_schedule.fee_schedule.get_total_students",
				args: {
				...
				callback: function(r) {
					if(!r.exc) {
						dataent.model.set_value(cdt, cdn, "total_students", r.message);
					}
				}
			});
		}
