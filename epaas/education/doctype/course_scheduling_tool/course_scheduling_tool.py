...
from dataent.utils import add_days, getdate
...
class CourseSchedulingTool(Document):
	...
	def schedule_course(self):
		...
		self.validate_date()
		self.instructor_name = dataent.db.get_value(
			"Instructor", self.instructor, "instructor_name")

		group_based_on, course = dataent.db.get_value(
	...
	def validate_mandatory(self):
		...
		for d in fields:
			if not self.get(d):
				dataent.throw(_("{0} is mandatory").format(
	...
	def delete_course_schedule(self, rescheduled, reschedule_errors):
		...
		schedules = dataent.get_list("Course Schedule",
		...
		for d in schedules:
			try:
				if self.day == calendar.day_name[getdate(d.schedule_date).weekday()]:
					dataent.delete_doc("Course Schedule", d.name)
