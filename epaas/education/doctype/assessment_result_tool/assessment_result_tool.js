...
dataent.ui.form.on('Assessment Result Tool', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (dataent.route_options) {
			...
			frm.set_value("assessment_plan", dataent.route_options.assessment_plan);
	...
	assessment_plan: function(frm) {
		...
		if(frm.doc.assessment_plan) {
			if (!frm.doc.student_group)
				return
			dataent.call({
				method: "epaas.education.api.get_assessment_students",
				args: {
	...
	get_marks: function(frm, criteria_list) {
		...
		var result_table = $(dataent.render_template('assessment_result_tool', {
			frm: frm,
		...
		result_table.on('change', 'input', function(e) {
			...
			if (Object.keys(student_scores["assessment_details"]).length === criteria_list.length) {
				...
				dataent.call({
					method: "epaas.education.api.mark_assessment_result",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
	...
	submit_result: function(frm) {
		if (frm.doc.show_submit) {
			frm.page.set_primary_action(__("Submit"), function() {
				dataent.call({
					method: "epaas.education.api.submit_assessment_results",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
						"student_group": frm.doc.student_group
					},
					callback: function(r) {
						if (r.message) {
							dataent.msgprint(__("{0} Result submittted", [r.message]));
						} else {
							dataent.msgprint(__("No Result to submit"));
						}
