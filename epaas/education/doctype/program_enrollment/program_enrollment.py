...
class ProgramEnrollment(Document):
	def validate(self):
		...
		if not self.student_name:
			self.student_name = dataent.db.get_value("Student", self.student, "title")
		if not self.courses:
	...
	def validate_duplication(self):
		enrollment = dataent.get_all("Program Enrollment", filters={
		...
		if enrollment:
			dataent.throw(_("Student is already enrolled."))
	
	def update_student_joining_date(self):
		date = dataent.db.sql("select min(enrollment_date) from `tabProgram Enrollment` where student= %s", self.student)
		dataent.db.set_value("Student", self.student, "joining_date", date)
	...
	def get_courses(self):
		return dataent.db.sql('''select course, course_name from `tabProgram Course` where parent = %s and required = 1''', (self.program), as_dict=1)
