...
from epaas.education.report.course_wise_assessment_report.course_wise_assessment_report import get_formatted_result
from epaas.education.report.course_wise_assessment_report.course_wise_assessment_report import get_child_assessment_groups
...
def preview_report_card(doc):
	...
	if not (doc.student_name and doc.student_batch):
		program_enrollment = dataent.get_all("Program Enrollment", fields=["student_batch_name", "student_name"],
	...
	template = "epaas/education/doctype/student_report_generation_tool/student_report_generation_tool.html"
	base_template_path = "dataent/www/printview.html"

	from dataent.www.printview import get_letter_head
	letterhead = get_letter_head(dataent._dict({"letter_head": doc.letterhead}), not doc.add_letterhead)

	html = dataent.render_template(template,
		{
			"doc": doc,
	...
	final_template = dataent.render_template(base_template_path, {"body": html, "title": "Report Card"})

	dataent.response.filename = "Report Card " + doc.students[0] + ".pdf"
	dataent.response.filecontent = get_pdf(final_template)
	dataent.response.type = "download"


def get_courses_criteria(courses):
	course_criteria = dataent._dict()
	for course in courses:
		course_criteria[course] = [d.assessment_criteria for d in dataent.get_all("Course Assessment Criteria",
...
def get_attendance_count(student, academic_year, academic_term=None):
	if academic_year:
		from_date, to_date = dataent.db.get_value("Academic Year", academic_year, ["year_start_date", "year_end_date"])
	elif academic_term:
		from_date, to_date = dataent.db.get_value("Academic Term", academic_term, ["term_start_date", "term_end_date"])
	if from_date and to_date:
		attendance = dict(dataent.db.sql('''select status, count(student) as no_of_days
		...
		dataent.throw(_("Provide the academic year and set the starting and ending date."))