...
QUnit.test('Test: Student Group Creation Tool', function(assert){
	...
	dataent.run_serially([
		...
		() => dataent.set_route('Form', 'Student Group Creation Tool'),
		() => dataent.timeout(0.5),
		() => {
			...
			dataent.tests.click_button('Get Courses');
		},
		() => dataent.timeout(1),
		() => {
		...
		() => dataent.timeout(0.5),
		() => dataent.tests.click_button("Create Student Groups"),
		() => dataent.timeout(0.5),
