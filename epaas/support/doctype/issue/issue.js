dataent.ui.form.on("Issue", {
	...
	timeline_refresh: function(frm) {
		...
		if(dataent.model.can_create('Help Article')) {
			...
			$('<button class="btn btn-xs btn-link btn-add-to-kb text-muted hidden-xs pull-right">'+
				...
				.on('click', function() {
					...
					var doc = dataent.model.get_new_doc('Help Article');
					doc.title = frm.doc.subject;
					doc.content = content;
					dataent.set_route('Form', 'Help Article', doc.name);
				});
		}
					...
					var dialog = new dataent.ui.Dialog({
						...
						primary_action: function() {
							frm.call("split_issue", {
								...
								dataent.msgprint(`New issue created: <a href="${result}/desk#Form/Issue/${r.message}">${r.message}</a>`)
								frm.reload_doc();
								dialog.hide();
							});
						}
