...
dataent.pages['support-analytics'].on_page_load = function(wrapper) {
	...
	new epaas.SupportAnalytics(wrapper);


	dataent.breadcrumbs.add("Support")

}

epaas.SupportAnalytics = dataent.views.GridReportWithPlot.extend({
	...
	prepare_data: function() {
		...
		$.each(dataent.report_dump.data["Issue"], function(i, d) {
			var dateobj = dataent.datetime.str_to_obj(d.creation);
			...
			if(col) {
				...
				if(d.status=="Closed") {
					...
					days_to_close[col.field] = flt(days_to_close[col.field])
						+ dataent.datetime.get_diff(d.resolution_date, d.creation);

					hours_to_close[col.field] = flt(hours_to_close[col.field])
						+ dataent.datetime.get_hour_diff(d.resolution_date, d.creation);
				...
				if (d.first_responded_on) {
					...
					hours_to_respond[col.field] = flt(hours_to_respond[col.field])
						+ dataent.datetime.get_hour_diff(d.first_responded_on, d.creation);
				}
			}
		});
