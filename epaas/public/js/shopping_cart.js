...
// shopping cart
dataent.provide("epaas.shopping_cart");
var shopping_cart = epaas.shopping_cart;

dataent.ready(function() {
	var full_name = dataent.session && dataent.session.user_fullname;
	// update user
	if(full_name) {
...
$.extend(shopping_cart, {
	show_shoppingcart_dropdown: function() {
		$(".shopping-cart").on('shown.bs.dropdown', function() {
			if (!$('.shopping-cart-menu .cart-container').length) {
				return dataent.call({
					method: 'epaas.shopping_cart.cart.get_shopping_cart_menu',
					callback: function(r) {
						if (r.message) {
	...
	update_cart: function(opts) {
		if(dataent.session.user==="Guest") {
			...
			return dataent.call({
				type: "POST",
				method: "epaas.shopping_cart.cart.update_cart",
				args: {
	...
	set_cart_count: function() {
		var cart_count = dataent.get_cookie("cart_count");
		if(dataent.session.user==="Guest") {
			cart_count = 0;
		}
