...
epaas.taxes_and_totals = epaas.payments.extend({
	setup: function() {},
	...
	calculate_discount_amount: function(){
		if (dataent.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate: function() {
		...
		var conversion_rate_label = dataent.meta.get_label(this.frm.doc.doctype, "conversion_rate",
			this.frm.doc.name);
		...
		if(!this.frm.doc.conversion_rate) {
			if(this.frm.doc.currency == company_currency) {
				...
				dataent.throw(repl('%(conversion_rate_label)s' +
	...
	calculate_item_values: function() {
		...
		if (!this.discount_amount_applied) {
			$.each(this.frm.doc["items"] || [], function(i, item) {
				dataent.model.round_floats_in(item);
	...
	initialize_taxes: function() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			dataent.model.round_floats_in(tax);
		});
	},
	...
	calculate_net_total: function() {
		...
		dataent.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
	},
	...
	manipulate_grand_total_for_inclusive_tax: function() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = dataent.utils.sum($.map(this.frm.doc.taxes || [],
					function(d) {
	...
	calculate_totals: function() {
		...
		if(in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice"], this.frm.doc.doctype)) {
			...
			if(tax_count) {
				...
				dataent.model.round_floats_in(this.frm.doc,
		...
		dataent.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total: function() {
		...
		if(dataent.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (dataent.sys_defaults.disable_rounded_total) {
			disable_rounded_total = dataent.sys_defaults.disable_rounded_total;
		}
		...
		if(dataent.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup: function() {
		...
		if(this.frm.doc["items"] && this.frm.doc["items"].length) {
			if(!dataent.meta.get_docfield(this.frm.doc["items"][0].doctype, "item_tax_amount", this.frm.doctype)) {
				$.each(this.frm.doc["items"] || [], function(i, item) {
		...
		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if(!dataent.meta.get_docfield(this.frm.doc["taxes"][0].doctype, "tax_amount_after_discount_amount", this.frm.doctype)) {
	...
	set_discount_amount: function() {
		if(this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(flt(this.frm.doc[dataent.scrub(this.frm.doc.apply_discount_on)])
	...
	apply_discount_amount: function() {
		...
		if (this.frm.doc.discount_amount) {
			if(!this.frm.doc.apply_discount_on)
				dataent.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance: function(update_paid_amount) {
		var total_allocated_amount = dataent.utils.sum($.map(this.frm.doc["advances"] || [], function(adv) {
	...
	calculate_outstanding_amount: function(update_paid_amount) {
		...
		dataent.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if(in_list(["Sales Invoice", "Purchase Invoice"], this.frm.doc.doctype)) {
			...
			dataent.model.round_floats_in(this.frm.doc, ["paid_amount"]);
