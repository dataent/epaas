...
// get tax rate
dataent.provide("epaas.taxes");
dataent.provide("epaas.taxes.flags");

dataent.ui.form.on(cur_frm.doctype, {
	setup: function(frm) {
		...
		$(frm.wrapper).on('grid-row-render', function(e, grid_row) {
			if(in_list(['Sales Taxes and Charges', 'Purchase Taxes and Charges'], grid_row.doc.doctype)) {
				epaas.taxes.set_conditional_mandatory_rate_or_amount(grid_row);
			}
		});
	},
	onload: function(frm) {
		if(frm.get_field("taxes")) {
			frm.set_query("account_head", "taxes", function(doc) {
				...
				return {
					query: "epaas.controllers.queries.tax_account_query",
					filters: {
	...
	taxes_on_form_rendered: function(frm) {
		epaas.taxes.set_conditional_mandatory_rate_or_amount(frm.open_grid_row());
	},
...
dataent.ui.form.on('Sales Invoice Payment', {
	mode_of_payment: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		get_payment_mode_account(frm, d.mode_of_payment, function(account){
			dataent.model.set_value(cdt, cdn, 'account', account)
		})
	}
});
...
dataent.ui.form.on('Purchase Invoice', {
...
dataent.ui.form.on("Payment Schedule", {
...
dataent.ui.form.on('Salary Structure', {
	mode_of_payment: function(frm) {
		get_payment_mode_account(frm, frm.doc.mode_of_payment, function(account){
...
var get_payment_mode_account = function(frm, mode_of_payment, callback) {
	if(!frm.doc.company) {
		dataent.throw(__("Please select the Company first"));
	}
	...
	return  dataent.call({
		method: "epaas.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
		args: {
...
cur_frm.cscript.account_head = function(doc, cdt, cdn) {
	...
	if(!d.charge_type && d.account_head){
		dataent.msgprint(__("Please select Charge Type first"));
		dataent.model.set_value(cdt, cdn, "account_head", "");
	} else if(d.account_head && d.charge_type!=="Actual") {
		dataent.call({
			type:"GET",
			method: "epaas.controllers.accounts_controller.get_tax_rate",
			args: {"account_head":d.account_head},
			callback: function(r) {
				dataent.model.set_value(cdt, cdn, "rate", r.message.tax_rate || 0);
				dataent.model.set_value(cdt, cdn, "description", r.message.account_name);
			}
		})
	} else if (d.charge_type == 'Actual' && d.account_head) {
		dataent.model.set_value(cdt, cdn, "description", d.account_head.split(' - ')[0]);
	}
}
...
cur_frm.cscript.validate_inclusive_tax = function(tax) {
	...
	if(cint(tax.included_in_print_rate)) {
		if(tax.charge_type == "Actual") {
			...
			dataent.throw(__("Valuation type charges can not marked as Inclusive"));
		}
	}
}

if(!epaas.taxes.flags[cur_frm.cscript.tax_table]) {
	epaas.taxes.flags[cur_frm.cscript.tax_table] = true;

	dataent.ui.form.on(cur_frm.cscript.tax_table, "row_id", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	dataent.ui.form.on(cur_frm.cscript.tax_table, "rate", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	dataent.ui.form.on(cur_frm.cscript.tax_table, "tax_amount", function(frm, cdt, cdn) {
		cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
	});

	dataent.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(frm, cdt, cdn) {
		frm.cscript.validate_taxes_and_charges(cdt, cdn);
		var open_form = frm.open_grid_row();
		if(open_form) {
			epaas.taxes.set_conditional_mandatory_rate_or_amount(open_form);
		} else {
			// apply in current row
			epaas.taxes.set_conditional_mandatory_rate_or_amount(frm.get_field('taxes').grid.get_row(cdn));
		}
	});

	dataent.ui.form.on(cur_frm.cscript.tax_table, "included_in_print_rate", function(frm, cdt, cdn) {
		var tax = dataent.get_doc(cdt, cdn);
		try {
			cur_frm.cscript.validate_taxes_and_charges(cdt, cdn);
...
epaas.taxes.set_conditional_mandatory_rate_or_amount = function(grid_row) {
	if(grid_row) {
...
cur_frm.pformat.taxes= function(doc){
	...
	function print_hide(fieldname) {
		var doc_field = dataent.meta.get_docfield(doc.doctype, fieldname, doc.name);
