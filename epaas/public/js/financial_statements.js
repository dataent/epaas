dataent.provide("epaas.financial_statements");
...
epaas.financial_statements = {
	...
	"formatter": function(value, row, column, data, default_formatter) {
		if (column.fieldname=="account") {
			...
			column.link_onclick =
				"epaas.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			column.is_tree = true;
		}
	...
	"open_general_ledger": function(data) {
		...
		var project = $.grep(dataent.query_report.filters, function(e){ return e.df.fieldname == 'project'; })
	...
	onload: function(report) {
		...
		epaas.financial_statements.filters = get_filters()

		report.page.add_inner_button(__("Balance Sheet"), function() {
			var filters = report.get_values();
			dataent.set_route('query-report', 'Balance Sheet', {company: filters.company});
		}, __('Financial Statements'));
		report.page.add_inner_button(__("Profit and Loss"), function() {
			var filters = report.get_values();
			dataent.set_route('query-report', 'Profit and Loss Statement', {company: filters.company});
		}, __('Financial Statements'));
		report.page.add_inner_button(__("Cash Flow Statement"), function() {
			var filters = report.get_values();
			dataent.set_route('query-report', 'Cash Flow', {company: filters.company});
		}, __('Financial Statements'));
	}
};
