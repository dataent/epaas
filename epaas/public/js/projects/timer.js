dataent.provide("epaas.timesheet");

epaas.timesheet.timer = function(frm, row, timestamp=0) {
	...
	epaas.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

epaas.timesheet.control_timer = function(frm, dialog, row, timestamp=0) {
	...
	$btn_start.click(function(e) {
		if (!initialised) {
			...
			row = dataent.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = dataent.datetime.get_datetime_as_string();
			...
			if(row.expected_hours) {
				...
				row.to_time = d.format(dataent.defaultDatetimeFormat);
			}
	...
	$btn_complete.click(function() {
		...
		grid_row.doc.to_time = dataent.datetime.now_datetime();
		grid_row.refresh();
		frm.save();
		reset();
		dialog.hide();
	...
	function updateStopwatch(increment) {
		...
		if(cur_dialog && cur_dialog.get_value('expected_hours') > 0) {
			if(flag && (currentIncrement >= (cur_dialog.get_value('expected_hours') * 3600))) {
				dataent.utils.play_sound("alert");
				dataent.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
