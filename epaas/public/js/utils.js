...
$.extend(epaas, {
	get_currency: function(company) {
		...
		if(company)
			return dataent.get_doc(":Company", company).default_currency || dataent.boot.sysdefaults.currency;
		else
			return dataent.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = dataent.boot.docs;
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(epaas.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", epaas.last_selected_company);
			}
		}
	},
	...
	is_perpetual_inventory_enabled: function(company) {
		if(company) {
			return dataent.get_doc(":Company", company).enable_perpetual_inventory
		}
	},

	stale_rate_allowed: () => {
		return cint(dataent.boot.sysdefaults.allow_stale);
	},
...
$.extend(epaas.utils, {
		...
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					epaas.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return dataent.call({
				method: 'epaas.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_bank_account: function(doctype, docname) {
		dataent.call({
			method: "epaas.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	make_subscription: function(doctype, docname) {
		dataent.call({
			method: "dataent.desk.doctype.auto_repeat.auto_repeat.make_auto_repeat",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	make_pricing_rule: function(doctype, docname) {
		dataent.call({
			method: "epaas.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	**/
	...
	get_tree_options: function(option) {
		...
		let unscrub_option = dataent.model.unscrub(option);
		let user_permission = dataent.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function(option) {
		...
		if(options.includes(dataent.defaults.get_default(option))) {
			return dataent.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
...
epaas.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new dataent.ui.Dialog({
				...
				fields: [{
					...
					onchange: function() {
						...
						if (item_code && warehouse) {
							dataent.call({
								method: "epaas.stock.utils.get_latest_stock_qty",
								args: {
									item_code: item_code,
									warehouse: warehouse
								},
					...
					get_query: (e) => {
						return {
							query: "epaas.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
		...
		primary_action: function() {
			...
			alternative_items.forEach(d => {
				let row = dataent.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				frm.script_manager.trigger(item_field, row.doctype, row.name)
					.then(() => {
						dataent.model.set_value(row.doctype, row.name, 'qty', qty);
						dataent.model.set_value(row.doctype, row.name,
...
epaas.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new dataent.ui.Dialog({
		...
		primary_action: function() {
			...
			dataent.call({
				method: 'epaas.controllers.accounts_controller.update_child_qty_rate',
				args: {
...
epaas.utils.map_current_doc = function(opts) {
	...
	var _map = function() {
		if($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = dataent.meta.get_docfield(cur_frm.doctype, 'items').options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			dataent.get_meta(items_doctype).fields.forEach(function(d) {
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					dataent.model.with_doc(opts.source_doctype, src, function(r) {
						var source_doc = dataent.model.get_doc(opts.source_doctype, src);
					...
					if(already_set) {
						dataent.msgprint(__("You have already selected items from {0} {1}",
		...
		return dataent.call({
			...
			type: "POST",
			method: 'dataent.model.mapper.map_docs',
			args: {
				"method": opts.method,
	...
	if(opts.source_doctype) {
		var d = new dataent.ui.form.MultiSelectDialog({
			...
			action: function(selections, args) {
				...
				if(values.length === 0){
					dataent.msgprint(__("Please select {0}", [opts.source_doctype]))
					return;
				}
...
epaas.utils.clear_duplicates = function() {
...
dataent.form.link_formatters['Item'] = function(value, doc) {
...
$(document).on('app_ready', function() {
	if(!dataent.datetime.is_timezone_same()) {
		$.each(["Stock Reconciliation", "Stock Entry", "Stock Ledger Entry",
			...
			dataent.ui.form.on(d, "onload", function(frm) {
				cur_frm.set_df_property("posting_time", "description",
					dataent.sys_defaults.time_zone);
			});
		});
	}
});
