...
epaas.StockAnalytics = epaas.StockGridReport.extend({
		...
		var args = {
			...
			tree_grid: {
				...
				formatter: function(item) {
					if(!item.is_group) {
						return repl("<a \
							onclick='dataent.cur_grid_report.show_stock_ledger(\"%(value)s\")'>\
							%(value)s</a>", {
								value: item.name,
							});
					} else {
	...
	prepare_balances: function() {
		...
		var data = dataent.report_dump.data["Stock Ledger Entry"];
		...
		for(var i=0, j=data.length; i<j; i++) {
			...
			var posting_datetime = dataent.datetime.str_to_obj(sl.posting_datetime);
			...
			if(me.is_default("warehouse") ? true : me.warehouse == sl.warehouse) {
				...
				if(me.value_or_qty!="Quantity") {
					...
					var valuation_method = item.valuation_method ?
						item.valuation_method : dataent.sys_defaults.valuation_method;
