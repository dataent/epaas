...
export default {
	...
	data() {
		return {
			...
			hub_item_name: dataent.get_route()[2],
	...
	mounted() {
		...
		// epaas.hub.item_view_cache = epaas.hub.item_view_cache || [];
		// if (epaas.hub.item_view_cache.includes(this.hub_item_name)) {
		// 	return;
		// }
		...
		this.item_received.then(() => {
			setTimeout(() => {
				...
				// .then(() => {
				// 	epaas.hub.item_view_cache.push(this.hub_item_name);
				// });
			}, 5000);
		});
	},
	methods: {
		...
		go_to_seller_profile_page(seller_name) {
			dataent.set_route(`marketplace/seller/${seller_name}`);
		},
		build_data() {
		...
		add_to_saved_items() {
			...
			.then(() => {
				...
				dataent.show_alert(saved_items_link);
		...
		add_to_featured_items() {
			...
			.then(() => {
				...
				dataent.show_alert(featured_items_link);
				epaas.hub.trigger('action:item_feature');
			})
			.catch(e => {
				console.error(e);
			});
		...
		make_contact_seller_dialog() {
			this.contact_seller_dialog = new dataent.ui.Dialog({
				...
				primary_action: ({ message }) => {
					...
					hub.call('send_message', {
						...
						.then(() => {
							...
							dataent.set_route('marketplace', 'buying', this.item.name);
							epaas.hub.trigger('action:send_message')
						});
				}
			});
		},
		...
		make_report_item_dialog() {
			this.report_item_dialog = new dataent.ui.Dialog({
				...
				primary_action: ({ message }) => {
					hub.call('add_reported_item', { hub_item_name: this.item.name, message })
						.then(() => {
							d.hide();
							dataent.show_alert(__('Item Reported'));
						});
				}
			});
		},
		...
		report_item() {
			if (!hub.is_seller_registered()) {
				dataent.throw(__('Please login as a Marketplace User to report this item.'));
			}
			this.report_item_dialog.show();
		},

		edit_details() {
			dataent.msgprint(__('This feature is under development...'));
		},
