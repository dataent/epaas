...
export default {
	...
	methods: {
		get_items() {
			hub.call('get_data_for_homepage', dataent.defaults ? {
				country: dataent.defaults.get_user_default('country')
			} : null)
			.then((data) => {
		...
		set_search_route() {
			dataent.set_route('marketplace', 'search', this.search_value);
		},
	}
}
</script>
