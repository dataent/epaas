...
dataent.provide('dataent.route');

dataent.utils.make_event_emitter(dataent.route);
dataent.utils.make_event_emitter(epaas.hub);

epaas.hub.Marketplace = class Marketplace {
	constructor({ parent }) {
		this.$parent = $(parent);
	...
	make_body() {
		...
		if (!hub.is_server) {
			epaas.hub.on('seller-registered', () => {
	...
	show_register_dialog() {
		if(dataent.session.user === 'Administrator') {
			dataent.msgprint(__('You need to be a user other than Administrator with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}

		if (!is_subset(['System Manager', 'Item Manager'], dataent.user_roles)) {
			dataent.msgprint(__('You need to be a user with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}
	...
	register_marketplace({company, company_description}) {
		dataent.call({
		    method: 'epaas.hub_node.api.register_marketplace',
		    args: {
				company,
				company_description
			}
			...
			if (r.message && r.message.ok) {
				...
				this.update_hub_settings()
					.then(() => {
						dataent.set_route('marketplace', 'publish');
						epaas.hub.trigger('seller-registered');
					});
			}
		});
	}
	...
	show_add_user_dialog() {
		if (!is_subset(['System Manager', 'Item Manager'], dataent.user_roles)) {
			dataent.msgprint(__('You need to be a user with System Manager and Item Manager roles to add users to Marketplace.'));
			return;
		}
			...
			.then(r => {
				...
				const d = new dataent.ui.Dialog({
					...
					primary_action({ users }) {
						...
						dataent.call('epaas.hub_node.api.register_users', {
						...
						.then(r => {
							...
							if (r.message && r.message.length) {
								dataent.show_alert(__('Added {0} users', [r.message.length]));
							}
						});
					}
				});
	...
	get_unregistered_users() {
		return dataent.call('epaas.hub_node.api.get_unregistered_users')
	}
...
Object.assign(hub, {
	...
	is_user_registered() {
		return this.is_seller_registered() && hub.settings.users
			.filter(hub_user => hub_user.user === dataent.session.user)
			.length === 1;
	},
	...
	get_settings() {
		...
		return dataent.db.get_doc('Marketplace Settings');
	}
});
