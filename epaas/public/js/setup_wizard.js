dataent.provide("epaas.setup");

dataent.pages['setup-wizard'].on_page_load = function(wrapper) {
	if(dataent.sys_defaults.company) {
		dataent.set_route("desk");
		return;
	}
};

dataent.setup.on("before_load", function () {
	epaas.setup.slides_settings.map(dataent.setup.add_slide);
});

epaas.setup.slides_settings = [
	{
		// Domain
		name: 'domain',
		...
		validate: function () {
			if (this.values.domains.length === 0) {
				dataent.msgprint(__("Please select at least one domain."));
				return false;
			}
			dataent.setup.domains = this.values.domains;
			return true;
		},
	},

	{
		...
		fields: [
			...
			},
			{
				...
				label: dataent.setup.domains.includes('Education') ?
		...
		bind_events: function (slide) {
			...
			}).val(dataent.boot.sysdefaults.company_name || "").trigger("change");

			slide.get_input("company_abbr").on("change", function () {
				if (slide.get_input("company_abbr").val().length > 5) {
					dataent.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
		...
		validate: function() {
			if ((this.values.company_name || "").toLowerCase() == "company") {
				dataent.msgprint(__("Company Name cannot be Company"));
				return false;
			}
	...
	},
	{
		...
		icon: "fa fa-building",
		// help: dataent.setup.domains.includes('Education') ?
		...
		fields: [
			{
				...
				fieldtype: 'Data',
				placeholder: dataent.setup.domains.includes('Education') ?
		...
		validate: function () {
			...
			if(me.values.bank_account){
				dataent.call({
					async: false,
					method: "epaas.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
					...
					callback: function (r) {
						if(r.message){
							...
							dataent.msgprint(__(`Account ${me.values.bank_account} already exists, enter a different name for your bank account`));
						}
					}
				});
		...
		validate_fy_dates: function() {
			...
			if (invalid || start_greater_than_end) {
				dataent.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = dataent.wizard.values.country;

			if (country) {
				var fy = epaas.setup.fiscal_years[country];
				...
				if (year_start_date > dataent.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date =
					dataent.datetime.add_days(dataent.datetime.add_months(start_date, 12), -1);
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new dataent.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: 'epaas.accounts.utils.get_coa',
				args: {
			...
			const buttons = $(form_container).find('.dataent-control');
