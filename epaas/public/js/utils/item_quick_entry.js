...
dataent.ui.form.ItemQuickEntryForm = dataent.ui.form.QuickEntryForm.extend({
	...
	init_post_render_dialog_operations: function() {
		this.dialog.fields_dict.attribute_html.$wrapper.append(dataent.render_template("item_quick_entry"));
	...
	open_doc: function() {
		...
		if (this.dialog.fields_dict.create_variant.$input.prop("checked")) {
			...
			if (template)
				dataent.set_route("Form", this.doctype, template);
		} else {
		...
		$.each(attributes, function(index, row) {
			...
			if (!row.numeric_values) {
				...
				me[row.attribute].$input.on("change", function() {
					...
					$(this).closest(".dataent-control").toggleClass("has-error", $(this).val() ? false : true);
				});
			}
		});
	},
	...
	init_make_control: function(fieldtype, row) {
		this[row.attribute] = dataent.ui.form.make_control({
			df: {
				"fieldtype": fieldtype,
				"label": row.attribute,
	...
	init_awesomplete_for_attribute: function(row) {
		...
		this[row.attribute].$input.on('input', function(e) {
			...
			$(e.target).closest(".dataent-control").toggleClass("has-error", e.target.value ? false : true);
		});
	},
	...
	get_variant_doc: function() {
		...
		if (Object.keys(attribute).length) {
			dataent.call({
				method: "epaas.controllers.item_variant.create_variant_doc_for_quick_entry",
				args: {
				...
				callback: function(r) {
					if (Object.prototype.toString.call(r.message) == "[object Object]") {
						...
						var msgprint_dialog = dataent.msgprint(__("Item Variant {0} already exists with same attributes", [repl('<a class="strong variant-click" data-item-code="%(item)s" \
								>%(item)s</a>', {
								item: r.message
							})]));
						...
						msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							...
							me.dialog.hide();
							if (dataent._from_link) {
								dataent._from_link.set_value($(this).attr("data-item-code"));
							} else {
								dataent.set_route('Form', "Item", $(this).attr("data-item-code"));
							}
						});
					}
				}
