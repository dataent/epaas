...
epaas.SerialNoBatchSelector = Class.extend({
	...
	make_dialog: function() {
		...
		let fields = [
			...
			{fieldtype:'Column Break'},
			{
				...
				get_query: function() {
					return {
						query: "epaas.controllers.queries.warehouse_query",
						filters: [
	...
	validate: function() {
		...
		if(!values.warehouse) {
			dataent.throw(__("Please select a warehouse"));
			return false;
		}
		if(this.has_batch) {
			if(values.batches.length === 0 || !values.batches) {
				dataent.throw(__("Please select batches for batched item "
					+ values.item_code));
				return false;
			}
			values.batches.map((batch, i) => {
				if(!batch.selected_qty || batch.selected_qty === 0 ) {
					if (!this.show_dialog) {
						dataent.throw(__("Please select quantity on row " + (i+1)));
						return false;
					}
				}
			});
			...
			if (!serial_nos || !serial_nos.replace(/\s/g, '').length) {
				if (!this.show_dialog) {
					dataent.throw(__("Please enter serial numbers for serialized item "
						+ values.item_code));
					return false;
				}
			}
				...
				fields: [
					{
						...
						change: function () {
							...
							if (selected_batches.includes(val)) {
								this.set_value("");
								dataent.throw(__(`Batch ${val} already selected.`));
								return;
							}
							if (me.warehouse_details.name) {
								dataent.call({
									method: 'epaas.stock.doctype.batch.batch.get_batch_qty',
									args: {
								...
								this.set_value("");
								dataent.throw(__(`Please select a warehouse to get available
									quantities`));
							}
					...
					},
					{
						...
						change: function () {
							...
							if (batch_no.length === 0 && parseInt(selected_qty) !== 0) {
								dataent.throw(__("Please select a batch"));
							}
							if (me.warehouse_details.type === 'Source Warehouse' &&
								parseFloat(available_qty) < parseFloat(selected_qty)) {

								this.set_value('0');
								dataent.throw(__(`For transfer from source, selected quantity cannot be
