epaas.ItemSelector = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		...
		if (!this.item_query) {
			this.item_query = epaas.queries.item().query;
		}
	...
	add_item: function(item_code) {
		...
		$.each(this.frm.doc.items || [], (i, d) => {
			if(d[this.item_field]===item_code) {
				dataent.model.set_value(d.doctype, d.name, 'qty', d.qty + 1);
				dataent.show_alert({message: __("Added {0} ({1})", [item_code, d.qty]), indicator: 'green'});
				added = true;
				return false;
			}
		});
		...
		if(!added) {
			...
			dataent.run_serially([
				...
				() => dataent.model.set_value(d.doctype, d.name, this.item_field, item_code),
				() => dataent.timeout(0.1),
				() => {
					dataent.model.set_value(d.doctype, d.name, 'qty', 1);
					dataent.show_alert({message: __("Added {0} ({1})", [item_code, 1]), indicator: 'green'});
				}
			]);
		}
	...
	render_items: function() {
		...
		var me = this;
		dataent.link_search("Item", args, function(r) {
			$.each(r.values, function(i, d) {
				if(!d.image) {
					d.abbr = dataent.get_abbr(d.item_name);
					d.color = dataent.get_palette(d.item_name);
				}
			});
			me.dialog.results.html(dataent.render_template('item_selector', {'data':r.values}));
		});
	}
});