...
def register_marketplace(company, company_description):
	...
	settings = dataent.get_single('Marketplace Settings')
	...
	if message.get('hub_seller_name'):
		...
		settings.add_hub_user(dataent.session.user)
...
def map_fields(items):
	...
	table_fields = [d.fieldname for d in dataent.get_meta('Item').get_table_fields()]

	hub_seller_name = dataent.db.get_value('Marketplace Settings' , 'Marketplace Settings', 'hub_seller_name')
...
def publish_selected_items(items_to_publish):
	...
	if not len(items_to_publish):
		dataent.throw(_('No items to publish'))

	for item in items_to_publish:
		item_code = item.get('item_code')
		dataent.db.set_value('Item', item_code, 'publish_in_hub', 1)
	...
	try:
		...
		dataent.log_error(message=e, title='Hub Sync Error')

@dataent.whitelist()
def get_unregistered_users():
	settings = dataent.get_single('Marketplace Settings')
	registered_users = [user.user for user in settings.users] + ['Administrator', 'Guest']
	all_users = [user.name for user in dataent.db.get_all('User', filters={'enabled': 1})]
...
def item_sync_preprocess(intended_item_publish_count):
	...
	if response:
		dataent.db.set_value("Marketplace Settings", "Marketplace Settings", "sync_in_progress", 1)
		return response
	else:
		dataent.throw(_('Unable to update remote activity'))


def item_sync_postprocess():
	response = call_hub_method('post_items_publish', {})
	if response:
		dataent.db.set_value('Marketplace Settings', 'Marketplace Settings', 'last_sync_datetime', dataent.utils.now())
	else:
		dataent.throw(_('Unable to update remote activity'))

	dataent.db.set_value('Marketplace Settings', 'Marketplace Settings', 'sync_in_progress', 0)
...
def convert_relative_image_urls_to_absolute(items):
	...
	for item in items:
		...
		if file_path.startswith('/files/'):
			item['image'] = urljoin(dataent.utils.get_url(), file_path)


def get_hub_connection():
	settings = dataent.get_single('Marketplace Settings')
	marketplace_url = settings.marketplace_url
	hub_user = settings.get_hub_user(dataent.session.user)

	if hub_user:
		password = hub_user.get_password()
		hub_connection = DataentClient(marketplace_url, hub_user.user, password)
		return hub_connection
	else:
		read_only_hub_connection = DataentClient(marketplace_url)
