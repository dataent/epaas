...
def get_hub_connection():
	if dataent.db.exists('Data Migration Connector', 'Hub Connector'):
		hub_connector = dataent.get_doc('Data Migration Connector', 'Hub Connector')
		hub_connection = hub_connector.get_connection()
		return hub_connection.connection

	# read-only connection
	hub_connection = DataentClient(dataent.conf.hub_url)
...
def make_opportunity(buyer_name, email_id):
	...
	o.lead = dataent.get_all("Lead", filters={"email_id": email_id}, fields = ["name"])[0]["name"]
...
def make_supplier(supplier):
	...
	supplier = dataent._dict(json.loads(supplier))

	if not dataent.db.exists('Supplier', {'supplier_name': supplier.supplier_name}):
		supplier_doc = dataent.get_doc({
			'doctype': 'Supplier',
		...
		supplier_doc = dataent.get_doc('Supplier', supplier.supplier_name)
...
def make_item(item):
	...
	item = dataent._dict(json.loads(item))

	if not dataent.db.exists('Item', {'item_code': item.item_code}):
		item_doc = dataent.get_doc({
			'doctype': 'Item',
...
def make_rfq(item, supplier, contact):
	...
	rfq = dataent.get_doc({
		...
		'status': 'Draft',
		'company': dataent.db.get_single_value('Marketplace Settings', 'company'),
...
def send_opportunity(contact):
	...
	doc = {
		'doctype': 'Lead',
		'lead_name': dataent.db.get_single_value('Marketplace Settings', 'company'),
		'email_id': dataent.db.get_single_value('Marketplace Settings', 'user')
	}

	args = dataent._dict(dict(
		doctype='Hub Message',
