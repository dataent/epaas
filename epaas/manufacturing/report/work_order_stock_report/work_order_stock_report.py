...
def get_item_list(wo_list, filters):
	...
	for wo_details in wo_list:
		desc = dataent.db.get_value("BOM", wo_details.bom_no, "description")

		for wo_item_details in dataent.db.get_values("Work Order Item",
			{"parent": wo_details.name}, ["item_code", "source_warehouse"], as_dict=1):

			item_list = dataent.db.sql("""SELECT
...
def get_work_orders():
	out =  dataent.get_all("Work Order", filters={"docstatus": 1, "status": ( "!=","Completed")},
