...
#	if not filters: filters = {}
	...
	for row in data:
		...
		last_pur_price = dataent.db.get_value("Item", row.item_code, "last_purchase_rate")
		if row.to_build > 0:
...
def get_item_details(item_code):
		items = dataent.db.sql("""select it.item_group, it.item_name, it.stock_uom, it.name, it.brand, it.description, it.manufacturer_part_no, it.manufacturer from tabItem it where it.item_code = %s""", item_code, as_dict=1)
