...
dataent.provide("epaas.bom");

dataent.ui.form.on("BOM", {
	setup: function(frm) {
		...
		if (frm.doc.routing) {
			dataent.call({
				...
				callback: function(r) {
					if (!r.exc) {
						frm.refresh_fields();
						epaas.bom.calculate_op_cost(frm.doc);
						epaas.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
epaas.bom.BomController = epaas.TransactionController.extend({
	conversion_rate: function(doc) {
		if(this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			epaas.bom.update_cost(doc);
		}
	},
	...
	conversion_factor: function(doc, cdt, cdn) {
		if(dataent.meta.get_docfield(cdt, "stock_qty", cdn)) {
...
$.extend(cur_frm.cscript, new epaas.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	epaas.bom.calculate_op_cost(doc);
	epaas.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return dataent.call({
			...
			callback: function(r) {
				...
				epaas.bom.calculate_rm_cost(doc);
				epaas.bom.calculate_scrap_materials_cost(doc);
				epaas.bom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.bom_no) {
		dataent.msgprint(__("You can not change rate if BOM mentioned agianst any item"));
		get_bom_material_detail(doc, cdt, cdn, scrap_items);
	} else {
		epaas.bom.calculate_rm_cost(doc);
		epaas.bom.calculate_scrap_materials_cost(doc);
...
epaas.bom.update_cost = function(doc) {
	epaas.bom.calculate_op_cost(doc);
	epaas.bom.calculate_rm_cost(doc);
	epaas.bom.calculate_scrap_materials_cost(doc);
	epaas.bom.calculate_total(doc);
...
epaas.bom.calculate_op_cost = function(doc) {
	...
	for(var i=0;i<op.length;i++) {
		...
		dataent.model.set_value('BOM Operation',op[i].name, "operating_cost", operating_cost);
		dataent.model.set_value('BOM Operation',op[i].name, "base_operating_cost", base_operating_cost);
...
// rm : raw material
epaas.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for(var i=0;i<rm.length;i++) {
		...
		dataent.model.set_value('BOM Item', rm[i].name, 'base_rate',
			flt(rm[i].rate) * flt(doc.conversion_rate));
		dataent.model.set_value('BOM Item', rm[i].name, 'amount', amount);
		dataent.model.set_value('BOM Item', rm[i].name, 'base_amount', base_amount);
		dataent.model.set_value('BOM Item', rm[i].name,
...
// sm : scrap material
epaas.bom.calculate_scrap_materials_cost = function(doc) {
	...
	for(var i=0;i<sm.length;i++) {
		...
		dataent.model.set_value('BOM Scrap Item',sm[i].name, 'base_rate', base_rate);
		dataent.model.set_value('BOM Scrap Item',sm[i].name, 'amount', amount);
		dataent.model.set_value('BOM Scrap Item',sm[i].name, 'base_amount', base_amount);
...
epaas.bom.calculate_total = function(doc) {
...
dataent.ui.form.on("BOM Operation", "operation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	dataent.call({
		...
		callback: function (data) {
			if(data.message.description) {
				dataent.model.set_value(d.doctype, d.name, "description", data.message.description);
			}
			if(data.message.workstation) {
				dataent.model.set_value(d.doctype, d.name, "workstation", data.message.workstation);
			}
		}
	});
});

dataent.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	dataent.call({
		...
		callback: function (data) {
			dataent.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			dataent.model.set_value(d.doctype, d.name, "hour_rate",
...
dataent.ui.form.on("BOM Item", "qty", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

dataent.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	dataent.db.get_value('Item', {name: d.item_code}, 'allow_alternative_item', (r) => {
		d.allow_alternative_item = r.allow_alternative_item
	})
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

dataent.ui.form.on("BOM Operation", "operations_remove", function(frm) {
	epaas.bom.calculate_op_cost(frm.doc);
	epaas.bom.calculate_total(frm.doc);
});

dataent.ui.form.on("BOM Item", "items_remove", function(frm) {
	epaas.bom.calculate_rm_cost(frm.doc);
	epaas.bom.calculate_total(frm.doc);
});
...
dataent.ui.form.on("BOM", "with_operations", function(frm) {
