...
from dataent.utils import cint, cstr, flt
from dataent import _
from epaas.setup.utils import get_exchange_rate
from dataent.website.website_generator import WebsiteGenerator
from epaas.stock.get_item_details import get_conversion_factor
from epaas.stock.get_item_details import get_price_list_rate
...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		names = dataent.db.sql_list("""select name from `tabBOM` where item=%s""", self.item)
	...
	def validate(self):
		self.route = dataent.scrub(self.name).replace('_', '-')
	...
	def on_cancel(self):
		dataent.db.set(self, "is_active", 0)
		dataent.db.set(self, "is_default", 0)
	...
	def get_item_det(self, item_code):
		item = dataent.db.sql("""select name, item_name, docstatus, description, image,
			is_sub_contracted_item, stock_uom, default_bom, last_purchase_rate, include_item_in_manufacturing
			from `tabItem` where name=%s""", item_code, as_dict = 1)

		if not item:
			dataent.throw(_("Item: {0} does not exist in the system").format(item_code))

		return item

	def get_routing(self):
		if self.routing:
			for d in dataent.get_all("BOM Operation", fields = ["*"],
	...
	def validate_rm_item(self, item):
		if (item[0]['name'] in [it.item_code for it in self.items]) and item[0]['name'] == self.item:
			dataent.throw(_("BOM #{0}: Raw material cannot be same as main Item").format(self.name))
	...
	def get_bom_material_detail(self, args=None):
		...
		if not args:
			args = dataent.form_dict.get('args')
	...
	def validate_bom_currecny(self, item):
		if item.get('bom_no') and dataent.db.get_value('BOM', item.get('bom_no'), 'currency') != self.currency:
			dataent.throw(_("Row {0}: Currency of the BOM #{1} should be equal to the selected currency {2}")
			...
			if arg.get('bom_no') and self.set_rate_of_sub_assembly_item_based_on_bom:
				...
				if self.rm_cost_as_per == 'Valuation Rate':
					...
					rate = (arg.get('last_purchase_rate') \
						or dataent.db.get_value("Item", arg['item_code'], "last_purchase_rate")) \
							* (arg.get("conversion_factor") or 1)
				elif self.rm_cost_as_per == "Price List":
					if not self.buying_price_list:
						dataent.throw(_("Please select Price List"))
					args = dataent._dict({
						"doctype": "BOM",
					...
					item_doc = dataent.get_doc("Item", arg.get("item_code"))
					out = dataent._dict()
				...
				if not rate:
					if self.rm_cost_as_per == "Price List":
						dataent.msgprint(_("Price not found for item {0} in price list {1}")
							.format(arg["item_code"], self.buying_price_list), alert=True)
					else:
						dataent.msgprint(_("{0} not found for item {1}")
	...
	def update_cost(self, update_parent=True, from_child_bom=False, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = dataent.db.sql_list("""select distinct parent from `tabBOM Item`
				where bom_no = %s and docstatus=1 and parenttype='BOM'""", self.name)

			for bom in parent_boms:
				dataent.get_doc("BOM", bom).update_cost(from_child_bom=True)

		if not from_child_bom:
			dataent.msgprint(_("Cost Updated"))

	def update_parent_cost(self):
		if self.total_cost:
			cost = self.total_cost / self.quantity

			dataent.db.sql("""update `tabBOM Item` set rate=%s, amount=stock_qty*%s
				where bom_no = %s and docstatus < 2 and parenttype='BOM'""",
				(cost, cost, self.name))

	def get_bom_unitcost(self, bom_no):
		bom = dataent.db.sql("""select name, base_total_cost/quantity as unit_cost from `tabBOM`
	...
	def get_valuation_rate(self, args):
		...
		for d in dataent.db.sql("""select actual_qty, stock_value from `tabBin`
		...
		if valuation_rate <= 0:
			last_valuation_rate = dataent.db.sql("""select valuation_rate
		...
		if not valuation_rate:
			valuation_rate = dataent.db.get_value("Item", args['item_code'], "valuation_rate")
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			from dataent.model.utils import set_default
			set_default(self, "item")
			item = dataent.get_doc("Item", self.item)
			if item.default_bom != self.name:
				dataent.db.set_value('Item', self.item, 'default_bom', self.name)
		else:
			dataent.db.set(self, "is_default", 0)
			item = dataent.get_doc("Item", self.item)
			if item.default_bom == self.name:
				dataent.db.set_value('Item', self.item, 'default_bom', None)
	...
	def validate_main_item(self):
		...
		if not item:
			dataent.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = dataent.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name= ret[2]

		if not self.quantity:
			dataent.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == 'Price List':
			price_list_currency = dataent.db.get_value('Price List', self.buying_price_list, 'currency')
			if price_list_currency not in (self.currency, self.company_currency()):
				dataent.throw(_("Currency of the price list {0} must be {1} or {2}")
	...
	def validate_materials(self):
		...
		if not self.get('items'):
			dataent.throw(_("Raw Materials cannot be blank."))
		check_list = []
		for m in self.get('items'):
			if m.bom_no:
				validate_bom_no(m.item_code, m.bom_no)
			if flt(m.qty) <= 0:
				dataent.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
		...
		if not self.allow_same_item_multiple_times:
			...
			if duplicate_items:
				...
				dataent.throw(_("Same item has been entered multiple times. {0}").format(duplicate_list))
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			return [cstr(d[0]) for d in dataent.db.sql("""select bom_no from `tabBOM Item`
	...
	def calculate_op_cost(self):
		...
		for d in self.get('operations'):
			if d.workstation:
				if not d.hour_rate:
					hour_rate = flt(dataent.db.get_value("Workstation", d.workstation, "hour_rate"))
	...
	def add_exploded_items(self):
		...
		dataent.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			act_pbom = dataent.db.sql("""select distinct bom_item.parent from `tabBOM Item` bom_item
				where bom_item.bom_no = %s and bom_item.docstatus = 1 and bom_item.parenttype='BOM'
				and exists (select * from `tabBOM` where name = bom_item.parent
					and docstatus = 1 and is_active = 1)""", self.name)

			if act_pbom and act_pbom[0][0]:
				dataent.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))

	def validate_operations(self):
		if self.with_operations and not self.get('operations'):
			dataent.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = dataent.db.get_value('Operation', d.operation, 'description')
...
def get_bom_items_as_dict(bom, company, qty=1, fetch_exploded=1, fetch_scrap_items=0, include_non_stock_items=False, fetch_qty_in_stock_uom=True):
	...
	if cint(fetch_exploded):
		...
		items = dataent.db.sql(query, { "parent": bom, "qty": qty, "bom": bom, "company": company }, as_dict=True)
		...
		items = dataent.db.sql(query, { "qty": qty, "bom": bom, "company": company }, as_dict=True)
	else:
	...
	for item, item_details in item_dict.items():
		for d in [["Account", "expense_account", "default_expense_account"],
			["Cost Center", "cost_center", "cost_center"], ["Warehouse", "default_warehouse", ""]]:
				company_in_record = dataent.db.get_value(d[0], item_details.get(d[1]), "company")
				if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
					item_dict[item][d[1]] = dataent.get_cached_value('Company',  company,  d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		dataent.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not getattr(dataent.flags, "in_test", False):
			dataent.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if bom.item.lower() == item.lower() or \
			bom.item.lower() == cstr(dataent.db.get_value("Item", item, "variant_of")).lower():
 				rm_item_exists = True
		if not rm_item_exists:
			dataent.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))
...
def get_boms_in_bottom_up_order(bom_no=None):
	...
	if bom_no:
		...
		# get all leaf BOMs
		bom_list = dataent.db.sql_list("""select name from `tabBOM` bom
...
def add_additional_cost(stock_entry, work_order):
	...
	bom = dataent.get_doc('BOM', work_order.bom_no)
	...
	non_stock_items = dataent.get_all('Item',
