...
from epaas.manufacturing.doctype.bom.bom import validate_bom_no
from dataent.utils import cstr, flt, cint, nowdate, add_days, comma_and, now_datetime, ceil
...
class ProductionPlan(Document):
	...
	def validate_data(self):
		for d in self.get('po_items'):
			if not d.bom_no:
				dataent.throw(_("Please select BOM for Item in Row {0}".format(d.idx)))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				dataent.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			dataent.msgprint(_("Sales orders are not available for production"))
	...
	def get_so_items(self):
		...
		item_condition = ""
		if self.item_code:
			item_condition = ' and so_item.item_code = "{0}"'.format(dataent.db.escape(self.item_code))

		items = dataent.db.sql("""select distinct parent, item_code, warehouse,
		...
		packed_items = dataent.db.sql("""select distinct pi.parent, pi.item_code, pi.warehouse as warehouse,
	...
	def get_mr_items(self):
		...
		item_condition = ""
		if self.item_code:
			item_condition = " and mr_item.item_code ='{0}'".format(dataent.db.escape(self.item_code))

		items = dataent.db.sql("""select distinct parent, name, item_code, warehouse,
	...
	def delete_draft_work_order(self):
		for d in dataent.get_all('Work Order', fields = ["name"],
			filters = {'docstatus': 0, 'production_plan': ("=", self.name)}):
			dataent.delete_doc('Work Order', d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			item_details.update({
				"project": self.project or dataent.db.get_value("Sales Order", d.sales_order, "project")
			})
	...
	def create_work_order(self, item):
		from epaas.manufacturing.doctype.work_order.work_order import OverProductionError, get_default_warehouse
		warehouse = get_default_warehouse()
		wo = dataent.new_doc("Work Order")
		wo.update(item)
...
def get_exploded_items(item_details, company, bom_no, include_non_stock_items, planned_qty=1):
	for d in dataent.db.sql("""select bei.item_code, item.default_bom as bom,
...
def get_material_request_items(row, sales_order, company, ignore_existing_ordered_qty, warehouse):
	...
	if row['purchase_uom'] != row['stock_uom']:
		if not row['conversion_factor']:
			dataent.throw(_("UOM Conversion factor ({0} -> {1}) not found for item: {2}")
				.format(row['purchase_uom'], row['stock_uom'], row.item_code))
		requested_qty = requested_qty / row['conversion_factor']

	if dataent.db.get_value("UOM", row['purchase_uom'], "must_be_whole_number"):
...
def get_items_for_material_requests(doc, sales_order=None, company=None):
	if isinstance(doc, string_types):
		doc = dataent._dict(json.loads(doc))

	doc['mr_items'] = []
	po_items = doc.get('po_items') if doc.get('po_items') else doc.get('items')
	company = doc.get('company')

	so_item_details = dataent._dict()
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				dataent.throw(_("For row {0}: Enter Planned Qty").format(data.get('idx')))
			...
			item_master = dataent.get_doc('Item', data['item_code']).as_dict()
			...
			item_details[item_master.name] = dataent._dict(
				{
		...
		for item_code, details in iteritems(item_details):
			so_item_details.setdefault(sales_order, dataent._dict())
