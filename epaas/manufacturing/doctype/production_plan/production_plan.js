...
dataent.ui.form.on('Production Plan', {
	setup: function(frm) {
		...
		frm.fields_dict['po_items'].grid.get_field('bom_no').get_query = function(doc, cdt, cdn) {
			...
			} else dataent.msgprint(__("Please enter Item first"));
		}
	...
	get_items_for_mr: function(frm) {
		dataent.call({
			method: "epaas.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {doc: frm.doc},
			callback: function(r) {
				if(r.message) {
...
dataent.ui.form.on("Material Request Plan Item", {
	warehouse: function(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.item_code) {
			dataent.call({
				method: "epaas.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row
				},
				callback: function(r) {
					dataent.model.set_value(cdt, cdn, 'actual_qty', r.message[1])
				}
			})
		}
	}
