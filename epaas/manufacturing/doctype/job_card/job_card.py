...
from dataent.utils import flt, time_diff_in_hours, get_datetime
...
class JobCard(Document):
		...
		if self.get('time_logs'):
			for d in self.get('time_logs'):
				if get_datetime(d.from_time) > get_datetime(d.to_time):
					dataent.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_overlap_for(self, args):
		existing = dataent.db.sql("""select jc.name as name from
	...
	def get_required_items(self):
		...
		doc = dataent.get_doc('Work Order', self.get('work_order'))
		if doc.transfer_material_against == 'Work Order' or doc.skip_transfer:
			return

		for d in doc.required_items:
			if not d.operation:
				dataent.throw(_("Row {0} : Operation is required against the raw material item {1}")
			...
			if self.get('operation') == d.operation:
				self.append('items', {
					...
					'uom': dataent.db.get_value("Item", d.item_code, 'stock_uom'),
	...
	def validate_job_card(self):
		if not self.time_logs:
			dataent.throw(_("Time logs are required for job card {0}").format(self.name))

		if self.total_completed_qty <= 0.0:
			dataent.throw(_("Total completed qty must be greater than zero"))

		if self.total_completed_qty > self.for_quantity:
			dataent.throw(_("Total completed qty can not be greater than for quantity"))
	...
	def update_work_order(self):
		...
		for d in dataent.get_all('Job Card',
			...
			doc = dataent.get_doc('Job Card', d.name)
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.items:
			self.transferred_qty = dataent.db.get_value('Stock Entry', {
				'job_card': self.name,
		...
		qty = 0
		if self.work_order:
			doc = dataent.get_doc('Work Order', self.work_order)
			if doc.transfer_material_against == 'Job Card' and not doc.skip_transfer:
				...
				if completed:
					job_cards = dataent.get_all('Job Card', filters = {'work_order': self.work_order,
