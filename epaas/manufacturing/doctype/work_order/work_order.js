...
dataent.ui.form.on("Work Order", {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			epaas.work_order.set_default_warehouse(frm);
		}
	},

	refresh: function(frm) {
		epaas.toggle_naming_series();
		epaas.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	make_job_card: function(frm) {
		...
		const dialog = dataent.prompt(fields, function(data) {
			if (data.qty > qty) {
				dataent.throw(__("For Quantity must be less than quantity {0}", [qty]));
			}

			if (data.qty <= 0) {
				dataent.throw(__("For Quantity must be greater than zero"));
			}

			dataent.call({
				method: "epaas.manufacturing.doctype.work_order.work_order.make_job_card",
				args: {
	...
	production_item: function(frm) {
		if (frm.doc.production_item) {
			dataent.call({
				method: "epaas.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function(r) {
					if(r.message) {
						...
						epaas.in_production_item_onchange = true;
						...
						epaas.in_production_item_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!epaas.in_production_item_onchange) {
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_item) {
			dataent.call({
				method: "epaas.manufacturing.doctype.work_order.work_order.query_sales_order",
...
dataent.ui.form.on("Work Order Item", {
	source_warehouse: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if(!row.item_code) {
			dataent.throw(__("Please set the Item Code first"));
...
dataent.ui.form.on("Work Order Operation", {
		...
		if (d.workstation) {
			dataent.call({
				...
				callback: function (data) {
					dataent.model.set_value(d.doctype, d.name, "hour_rate", data.message.hour_rate);
					epaas.work_order.calculate_cost(frm.doc);
					epaas.work_order.calculate_total_cost(frm);
				}
			})
		}
	},
...
epaas.work_order = {
		...
		if (doc.docstatus === 1) {
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					epaas.work_order.stop_work_order(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					epaas.work_order.stop_work_order(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn){
				if ((flt(doc.material_transferred_for_manufacturing) < flt(doc.qty))
					...
					var start_btn = frm.add_custom_button(__('Start'), function() {
						epaas.work_order.make_se(frm, 'Material Transfer for Manufacture');
					});
			...
			if(!frm.doc.skip_transfer){
				...
				&& frm.doc.status != 'Stopped') {
					...
					if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								epaas.work_order.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					var finish_btn = frm.add_custom_button(__('Finish'), function() {
						epaas.work_order.make_se(frm, 'Manufacture');
					});
	...
	calculate_cost: function(doc) {
		if (doc.operations){
			...
			for(var i=0;i<op.length;i++) {
				...
				dataent.model.set_value('Work Order Operation', op[i].name,
	...
	set_default_warehouse: function(frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			dataent.call({
				method: "epaas.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function(r) {
					if(!r.exe) {
	...
	make_se: function(frm, purpose) {
		...
		dataent.prompt({fieldtype:"Float", label: __("Qty for {0}", [purpose]), fieldname:"qty",
			description: __("Max: {0}", [max]), 'default': max }, function(data)
		{
			if(data.qty > max) {
				dataent.msgprint(__("Quantity must not be more than {0}", [max]));
				return;
			}
			dataent.call({
				method:"epaas.manufacturing.doctype.work_order.work_order.make_stock_entry",
				args: {
	...
	stop_work_order: function(frm, status) {
		dataent.call({
			method: "epaas.manufacturing.doctype.work_order.work_order.stop_unstop",
			args: {
				work_order: frm.doc.name,
				status: status
			},
