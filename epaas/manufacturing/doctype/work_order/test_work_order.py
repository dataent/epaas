...
import unittest
import dataent
from dataent.utils import flt, time_diff_in_hours, now, add_days, cint
...
from epaas.stock.doctype.stock_entry import test_stock_entry
...
class TestWorkOrder(unittest.TestCase):
	...
	def check_planned_qty(self):
		...
		planned0 = dataent.db.get_value("Bin", {"item_code": "_Test FG Item",
			"warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty") or 0

		wo_order = make_wo_order_test_record()

		planned1 = dataent.db.get_value("Bin", {"item_code": "_Test FG Item",
		...
		# from stores to wip
		s = dataent.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = dataent.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(dataent.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = dataent.db.get_value("Bin", {"item_code": "_Test FG Item",
			"warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty")
	...
	def test_over_production(self):
		...
		s = dataent.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_production_item(self):
		...
		dataent.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(dataent.ValidationError, wo_order.save)

		dataent.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		dataent.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(dataent.ValidationError, wo_order.save)

		dataent.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = dataent.get_doc(make_stock_entry(self.wo_order.name,
		...
		s = dataent.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_scrap_material_qty(self):
		...
		s = dataent.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()
		...
		wo_order_details = dataent.db.get_value("Work Order", wo_order.name,
	...
	def test_allow_overproduction(self):
		...
		s = dataent.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not dataent.db.get_value('Item Price', {'item_code': '_Test FG Non Stock Item'}):
			dataent.get_doc({
				'doctype': 'Item Price',
		...
		se = dataent.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()
	...
	def test_job_card(self):
		data = dataent.get_cached_value('BOM',
			{'docstatus': 1, 'with_operations': 1, 'company': '_Test Company'}, ['name', 'item'])

		if data:
			bom, bom_item = data

			bom_doc = dataent.get_doc('BOM', bom)
			work_order = make_wo_order_test_record(item=bom_item, qty=1, bom_no=bom)

			job_cards = dataent.get_all('Job Card', filters = {'work_order': work_order})
	...
	def test_work_order_with_non_transfer_item(self):
		...
		wo = make_wo_order_test_record(production_item = fg_item)
		ste = dataent.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
...
def get_scrap_item_details(bom_no):
	scrap_items = {}
	for item in dataent.db.sql("""select item_code, stock_qty from `tabBOM Scrap Item`
...
def allow_overproduction(fieldname, percentage):
	doc = dataent.get_doc("Manufacturing Settings")
	doc.update({
...
def make_wo_order_test_record(**args):
	...
	wo_order = dataent.new_doc("Work Order")
	wo_order.production_item = args.production_item or args.item or args.item_code or "_Test FG Item"
	wo_order.bom_no = args.bom_no or dataent.db.get_value("BOM", {"item": wo_order.production_item,
...
test_records = dataent.get_test_records('Work Order')
