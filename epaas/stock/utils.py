...
import json
from dataent.utils import flt, cstr, nowdate, nowtime

from six import string_types

class InvalidWarehouseCompany(dataent.ValidationError): pass
...
def get_stock_value_from_bin(warehouse=None, item_code=None):
	...
	stock_value = dataent.db.sql(query, values)
...
def get_stock_value_on(warehouse=None, posting_date=None, item_code=None):
	...
	if warehouse:
		...
		lft, rgt, is_group = dataent.db.get_value("Warehouse", warehouse, ["lft", "rgt", "is_group"])
...
def get_latest_stock_qty(item_code, warehouse=None):
	...
	actual_qty = dataent.db.sql("""select sum(actual_qty) from tabBin
...
def get_latest_stock_balance():
	bin_map = {}
	for d in dataent.db.sql("""SELECT item_code, warehouse, stock_value as stock_value
...
def get_bin(item_code, warehouse):
	bin = dataent.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
	if not bin:
		bin_obj = dataent.get_doc({
			"doctype": "Bin",
			"item_code": item_code,
			"warehouse": warehouse,
		})
		...
		bin_obj = dataent.get_doc('Bin', bin)
	bin_obj.flags.ignore_permissions = True
	return bin_obj

def update_bin(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	is_stock_item = dataent.db.get_value('Item', args.get("item_code"), 'is_stock_item')
	if is_stock_item:
		bin = get_bin(args.get("item_code"), args.get("warehouse"))
		bin.update_stock(args, allow_negative_stock, via_landed_cost_voucher)
		return bin
	else:
		dataent.msgprint(_("Item {0} ignored since it is not a stock item").format(args.get("item_code")))
...
def get_avg_purchase_rate(serial_nos):
	...
	return flt(dataent.db.sql("""select avg(purchase_rate) from `tabSerial No`
		where name in (%s)""" % ", ".join(["%s"] * len(serial_nos)),
		tuple(serial_nos))[0][0])

def get_valuation_method(item_code):
	"""get valuation method from item or default"""
	val_method = dataent.db.get_value('Item', item_code, 'valuation_method')
	if not val_method:
		val_method = dataent.db.get_value("Stock Settings", None, "valuation_method") or "FIFO"
	return val_method
	...
	for val in serial_nos:
		if val:
			...
			if val in valid_serial_nos:
				dataent.throw(_("Serial number {0} entered more than once").format(val))
			else:
				valid_serial_nos.append(val)

	if qty and len(valid_serial_nos) != abs(qty):
		dataent.throw(_("{0} valid serial nos for Item {1}").format(abs(qty), item_code))

	return valid_serial_nos

def validate_warehouse_company(warehouse, company):
	warehouse_company = dataent.db.get_value("Warehouse", warehouse, "company")
	if warehouse_company and warehouse_company != company:
		dataent.throw(_("Warehouse {0} does not belong to company {1}").format(warehouse, company),
			InvalidWarehouseCompany)

def is_group_warehouse(warehouse):
	if dataent.db.get_value("Warehouse", warehouse, "is_group"):
		dataent.throw(_("Group node warehouse is not allowed to select for transactions"))
