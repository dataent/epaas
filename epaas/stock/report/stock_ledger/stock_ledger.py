...
def get_stock_ledger_entries(filters, items):
	...
	if items:
		item_conditions_sql = 'and sle.item_code in ({})'\
			.format(', '.join(['"' + dataent.db.escape(i) + '"' for i in items]))

	return dataent.db.sql("""select concat_ws(" ", posting_date, posting_time) as date,
...
def get_warehouse_condition(warehouse):
	warehouse_details = dataent.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
...
def get_item_group_condition(item_group):
	item_group_details = dataent.db.get_value("Item Group", item_group, ["lft", "rgt"], as_dict=1)
