...
dataent.query_reports["Total Stock Summary"] = {
	"filters": [
		{
			"fieldname":"group_by",
			"label": __("Group By"),
			...
			"change": function() {
				let group_by = dataent.query_report.get_filter_value("group_by")
				let company_filter = dataent.query_report.get_filter("company")
				if (group_by == "Company") {
					company_filter.df.reqd = 0;
					company_filter.df.hidden = 1;
					dataent.query_report.set_filter_value("company", "");
