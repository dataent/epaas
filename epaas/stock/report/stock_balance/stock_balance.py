...
from dataent.utils import flt, cint, getdate, now
from epaas.stock.utils import update_included_uom_in_report
from epaas.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def get_conditions(filters):
	...
	if not filters.get("from_date"):
		dataent.throw(_("'From Date' is required"))

	if filters.get("to_date"):
		conditions += " and sle.posting_date <= '%s'" % dataent.db.escape(filters.get("to_date"))
	else:
		dataent.throw(_("'To Date' is required"))

	if filters.get("warehouse"):
		warehouse_details = dataent.db.get_value("Warehouse",
...
def get_stock_ledger_entries(filters, items):
	...
	if items:
		item_conditions_sql = ' and sle.item_code in ({})'\
			.format(', '.join(['"' + dataent.db.escape(i, percent=False) + '"' for i in items]))
...
def filter_items_with_no_transactions(iwb_map):
	for (company, item, warehouse) in sorted(iwb_map):
		...
		float_precision = cint(dataent.db.get_default("float_precision")) or 3
...
def get_items(filters):
	...
	items = []
	if conditions:
		items = dataent.db.sql_list("""select name from `tabItem` item where {}"""
...
def get_item_details(items, sle, filters):
	...
	if filters.get("include_uom"):
		...
		cf_join = "left join `tabUOM Conversion Detail` ucd on ucd.parent=item.name and ucd.uom='%s'" \
			% dataent.db.escape(filters.get("include_uom"))

	item_codes = ', '.join(['"' + dataent.db.escape(i, percent=False) + '"' for i in items])
	res = dataent.db.sql("""
		select
...
def get_item_reorder_details(items):
	item_reorder_details = dataent._dict()

	if items:
		item_reorder_details = dataent.db.sql("""
			select parent, warehouse, warehouse_reorder_qty, warehouse_reorder_level
			from `tabItem Reorder`
			where parent in ({0})
		""".format(', '.join(['"' + dataent.db.escape(i, percent=False) + '"' for i in items])), as_dict=1)

	return dict((d.parent + d.warehouse, d) for d in item_reorder_details)

def validate_filters(filters):
	if not (filters.get("item_code") or filters.get("warehouse")):
		sle_count = flt(dataent.db.sql("""select count(name) from `tabStock Ledger Entry`""")[0][0])
		if sle_count > 500000:
			dataent.throw(_("Please set filter based on Item or Warehouse"))

def get_variants_attributes():
	'''Return all item variant attributes.'''
	return [i.name for i in dataent.get_all('Item Attribute')]

def get_variant_values_for(items):
	'''Returns variant values for items.'''
	attribute_map = {}
	for attr in dataent.db.sql('''select parent, attribute, attribute_value
