...
from dataent.utils import date_diff
...
class DelayedItemReport(object):
	...
	def get_data(self, consolidated=False):
		...
		if self.filters.get("item_group"):
			conditions += " and `tab%s`.item_group = '%s'" % (child_doc,
				dataent.db.escape(self.filters.get("item_group")))

		for field in ["customer", "customer_group", "company"]:
			if self.filters.get(field):
				conditions += " and `tab%s`.%s = '%s'" % (doctype,
					field, dataent.db.escape(self.filters.get(field)))
		...
		self.transactions = dataent.db.sql(""" SELECT `tab{child_doc}`.item_code, `tab{child_doc}`.item_name,
	...
	def filter_transactions_data(self, consolidated=False):
		...
		so_data = {}
		for d in dataent.get_all(doctype, filters = filters,
