...
from dataent.utils import getdate, flt
...
def get_item_info(filters):
	...
	return dataent.db.sql("""select name, item_name, description, brand, item_group,
		safety_stock, lead_time_days from `tabItem` item where {}"""
		.format(" and ".join(conditions)), filters, as_dict=1)


def get_consumed_items(condition):
	cn_items = dataent.db.sql("""select se_item.item_code,
...
def get_delivered_items(condition):
	dn_items = dataent.db.sql("""select dn_item.item_code, sum(dn_item.stock_qty) as dn_qty
		from `tabDelivery Note` dn, `tabDelivery Note Item` dn_item
		where dn.name = dn_item.parent and dn.docstatus = 1 %s
		group by dn_item.item_code""" % (condition), as_dict=1)

	si_items = dataent.db.sql("""select si_item.item_code, sum(si_item.stock_qty) as si_qty
...
def get_condition(filters):
	...
	if filters.get("from_date") and filters.get("to_date"):
		...
		dataent.throw(_("From and To dates required"))
	return conditions
