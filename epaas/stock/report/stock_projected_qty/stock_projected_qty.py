...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		company = warehouse_company.setdefault(bin.warehouse,
			dataent.db.get_value("Warehouse", bin.warehouse, "company"))
...
def get_bin_list(filters):
	...
	if filters.warehouse:
		warehouse_details = dataent.db.get_value("Warehouse", filters.warehouse, ["lft", "rgt"], as_dict=1)
	...
	bin_list = dataent.db.sql("""select item_code, warehouse, actual_qty, planned_qty, indented_qty,
...
def get_item_map(item_code, include_uom):
	...
	condition = ""
	if item_code:
		condition = 'and item_code = "{0}"'.format(dataent.db.escape(item_code, percent=False))
	...
	condition = ""
	if item_code:
		condition = 'where parent="{0}"'.format(dataent.db.escape(item_code, percent=False))

	reorder_levels = dataent._dict()
	for ir in dataent.db.sql("""select * from `tabItem Reorder` {condition}""".format(condition=condition), as_dict=1):
		if ir.parent not in reorder_levels:
			reorder_levels[ir.parent] = []

		reorder_levels[ir.parent].append(ir)

	item_map = dataent._dict()
	for item in items:
