...
epaas.stock.ItemDashboard = Class.extend({
	init: function(opts) {
		$.extend(this, opts);
		this.make();
	},
	make: function() {
		...
		this.content = $(dataent.render_template('item_dashboard')).appendTo(this.parent);
		this.result = this.content.find('.result');

		// move
		this.content.on('click', '.btn-move', function() {
			epaas.stock.move_item(unescape($(this).attr('data-item')), $(this).attr('data-warehouse'),
				null, $(this).attr('data-actual_qty'), null, function() { me.refresh(); });
		});

		this.content.on('click', '.btn-add', function() {
			epaas.stock.move_item(unescape($(this).attr('data-item')), null, $(this).attr('data-warehouse'),
	...
	refresh: function() {
		...
		var me = this;
		dataent.call({
			method: 'epaas.stock.dashboard.item_dashboard.get_data',
			args: {
	...
	render: function(data) {
		...
		if (context.data.length > 0) {
			$(dataent.render_template('item_dashboard_list', context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data: function(data, max_count, show_item) {
		...
		var can_write = 0;
		if(dataent.boot.user.can_write.indexOf("Stock Entry")>=0){
			can_write = 1;
		}
...
epaas.stock.move_item = function(item, source, target, actual_qty, rate, callback) {
	...
	dialog.set_primary_action(__('Submit'), function() {
		...
		if(source && values.qty > actual_qty) {
			dataent.msgprint(__('Quantity must be less than or equal to {0}', [actual_qty]));
			return;
		}
		if(values.source === values.target) {
			dataent.msgprint(__('Source and target warehouse must be different'));
		}

		dataent.call({
			method: 'epaas.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry',
			args: values,
			freeze: true,
			callback: function(r) {
				dataent.show_alert(__('Stock Entry {0} created',
	...
	$('<p style="margin-left: 10px;"><a class="link-open text-muted small">'
		...
		.find('.link-open')
		.on('click', function() {
			dataent.model.with_doctype('Stock Entry', function() {
				var doc = dataent.model.get_new_doc('Stock Entry');
				doc.from_warehouse = dialog.get_value('source');
				doc.to_warehouse = dialog.get_value('target');
				var row = dataent.model.add_child(doc, 'items');
