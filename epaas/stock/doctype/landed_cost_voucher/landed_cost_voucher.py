...
class LandedCostVoucher(Document):
	def get_items_from_purchase_receipts(self):
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				pr_items = dataent.db.sql("""select pr_item.item_code, pr_item.description,
				...
				for d in pr_items:
					...
					item.cost_center = d.cost_center or \
						epaas.get_default_cost_center(self.company)
	...
	def check_mandatory(self):
		if not self.get("purchase_receipts"):
			dataent.throw(_("Please enter Receipt Document"))
	...
	def validate_purchase_receipts(self):
		...
		for d in self.get("purchase_receipts"):
			if dataent.db.get_value(d.receipt_document_type, d.receipt_document, "docstatus") != 1:
				dataent.throw(_("Receipt document must be submitted"))
			else:
				receipt_documents.append(d.receipt_document)

		for item in self.get("items"):
			if not item.receipt_document:
				dataent.throw(_("Item must be added using 'Get Items from Purchase Receipts' button"))
			elif item.receipt_document not in receipt_documents:
				dataent.throw(_("Item Row {0}: {1} {2} does not exist in above '{1}' table")
					.format(item.idx, item.receipt_document_type, item.receipt_document))

			if not item.cost_center:
				dataent.throw(_("Row {0}: Cost center is required for an item {1}")
	...
	def validate_applicable_charges_for_item(self):
		...
		if not total:
			dataent.throw(_("Total {0} for all items is zero, may be you should change 'Distribute Charges Based On'").format(based_on))

		total_applicable_charges = sum([flt(d.applicable_charges) for d in self.get("items")])

		precision = get_field_precision(dataent.get_meta("Landed Cost Item").get_field("applicable_charges"),
		...
		if abs(diff) < (2.0 / (10**precision)):
			...
			dataent.throw(_("Total Applicable Charges in Purchase Receipt Items table must be same as Total Taxes and Charges"))
	...
	def update_landed_cost(self):
		for d in self.get("purchase_receipts"):
			doc = dataent.get_doc(d.receipt_document_type, d.receipt_document)
	...
	def update_rate_in_serial_no(self, receipt_document):
		for item in receipt_document.get("items"):
			if item.serial_no:
				...
				if serial_nos:
					dataent.db.sql("update `tabSerial No` set purchase_rate=%s where name in ({0})"
