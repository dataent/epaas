...
class TestLandedCostVoucher(unittest.TestCase):
	def test_landed_cost_voucher(self):
		...
		last_sle = dataent.db.get_value("Stock Ledger Entry", {
		...
		pr_lc_value = dataent.db.get_value("Purchase Receipt Item", {"parent": pr.name}, "landed_cost_voucher_amount")
		self.assertEqual(pr_lc_value, 25.0)

		last_sle_after_landed_cost = dataent.db.get_value("Stock Ledger Entry", {
				"voucher_type": pr.doctype,
				"voucher_no": pr.name,
				"item_code": "_Test Item",
				"warehouse": "_Test Warehouse - _TC"
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = dataent.db.get_value("Purchase Invoice Item", {"parent": pi.name}, 
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		set_perpetual_inventory(1)
		dataent.db.sql("delete from `tabSerial No` where name in ('SN001', 'SN002', 'SN003', 'SN004', 'SN005')")
		...
		serial_no_rate = dataent.db.get_value("Serial No", "SN001", "purchase_rate")

		submit_landed_cost_voucher("Purchase Receipt", pr.name)

		serial_no = dataent.db.get_value("Serial No", "SN001",
...
def submit_landed_cost_voucher(receipt_document_type, receipt_document, charges=50):
	ref_doc = dataent.get_doc(receipt_document_type, receipt_document)
	
	lcv = dataent.new_doc("Landed Cost Voucher")
...
test_records = dataent.get_test_records('Landed Cost Voucher')
