...
dataent.ui.form.on("Item", {
	...
	refresh: function(frm) {
		if(frm.doc.is_stock_item) {
			frm.add_custom_button(__("Balance"), function() {
				...
				dataent.set_route("query-report", "Stock Balance");
			}, __("View"));
			...
			frm.add_custom_button(__("Projected"), function() {
				...
				dataent.set_route("query-report", "Stock Projected Qty");
			}, __("View"));
		}

		if(!frm.doc.is_fixed_asset) {
			epaas.item.make_dashboard(frm);
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(__("Show Variants"), function() {
				dataent.set_route("List", "Item", {"variant_of": frm.doc.name});
			}, __("View"));

			frm.add_custom_button(__("Variant Details Report"), function() {
				dataent.set_route("query-report", "Item Variant Details", {"item": frm.doc.name});
			}, __("View"));

			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					epaas.item.show_single_variant_dialog(frm);
				}, __("Make"));
				frm.add_custom_button(__("Multiple Variants"), function() {
					epaas.item.show_multiple_variants_dialog(frm);
				}, __("Make"));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					epaas.item.show_modal_for_manufacturers(frm);
				}, __("Make"));
			}
		...
		if (dataent.defaults.get_default("item_naming_by")!="Naming Series" || frm.doc.variant_of) {
			frm.toggle_display("naming_series", false);
		} else {
			epaas.toggle_naming_series();
		}

		epaas.item.edit_prices_button(frm);
		epaas.item.toggle_attributes(frm);

		frm.add_custom_button(__('Duplicate'), function() {
			var new_item = dataent.model.copy_doc(frm.doc);
			...
			dataent.set_route('Form', 'Item', new_item.name);
		});

		if(frm.doc.has_variants) {
			frm.add_custom_button(__("Item Variant Settings"), function() {
				dataent.set_route("Form", "Item Variant Settings");
			}, __("View"));
		}
	...
	validate: function(frm){
		epaas.item.weight_to_validate(frm);
	},
	...
	page_name: dataent.utils.warn_page_name_change,
...
dataent.ui.form.on('Item Reorder', {
...
dataent.ui.form.on('Item Customer Detail', {
	customer_items_add: function(frm, cdt, cdn) {
		dataent.model.set_value(cdt, cdn, 'customer_group', "");
	},
	...
	customer_group: function(frm, cdt, cdn) {
		if(set_customer_group(frm, cdt, cdn)){
			dataent.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	}
});
...
var set_customer_group = function(frm, cdt, cdn) {
	...
	dataent.model.with_doc("Customer", row.customer_name, function() {
		var customer = dataent.model.get_doc("Customer", row.customer_name);
...
$.extend(epaas.item, {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			dataent.require('assets/js/item-dashboard.min.js', function() {
				var section = frm.dashboard.add_section('<h5 style="margin-top: 0px;">\
					<a href="#stock-balance">' + __("Stock Levels") + '</a></h5>');
				epaas.item.item_dashboard = new epaas.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name
				});
				epaas.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function(frm) {
		frm.add_custom_button(__("Add / Edit Prices"), function() {
			dataent.set_route("List", "Item Price", {"item_code": frm.doc.name});
		}, __("View"));
	},

	weight_to_validate: function(frm){
		if((frm.doc.nett_weight || frm.doc.gross_weight) && !frm.doc.weight_uom) {
			dataent.msgprint(__('Weight is mentioned,\nPlease mention "Weight UOM" too'));
			dataent.validated = 0;
		}
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Make'), function() {
			...
			dataent.call({
				method: "epaas.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new dataent.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__("Make Variants"), () => {
				...
				dataent.call({
					method: "epaas.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find('.form-column'))
				.find('.dataent-control')).css('margin-bottom', '0px');
			...
			dataent.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						dataent.msgprint_dialog = dataent.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a href="#Form/Item/%(item_encoded)s" class="strong variant-click">%(item)s</a>', {
								item_encoded: encodeURIComponent(variant),
								item: variant
							})]
						));
						dataent.msgprint_dialog.hide_on_page_refresh = true;
						dataent.msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							d.hide();
						});
					} else {
						d.hide();
						dataent.call({
							method: "epaas.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					dataent.call({
						method: "epaas.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
dataent.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			dataent.call({
				method: "epaas.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
				...
				callback: function(r) {
					if (!r.exc && r.message) {
						dataent.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				}
			});
		}
