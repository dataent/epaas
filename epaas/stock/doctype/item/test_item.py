...
from epaas.controllers.item_variant import (create_variant, ItemVariantExistsError,
	InvalidItemAttributeValueError, get_variant)
from epaas.stock.doctype.item.item import StockExistsForTemplate, InvalidBarcode
from epaas.stock.doctype.item.item import get_uom_conv_factor
...
def make_item(item_code, properties=None):
	if dataent.db.exists("Item", item_code):
		return dataent.get_doc("Item", item_code)
...
class TestItem(unittest.TestCase):
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in dataent.db.get_all('Item', filters={'variant_of':
	...
	def test_item_merging(self):
		...
		self.assertFalse(dataent.db.exists("Item", "Test Item for Merging 1"))

		self.assertTrue(dataent.db.get_value("Bin",
	...
	def test_uom_conversion_factor(self):
		if dataent.db.exists('Item', 'Test Item UOM'):
			dataent.delete_doc('Item', 'Test Item UOM')
	...
	def test_item_variant_by_manufacturer(self):
		...
		if dataent.db.exists('Item', '_Test Variant Mfg'):
			dataent.delete_doc('Item', '_Test Variant Mfg')
		if dataent.db.exists('Item', '_Test Variant Mfg-1'):
			dataent.delete_doc('Item', '_Test Variant Mfg-1')
		if dataent.db.exists('Manufacturer', 'MSG1'):
			dataent.delete_doc('Manufacturer', 'MSG1')

		template = dataent.get_doc(dict(
			doctype='Item',
		...
		manufacturer = dataent.get_doc(dict(
	...
	def test_stock_exists_against_template_item(self):
		stock_item = dataent.get_all('Stock Ledger Entry', fields = ["item_code"], limit=1)
		if stock_item:
			item_code = stock_item[0].item_code

			item_doc = dataent.get_doc('Item', item_code)
			item_doc.has_variants = 1
			self.assertRaises(StockExistsForTemplate, item_doc.save)

	def test_add_item_barcode(self):
		# Clean up
		dataent.db.sql("""delete from `tabItem Barcode`""")
		item_code = "Test Item Barcode"
		if dataent.db.exists("Item", item_code):
			dataent.delete_doc("Item", item_code)
		...
		self.assertRaises(dataent.DuplicateEntryError, item_doc.save)
...
def make_item_variant():
	if not dataent.db.exists("Item", "_Test Variant Item-S"):
		variant = create_variant("_Test Variant Item", """{"Test Size": "Small"}""")
		variant.item_code = "_Test Variant Item-S"
		variant.item_name = "_Test Variant Item-S"
		variant.save()

test_records = dataent.get_test_records('Item')
