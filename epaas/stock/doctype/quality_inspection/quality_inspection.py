...
from epaas.stock.doctype.quality_inspection_template.quality_inspection_template \
...
class QualityInspection(Document):
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = dataent.db.get_value('Item',
	...
	def get_quality_inspection_template(self):
		template = ''
		if self.bom_no:
			template = dataent.db.get_value('BOM', self.bom_no, 'quality_inspection_template')

		if not template:
			template = dataent.db.get_value('BOM', self.item_code, 'quality_inspection_template')
	...
	def update_qc_reference(self):
		...
		if self.reference_type and self.reference_name:
			dataent.db.sql("""update `tab{child_doc}` t1, `tab{parent_doc}` t2
