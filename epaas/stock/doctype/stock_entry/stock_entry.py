...
from dataent.utils import cstr, cint, flt, comma_or, getdate, nowdate, formatdate, format_time
from epaas.stock.utils import get_incoming_rate
from epaas.stock.stock_ledger import get_previous_sle, NegativeStockError, get_valuation_rate
from epaas.stock.get_item_details import get_bin_details, get_default_cost_center, get_conversion_factor, get_reserved_qty_for_so
from epaas.setup.doctype.item_group.item_group import get_item_group_defaults
from epaas.stock.doctype.batch.batch import get_batch_no, set_batch_nos, get_batch_qty
from epaas.stock.doctype.item.item import get_item_defaults
from epaas.manufacturing.doctype.bom.bom import validate_bom_no, add_additional_cost
from epaas.stock.utils import get_bin
from epaas.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit, get_serial_nos
from epaas.stock.doctype.stock_reconciliation.stock_reconciliation import OpeningEntryAccountError

import json

from six import string_types, itervalues, iteritems

class IncorrectValuationRateError(dataent.ValidationError): pass
class DuplicateEntryForWorkOrderError(dataent.ValidationError): pass
class OperationsNotCompleteError(dataent.ValidationError): pass
class MaxSampleAlreadyRetainedError(dataent.ValidationError): pass

from epaas.controllers.stock_controller import StockController
...
class StockEntry(StockController):
	...
	def validate(self):
		self.pro_doc = dataent._dict()
		if self.work_order:
			self.pro_doc = dataent.get_doc('Work Order', self.work_order)
	...
	def set_job_card_data(self):
		if self.job_card and not self.work_order:
			data = dataent.db.get_value('Job Card',
	...
	def validate_work_order_status(self):
		pro_doc = dataent.get_doc("Work Order", self.work_order)
		if pro_doc.status == 'Completed':
			dataent.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			dataent.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))

		if self.job_card and self.purpose != 'Material Transfer for Manufacture':
			dataent.throw(_("For job card {0}, you can only make the 'Material Transfer for Manufacture' type stock entry")
				.format(self.job_card))

	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty):
				dataent.throw(_("Row {0}: Qty is mandatory").format(item.idx))
			if not flt(item.conversion_factor):
				dataent.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
			item.transfer_qty = flt(flt(item.qty) * flt(item.conversion_factor),
				self.precision("transfer_qty", item))

	def update_cost_in_project(self):
		if self.project:
			amount = dataent.db.sql(""" select ifnull(sum(sed.amount), 0)
				from
			...
			additional_costs = dataent.db.sql(""" select ifnull(sum(sed.amount), 0)
				from
			...
			dataent.db.set_value('Project', self.project, 'total_consumed_material_cost', amount)
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			if item.item_code not in stock_items:
				dataent.throw(_("{0} is not a stock Item").format(item.item_code))

			item_details = self.get_item_details(dataent._dict(
			...
			if (self.purpose in ("Material Transfer", "Material Transfer for Manufacture")
				...
				dataent.throw(_("Row #{0}: Please specify Serial No for Item {1}").format(item.idx, item.item_code),
					dataent.MandatoryError)

	def validate_qty(self):
		manufacture_purpose = ["Manufacture", "Material Consumption for Manufacture"]

		if self.purpose in manufacture_purpose and self.work_order:
			if not dataent.get_value('Work Order', self.work_order, 'skip_transfer'):
				item_code = []
				for item in self.items:
					if cstr(item.t_warehouse) == '':
						req_items = dataent.get_all('Work Order Item',
										filters={'parent': self.work_order, 'item_code': item.item_code}, fields=["item_code"])

						transferred_materials = dataent.db.sql("""
									select
										sum(qty) as qty
	...
	def validate_fg_completed_qty(self):
		if self.purpose == "Manufacture" and self.work_order:
			production_item = dataent.get_value('Work Order', self.work_order, 'production_item')
			for item in self.items:
				if item.item_code == production_item and item.qty != self.fg_completed_qty:
					dataent.throw(_("Finished product quantity <b>{0}</b> and For Quantity <b>{1}</b> cannot be different")
	...
	def validate_difference_account(self):
		...
		for d in self.get("items"):
			if not d.expense_account:
				dataent.throw(_("Please enter Difference Account"))
			elif self.is_opening == "Yes" and dataent.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss":
				dataent.throw(_("Difference Account must be a Asset/Liability type account, since this Stock Entry is an Opening Entry"), OpeningEntryAccountError)
	...
	def validate_warehouse(self):
		...
		for d in self.get('items'):
			...
			if not (d.s_warehouse or d.t_warehouse):
				dataent.throw(_("Atleast one warehouse is mandatory"))

			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					d.s_warehouse = self.from_warehouse
				else:
					dataent.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					dataent.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
				...
				if validate_for_manufacture_repack:
					if d.bom_no:
						...
						if not d.t_warehouse:
							...
							dataent.throw(_("Target warehouse in row {0} must be same as Work Order").format(d.idx))
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and not self.purpose == "Material Transfer for Manufacture":
				dataent.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))
		...
		if self.purpose in ("Manufacture", "Material Transfer for Manufacture", "Material Consumption for Manufacture"):
			...
			if (self.purpose=="Manufacture" or self.purpose=="Material Consumption for Manufacture") \
				...
				if not self.fg_completed_qty:
					dataent.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def check_if_operations_completed(self):
		...
		prod_order = dataent.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(dataent.db.get_single_value("Manufacturing Settings",
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				job_card = dataent.db.get_value('Job Card', {'operation_id': d.name}, 'name')
				dataent.throw(_("Row #{0}: Operation {1} is not completed for {2} qty of finished goods in Work Order # {3}. Please update operation status via Job Card # {4}")
					.format(d.idx, d.operation, total_completed_qty, self.work_order, job_card), OperationsNotCompleteError)

	def check_duplicate_entry_for_work_order(self):
		other_ste = [t[0] for t in dataent.db.get_values("Stock Entry",  {
		...
		if other_ste:
			production_item, qty = dataent.db.get_value("Work Order",
				self.work_order, ["production_item", "qty"])
			args = other_ste + [production_item]
			fg_qty_already_entered = dataent.db.sql("""select sum(transfer_qty)
				from `tabStock Entry Detail`
				where parent in (%s)
					and item_code = %s
					and ifnull(s_warehouse,'')='' """ % (", ".join(["%s" * len(other_ste)]), "%s"), args)[0][0]
			if fg_qty_already_entered and fg_qty_already_entered >= qty:
				dataent.throw(_("Stock Entries already created for Work Order ")
	...
	def set_actual_qty(self):
		allow_negative_stock = cint(dataent.db.get_value("Stock Settings", None, "allow_negative_stock"))
		...
		for d in self.get('items'):
			...
			if d.docstatus==1 and d.s_warehouse and not allow_negative_stock and flt(d.actual_qty, d.precision("actual_qty")) < flt(d.transfer_qty, d.precision("actual_qty")):
				dataent.throw(_("Row {0}: Qty not available for {4} in warehouse {1} at posting time of the entry ({2} {3})").format(d.idx,
					dataent.bold(d.s_warehouse), formatdate(self.posting_date),
					format_time(self.posting_time), dataent.bold(d.item_code))
					+ '<br><br>' + _("Available qty is {0}, you need {1}").format(dataent.bold(d.actual_qty),
						dataent.bold(d.transfer_qty)),
					NegativeStockError, title=_('Insufficient Stock'))

	def set_serial_nos(self, work_order):
		previous_se = dataent.db.get_value("Stock Entry", {"work_order": work_order,
				"purpose": "Material Transfer for Manufacture"}, "name")

		for d in self.get('items'):
			transferred_serial_no = dataent.db.get_value("Stock Entry Detail",{"parent": previous_se,
		...
		for d in self.get('items'):
			...
			if d.bom_no:
				if not flt(d.basic_rate) and not d.allow_zero_valuation_rate and \
					getattr(self, "pro_doc", dataent._dict()).scrap_warehouse == d.t_warehouse:
				...
				if getattr(self, "pro_doc", dataent._dict()).scrap_warehouse == d.t_warehouse:
	...
	def set_basic_rate_for_finished_goods(self, raw_material_cost, scrap_material_cost):
		if self.purpose in ["Manufacture", "Repack"]:
			for d in self.get("items"):
				if d.transfer_qty and (d.bom_no or d.t_warehouse) and (getattr(self, "pro_doc", dataent._dict()).scrap_warehouse != d.t_warehouse):
	...
	def validate_purchase_order(self):
		...
		backflush_raw_materials_based_on = dataent.db.get_single_value("Buying Settings",
			"backflush_raw_materials_of_subcontract_based_on")

		if (self.purpose == "Subcontract" and self.purchase_order and
			backflush_raw_materials_based_on == 'BOM'):
			purchase_order = dataent.get_doc("Purchase Order", self.purchase_order)
			for se_item in self.items:
				...
				if not total_allowed:
					dataent.throw(_("Item {0} not found in 'Raw Materials Supplied' table in Purchase Order {1}")
						.format(se_item.item_code, self.purchase_order))
				total_supplied = dataent.db.sql("""select sum(transfer_qty)
				...
				if flt(total_supplied, precision) > flt(total_allowed, precision):
					dataent.throw(_("Row {0}# Item {1} cannot be transferred more than {2} against Purchase Order {3}")
						.format(se_item.idx, se_item.item_code, total_allowed, self.purchase_order))

	def validate_bom(self):
		for d in self.get('items'):
			if d.bom_no and (d.t_warehouse != getattr(self, "pro_doc", dataent._dict()).scrap_warehouse):
	...
	def validate_finished_goods(self):
		...
		for d in self.get('items'):
			if self.purpose != "Subcontract" and d.bom_no and flt(d.transfer_qty) > flt(self.fg_completed_qty) and (d.t_warehouse != getattr(self, "pro_doc", dataent._dict()).scrap_warehouse):
				dataent.throw(_("Quantity in row {0} ({1}) must be same as manufactured quantity {2}"). \
		...
		if self.work_order and self.purpose == "Manufacture":
			production_item, wo_qty = dataent.db.get_value("Work Order",
				self.work_order, ["production_item", "qty"])

			allowed_qty = wo_qty + (allowance_percentage/100 * wo_qty)
			if self.fg_completed_qty > allowed_qty:
				dataent.throw(_("For quantity {0} should not be grater than work order quantity {1}")
					.format(flt(self.fg_completed_qty), wo_qty))

			if production_item not in items_with_target_warehouse:
				dataent.throw(_("Finished Item {0} must be entered for Manufacture type entry")
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			if flt(pro_doc.docstatus) != 1:
				dataent.throw(_("Work Order {0} must be submitted").format(self.work_order))

			if pro_doc.status == 'Stopped':
				dataent.throw(_("Transaction not allowed against stopped Work Order {0}").format(self.work_order))

		if self.job_card:
			job_doc = dataent.get_doc('Job Card', self.job_card)
	...
	def get_item_details(self, args=None, for_update=False):
		item = dataent.db.sql("""select i.name, i.stock_uom, i.description, i.image, i.item_name, i.item_group,
		...
		if self.purpose == 'Material Issue':
			ret["expense_account"] = (item.get("expense_account") or
				...
				dataent.get_cached_value('Company',  self.company,  "default_expense_account"))

		for company_field, field in {'stock_adjustment_account': 'expense_account',
			'cost_center': 'cost_center'}.items():
			if not ret.get(field):
				ret[field] = dataent.get_cached_value('Company',  self.company,  company_field)
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			dataent.throw(_("Posting date and posting time is mandatory"))
		...
		if self.bom_no:
			...
			if self.purpose in ["Material Issue", "Material Transfer", "Manufacture", "Repack",
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and not self.pro_doc.skip_transfer and dataent.db.get_single_value("Manufacturing Settings",
					...
					dataent.db.get_single_value("Manufacturing Settings", "backflush_raw_materials_based_on")== "BOM" and \
					dataent.db.get_single_value("Manufacturing Settings", "material_consumption")== 1:
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						dataent.throw(_("Manufacturing Quantity is mandatory"))
					...
					if self.purchase_order and self.purpose == "Subcontract":
						#Get PO Supplied Items Details
						item_wh = dataent._dict(dataent.db.sql("""
			...
			if self.work_order and self.purpose == "Manufacture":
				...
				work_order = dataent.get_doc('Work Order', self.work_order)
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = dataent.db.get_value("BOM", self.bom_no, "item")
	...
	def get_bom_raw_materials(self, qty):
		...
		for item in itervalues(item_dict):
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = dataent.db.get_value('Work Order',
	...
	def get_unconsumed_raw_materials(self):
		wo = dataent.get_doc("Work Order", self.work_order)
		wo_items = dataent.get_all('Work Order Item',
	...
	def get_transfered_raw_materials(self):
		...
		materials_already_backflushed = dataent.db.sql("""
			select
		...
		po_qty = dataent.db.sql("""select qty, produced_qty, material_transferred_for_manufacturing from
		...
		for item in transferred_materials:
			...
			if not req_items:
				dataent.msgprint(_("Did not found transfered item {0} in Work Order {1}, the item not added in Stock Entry")
	...
	def get_pending_raw_materials(self):
		...
		if not len(item_dict):
			dataent.msgprint(_("""All items have already been transferred for this Work Order."""))

		return item_dict

	def get_pro_order_required_items(self):
		item_dict = dataent._dict()
		pro_order = dataent.get_doc("Work Order", self.work_order)
		if not dataent.db.get_value("Warehouse", pro_order.wip_warehouse, "is_group"):
		...
		for d in pro_order.get("required_items"):
			if (flt(d.required_qty) > flt(d.transferred_qty) and
				...
				item_row = d.as_dict()
				if d.source_warehouse and not dataent.db.get_value("Warehouse", d.source_warehouse, "is_group"):
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		...
		for d in item_dict:
			stock_uom = item_dict[d].get("stock_uom") or dataent.db.get_value("Item", d, "stock_uom")
	...
	def validate_with_material_request(self):
		for item in self.get("items"):
			if item.material_request:
				mreq_item = dataent.db.get_value("Material Request Item",
					{"name": item.material_request_item, "parent": item.material_request},
					["item_code", "warehouse", "idx"], as_dict=True)
				if mreq_item.item_code != item.item_code or \
				mreq_item.warehouse != (item.s_warehouse if self.purpose== "Material Issue" else item.t_warehouse):
					dataent.throw(_("Item or Warehouse for row {0} does not match Material Request").format(item.idx),
						dataent.MappingMismatchError)

	def validate_batch(self):
		if self.purpose in ["Material Transfer for Manufacture", "Manufacture", "Repack", "Subcontract"]:
			for item in self.get("items"):
				if item.batch_no:
					disabled = dataent.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = dataent.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
							if getdate(self.posting_date) > getdate(expiry_date):
								dataent.throw(_("Batch {0} of Item {1} has expired.")
									.format(item.batch_no, item.item_code))
					else:
						dataent.throw(_("Batch {0} of Item {1} is disabled.")
							.format(item.batch_no, item.item_code))
	...
	def update_so_in_serial_number(self):
		so_name, item_code = dataent.db.get_value("Work Order", self.work_order, ["sales_order", "production_item"])
		if so_name and item_code:
			qty_to_reserve = get_reserved_qty_for_so(so_name, item_code)
			if qty_to_reserve:
				reserved_qty = dataent.db.sql("""select count(name) from `tabSerial No` where item_code=%s and
					...
					for item in self.items:
						if item.item_code == item_code:
							...
							for serial_no in serial_nos:
								if qty_to_reserve > 0:
									dataent.db.set_value("Serial No", serial_no, "sales_order", so_name)
									qty_to_reserve -=1
		...
		for item in self.items:
			if item.s_warehouse and not item.t_warehouse and item.serial_no:
				for sr in get_serial_nos(item.serial_no):
					sales_order = dataent.db.get_value("Serial No", sr, "sales_order")
					if sales_order:
						dataent.throw(_("Item {0} (Serial No: {1}) cannot be consumed as is reserverd\
...
def move_sample_to_retention_warehouse(company, items):
	...
	retention_warehouse = dataent.db.get_single_value('Stock Settings', 'sample_retention_warehouse')
	stock_entry = dataent.new_doc("Stock Entry")
...
def get_work_order_details(work_order):
	work_order = dataent.get_doc("Work Order", work_order)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if not operating_cost_per_unit and bom_no:
		bom = dataent.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_used_alternative_items(purchase_order=None, work_order=None):
	...
	data = dataent.db.sql(""" select sted.original_item, sted.uom, sted.conversion_factor,
...
def get_uom_details(item_code, uom, qty):
	...
	if not conversion_factor:
		dataent.msgprint(_("UOM coversion factor required for UOM: {0} in Item: {1}")
...
def get_expired_batch_items():
	return dataent.db.sql("""select b.item, sum(sle.actual_qty) as qty, sle.batch_no, sle.warehouse, sle.stock_uom\
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no = None):
	if cint(qty) < cint(sample_quantity):
		dataent.throw(_("Sample quantity {0} cannot be more than received quantity {1}").format(sample_quantity, qty))
	retention_warehouse = dataent.db.get_single_value('Stock Settings', 'sample_retention_warehouse')
	retainted_qty = 0
	if batch_no:
		retainted_qty = get_batch_qty(batch_no, retention_warehouse, item_code)
	max_retain_qty = dataent.get_value('Item', item_code, 'sample_quantity')
	if retainted_qty >= max_retain_qty:
		dataent.msgprint(_("Maximum Samples - {0} have already been retained for Batch {1} and Item {2} in Batch {3}.").
	...
	if cint(sample_quantity) > cint(qty_diff):
		dataent.msgprint(_("Maximum Samples - {0} can be retained for Batch {1} and Item {2}.").
