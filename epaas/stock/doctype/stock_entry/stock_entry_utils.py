...
def make_stock_entry(**args):
	...
	if not args.company:
		if args.source:
			args.company = dataent.db.get_value('Warehouse', args.source, 'company')
		elif args.target:
			args.company = dataent.db.get_value('Warehouse', args.target, 'company')
	...
	s.company = args.company or epaas.get_default_company()
	s.purchase_receipt_no = args.purchase_receipt_no
	s.delivery_note_no = args.delivery_note_no
	s.sales_invoice_no = args.sales_invoice_no
	s.is_opening = args.is_opening or "No"
	if not args.cost_center:
		args.cost_center = dataent.get_value('Company', s.company, 'cost_center')

	if not args.expense_account and s.is_opening == "No":
		args.expense_account = dataent.get_value('Company', s.company, 'stock_adjustment_account')
	...
	if not args.serial_no and args.qty and args.batch_no:
		serial_number_list = dataent.get_list(
