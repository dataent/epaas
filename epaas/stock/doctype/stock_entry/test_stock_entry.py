...
from dataent.utils import flt, nowdate, nowtime
from epaas.stock.doctype.serial_no.serial_no import *
from epaas import set_perpetual_inventory
from epaas.stock.doctype.stock_ledger_entry.stock_ledger_entry import StockFreezeError
...
from epaas.stock.doctype.item.test_item import set_item_variant_settings, make_item_variant, create_item
from epaas.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from epaas.accounts.doctype.account.test_account import get_inventory_account
from epaas.stock.doctype.stock_entry.stock_entry import move_sample_to_retention_warehouse
...
def get_sle(**args):
	...
	return dataent.db.sql("""select * from `tabStock Ledger Entry` %s
...
class TestStockEntry(unittest.TestCase):
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 20],[1, 30]], dataent.safe_eval(sle.stock_queue))
	...
	def _test_auto_material_request(self, item_code, material_request_type="Purchase", warehouse="_Test Warehouse - _TC"):
		variant = dataent.get_doc("Item", item_code)

		projected_qty, actual_qty = dataent.db.get_value("Bin", {"item_code": item_code,
		...
		projected_qty = dataent.db.get_value("Bin", {"item_code": item_code,
			"warehouse": warehouse}, "projected_qty") or 0

		dataent.db.set_value("Stock Settings", None, "auto_indent", 1)
		...
		from epaas.stock.reorder_item import reorder_item
		mr_list = reorder_item()

		dataent.db.set_value("Stock Settings", None, "auto_indent", 0)
	...
	def test_material_receipt_gl_entry(self):
		...
		self.assertFalse(dataent.db.sql("""select * from `tabStock Ledger Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mr.name))

		self.assertFalse(dataent.db.sql("""select * from `tabGL Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mr.name))
	...
	def test_material_issue_gl_entry(self):
		...
		stock_value_diff = abs(dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Stock Entry",
		...
		self.assertFalse(dataent.db.sql("""select name from `tabStock Ledger Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mi.name))

		self.assertFalse(dataent.db.sql("""select name from `tabGL Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mi.name))
	...
	def test_repack_with_additional_costs(self):
		...
		rm_stock_value_diff = abs(dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Stock Entry",
	...
	def check_stock_ledger_entries(self, voucher_type, voucher_no, expected_sle):
		...
		sle = dataent.db.sql("""select item_code, warehouse, actual_qty
	...
	def test_serial_no_transfer_in(self):
		...
		self.assertTrue(dataent.db.exists("Serial No", "ABCD"))
		self.assertTrue(dataent.db.exists("Serial No", "EFGH"))

		se.cancel()
		self.assertFalse(dataent.db.get_value("Serial No", "ABCD", "warehouse"))

	def test_serial_no_not_exists(self):
		dataent.db.sql("delete from `tabSerial No` where name in ('ABCD', 'EFGH')")
	...
	def test_serial_by_series(self):
		...
		self.assertTrue(dataent.db.exists("Serial No", serial_nos[0]))
		self.assertTrue(dataent.db.exists("Serial No", serial_nos[1]))

		return se, serial_nos

	def test_serial_item_error(self):
		se, serial_nos = self.test_serial_by_series()
		if not dataent.db.exists('Serial No', 'ABCD'):
	...
	def test_serial_move(self):
		...
		se.insert()
		se.submit()
		self.assertTrue(dataent.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC")

		se.cancel()
		self.assertTrue(dataent.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC")
	...
	def test_serial_cancel(self):
		...
		serial_no = get_serial_nos(se.get("items")[0].serial_no)[0]
		self.assertFalse(dataent.db.get_value("Serial No", serial_no, "warehouse"))

	def test_warehouse_company_validation(self):
		company = dataent.db.get_value('Warehouse', '_Test Warehouse 2 - _TC1', 'company')
		set_perpetual_inventory(0, company)
		dataent.get_doc("User", "test2@example.com")\
	...
	def test_warehouse_user(self):
		...
		test_user = dataent.get_doc("User", "test@example.com")
	...
	def test_freeze_stocks(self):
		dataent.db.set_value('Stock Settings', None,'stock_auth_role', '')

		# test freeze_stocks_upto
		dataent.db.set_value("Stock Settings", None, "stock_frozen_upto", add_days(nowdate(), 5))
		se = dataent.copy_doc(test_records[0]).insert()
		self.assertRaises(StockFreezeError, se.submit)

		dataent.db.set_value("Stock Settings", None, "stock_frozen_upto", '')

		# test freeze_stocks_upto_days
		dataent.db.set_value("Stock Settings", None, "stock_frozen_upto_days", 7)
		se = dataent.copy_doc(test_records[0])
		se.set_posting_time = 1
		se.posting_date = add_days(nowdate(), -15)
		se.insert()
		self.assertRaises(StockFreezeError, se.submit)
		dataent.db.set_value("Stock Settings", None, "stock_frozen_upto_days", 0)

	def test_work_order(self):
		from epaas.manufacturing.doctype.work_order.work_order \
			import make_stock_entry as _make_stock_entry
		bom_no, bom_operation_cost = dataent.db.get_value("BOM", {"item": "_Test FG Item 2",
			"is_default": 1, "docstatus": 1}, ["name", "operating_cost"])

		work_order = dataent.new_doc("Work Order")
		work_order.update({
	...
	def test_variant_work_order(self):
		bom_no = dataent.db.get_value("BOM", {"item": "_Test Variant Item",
		...
		stock_entry = dataent.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_retain_sample(self):
		...
		from epaas.stock.doctype.batch.batch import get_batch_qty

		create_warehouse("Test Warehouse for Sample Retention")
		dataent.db.set_value("Stock Settings", None, "sample_retention_warehouse", "Test Warehouse for Sample Retention - _TC")

		test_item_code = "Retain Sample Item"
		if not dataent.db.exists('Item', test_item_code):
		...
		receipt_entry = dataent.new_doc("Stock Entry")
		...
		retention_entry = dataent.new_doc("Stock Entry")
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(dataent.ValidationError, repack.submit)

	def test_material_consumption(self):
		from epaas.manufacturing.doctype.work_order.work_order \
			import make_stock_entry as _make_stock_entry
		bom_no = dataent.db.get_value("BOM", {"item": "_Test FG Item 2",
			"is_default": 1, "docstatus": 1})
		...
		stock_entry = dataent.get_doc(_make_stock_entry(work_order.name, "Material Consumption for Manufacture", 2))
		for d in stock_entry.get('items'):
...
test_records = dataent.get_test_records('Stock Entry')
