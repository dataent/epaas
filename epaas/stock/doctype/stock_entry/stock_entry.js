// Copyright (c) 2015, Dataent Technologies Pvt. Ltd. and Contributors // License: GNU General Public License v3. See license.txt

dataent.provide("epaas.stock");

dataent.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		dataent.db.get_value('Stock Settings', {name: 'Stock Settings'}, 'sample_retention_warehouse', (r) => {
		...
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if(!item.item_code) {
				dataent.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (in_list(["Material Transfer for Manufacture", "Manufacture", "Repack", "Subcontract"], doc.purpose)) {
					var filters = {
						'item_code': item.item_code,
						'posting_date': frm.doc.posting_date || dataent.datetime.nowdate()
					}
				} else {
					var filters = {
						'item_code': item.item_code
	...
	refresh: function(frm) {
		if(!frm.doc.docstatus) {
			...
			frm.add_custom_button(__('Make Material Request'), function() {
				dataent.model.with_doctype('Material Request', function() {
					var mr = dataent.model.get_new_doc('Material Request');
					var items = frm.get_field('items').grid.get_selected_children();
					if(!items.length) {
						items = frm.doc.items;
					}
					items.forEach(function(item) {
						var mr_item = dataent.model.add_child(mr, 'items');
						...
						mr_item.required_date = dataent.datetime.nowdate();
					});
					dataent.set_route('Form', 'Material Request', mr.name);
				});
			});
		}
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					epaas.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
		...
		if (frm.doc.docstatus===0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				epaas.utils.map_current_doc({
					method: "epaas.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				dataent.call({
					method: "epaas.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		dataent.call({
			method: "epaas.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
				&& frm.doc.purpose=="Material Consumption for Manufacture") {
				frm.set_value("purpose", 'Manufacture');
				dataent.throw(__('Material Consumption is not set in Manufacturing Settings.'));
			}
		});
	},
	...
	company: function(frm) {
		if(frm.doc.company) {
			var company_doc = dataent.get_doc(":Company", frm.doc.company);
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		dataent.call({
			method: "epaas.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe){
					dataent.model.set_value(cdt, cdn, "serial_no", r.message);
				}
	...
	set_basic_rate: function(frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			dataent.call({
				method: "epaas.stock.utils.get_incoming_rate",
				args: {
					args: args
				},
				callback: function(r) {
					dataent.model.set_value(cdt, cdn, 'basic_rate', (r.message || 0.0));
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			dataent.call({
				method: "epaas.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
	...
	calculate_amount: function(frm) {
		...
		const total_basic_amount = dataent.utils.sum(
	...
	calculate_total_additional_costs: function(frm) {
		const total_additional_costs = dataent.utils.sum(
	...
	source_warehouse_address: function(frm) {
		epaas.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		epaas.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	}
})

dataent.ui.form.on('Stock Entry Detail', {
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return dataent.call({
				method: "epaas.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
				...
				callback: function(r) {
					if(r.message) {
						dataent.model.set_value(cdt, cdn, r.message);
					}
				}
			});
		}
			...
			return dataent.call({
				...
				callback: function(r) {
					if(r.message) {
						...
						refresh_field("items");
						epaas.stock.select_batch_and_serial_no(frm, d);
					}
				}
			});
		}
	...
	expense_account: function(frm, cdt, cdn) {
		epaas.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		epaas.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
...
dataent.ui.form.on('Landed Cost Taxes and Charges', {
	amount: function(frm) {
		frm.events.calculate_amount(frm);
	}
});

epaas.stock.StockEntry = epaas.stock.StockController.extend({
	setup: function() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field('item_code').get_query = function() {
			return epaas.queries.item({is_stock_item: 1});
		};
		...
		this.frm.set_query("supplier_address", epaas.queries.address_query)
	},
	...
	refresh: function() {
		...
		if (this.frm.doc.docstatus===1 && epaas.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		epaas.hide_company();
		epaas.utils.add_item(this.frm);
	},

	scan_barcode: function() {
		let transaction_controller= new epaas.TransactionController({frm:this.frm});
	...
	set_default_account: function(callback) {
		...
		if(this.frm.doc.company && epaas.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "epaas.accounts.utils.get_company_default",
				args: {
	...
	clean_up: function() {
		...
		if(this.frm.doc.work_order &&
			...
			dataent.model.remove_from_locals("Work Order",
	...
	get_items: function() {
		...
		if(!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no)
			dataent.throw(__("BOM and Manufacturing Quantity are required"));
	...
	work_order: function() {
		...
		return dataent.call({
			method: "epaas.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button: function() {
		if(dataent.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(__("Excise Invoice"), function() {
				var excise = dataent.model.make_new_doc_and_get_name('Journal Entry');
				excise = locals['Journal Entry'][excise];
				excise.voucher_type = 'Excise Entry';
				dataent.set_route('Form', 'Journal Entry', excise.name);
			}, __("Make"));
	},
	...
	set_warehouse_if_different: function(fieldname, value, condition) {
		...
		for (var i=0, l=(this.frm.doc.items || []).length; i<l; i++) {
			...
			if (row[fieldname] != value) {
				...
				dataent.model.set_value(row.doctype, row.name, fieldname, value, "Link");
				changed = true;
			}
		}
		refresh_field("items");
	...
	supplier: function(doc) {
		epaas.utils.get_party_details(this.frm, null, null, null);
	}
});

epaas.stock.select_batch_and_serial_no = (frm, item) => {
...
$.extend(cur_frm.cscript, new epaas.stock.StockEntry({frm: cur_frm}));
