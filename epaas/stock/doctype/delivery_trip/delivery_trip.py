...
from dataent.utils import cint, get_datetime, get_link_to_form
...
class DeliveryTrip(Document):
	def __init__(self, *args, **kwargs):
		...
		self.default_distance_uom = dataent.db.get_single_value("Global Defaults", "default_distance_unit") or "Meter"
		self.uom_conversion_factor = dataent.db.get_value("UOM Conversion Factor",
	...
	def validate_stop_addresses(self):
		for stop in self.delivery_stops:
			if not stop.customer_address:
				stop.customer_address = get_address_display(dataent.get_doc("Address", stop.address).as_dict())
	...
	def update_delivery_notes(self, delete=False):
		...
		dataent.msgprint(_("Delivery Notes {0} updated".format(", ".join(delivery_notes))))
	...
	def process_route(self, optimize):
		...
		if not dataent.db.get_single_value("Google Maps Settings", "enabled"):
			dataent.throw(_("Cannot process route, since Google Maps Settings is disabled."))
		...
		for route in route_list:
			...
			if directions:
				...
				for leg in legs:
					...
					stop_delay = dataent.db.get_single_value("Delivery Settings", "stop_delay")
	...
	def form_route_list(self, optimize):
		...
		home_address = get_address_display(dataent.get_doc("Address", settings.home_address).as_dict())
...
def get_default_contact(out, name):
	contact_persons = dataent.db.sql(
		"""
			SELECT parent,
...
def get_default_address(out, name):
	shipping_addresses = dataent.db.sql(
		"""
			SELECT parent,
...
def notify_customers(delivery_trip):
	...
	if delivery_trip.driver:
		context.update(dataent.db.get_value("Driver", delivery_trip.driver, "cell_number", as_dict=1))

	email_recipients = []

	for stop in delivery_trip.delivery_stops:
		contact_info = dataent.db.get_value("Contact", stop.contact, ["first_name", "last_name", "email_id"], as_dict=1)

		context.update({"items": []})
		if stop.delivery_note:
			items = dataent.get_all("Delivery Note Item", filters={"parent": stop.delivery_note, "docstatus": 1}, fields=["*"])
		...
		if contact_info and contact_info.email_id:
			...
			dispatch_template_name = dataent.db.get_single_value("Delivery Settings", "dispatch_template")
			dispatch_template = dataent.get_doc("Email Template", dispatch_template_name)

			dataent.sendmail(recipients=contact_info.email_id,
				subject=dispatch_template.subject,
				message=dataent.render_template(dispatch_template.response, context),
	...
	if email_recipients:
		dataent.msgprint(_("Email sent to {0}").format(", ".join(email_recipients)))
...
def get_attachments(delivery_stop):
	if not (dataent.db.get_single_value("Delivery Settings", "send_with_attachment") and delivery_stop.delivery_note):
		return []

	dispatch_attachment = dataent.db.get_single_value("Delivery Settings", "dispatch_attachment")
	attachments = dataent.attach_print("Delivery Note", delivery_stop.delivery_note,
