...
from epaas.buying.doctype.purchase_order.purchase_order import make_purchase_receipt, make_rm_stock_entry
import unittest
...
class TestItemAlternative(unittest.TestCase):
	...
	def test_alternative_item_for_subcontract_rm(self):
		...
		reserved_qty_for_sub_contract = dataent.db.get_value('Bin',
			{'item_code': 'Test FG A RW 1', 'warehouse': '_Test Warehouse - _TC'}, 'reserved_qty_for_sub_contract')

		se = dataent.get_doc(make_rm_stock_entry(po.name, rm_item_string))
		se.to_warehouse = supplier_warehouse
		se.insert()

		doc = dataent.get_doc('Stock Entry', se.name)
		...
		doc.save()
		doc.submit()
		after_transfer_reserved_qty_for_sub_contract = dataent.db.get_value('Bin',
			{'item_code': 'Test FG A RW 1', 'warehouse': '_Test Warehouse - _TC'}, 'reserved_qty_for_sub_contract')
		...
		pr = dataent.get_doc('Purchase Receipt', pr.name)
		status = False
	...
	def test_alternative_item_for_production_rm(self):
		...
		reserved_qty_for_production = dataent.db.get_value('Bin',
			{'item_code': 'Test FG A RW 1', 'warehouse': '_Test Warehouse - _TC'}, 'reserved_qty_for_production')

		ste = dataent.get_doc(make_stock_entry(pro_order.name, "Material Transfer for Manufacture", 5))
		ste.insert()
		...
		ste.submit()
		reserved_qty_for_production_after_transfer = dataent.db.get_value('Bin',
			{'item_code': 'Test FG A RW 1', 'warehouse': '_Test Warehouse - _TC'}, 'reserved_qty_for_production')

		self.assertEqual(reserved_qty_for_production_after_transfer, flt(reserved_qty_for_production - 5))
		ste1 = dataent.get_doc(make_stock_entry(pro_order.name, "Manufacture", 5))
...
def make_items():
	...
	if dataent.db.exists('Item', 'Test FG A RW 1'):
		doc = dataent.get_doc('Item', 'Test FG A RW 1')
		doc.allow_alternative_item = 1
		doc.save()

	if dataent.db.exists('Item', 'Test Finished Goods - A'):
		doc = dataent.get_doc('Item', 'Test Finished Goods - A')
		doc.is_sub_contracted_item = 1
		doc.save()

	if not dataent.db.get_value('BOM',
		{'item': 'Test Finished Goods - A', 'docstatus': 1}):
		make_bom(item = 'Test Finished Goods - A', raw_materials = ['Test FG A RW 1', 'Test FG A RW 2'])

	if not dataent.db.get_value('Warehouse', {'warehouse_name': 'Test Supplier Warehouse'}):
		dataent.get_doc({
			'doctype': 'Warehouse',
