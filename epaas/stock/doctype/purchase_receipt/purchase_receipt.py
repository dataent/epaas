...
from dataent.utils import flt, cint, nowdate, getdate
...
class PurchaseReceipt(BuyingController):
		...
		if dataent.db.get_value("Buying Settings", None, "po_required") == 'Yes':
			for d in self.get('items'):
				if not d.purchase_order:
					dataent.throw(_("Purchase Order number required for Item {0}").format(d.item_code))

	def get_already_received_qty(self, po, po_detail):
		qty = dataent.db.sql("""select sum(qty) from `tabPurchase Receipt Item`
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = dataent.db.get_value("Purchase Order Item", po_detail,
			["qty", "warehouse"])
	...
	def check_next_docstatus(self):
		submit_rv = dataent.db.sql("""select t1.name
			...
			(self.name))
		if submit_rv:
			dataent.throw(_("Purchase Invoice {0} is already submitted").format(self.submit_rv[0][0]))
	...
	def on_cancel(self):
		...
		submitted = dataent.db.sql("""select t1.name
			from `tabPurchase Invoice` t1,`tabPurchase Invoice Item` t2
			where t1.name = t2.parent and t2.purchase_receipt = %s and t1.docstatus = 1""",
			self.name)
		if submitted:
			dataent.throw(_("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
	...
	def get_current_stock(self):
		for d in self.get('supplied_items'):
			if self.supplier_warehouse:
				bin = dataent.db.sql("select actual_qty from `tabBin` where item_code = %s and warehouse = %s", (d.rm_item_code, self.supplier_warehouse), as_dict = 1)
				d.current_stock = bin and flt(bin[0]['actual_qty']) or 0

	def get_gl_entries(self, warehouse_account=None):
		from epaas.accounts.general_ledger import process_gl_map
		...
		for d in self.get("items"):
			if d.item_code in stock_items and flt(d.valuation_rate) and flt(d.qty):
				if warehouse_account.get(d.warehouse):
					stock_value_diff = dataent.db.get_value("Stock Ledger Entry",
		...
		if negative_expense_to_be_booked and valuation_tax:
			...
			negative_expense_booked_in_pi = dataent.db.sql("""select name from `tabPurchase Invoice Item` pi
		...
		if warehouse_with_no_account:
			dataent.msgprint(_("No accounting entries for the following warehouses") + ": \n" +
	...
	def update_billing_status(self, update_modified=True):
		...
		for pr in set(updated_pr):
			pr_doc = self if (pr == self.name) else dataent.get_doc("Purchase Receipt", pr)
...
def update_billed_amount_based_on_po(po_detail, update_modified=True):
	...
	billed_against_po = dataent.db.sql("""select sum(amount) from `tabPurchase Invoice Item`
		where po_detail=%s and (pr_detail is null or pr_detail = '') and docstatus=1""", po_detail)
	billed_against_po = billed_against_po and billed_against_po[0][0] or 0

	# Get all Delivery Note Item rows against the Sales Order Item row
	pr_details = dataent.db.sql("""select pr_item.name, pr_item.amount, pr_item.parent
	...
	for pr_item in pr_details:
		...
		billed_amt_agianst_pr = dataent.db.sql("""select sum(amount) from `tabPurchase Invoice Item`
		...
		dataent.db.set_value("Purchase Receipt Item", pr_item.name, "billed_amt", billed_amt_agianst_pr, update_modified=update_modified)
...
def make_purchase_invoice(source_name, target_doc=None):
	...
	doc = dataent.get_doc('Purchase Receipt', source_name)
	returned_qty_map = get_returned_qty_map(source_name)
	invoiced_qty_map = get_invoiced_qty_map(source_name)

	def set_missing_values(source, target):
		if len(target.get("items")) == 0:
			dataent.throw(_("All items have already been invoiced"))

		doc = dataent.get_doc(target)
...
def get_invoiced_qty_map(purchase_receipt):
	...
	for pr_detail, qty in dataent.db.sql("""select pr_detail, qty from `tabPurchase Invoice Item`
...
def get_returned_qty_map(purchase_receipt):
	...
	returned_qty_map = dataent._dict(dataent.db.sql("""select pr_item.item_code, sum(abs(pr_item.qty)) as qty
		from `tabPurchase Receipt Item` pr_item, `tabPurchase Receipt` pr
...
def update_purchase_receipt_status(docname, status):
	pr = dataent.get_doc("Purchase Receipt", docname)
