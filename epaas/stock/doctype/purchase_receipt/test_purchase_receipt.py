...
from dataent.utils import cint, flt, cstr, today, random_string
...
from epaas.stock.doctype.serial_no.serial_no import SerialNoDuplicateError
...
class TestPurchaseReceipt(unittest.TestCase):
	def setUp(self):
		dataent.db.set_value("Buying Settings", None, "allow_multiple_items", 1)

	def test_make_purchase_invoice(self):
		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(dataent.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(dataent.ValidationError, dataent.get_doc(pi).submit)

	def test_purchase_receipt_no_gl_entry(self):
		company = dataent.db.get_value('Warehouse', '_Test Warehouse - _TC', 'company')
		set_perpetual_inventory(0, company)

		existing_bin_stock_value = dataent.db.get_value("Bin", {"item_code": "_Test Item",
			"warehouse": "_Test Warehouse - _TC"}, "stock_value")

		pr = make_purchase_receipt()

		stock_value_difference = dataent.db.get_value("Stock Ledger Entry",
		...
		current_bin_stock_value = dataent.db.get_value("Bin", {"item_code": "_Test Item",
			"warehouse": "_Test Warehouse - _TC"}, "stock_value")
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(epaas.is_perpetual_inventory_enabled(pr.company)), 1)
		pr.insert()
		pr.submit()
	...
	def test_serial_no_supplier(self):
		...
		self.assertEqual(dataent.db.get_value("Serial No", pr.get("items")[0].serial_no, "supplier"),
			pr.supplier)

		pr.cancel()
		self.assertFalse(dataent.db.get_value("Serial No", pr.get("items")[0].serial_no, "warehouse"))
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(dataent.db.get_value("Serial No", serial_no, "warehouse"),
	...
	def test_purchase_return(self):
		...
		# check sle
		outgoing_rate = dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_rejected_qty(self):
		...
		actual_qty = dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = dataent.get_doc("Serial No", serial_no)
	...
	def test_closed_purchase_receipt(self):
		from epaas.stock.doctype.purchase_receipt.purchase_receipt import update_purchase_receipt_status

		pr = make_purchase_receipt(do_not_submit=True)
		pr.submit()

		update_purchase_receipt_status(pr.name, "Closed")
		self.assertEqual(dataent.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_not_accept_duplicate_serial_no(self):
		...
		item_code = dataent.db.get_value('Item', {'has_serial_no': 1, 'is_fixed_asset': 0})
		if not item_code:
	...
	def test_serialized_asset_item(self):
		...
		asset = dataent.db.get_value('Asset', {'purchase_receipt': pr.name}, 'name')
		asset_movement = dataent.db.get_value('Asset Movement', {'reference_name': pr.name}, 'name')
		serial_nos = dataent.get_all('Serial No', {'asset': asset}, 'name')

		self.assertEquals(len(serial_nos), 3)

		location = dataent.db.get_value('Serial No', serial_nos[0].name, 'location')
		self.assertEquals(location, "Test Location")

		pr.cancel()
		serial_nos = dataent.get_all('Serial No', {'asset': asset}, 'name') or []
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		from epaas.buying.doctype.purchase_order.test_purchase_order import create_purchase_order, create_pr_against_po
...
def get_gl_entries(voucher_type, voucher_no):
	return dataent.db.sql("""select account, debit, credit, cost_center
...
def make_purchase_receipt(**args):
	...
	dataent.db.set_value("Buying Settings", None, "allow_multiple_items", 1)
	pr = dataent.new_doc("Purchase Receipt")
	...
	pr.append("items", {
		...
		"cost_center": args.cost_center or dataent.get_cached_value('Company',  pr.company,  'cost_center'),
