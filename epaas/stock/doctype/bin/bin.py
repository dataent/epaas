...
class Bin(Document):
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = dataent.get_cached_value('Item', self.item_code, 'stock_uom')
	...
	def update_reserved_qty_for_production(self):
		'''Update qty reserved for production from Production Item tables
			in open work orders'''
		self.reserved_qty_for_production = dataent.db.sql('''
	...
	def update_reserved_qty_for_sub_contracting(self):
		#reserved qty
		reserved_qty_for_sub_contract = dataent.db.sql('''
		...
		materials_transferred = dataent.db.sql("""
			select
...
def on_doctype_update():
	dataent.db.add_index("Bin", ["item_code", "warehouse"])
