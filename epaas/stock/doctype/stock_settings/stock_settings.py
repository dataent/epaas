...
class StockSettings(Document):
	def validate(self):
		...
		if submitted_stock_frozen > stock_frozen_limit:
			...
			dataent.msgprint (_("`Freeze Stocks Older Than` should be smaller than %d days.") %stock_frozen_limit)

		# show/hide barcode field
		for name in ["barcode", "barcodes", "scan_barcode"]:
			dataent.make_property_setter({'fieldname': name, 'property': 'hidden',
	...
	def cant_change_valuation_method(self):
		db_valuation_method = dataent.db.get_single_value("Stock Settings", "valuation_method")
		...
		if db_valuation_method and db_valuation_method != self.valuation_method:
			...
			sle = dataent.db.sql("""select name from `tabStock Ledger Entry` sle
				where exists(select name from tabItem
					where name=sle.item_code and (valuation_method is null or valuation_method='')) limit 1
			""")

			if sle:
				dataent.throw(_("Can't change valuation method, as there are transactions against some items which does not have it's own valuation method"))
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) \
			...
			# changed to text
			dataent.enqueue('epaas.stock.doctype.stock_settings.stock_settings.clean_all_descriptions', now=dataent.flags.in_test)


def clean_all_descriptions():
	for item in dataent.get_all('Item', ['name', 'description']):
		if item.description:
			clean_description = clean_html(item.description)
		if item.description != clean_description:
			dataent.db.set_value('Item', item.name, 'description', clean_description)
