...
from dataent.utils import cint, nowdate, nowtime, cstr, add_days, flt, today
...
from epaas.stock.doctype.delivery_note.delivery_note import make_sales_invoice, make_delivery_trip
from epaas.stock.doctype.stock_entry.test_stock_entry \
	import make_stock_entry, make_serialized_item, get_qty_after_transaction
from epaas.stock.doctype.serial_no.serial_no import get_serial_nos, SerialNoWarehouseError
from epaas.stock.doctype.stock_reconciliation.test_stock_reconciliation \
	import create_stock_reconciliation, set_valuation_method
from epaas.selling.doctype.sales_order.test_sales_order import make_sales_order, create_dn_against_so
...
class TestDeliveryNote(unittest.TestCase):
	def tearDown(self):
		...
		if not dataent.db.exists("Account", target_warehouse):
			...
			dataent.db.set_value('Warehouse', target_warehouse, 'account', account)

	def test_over_billing_against_dn(self):
		dataent.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		dn = create_delivery_note(do_not_submit=True)
		self.assertRaises(dataent.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(dataent.ValidationError, dataent.get_doc(si).insert)
	...
	def test_sales_return_for_non_bundled_items(self):
		...
		# outgoing_rate
		outgoing_rate = dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Delivery Note",
		...
		gle_warehouse_amount = dataent.db.get_value("GL Entry", {"voucher_type": "Delivery Note",
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = dataent.db.get_value("Stock Ledger Entry",
			{"voucher_type": "Delivery Note", "voucher_no": dn.name,
		...
		stock_value_difference = abs(dataent.db.sql("""select sum(stock_value_difference)
	...
	def test_closed_delivery_note(self):
		from epaas.stock.doctype.delivery_note.delivery_note import update_delivery_note_status

		dn = create_delivery_note(do_not_submit=True)
		dn.submit()

		update_delivery_note_status(dn.name, "Closed")
		self.assertEqual(dataent.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
