...
dataent.provide("epaas.stock.delivery_note");

dataent.ui.form.on("Delivery Note", {
	setup: function(frm) {
	...
	print_without_amount: function(frm) {
		epaas.stock.delivery_note.set_print_hide(frm.doc);
	}
});

dataent.ui.form.on("Delivery Note Item", {
...
epaas.stock.DeliveryNoteController = epaas.selling.SellingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc, dt, dn) {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					dataent.model.open_mapped_doc({
						method: "epaas.stock.doctype.delivery_note.delivery_note.make_packing_slip",
						frm: me.frm
					}) }, __("Make"));
			}
		...
		epaas.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_sales_return: function() {
		dataent.model.open_mapped_doc({
			method: "epaas.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	},
	...
	update_status: function(status) {
		...
		dataent.call({
			method:"epaas.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
$.extend(cur_frm.cscript, new epaas.stock.DeliveryNoteController({frm: cur_frm}));
...
dataent.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = epaas.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


epaas.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
	var dn_fields = dataent.meta.docfield_map['Delivery Note'];
	var dn_item_fields = dataent.meta.docfield_map['Delivery Note Item'];
