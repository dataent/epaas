...
def get_product_bundle_items(item_code):
	return dataent.db.sql("""select t1.item_code, t1.qty, t1.uom, t1.description
...
def get_bin_qty(item, warehouse):
	det = dataent.db.sql("""select actual_qty, projected_qty from `tabBin`
		where item_code = %s and warehouse = %s""", (item, warehouse), as_dict = 1)
	return det and det[0] or dataent._dict()
...
def make_packing_list(doc):
	...
	for d in doc.get("items"):
		if dataent.db.get_value("Product Bundle", {"new_item_code": d.item_code}):
...
def on_doctype_update():
	dataent.db.add_index("Packed Item", ["item_code", "warehouse"])