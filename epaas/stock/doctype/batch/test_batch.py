...
from epaas.stock.doctype.batch.batch import get_batch_qty, UnableToSelectBatchError, get_batch_no
...
class TestBatch(unittest.TestCase):
	...
	def test_item_has_batch_enabled(self):
		self.assertRaises(ValidationError, dataent.get_doc({
			"doctype": "Batch",
			"name": "_test Batch",
			"item": "_Test Item"
		}).save)
	...
	def test_delivery_note(self):
		...
		delivery_note = dataent.get_doc(dict(
	...
	def test_batch_split(self):
		...
		from epaas.stock.doctype.batch.batch import split_batch
	...
	def make_new_batch_and_entry(cls, item_name, batch_name, warehouse):
		...
		if not dataent.db.exists("Batch", batch_name):
			batch = dataent.get_doc(dict(
				doctype = 'Batch',
				item = item_name,
				batch_id = batch_name
	...
	def test_batch_name_with_naming_series(self):
		...
		if not use_naming_series:
			dataent.set_value('Stock Settings', 'Stock Settings', 'use_naming_series', 1)
		...
		if not use_naming_series:
			dataent.set_value('Stock Settings', 'Stock Settings', 'use_naming_series', 0)

	def make_new_batch(self, item_name, batch_id=None, do_not_insert=0):
		batch = dataent.new_doc('Batch')
