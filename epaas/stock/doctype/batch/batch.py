...
from dataent.model.naming import make_autoname, revert_series_if_last
from dataent.utils import flt, cint
from dataent.utils.jinja import render_template
from dataent.utils.data import add_days
from six import string_types

class UnableToSelectBatchError(dataent.ValidationError):
	pass
...
def get_name_from_hash():
	...
	temp = None
	while not temp:
		temp = dataent.generate_hash()[:7].upper()
		if dataent.db.exists('Batch', temp):
			temp = None
...
def batch_uses_naming_series():
	...
	:return: bool
	"""
	use_naming_series = cint(dataent.db.get_single_value('Stock Settings', 'use_naming_series'))
...
def _get_batch_prefix():
	...
	"""
	naming_series_prefix = dataent.db.get_single_value('Stock Settings', 'naming_series_prefix')
...
class Batch(Document):
	...
	def onload(self):
		self.image = dataent.db.get_value('Item', self.item, 'image')
	...
	def item_has_batch_enabled(self):
		if dataent.db.get_value("Item", self.item, "has_batch_no") == 0:
			dataent.throw(_("The selected item cannot have Batch"))
...
def get_batch_qty(batch_no=None, warehouse=None, item_code=None):
	...
	if batch_no and warehouse:
		out = float(dataent.db.sql("""select sum(actual_qty)
			from `tabStock Ledger Entry`
			where warehouse=%s and batch_no=%s""",
			(warehouse, batch_no))[0][0] or 0)

	if batch_no and not warehouse:
		out = dataent.db.sql('''select warehouse, sum(actual_qty) as qty
			from `tabStock Ledger Entry`
			where batch_no=%s
			group by warehouse''', batch_no, as_dict=1)

	if not batch_no and item_code and warehouse:
		out = dataent.db.sql('''select batch_no, sum(actual_qty) as qty
			from `tabStock Ledger Entry`
...
def get_batches_by_oldest(item_code, warehouse):
	...
	batches_dates = [[batch, dataent.get_value('Batch', batch.batch_no, 'expiry_date')] for batch in batches]
...
def split_batch(batch_no, item_code, warehouse, qty, new_batch_id=None):
	...
	batch = dataent.get_doc(dict(doctype='Batch', item=item_code, batch_id=new_batch_id)).insert()
	stock_entry = dataent.get_doc(dict(
		doctype='Stock Entry',
		purpose='Repack',
		items=[
			dict(
...
def set_batch_nos(doc, warehouse_field, throw=False):
	...
	for d in doc.items:
		...
		has_batch_no = dataent.db.get_value('Item', d.item_code, 'has_batch_no')
		...
		if has_batch_no and warehouse and qty > 0:
			if not d.batch_no:
				...
				if flt(batch_qty, d.precision("qty")) < flt(qty, d.precision("qty")):
					dataent.throw(_("Row #{0}: The batch {1} has only {2} qty. Please select another batch which has {3} qty available or split the row into multiple rows, to deliver/issue from multiple batches").format(d.idx, d.batch_no, batch_qty, qty))
