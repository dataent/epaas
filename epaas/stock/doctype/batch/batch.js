...
dataent.ui.form.on('Batch', {
	...
	item: (frm) => {
		// dataent.db.get_value('Item', {name: frm.doc.item}, 'has_expiry_date', (r) => {
		// 	frm.toggle_reqd('expiry_date', r.has_expiry_date);
		// });
		dataent.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
			if (r.has_expiry_date && r.shelf_life_in_days) {
				// Calculate expiry date based on shelf_life_in_days
				frm.set_value('expiry_date', dataent.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
			...
			dataent.call({
				...
				callback: (r) => {
					...
					rows.find('.btn-split').on('click', function() {
						...
						(data) => {
							dataent.call({
								method: 'epaas.stock.doctype.batch.batch.split_batch',
								args: {
									item_code: frm.doc.item,
									batch_no: frm.doc.name,
									qty: data.qty,
...
dataent.ui.form.on('Batch', 'manufacturing_date', function (frm){
	dataent.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
		if (r.has_expiry_date && r.shelf_life_in_days) {
			// Calculate expiry date based on shelf_life_in_days
			frm.set_value('expiry_date', dataent.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
