...
from epaas.stock.utils import get_stock_balance

class OpeningEntryAccountError(dataent.ValidationError): pass
class EmptyStockReconciliationItemsError(dataent.ValidationError): pass
...
class StockReconciliation(StockController):
	...
	def validate(self):
		if not self.expense_account:
			self.expense_account = dataent.get_cached_value('Company',  self.company,  "stock_adjustment_account")
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			dataent.throw(_("None of the items have any change in quantity or value."),
			...
			for i, item in enumerate(self.items):
				item.idx = i + 1
			dataent.msgprint(_("Removed items with no change in quantity or value."))
		...
		for row_num, row in enumerate(self.items):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = dataent.db.get_value("Item Price", {"item_code": row.item_code,
					...
					if buying_rate:
						...
						row.valuation_rate = dataent.get_value('Item', row.item_code, 'valuation_rate')
	...
	def validate_item(self, item_code, row_num):
		...
		try:
			...
			if item.has_serial_no == 1:
				raise dataent.ValidationError(_("Serialized Item {0} cannot be updated using Stock Reconciliation, please use Stock Entry").format(item_code))

			# item managed batch-wise not allowed
			if item.has_batch_no == 1:
				raise dataent.ValidationError(_("Batched Item {0} cannot be updated using Stock Reconciliation, instead use Stock Entry").format(item_code))
	...
	def update_stock_ledger(self):
		...
		for row in self.items:
			...
			if row.qty and not row.valuation_rate:
				dataent.throw(_("Valuation Rate required for Item in row {0}").format(row.idx))
	...
	def insert_entries(self, row):
		...
		args = dataent._dict({
			...
			"stock_uom": dataent.db.get_value("Item", row.item_code, "stock_uom"),
			"is_cancelled": "No",
	...
	def delete_and_repost_sle(self):
		...
		existing_entries = dataent.db.sql("""select distinct item_code, warehouse
			from `tabStock Ledger Entry` where voucher_type=%s and voucher_no=%s""",
			(self.doctype, self.name), as_dict=1)

		# delete entries
		dataent.db.sql("""delete from `tabStock Ledger Entry`
	...
	def validate_expense_account(self):
		...
		elif self.purpose == "Opening Stock" or not dataent.db.sql("""select name from `tabStock Ledger Entry` limit 1"""):
			if dataent.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				dataent.throw(_("Difference Account must be a Asset/Liability type account, since this Stock Reconciliation is an Opening Entry"), OpeningEntryAccountError)
...
def get_items(warehouse, posting_date, posting_time, company):
	...
	for d in set(items):
		...
		if dataent.db.get_value("Item", d[0], "disabled") == 0:
			res.append({
				"item_code": d[0],
				"warehouse": d[2],
				"qty": stock_bal[0],
...
def get_stock_balance_for(item_code, warehouse, posting_date, posting_time):
	dataent.has_permission("Stock Reconciliation", "write", throw = True)
...
def get_difference_account(purpose, company):
	if purpose == 'Stock Reconciliation':
		...
		account = dataent.db.get_value('Account', {'is_group': 0,
