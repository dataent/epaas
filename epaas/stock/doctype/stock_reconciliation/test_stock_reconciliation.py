...
from epaas.stock.stock_ledger import get_previous_sle, update_entries_after
from epaas.stock.doctype.stock_reconciliation.stock_reconciliation import EmptyStockReconciliationItemsError, get_items
	...
	def _test_reco_sle_gle(self, valuation_method):
		...
		for d in input_data:
			...
			if qty_after_transaction == last_sle.get("qty_after_transaction") \
				...
				# no gl entries
				self.assertTrue(dataent.db.get_value("Stock Ledger Entry",
					{"voucher_type": "Stock Reconciliation", "voucher_no": stock_reco.name}))
				self.assertFalse(get_stock_and_account_difference(["_Test Account Stock In Hand - _TC"]))

			stock_reco.cancel()

			self.assertFalse(dataent.db.get_value("Stock Ledger Entry",
				{"voucher_type": "Stock Reconciliation", "voucher_no": stock_reco.name}))
...
def set_valuation_method(item_code, valuation_method):
	dataent.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in dataent.get_all("Warehouse", filters={"company": "_Test Company"}, fields=["name", "is_group"]):
