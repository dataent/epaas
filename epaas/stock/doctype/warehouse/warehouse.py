...
from dataent.utils import cint, nowdate
from dataent import throw, _
from dataent.utils.nestedset import NestedSet
from epaas.stock import get_warehouse_account
...
class Warehouse(NestedSet):
	...
	def autoname(self):
		if self.company:
			suffix = " - " + dataent.get_cached_value('Company',  self.company,  "abbr")
	...
	def onload(self):
		...
		if self.company and cint(dataent.db.get_value("Company", self.company, "enable_perpetual_inventory")):
	...
	def on_trash(self):
		# delete bin
		bins = dataent.db.sql("select * from `tabBin` where warehouse = %s",
			self.name, as_dict=1)
		for d in bins:
			if d['actual_qty'] or d['reserved_qty'] or d['ordered_qty'] or \
				...
				dataent.db.sql("delete from `tabBin` where name = %s", d['name'])
	...
	def check_if_sle_exists(self):
		return dataent.db.sql("""select name from `tabStock Ledger Entry`
			where warehouse = %s limit 1""", self.name)

	def check_if_child_exists(self):
		return dataent.db.sql("""select name from `tabWarehouse`
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			if not dataent.db.exists("Warehouse", new_warehouse):
				dataent.throw(_("Warehouse {0} does not exist").format(new_warehouse))

			if self.company != dataent.db.get_value("Warehouse", new_warehouse, "company"):
	...
	def get_new_warehouse_name_without_abbr(self, name):
		company_abbr = dataent.get_cached_value('Company',  self.company,  "abbr")
		parts = name.rsplit(" - ", 1)
	...
	def recalculate_bin_qty(self, new_name):
		...
		repost_stock_for_items = dataent.db.sql_list("""select distinct item_code
			from tabBin where warehouse=%s""", new_name)

		# Delete all existing bins to avoid duplicate bins for the same item and warehouse
		dataent.db.sql("delete from `tabBin` where warehouse=%s", new_name)
	...
	def convert_to_ledger(self):
		if self.check_if_child_exists():
			dataent.throw(_("Warehouses with child nodes cannot be converted to ledger"))
...
def get_children(doctype, parent=None, company=None, is_root=False):
	from epaas.stock.utils import get_stock_value_from_bin
	...
	warehouses = dataent.get_list(doctype, fields=fields, filters=filters, order_by='name')

	# return warehouses
	for wh in warehouses:
		wh["balance"] = get_stock_value_from_bin(warehouse=wh.value)
		if company:
			wh["company_currency"] = dataent.db.get_value('Company', company, 'default_currency')
	return warehouses
...
def convert_to_group_or_ledger():
	...
	return dataent.get_doc("Warehouse", args.docname).convert_to_group_or_ledger()
