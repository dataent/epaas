...
from dataent.utils import cint, cstr, flt, add_days, nowdate, getdate
from epaas.stock.get_item_details import get_reserved_qty_for_so

from dataent import _, ValidationError
...
class SerialNo(StockController):
	...
	def validate(self):
		if self.get("__islocal") and self.warehouse:
			dataent.throw(_("New Serial No cannot have Warehouse. Warehouse must be set by Stock Entry or Purchase Receipt"), SerialNoCannotCreateDirectError)
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = dataent.db.get_value("Serial No",
				self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				dataent.throw(_("Item Code cannot be changed for Serial No."),
					SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				dataent.throw(_("Warehouse cannot be changed for Serial No."),
					SerialNoCannotCannotChangeError)
	...
	def validate_item(self):
		...
		"""
		item = dataent.get_doc("Item", self.item_code)
		if item.has_serial_no!=1:
			dataent.throw(_("Item {0} is not setup for Serial Nos. Check Item master").format(self.item_code))
		...
		if purchase_sle:
			...
			if purchase_sle.voucher_type in ("Purchase Receipt", "Purchase Invoice"):
				self.supplier, self.supplier_name = \
					dataent.db.get_value(purchase_sle.voucher_type, purchase_sle.voucher_no,
		...
		if delivery_sle:
			...
			if delivery_sle.voucher_type  in ("Delivery Note", "Sales Invoice"):
				self.customer, self.customer_name = \
					dataent.db.get_value(delivery_sle.voucher_type, delivery_sle.voucher_no,
	...
	def get_stock_ledger_entries(self):
		sle_dict = {}
		for sle in dataent.db.sql("""select * from `tabStock Ledger Entry`
	...
	def on_trash(self):
		sl_entries = dataent.db.sql("""select serial_no from `tabStock Ledger Entry`
			where serial_no like %s and item_code=%s and ifnull(is_cancelled, 'No')='No'""",
		...
		if sle_exists:
			dataent.throw(_("Cannot delete Serial No {0}, as it is used in stock transactions").format(self.name))

	def before_rename(self, old, new, merge=False):
		if merge:
			dataent.throw(_("Sorry, Serial Nos cannot be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		for dt in dataent.db.sql("""select parent from tabDocField
			...
			for item in dataent.db.sql("""select name, serial_no from `tab%s`
				where serial_no like '%%%s%%'""" % (dt[0], dataent.db.escape(old))):

				serial_nos = map(lambda i: new if i.upper()==old.upper() else i, item[1].split('\n'))
				dataent.db.sql("""update `tab%s` set serial_no = %s
...
def validate_serial_no(sle, item_det):
	...
	if item_det.has_serial_no==0:
		if serial_nos:
			dataent.throw(_("Item {0} is not setup for Serial Nos. Column must be blank").format(sle.item_code),
				SerialNoNotRequiredError)
	elif sle.is_cancelled == "No":
		if serial_nos:
			if cint(sle.actual_qty) != flt(sle.actual_qty):
				dataent.throw(_("Serial No {0} quantity {1} cannot be a fraction").format(sle.item_code, sle.actual_qty))

			if len(serial_nos) and len(serial_nos) != abs(cint(sle.actual_qty)):
				dataent.throw(_("{0} Serial Numbers required for Item {1}. You have provided {2}.").format(abs(sle.actual_qty), sle.item_code, len(serial_nos)),
					SerialNoQtyError)

			if len(serial_nos) != len(set(serial_nos)):
				dataent.throw(_("Duplicate Serial No entered for Item {0}").format(sle.item_code), SerialNoDuplicateError)

			for serial_no in serial_nos:
				if dataent.db.exists("Serial No", serial_no):
					sr = dataent.get_doc("Serial No", serial_no)

					if sr.item_code!=sle.item_code:
						if not allow_serial_nos_with_different_item(serial_no, sle):
							dataent.throw(_("Serial No {0} does not belong to Item {1}").format(serial_no,
								sle.item_code), SerialNoItemError)

					if cint(sle.actual_qty) > 0 and has_duplicate_serial_no(sr, sle):
						dataent.throw(_("Serial No {0} has already been received").format(serial_no),
							SerialNoDuplicateError)

					if (sr.delivery_document_no and sle.voucher_type != 'Stock Entry'
						and sle.voucher_type == sr.delivery_document_type):
						return_against = dataent.db.get_value(sle.voucher_type, sle.voucher_no, 'return_against')
						if return_against and return_against != sr.delivery_document_no:
							dataent.throw(_("Serial no {0} has been already returned").format(sr.name))

					if cint(sle.actual_qty) < 0:
						if sr.warehouse!=sle.warehouse:
							dataent.throw(_("Serial No {0} does not belong to Warehouse {1}").format(serial_no,
								sle.warehouse), SerialNoWarehouseError)

						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):

							if sr.batch_no and sr.batch_no != sle.batch_no:
								dataent.throw(_("Serial No {0} does not belong to Batch {1}").format(serial_no,
									sle.batch_no), SerialNoBatchError)

							if sle.is_cancelled=="No" and not sr.warehouse:
								dataent.throw(_("Serial No {0} does not belong to any Warehouse")
							...
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									if not dataent.db.exists("Sales Invoice Item", {"parent": sle.voucher_no,
										"item_code": sle.item_code, "sales_order": sr.sales_order}):
										dataent.throw(_("Cannot deliver Serial No {0} of item {1} as it is reserved \
											to fullfill Sales Order {2}").format(sr.name, sle.item_code, sr.sales_order))
								elif sle.voucher_type == "Delivery Note":
									if not dataent.db.exists("Delivery Note Item", {"parent": sle.voucher_no,
										"item_code": sle.item_code, "against_sales_order": sr.sales_order}):
										invoice = dataent.db.get_value("Delivery Note Item", {"parent": sle.voucher_no,
											"item_code": sle.item_code}, "against_sales_invoice")
										if not invoice or dataent.db.exists("Sales Invoice Item",
											{"parent": invoice, "item_code": sle.item_code,
											"sales_order": sr.sales_order}):
											dataent.throw(_("Cannot deliver Serial No {0} of item {1} as it is reserved to \
							...
							if sle.voucher_type == "Sales Invoice":
								sales_order = dataent.db.get_value("Sales Invoice Item", {"parent": sle.voucher_no,
								...
								sales_order = dataent.get_value("Delivery Note Item", {"parent": sle.voucher_no,
									"item_code": sle.item_code}, "against_sales_order")
								if sales_order and get_reserved_qty_for_so(sales_order, sle.item_code):
									validate_so_serial_no(sr, sales_order)
								else:
									sales_invoice = dataent.get_value("Delivery Note Item", {"parent": sle.voucher_no,
										"item_code": sle.item_code}, "against_sales_invoice")
									if sales_invoice:
										sales_order = dataent.db.get_value("Sales Invoice Item", {
											"parent": sales_invoice, "item_code": sle.item_code}, "sales_order")
										if sales_order and get_reserved_qty_for_so(sales_order, sle.item_code):
											validate_so_serial_no(sr, sales_order)
				elif cint(sle.actual_qty) < 0:
					# transfer out
					dataent.throw(_("Serial No {0} not in stock").format(serial_no), SerialNoNotExistsError)
		elif cint(sle.actual_qty) < 0 or not item_det.serial_no_series:
			dataent.throw(_("Serial Nos Required for Serialized Item {0}").format(sle.item_code),
				SerialNoRequiredError)
	elif serial_nos:
		for serial_no in serial_nos:
			sr = dataent.db.get_value("Serial No", serial_no, ["name", "warehouse"], as_dict=1)
			if sr and cint(sle.actual_qty) < 0 and sr.warehouse != sle.warehouse:
				dataent.throw(_("Cannot cancel {0} {1} because Serial No {2} does not belong to the warehouse {3}")
					.format(sle.voucher_type, sle.voucher_no, serial_no, sle.warehouse))

def validate_so_serial_no(sr, sales_order,):
	if not sr.sales_order or sr.sales_order!= sales_order:
		dataent.throw(_("""Sales Order {0} has reservation for item {1}, you can
...
def has_duplicate_serial_no(sn, sle):
	...
	if sn.purchase_document_no:
		...
		if status and sle.voucher_type == 'Stock Entry' and \
			dataent.db.get_value('Stock Entry', sle.voucher_no, 'purpose') != 'Material Receipt':
			status = False
...
def allow_serial_nos_with_different_item(sle_serial_no, sle):
	...
	if sle.voucher_type=="Stock Entry" and cint(sle.actual_qty) > 0:
		stock_entry = dataent.get_doc("Stock Entry", sle.voucher_no)
...
def update_serial_nos(sle, item_det):
	if sle.is_cancelled == "No" and not sle.serial_no and cint(sle.actual_qty) > 0 \
		...
		dataent.db.set(sle, "serial_no", serial_nos)
		validate_serial_no(sle, item_det)
	if sle.serial_no:
...
def get_item_details(item_code):
	return dataent.db.sql("""select name, has_batch_no, docstatus,
...
def make_serial_no(serial_no, args):
	...
	dataent.msgprint(_("Serial No {0} created").format(sr.name))
	return sr.name

def update_serial_nos_after_submit(controller, parentfield):
	stock_ledger_entries = dataent.db.sql("""select voucher_detail_no, serial_no, actual_qty, warehouse
			...
			if sle.voucher_detail_no==d.name:
				if not accepted_serial_nos_updated and qty and abs(sle.actual_qty)==qty \
					and sle.warehouse == warehouse and sle.serial_no != d.serial_no:
						...
						dataent.db.set_value(d.doctype, d.name, "serial_no", sle.serial_no)
					...
					and sle.warehouse == d.rejected_warehouse and sle.serial_no != d.rejected_serial_no:
						...
						dataent.db.set_value(d.doctype, d.name, "rejected_serial_no", sle.serial_no)
						update_rejected_serial_nos = False
						if accepted_serial_nos_updated:
							break

def update_maintenance_status():
	serial_nos = dataent.db.sql('''select name from `tabSerial No` where (amc_expiry_date<%s or
		warranty_expiry_date<%s) and maintenance_status not in ('Out of Warranty', 'Out of AMC')''',
		(nowdate(), nowdate()))
	for serial_no in serial_nos:
		doc = dataent.get_doc("Serial No", serial_no[0])
		doc.set_maintenance_status()
		dataent.db.set_value('Serial No', doc.name, 'maintenance_status', doc.maintenance_status)

def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ''
	dn_serial_nos = dataent.db.sql_list(""" select name from `tabSerial No`
