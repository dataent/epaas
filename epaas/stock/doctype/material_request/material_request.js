...
dataent.ui.form.on('Material Request', {
	setup: function(frm) {
	...
	update_status: function(frm, stop_status) {
		dataent.call({
			method: 'epaas.stock.doctype.material_request.material_request.update_status',
		...
		frm.call({
			...
			args: {
				args: {
					...
					buying_price_list: dataent.defaults.get_default('buying_price_list'),
					currency: dataent.defaults.get_default('Currency'),
					name: frm.doc.name,
					qty: item.qty || 1,
	...
	get_items_from_bom: function(frm) {
		...
		d.get_input("fetch").on("click", function() {
			...
			if(!frm.doc.company) dataent.throw(__("Company field is required"));
			dataent.call({
				method: "epaas.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function(r) {
					if (!r.message) {
						dataent.throw(__("BOM does not contain any stock item"));
					} else {
						epaas.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function(i, item) {
							var d = dataent.model.add_child(cur_frm.doc, "Material Request Item", "items");
		...
		dataent.prompt(
			...
			(values) => {
				dataent.model.open_mapped_doc({
					method: "epaas.stock.doctype.material_request.material_request.make_purchase_order",
					frm: frm,
	...
	make_stock_entry: function(frm) {
		dataent.model.open_mapped_doc({
			method: "epaas.stock.doctype.material_request.material_request.make_stock_entry",
			frm: frm
		});
	},
	...
	raise_work_orders: function(frm) {
		dataent.call({
			method:"epaas.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
dataent.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		var d = locals[doctype][name];
		if (flt(d.qty) < flt(d.min_order_qty)) {
			dataent.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
	...
	schedule_date: function(frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				epaas.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
epaas.buying.MaterialRequestController = epaas.buying.BuyingController.extend({
	tc_name: function() {
		this.get_terms();
	},
...
$.extend(cur_frm.cscript, new epaas.buying.MaterialRequestController({frm: cur_frm}));

function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		epaas.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
