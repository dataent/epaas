...
from dataent.utils import cstr, flt, getdate, new_line_sep, nowdate, add_days
...
from epaas.manufacturing.doctype.work_order.work_order import get_item_details
from epaas.buying.utils import check_for_closed_status, validate_for_items
...
class MaterialRequest(BuyingController):
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = dataent.db.sql("""select sum(qty)
					from `tabMaterial Request Item`
					where item_code = %s and sales_order = %s and
					docstatus = 1 and parent != %s""", (item, so_no, self.name))
				already_indented = already_indented and flt(already_indented[0][0]) or 0

				actual_so_qty = dataent.db.sql("""select sum(stock_qty) from `tabSales Order Item`
					where parent = %s and item_code = %s and docstatus = 1""", (so_no, item))
				actual_so_qty = actual_so_qty and flt(actual_so_qty[0][0]) or 0

				if actual_so_qty and (flt(so_items[so_no][item]) + already_indented > actual_so_qty):
					dataent.throw(_("Material Request of maximum {0} can be made for Item {1} against Sales Order {2}").format(actual_so_qty - already_indented, item, so_no))
	...
	def check_modified_date(self):
		mod_db = dataent.db.sql("""select modified from `tabMaterial Request` where name = %s""",
			self.name)
		date_diff = dataent.db.sql("""select TIMEDIFF('%s', '%s')"""
			% (mod_db[0][0], cstr(self.modified)))

		if date_diff and date_diff[0][0]:
			dataent.throw(_("{0} {1} has been modified. Please refresh.").format(_(self.doctype), self.name))
		...
		for d in self.get("items"):
			if d.name in mr_items:
				if self.material_request_type in ("Material Issue", "Material Transfer"):
					d.ordered_qty =  flt(dataent.db.sql("""select sum(transfer_qty)
						from `tabStock Entry Detail` where material_request = %s
						and material_request_item = %s and docstatus = 1""",
						(self.name, d.name))[0][0])

					if d.ordered_qty and d.ordered_qty > d.stock_qty:
						dataent.throw(_("The total Issue / Transfer quantity {0} in Material Request {1}  \
							cannot be greater than requested quantity {2} for Item {3}").format(d.ordered_qty, d.parent, d.qty, d.item_code))

				elif self.material_request_type == "Manufacture":
					d.ordered_qty = flt(dataent.db.sql("""select sum(qty)
						from `tabWork Order` where material_request = %s
						and material_request_item = %s and docstatus = 1""",
						(self.name, d.name))[0][0])

				dataent.db.set_value(d.doctype, d.name, "ordered_qty", d.ordered_qty)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (not mr_item_rows or d.name in mr_item_rows) and [d.item_code, d.warehouse] not in item_wh_list \
					and dataent.db.get_value("Item", d.item_code, "is_stock_item") == 1 and d.warehouse:
	...
	def update_requested_qty_in_production_plan(self):
		...
		for d in self.get('items'):
			if d.production_plan and d.material_request_plan_item:
				...
				dataent.db.set_value('Material Request Plan Item',
		...
		for production_plan in production_plans:
			doc = dataent.get_doc('Production Plan', production_plan)
			doc.set_status()
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				...
				if mr_obj.status in ["Stopped", "Cancelled"]:
					dataent.throw(_("{0} {1} is cancelled or stopped").format(_("Material Request"), mr),
						dataent.InvalidStatusError)
...
def update_status(name, status):
	material_request = dataent.get_doc('Material Request', name)
...
def make_purchase_order(source_name, target_doc=None):
	...
	def postprocess(source, target_doc):
		if dataent.flags.args and dataent.flags.args.default_supplier:
			# items only for given default supplier
			supplier_items = []
			for d in target_doc.items:
				default_supplier = get_item_defaults(d.item_code, target_doc.company).get('default_supplier')
				if dataent.flags.args.default_supplier == default_supplier:
...
def get_material_requests_based_on_supplier(supplier):
	supplier_items = [d.parent for d in dataent.db.get_all("Item Default",
		{"default_supplier": supplier}, 'parent')]
	if supplier_items:
		material_requests = dataent.db.sql_list("""select distinct mr.name
...
def raise_work_orders(material_request):
	mr= dataent.get_doc("Material Request", material_request)
	errors =[]
	work_orders = []
	default_wip_warehouse = dataent.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")

	for d in mr.items:
		if (d.qty - d.ordered_qty) >0:
			if dataent.db.exists("BOM", {"item": d.item_code, "is_default": 1}):
	...
	if errors:
		dataent.throw(_("Productions Orders cannot be raised for:") + '\n' + new_line_sep(errors))
