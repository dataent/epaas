...
class PackingSlip(Document):
	...
	def validate_delivery_note(self):
		...
		"""
		if cint(dataent.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
			dataent.throw(_("Delivery Note {0} must not be submitted").format(self.delivery_note))

	def validate_items_mandatory(self):
		rows = [d.item_code for d in self.get("items")]
		if not rows:
			dataent.msgprint(_("No Items to pack"), raise_exception=1)
	...
	def validate_case_nos(self):
		...
		if not cint(self.from_case_no):
			dataent.msgprint(_("Please specify a valid 'From Case No.'"), raise_exception=1)
		elif not self.to_case_no:
			self.to_case_no = self.from_case_no
		elif self.from_case_no > self.to_case_no:
			dataent.msgprint(_("'To Case No.' cannot be less than 'From Case No.'"),
				raise_exception=1)


		res = dataent.db.sql("""SELECT name FROM `tabPacking Slip`
		...
		if res:
			dataent.throw(_("""Case No(s) already in use. Try from Case No {0}""").format(self.get_recommended_case_no()))
	...
	def get_details_for_packing(self):
		...
		custom_fields = ', '.join(['dni.`{0}`'.format(d.fieldname)
			for d in dataent.get_meta("Delivery Note Item").get_custom_fields()
		...
		res = dataent.db.sql("""select item_code, sum(qty) as qty,
	...
	def recommend_new_qty(self, item, ps_item_qty, no_of_cases):
		...
		dataent.throw(_("Quantity for Item {0} must be less than {1}").format(item.get("item_code"), item.get("recommended_qty")))
	...
	def update_item_details(self):
		...
		for d in self.get("items"):
			res = dataent.db.get_value("Item", d.item_code,
	...
	def get_recommended_case_no(self):
		"""
			...
			note
		"""
		recommended_case_no = dataent.db.sql("""SELECT MAX(to_case_no) FROM `tabPacking Slip`
	...
	def get_items(self):
		...
		custom_fields = dataent.get_meta("Delivery Note Item").get_custom_fields()
...
def item_details(doctype, txt, searchfield, start, page_len, filters):
	...
	return dataent.db.sql("""select name, item_name, description from `tabItem`
