...
cur_frm.fields_dict['items'].grid.get_field('item_code').get_query = function(doc, cdt, cdn) {
	if(!doc.delivery_note) {
		dataent.throw(__("Please Delivery Note first"))
	} else {
		return {
			query: "epaas.stock.doctype.packing_slip.packing_slip.item_details",
...
cur_frm.cscript.validate_case_nos = function(doc) {
	...
	if(cint(doc.from_case_no)==0) {
		dataent.msgprint(__("The 'From Package No.' field must neither be empty nor it's value less than 1."));
		...
		dataent.msgprint(__("'To Case No.' cannot be less than 'From Case No.'"));
		dataent.validated = false;
	}
}
	...
	for(var i=0; i<ps_detail.length; i++) {
		for(var j=0; j<ps_detail.length; j++) {
			if(i!=j && ps_detail[i].item_code && ps_detail[i].item_code==ps_detail[j].item_code) {
				dataent.msgprint(__("You have entered duplicate items. Please rectify and try again."));
				dataent.validated = false;
				return;
			}
		}
		if(flt(ps_detail[i].qty)<=0) {
			dataent.msgprint(__("Invalid quantity specified for item {0}. Quantity should be greater than 0.", [ps_detail[i].item_code]));
			dataent.validated = false;
		}
	}
}
...
cur_frm.cscript.calc_net_total_pkg = function(doc, ps_detail) {
	...
	for(var i=0; i<ps_detail.length; i++) {
		...
		if(item.weight_uom != doc.net_weight_uom) {
			dataent.msgprint(__("Different UOM for items will lead to incorrect (Total) Net Weight value. Make sure that Net Weight of each item is in the same UOM."));
			dataent.validated = false;
		}
