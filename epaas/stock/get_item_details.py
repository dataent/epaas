...
from dataent.utils import flt, cint, add_days, cstr, add_months
import json
from epaas.accounts.doctype.pricing_rule.pricing_rule import get_pricing_rule_for_item, set_transaction_type
from epaas.setup.utils import get_exchange_rate
from dataent.model.meta import get_field_precision
from epaas.stock.doctype.batch.batch import get_batch_no
from epaas import get_company_currency
from epaas.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
...
def set_valuation_rate(out, args):
	if dataent.db.exists("Product Bundle", args.item_code, cache=True):
		valuation_rate = 0.0
		bundled_items = dataent.get_doc("Product Bundle", args.item_code)
...
def get_item_code(barcode=None, serial_no=None):
	if barcode:
		item_code = dataent.db.get_value("Item Barcode", {"barcode": barcode}, fieldname=["parent"])
		if not item_code:
			dataent.throw(_("No Item with Barcode {0}").format(barcode))
	elif serial_no:
		item_code = dataent.db.get_value("Serial No", serial_no, "item_code")
		if not item_code:
			dataent.throw(_("No Item with Serial No {0}").format(serial_no))
...
def get_basic_details(args, item):
	...
	if not item:
		item = dataent.get_doc("Item", args.get("item_code"))
	...
	if args.get('doctype') == "Material Request" and not args.get('material_request_type'):
		args['material_request_type'] = dataent.db.get_value('Material Request',
	...
	if args.get('doctype') in purchase_doctypes:
		from epaas.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	for d in [
		...
		["Warehouse", "warehouse", ""]]:
			if not out[d[1]]:
				out[d[1]] = dataent.get_cached_value('Company',  args.company,  d[2]) if d[2] else None

	for fieldname in ("item_name", "item_group", "barcodes", "brand", "stock_uom"):
		out[fieldname] = item.get(fieldname)

	child_doctype = args.doctype + ' Item'
	meta = dataent.get_meta(child_doctype)
...
def update_barcode_value(out):
	from epaas.accounts.doctype.sales_invoice.pos import get_barcode_data
...
def get_default_deferred_account(args, item, fieldname=None):
	if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
		return (item.get(fieldname)
			...
			or dataent.get_cached_value('Company',  args.company,  "default_"+fieldname))
	else:
		return None
...
def get_default_cost_center(args, item, item_group, company=None):
	cost_center = None
	if args.get('project'):
		cost_center = dataent.db.get_value("Project", args.get("project"), "cost_center", cache=True)
	...
	if (company and cost_center
		and dataent.get_cached_value("Cost Center", cost_center, "company") != company):
		return None
...
def insert_item_price(args):
	...
	if dataent.db.get_value("Price List", args.price_list, "currency", cache=True) == args.currency \
		and cint(dataent.db.get_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing")):
		if dataent.has_permission("Item Price", "write"):
			price_list_rate = (args.rate / args.get('conversion_factor')
				if args.get("conversion_factor") else args.rate)

			item_price = dataent.db.get_value('Item Price',
				{'item_code': args.item_code, 'price_list': args.price_list, 'currency': args.currency},
				['name', 'price_list_rate'], as_dict=1)
			if item_price and item_price.name:
				if item_price.price_list_rate != price_list_rate:
					dataent.db.set_value('Item Price', item_price.name, "price_list_rate", price_list_rate)
					dataent.msgprint(_("Item Price updated for {0} in Price List {1}").format(args.item_code,
				...
				item_price.insert()
				dataent.msgprint(_("Item Price added for {0} in Price List {1}").format(args.item_code,
					args.price_list), alert=True)
...
def get_item_price(args, item_code, ignore_party=False):
	"""
		...
		:param args: dict (or dataent._dict) with mandatory fields price_list, uom
	...
	return dataent.db.sql(""" select name, price_list_rate, uom
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	flag = True
	item_price = dataent.get_doc("Item Price", price_list_rate_name)
...
def validate_price_list(args):
	if args.get("price_list"):
		if not dataent.db.get_value("Price List",
...
def get_party_item_code(args, item_doc, out):
	if args.transaction_type=="selling" and args.customer:
		...
		if customer_item_code:
			...
			customer_group = dataent.get_cached_value("Customer", args.customer, "customer_group")
...
def get_pos_profile_item_details(company, args, pos_profile=None, update_data=False):
	...
	if not dataent.flags.pos_profile and not pos_profile:
		pos_profile = dataent.flags.pos_profile = get_pos_profile(company, args.get('pos_profile'))
...
def get_serial_nos_by_fifo(args, sales_order=None):
	if dataent.db.get_single_value("Stock Settings", "automatically_set_serial_nos_based_on_fifo"):
		return "\n".join(dataent.db.sql_list("""select name from `tabSerial No`
...
def get_conversion_factor(item_code, uom):
	variant_of = dataent.db.get_value("Item", item_code, "variant_of", cache=True)
	filters = {"parent": item_code, "uom": uom}
	if variant_of:
		filters["parent"] = ("in", (item_code, variant_of))
	conversion_factor = dataent.db.get_value("UOM Conversion Detail",
		filters, "conversion_factor")
	if not conversion_factor:
		stock_uom = dataent.db.get_value("Item", item_code, "stock_uom")
		conversion_factor = get_uom_conv_factor(uom, stock_uom)
	return {"conversion_factor": conversion_factor or 1.0}

@dataent.whitelist()
def get_projected_qty(item_code, warehouse):
	return {"projected_qty": dataent.db.get_value("Bin",
		{"item_code": item_code, "warehouse": warehouse}, "projected_qty")}

@dataent.whitelist()
def get_bin_details(item_code, warehouse):
	return dataent.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse},
		["projected_qty", "actual_qty", "reserved_qty"], as_dict=True, cache=True) \
			or {"projected_qty": 0, "actual_qty": 0, "reserved_qty": 0}

@dataent.whitelist()
def get_serial_no_details(item_code, warehouse, stock_qty, serial_no):
	args = dataent._dict({"item_code":item_code, "warehouse":warehouse, "stock_qty":stock_qty, "serial_no":serial_no})
...
def get_bin_details_and_serial_nos(item_code, warehouse, has_batch_no, stock_qty=None, serial_no=None):
	...
	if flt(stock_qty) > 0:
		if has_batch_no:
			args = dataent._dict({"item_code":item_code, "warehouse":warehouse, "stock_qty":stock_qty})
			serial_no = get_serial_no(args)
...
def get_batch_qty_and_serial_no(batch_no, stock_qty, warehouse, item_code, has_serial_no):
	...
	if (flt(batch_qty_and_serial_no.get('actual_batch_qty')) >= flt(stock_qty)) and has_serial_no:
		args = dataent._dict({"item_code":item_code, "warehouse":warehouse, "stock_qty":stock_qty, "batch_no":batch_no})
		serial_no = get_serial_no(args)
...
def get_batch_qty(batch_no, warehouse, item_code):
	from epaas.stock.doctype.batch import batch
	if batch_no:
...
def apply_price_list(args, as_doc=False):
	...
	if "items" in args:
		...
		for item in item_list:
			args_copy = dataent._dict(args.copy())
...
def apply_price_list_on_item(args):
	...
	item_doc = dataent.get_doc("Item", args.item_code)
...
def get_price_list_currency(price_list):
	if price_list:
		result = dataent.db.get_value("Price List", {"name": price_list,
...
def get_default_bom(item_code=None):
	if item_code:
		bom = dataent.db.get_value("BOM", {"docstatus": 1, "is_default": 1, "is_active": 1, "item": item_code})
		if bom:
			return bom
...
def get_valuation_rate(item_code, company, warehouse=None):
	...
	if item.get("is_stock_item"):
		...
		valuation_rate =dataent.db.sql("""select sum(base_net_amount) / sum(qty*conversion_factor)
...
def get_serial_no(args, serial_nos=None, sales_order=None):
	...
	if args.get('warehouse') and args.get('stock_qty') and args.get('item_code'):
		has_serial_no = dataent.get_value('Item', {'item_code': args.item_code}, "has_serial_no")
...
def get_blanket_order_details(args):
	...
	if args.item_code:
		...
		blanket_order_details = dataent.db.sql('''
...
def get_so_reservation_for_item(args):
	...
	if args.get('against_sales_order'):
		...
		sales_order = dataent.db.sql("""select sales_order from `tabSales Invoice Item` where
...
def get_reserved_qty_for_so(sales_order, item_code):
	reserved_qty = dataent.db.sql("""select sum(qty) from `tabSales Order Item`
