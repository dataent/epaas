...
import dataent
import epaas
from dataent.utils import flt, nowdate, add_days, cint
from dataent import _

def reorder_item():
	""" Reorder item if stock reaches reorder level"""
	# if initial setup not completed, return
	if not (dataent.db.a_row_exists("Company") and dataent.db.a_row_exists("Fiscal Year")):
		return

	if cint(dataent.db.get_value('Stock Settings', None, 'auto_indent')):
		return _reorder_item()

def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = dataent._dict(dataent.db.sql("""select name, company from `tabWarehouse`
		where disabled=0"""))
	default_company = (epaas.get_default_company() or
		dataent.db.sql("""select name from tabCompany limit 1""")[0][0])

	items_to_consider = dataent.db.sql_list("""select name from `tabItem` item
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in dataent.db.sql("""select item_code, warehouse, projected_qty
		...
		warehouse_doc = dataent.get_doc("Warehouse", warehouse)
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = dataent.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception():
		if dataent.local.message_log:
			exceptions_list.extend(dataent.local.message_log)
			dataent.local.message_log = []
		else:
			exceptions_list.append(dataent.get_traceback())
	...
	for request_type in material_requests:
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					d = dataent._dict(d)
					item = dataent.get_doc("Item", d.item_code)
					uom = item.stock_uom
	...
	if mr_list:
		if getattr(dataent.local, "reorder_email_notify", None) is None:
			dataent.local.reorder_email_notify = cint(dataent.db.get_value('Stock Settings', None,
				'reorder_email_notify'))

		if(dataent.local.reorder_email_notify):
...
def notify_errors(exceptions_list):
	subject = "[Important] [EPAAS] Auto Reorder Errors"
