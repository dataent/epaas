...
from dataent.utils import cint, flt, cstr, now
from epaas.stock.utils import get_valuation_method
import json

from six import iteritems

# future reposting
class NegativeStockError(dataent.ValidationError): pass

_exceptions = dataent.local('stockledger_exceptions')
# _exceptions = []

def make_sl_entries(sl_entries, is_amended=None, allow_negative_stock=False, via_landed_cost_voucher=False):
	if sl_entries:
		from epaas.stock.utils import update_bin
...
def set_as_cancel(voucher_type, voucher_no):
	dataent.db.sql("""update `tabStock Ledger Entry` set is_cancelled='Yes',
...
class update_entries_after(object):
	...
	def __init__(self, args, allow_zero_rate=False, allow_negative_stock=None, via_landed_cost_voucher=False, verbose=1):
		...
		if not self.allow_negative_stock:
			self.allow_negative_stock = cint(dataent.db.get_single_value("Stock Settings",
		...
		self.previous_sle = self.previous_sle[0] if self.previous_sle else dataent._dict()

		for key in ("qty_after_transaction", "valuation_rate", "stock_value"):
			setattr(self, key, flt(self.previous_sle.get(key)))

		self.company = dataent.db.get_value("Warehouse", self.warehouse, "company")
		self.precision = get_field_precision(dataent.get_meta("Stock Ledger Entry").get_field("stock_value"),
	...
	def update_bin(self):
		# update bin
		bin_name = dataent.db.get_value("Bin", {
			"item_code": self.item_code,
			"warehouse": self.warehouse
		})

		if not bin_name:
			bin_doc = dataent.get_doc({
				"doctype": "Bin",
				"item_code": self.item_code,
				"warehouse": self.warehouse
			})
			...
			bin_doc = dataent.get_doc("Bin", bin_name)
	...
	def get_serialized_values(self, sle):
		...
		if incoming_rate:
			...
			stock_value_change = -1 * flt(dataent.db.sql("""select sum(purchase_rate)
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		return dataent.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
	...
	def get_sle_after_datetime(self):
		...
		return get_stock_ledger_entries(self.previous_sle or dataent._dict({
	...
	def raise_exceptions(self):
		...
		if ((self.exceptions[0]["voucher_type"], self.exceptions[0]["voucher_no"]) in
			dataent.local.flags.currently_saving):

			msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
				abs(deficiency), dataent.get_desk_link('Item', self.item_code),
				dataent.get_desk_link('Warehouse', self.warehouse))
		else:
			msg = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
				abs(deficiency), dataent.get_desk_link('Item', self.item_code),
				dataent.get_desk_link('Warehouse', self.warehouse),
				self.exceptions[0]["posting_date"], self.exceptions[0]["posting_time"],
				dataent.get_desk_link(self.exceptions[0]["voucher_type"], self.exceptions[0]["voucher_no"]))

		if self.verbose:
			dataent.throw(msg, NegativeStockError, title='Insufficent Stock')
		else:
...
def get_stock_ledger_entries(previous_sle, operator=None, order="desc", limit=None, for_update=False, debug=False):
	...
	return dataent.db.sql("""select *, timestamp(posting_date, posting_time) as "timestamp" from `tabStock Ledger Entry`
...
def get_valuation_rate(item_code, warehouse, voucher_type, voucher_no,
	...
	if not valuation_rate:
		...
		valuation_rate = dataent.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = dataent.db.get_value('Item Price',
	...
	if not allow_zero_rate and not valuation_rate and raise_error_if_no_rate \
		...
		dataent.throw(_("Valuation rate not found for the Item {0}, which is required to do accounting entries for {1} {2}. If the item is transacting as a zero valuation rate item in the {1}, please mention that in the {1} Item table. Otherwise, please create an incoming stock transaction for the item or mention valuation rate in the Item record, and then try submiting/cancelling this entry").format(item_code, voucher_type, voucher_no))
