...
def repost(only_actual=False, allow_negative_stock=False, allow_zero_rate=False, only_bin=False):
	...
	for d in dataent.db.sql("""select distinct item_code, warehouse from
...
def get_balance_qty_from_sle(item_code, warehouse):
	balance_qty = dataent.db.sql("""select qty_after_transaction from `tabStock Ledger Entry`
...
def get_indented_qty(item_code, warehouse):
	indented_qty = dataent.db.sql("""select sum(mr_item.qty - mr_item.ordered_qty)
...
def set_stock_balance_as_per_serial_no(item_code=None, posting_date=None, posting_time=None,
	...
	bin = dataent.db.sql("""select bin.item_code, bin.warehouse, bin.actual_qty, item.stock_uom
		from `tabBin` bin, tabItem item
		where bin.item_code = item.name and item.has_serial_no = 1 %s""" % condition)

	for d in bin:
		serial_nos = dataent.db.sql("""select count(name) from `tabSerial No`
			where item_code=%s and warehouse=%s and docstatus < 2""", (d[0], d[1]))

		if serial_nos and flt(serial_nos[0][0]) != flt(d[2]):
			print(d[0], d[1], d[2], serial_nos[0][0])

		sle = dataent.db.sql("""select valuation_rate, company from `tabStock Ledger Entry`
...
def reset_serial_no_status_and_warehouse(serial_nos=None):
	if not serial_nos:
		serial_nos = dataent.db.sql_list("""select name from `tabSerial No` where docstatus = 0""")
...
def repost_all_stock_vouchers():
	warehouses_with_account = dataent.db.sql_list("""select warehouse from tabAccount
		where ifnull(account_type, '') = 'Stock' and (warehouse is not null and warehouse != '')
		and is_group=0""")

	vouchers = dataent.db.sql("""select distinct voucher_type, voucher_no
	...
	for voucher_type, voucher_no in vouchers:
		...
		try:
			for dt in ["Stock Ledger Entry", "GL Entry"]:
				dataent.db.sql("""delete from `tab%s` where voucher_type=%s and voucher_no=%s"""%
					(dt, '%s', '%s'), (voucher_type, voucher_no))

			doc = dataent.get_doc(voucher_type, voucher_no)
