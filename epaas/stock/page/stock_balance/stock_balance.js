dataent.pages['stock-balance'].on_page_load = function(wrapper) {
	...
	page.sort_selector = new dataent.ui.SortSelector({
	...
	dataent.require('assets/js/item-dashboard.min.js', function() {
		page.item_dashboard = new epaas.stock.ItemDashboard({
			parent: page.main,
		})
		...
		var setup_click = function(doctype) {
			page.main.on('click', 'a[data-type="'+ doctype.toLowerCase() +'"]', function() {
				...
				if(field.get_value()===name) {
					dataent.set_route('Form', doctype, name)
				} else {
