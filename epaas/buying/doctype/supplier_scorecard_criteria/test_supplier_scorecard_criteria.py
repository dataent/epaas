...
class TestSupplierScorecardCriteria(unittest.TestCase):
	def test_variables_exist(self):
		...
		self.assertRaises(dataent.ValidationError,dataent.get_doc(test_bad_criteria[0]).insert)

	def test_formula_validate(self):
		delete_test_scorecards()
		self.assertRaises(dataent.ValidationError,dataent.get_doc(test_bad_criteria[1]).insert)
		self.assertRaises(dataent.ValidationError,dataent.get_doc(test_bad_criteria[2]).insert)

def delete_test_scorecards():
	# Delete all the periods so we can delete all the criteria
	dataent.db.sql("""delete from `tabSupplier Scorecard Period`""")
	dataent.db.sql("""delete from `tabSupplier Scorecard Scoring Criteria` where parenttype = 'Supplier Scorecard Period'""")
	dataent.db.sql("""delete from `tabSupplier Scorecard Scoring Standing` where parenttype = 'Supplier Scorecard Period'""")
	dataent.db.sql("""delete from `tabSupplier Scorecard Scoring Variable` where parenttype = 'Supplier Scorecard Period'""")

	for d in test_good_criteria:
		if dataent.db.exists("Supplier Scorecard Criteria", d.get("name")):
			# Delete all the periods, then delete the scorecard
			dataent.delete_doc(d.get("doctype"), d.get("name"))
