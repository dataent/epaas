...
class VariablePathNotFound(dataent.ValidationError): pass
...
class SupplierScorecardVariable(Document):
	...
	def validate_path_exists(self):
		if '.' in self.path:
			try:
				from epaas.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import import_string_path
				import_string_path(self.path)
			except AttributeError:
				dataent.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	""" Gets the number of days in this period"""
	supplier = dataent.get_doc('Supplier', scorecard.supplier)
	total_item_days = dataent.db.sql("""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = dataent.db.sql("""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = dataent.get_doc('Supplier', scorecard.supplier)
	total_delivered_late_days = dataent.db.sql("""
			SELECT
	...
	total_missed_late_days = dataent.db.sql("""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = dataent.db.sql("""
			SELECT
				COUNT(pr_item.qty)
			FROM
				`tabPurchase Order Item` po_item,
