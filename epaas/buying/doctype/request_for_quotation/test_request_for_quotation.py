...
import dataent
from epaas.templates.pages.rfq import check_supplier_has_docname_access
...
class TestRequestforQuotation(unittest.TestCase):
	...
	def test_make_supplier_quotation_from_portal(self):
		from epaas.buying.doctype.request_for_quotation.request_for_quotation import create_supplier_quotation
		rfq = make_request_for_quotation()
		rfq.get('items')[0].rate = 100
		rfq.supplier = rfq.suppliers[0].supplier
		supplier_quotation_name = create_supplier_quotation(rfq)

		supplier_quotation_doc = dataent.get_doc('Supplier Quotation', supplier_quotation_name)
...
def make_request_for_quotation(supplier_data=None):
	...
	rfq = dataent.new_doc('Request for Quotation')
