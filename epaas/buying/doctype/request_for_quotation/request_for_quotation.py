...
from dataent.utils import get_url, cint
from dataent.utils.user import get_user_fullname
from dataent.utils.print_format import download_pdf
from dataent.desk.form.load import get_attachments
from dataent.core.doctype.communication.email import make
from epaas.accounts.party import get_party_account_currency, get_party_details
from epaas.stock.doctype.material_request.material_request import set_missing_values
from epaas.controllers.buying_controller import BuyingController
from epaas.buying.utils import validate_for_items
...
class RequestforQuotation(BuyingController):
	...
	def validate_duplicate_supplier(self):
		...
		if len(supplier_list) != len(set(supplier_list)):
			dataent.throw(_("Same supplier has been entered multiple times"))

	def validate_supplier_list(self):
		for d in self.suppliers:
			prevent_rfqs = dataent.db.get_value("Supplier", d.supplier, 'prevent_rfqs')
			if prevent_rfqs:
				standing = dataent.db.get_value("Supplier Scorecard",d.supplier, 'status')
				dataent.throw(_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(d.supplier, standing))
			warn_rfqs = dataent.db.get_value("Supplier", d.supplier, 'warn_rfqs')
			if warn_rfqs:
				standing = dataent.db.get_value("Supplier Scorecard",d.supplier, 'status')
				dataent.msgprint(_("{0} currently has a {1} Supplier Scorecard standing, and RFQs to this supplier should be issued with caution.").format(d.supplier, standing), title=_("Caution"), indicator='orange')

	def update_email_id(self):
		for rfq_supplier in self.suppliers:
			if not rfq_supplier.email_id:
				rfq_supplier.email_id = dataent.db.get_value("Contact", rfq_supplier.contact, "email_id")

	def validate_email_id(self, args):
		if not args.email_id:
			dataent.throw(_("Row {0}: For supplier {0} Email Address is required to send email").format(args.idx, args.supplier))

	def on_submit(self):
		dataent.db.set(self, 'status', 'Submitted')
	...
	def update_supplier_part_no(self, args):
		...
		for item in self.items:
			item.supplier_part_no = dataent.db.get_value('Item Supplier',
	...
	def update_supplier_contact(self, rfq_supplier, link):
		...
		if dataent.db.exists("User", rfq_supplier.email_id):
			user = dataent.get_doc("User", rfq_supplier.email_id)
		else:
	...
	def update_contact_of_supplier(self, rfq_supplier, user):
		if rfq_supplier.contact:
			contact = dataent.get_doc("Contact", rfq_supplier.contact)
		else:
	...
	def supplier_rfq_mail(self, data, update_password_link, rfq_link):
		...
		args = {
			...
			'message': dataent.render_template(self.message_for_supplier, data.as_dict()),
			'rfq_link': rfq_link,
	...
	def send_email(self, data, sender, subject, message, attachments):
		...
		dataent.msgprint(_("Email sent to supplier {0}").format(data.supplier))

	def get_attachments(self):
		attachments = [d.name for d in get_attachments(self.doctype, self.name)]
		attachments.append(dataent.attach_print(self.doctype, self.name, doc=self))
		return attachments
...
def send_supplier_emails(rfq_name):
	...
	rfq = dataent.get_doc("Request for Quotation", rfq_name)
	if rfq.docstatus==1:
		rfq.send_to_supplier()

def check_portal_enabled(reference_doctype):
	if not dataent.db.get_value('Portal Menu Item',
		{'reference_doctype': reference_doctype}, 'enabled'):
		dataent.throw(_("Request for Quotation is disabled to access from portal, for more check portal settings."))
...
def get_supplier_contacts(doctype, txt, searchfield, start, page_len, filters):
	return dataent.db.sql("""select `tabContact`.name from `tabContact`, `tabDynamic Link`
...
def make_supplier_quotation(source_name, for_supplier, target_doc=None):
	def postprocess(source, target_doc):
		...
		target_doc.buying_price_list = args.buying_price_list or dataent.db.get_value('Buying Settings', None, 'buying_price_list')
...
def create_supplier_quotation(doc):
	...
	try:
		sq_doc = dataent.get_doc({
			"doctype": "Supplier Quotation",
			"supplier": doc.get('supplier'),
			"terms": doc.get("terms"),
			"company": doc.get("company"),
			"currency": doc.get('currency') or get_party_account_currency('Supplier', doc.get('supplier'), doc.get('company')),
			"buying_price_list": doc.get('buying_price_list') or dataent.db.get_value('Buying Settings', None, 'buying_price_list')
		})
		add_items(sq_doc, doc.get('supplier'), doc.get('items'))
		sq_doc.flags.ignore_permissions = True
		sq_doc.run_method("set_missing_values")
		sq_doc.save()
		dataent.msgprint(_("Supplier Quotation {0} created").format(sq_doc.name))
		return sq_doc.name
	except Exception:
		return None
...
def create_rfq_items(sq_doc, supplier, data):
	sq_doc.append('items', {
		...
		"qty": data.qty,
		"rate": data.rate,
		"supplier_part_no": dataent.db.get_value("Item Supplier", {'parent': data.item_code, 'supplier': supplier}, "supplier_part_no"),
