...
from dataent.utils import flt, nowdate, add_days
...
class SupplierQuotation(BuyingController):
	...
	def update_rfq_supplier_status(self, include_me):
		...
		for rfq in rfq_list:
			doc = dataent.get_doc('Request for Quotation', rfq)
			doc_sup = dataent.get_all('Request for Quotation Supplier', filters=
				{'parent': doc.name, 'supplier': self.supplier}, fields=['name', 'quote_status'])

			doc_sup = doc_sup[0] if doc_sup else None
			if not doc_sup:
				dataent.throw(_("Supplier {0} not found in {1}").format(self.supplier,
			...
			if quote_status == _('Received') and doc_sup.quote_status == _('No Quote'):
				dataent.msgprint(_("{0} indicates that {1} will not provide a quotation, but all items \
					have been quoted. Updating the RFQ quote status.").format(doc.name, self.supplier))
				dataent.db.set_value('Request for Quotation Supplier', doc_sup.name, 'quote_status', quote_status)
				dataent.db.set_value('Request for Quotation Supplier', doc_sup.name, 'no_quote', 0)
...
def make_quotation(source_name, target_doc=None):
	doclist = get_mapped_doc("Supplier Quotation", source_name, {
		...
		"Supplier Quotation Item": {
			...
			"condition": lambda doc: dataent.db.get_value("Item", doc.item_code, "is_sales_item")==1,
			"add_if_empty": True
		}
	}, target_doc)
