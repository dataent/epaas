...
QUnit.test("test: purchase order", function(assert) {
	...
	dataent.run_serially([
		...
		() => {
			...
			assert.ok(cur_frm.doc.schedule_date == dataent.datetime.add_days(dataent.datetime.now_date(), 1), "Schedule Date correct");
			...
			assert.ok(cur_frm.doc.items[0].schedule_date == dataent.datetime.add_days(dataent.datetime.now_date(), 2), "Schedule Date correct");
