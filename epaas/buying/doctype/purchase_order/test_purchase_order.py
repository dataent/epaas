...
from dataent.utils import flt, add_days, nowdate, getdate
...
from epaas.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice as make_pi_from_pr
from epaas.stock.doctype.material_request.test_material_request import make_material_request
...
class TestPurchaseOrder(unittest.TestCase):
	def test_make_purchase_receipt(self):
		...
		self.assertRaises(dataent.ValidationError, make_purchase_receipt, po.name)
		po.submit()
	...
	def test_return_against_purchase_order(self):
		...
		from epaas.stock.doctype.purchase_receipt.test_purchase_receipt \
				import make_purchase_receipt as make_purchase_receipt_return
		from epaas.accounts.doctype.purchase_invoice.test_purchase_invoice \
	...
	def test_uom_integer_validation(self):
		from epaas.utilities.transaction_base import UOMMustBeIntegerError
	...
	def test_group_same_items(self):
		...
		dataent.get_doc({
			...
			"currency" : dataent.get_cached_value('Company',  "_Test Company",  "default_currency"),
			"conversion_factor" : 1,
	...
	def test_po_for_blocked_supplier_all(self):
		...
		self.assertRaises(dataent.ValidationError, create_purchase_order)
	...
	def test_po_for_blocked_supplier_payments(self):
		...
		self.assertRaises(
			dataent.ValidationError, get_payment_entry, dt='Purchase Order', dn=po.name, bank_account="_Test Bank - _TC")
	...
	def test_reserved_qty_subcontract_po(self):
		...
		se = dataent.get_doc(make_subcontract_transfer_entry(po.name, rm_item_string))
		se.to_warehouse = "_Test Warehouse 1 - _TC"
		se.save()
		se.submit()

		bin3 = dataent.db.get_value("Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
		...
		# Cancel Stock Entry
		se.cancel()
		bin10 = dataent.db.get_value("Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract", as_dict=1)
		...
		po.reload()
		po.cancel()
		bin11 = dataent.db.get_value("Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract", as_dict=1)
	...
	def test_exploded_items_in_subcontracted(self):
		...
		name = dataent.db.get_value('BOM', {'item': item_code}, 'name')
		bom = dataent.get_doc('BOM', name)
...
def update_backflush_based_on(based_on):
	doc = dataent.get_doc('Buying Settings')
...
def create_purchase_order(**args):
	...
	po.currency = args.currency or dataent.get_cached_value('Company',  po.company,  "default_currency")
