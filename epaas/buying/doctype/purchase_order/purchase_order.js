...
dataent.ui.form.on("Purchase Order", {
	setup: function(frm) {
...
dataent.ui.form.on("Purchase Order Item", {
...
epaas.buying.PurchaseOrderController = epaas.buying.BuyingController.extend({
	...
	get_items_from_open_material_requests: function() {
		epaas.utils.map_current_doc({
			method: "epaas.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			...
			if(me.values) {
				me.values.sub_con_rm_items.map((row,i) => {
					if (!row.item_code || !row.rm_item_code || !row.warehouse || !row.qty || row.qty === 0) {
						dataent.throw(__("Item Code, warehouse, quantity are required on row" + (i+1)));
					}
				})
	...
	_make_rm_stock_entry: function(rm_items) {
		dataent.call({
			method:"epaas.buying.doctype.purchase_order.purchase_order.make_rm_stock_entry",
			args: {
					...
					while (i < item_length) {
						...
						(r.message[0] || []).forEach(function(d) {
							if (d.qty > 0 && qty > 0 && me.frm.doc.items[i].item_code == d.item_code && !me.frm.doc.items[i].material_request_item)
							{
								...
								dataent.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + me.frm.doc.items[i].idx + ")");
								if (qty > 0) {
									dataent.msgprint("Splitting " + qty + " units of " + d.item_code);
									var new_row = dataent.model.add_child(me.frm.doc, me.frm.doc.items[i].doctype, "items");
									item_length++;
...
$.extend(cur_frm.cscript, new epaas.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	dataent.call({
		method: "epaas.buying.doctype.purchase_order.purchase_order.update_status",
...
dataent.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
