...
from dataent.utils import nowdate, get_last_day, getdate, add_days, add_years
from epaas.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import make_supplier_scorecard
...
class SupplierScorecard(Document):
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = dataent.get_doc('Supplier Scorecard Period', scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and \
				...
				for fieldname in ('prevent_pos', 'prevent_rfqs','warn_rfqs','warn_pos'):
					...
					dataent.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@dataent.whitelist()
def get_timeline_data(doctype, name):
	# Get a list of all the associated scorecards
	scs = dataent.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = dataent.db.sql("""
		SELECT
	...
	for sc in scorecards:
		start_date, end_date, total_score = dataent.db.get_value('Supplier Scorecard Period', sc.name, ['start_date', 'end_date', 'total_score'])
...
def make_all_scorecards(docname):
	...
	sc = dataent.get_doc('Supplier Scorecard', docname)
	supplier = dataent.get_doc('Supplier',sc.supplier)
	...
	while (start_date < todays) and (end_date <= todays):
		...
		end_date = get_scorecard_date(sc.period, start_date)
	if scp_count > 0:
		dataent.msgprint(_("Created {0} scorecards for {1} between: ").format(scp_count, sc.supplier) + str(first_start_date) + " - " + str(last_end_date))
	return scp_count
