...
dataent.query_reports["Quoted Item Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = dataent.query_report.get_filter_value('supplier_quotation');
				if (quote != "") {
					return {
						query: "epaas.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
	...
	onload: (report) => {
		...
		report.page.add_inner_button(__("Select Default Supplier"), () => {
			let reporter = dataent.query_reports["Quoted Item Comparison"];
		...
		dialog.set_primary_action("Set Default Supplier", () => {
			...
			if(values) {
				...
				dataent.call({
					...
					freeze: true,
					callback: (r) => {
						dataent.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					}
				});
			}
