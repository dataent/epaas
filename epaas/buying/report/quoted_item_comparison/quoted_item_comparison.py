...
def get_quote_list(item, qty_list):
	...
	suppliers = []
	price_data = []
	company_currency = dataent.db.get_default("currency")
	float_precision = cint(dataent.db.get_default("float_precision")) or 2 
	# Get the list of suppliers
	for root in dataent.db.sql("""select parent, qty, rate from `tabSupplier Quotation Item`
		where item_code=%s and docstatus < 2""", item, as_dict=1):
		for splr in dataent.db.sql("""select supplier from `tabSupplier Quotation`
	...
	for root in set(suppliers):
		supplier_currency = dataent.db.get_value("Supplier", root, "default_currency")
...
def get_quantity_list(item):
	...
	if item:
		qty_list = dataent.db.sql("""select distinct qty from `tabSupplier Quotation Item`
