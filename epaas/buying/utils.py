	...
	for d in doc.get("items"):
		if not d.qty:
			if doc.doctype == "Purchase Receipt" and d.rejected_qty:
				continue
			dataent.throw(_("Please enter quantity for Item {0}").format(d.item_code))

		# update with latest quantities
		bin = dataent.db.sql("""select projected_qty from `tabBin` where
		...
		item = dataent.db.sql("""select is_stock_item,
		...
		if item.is_stock_item==1 and d.qty and not d.warehouse and not d.get("delivered_by_supplier"):
			dataent.throw(_("Warehouse is mandatory for stock Item {0} in row {1}").format(d.item_code, d.idx))

		items.append(cstr(d.item_code))

	if items and len(items) != len(set(items)) and \
		not cint(dataent.db.get_single_value("Buying Settings", "allow_multiple_items") or 0):
		dataent.throw(_("Same item cannot be entered multiple times."))

def check_for_closed_status(doctype, docname):
	status = dataent.db.get_value(doctype, docname, "status")

	if status == "Closed":
		dataent.throw(_("{0} {1} status is {2}").format(doctype, docname, status), dataent.InvalidStatusError)
...
def get_linked_material_requests(items):
	...
	mr_list = []
	for item in items:
		material_request = dataent.db.sql("""SELECT distinct mr.name AS mr_name, 
