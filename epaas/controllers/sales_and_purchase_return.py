...
from dataent.utils import flt, get_datetime, format_datetime

class StockOverReturnError(dataent.ValidationError): pass
...
def validate_return_against(doc):
	...
	if not dataent.db.exists(filters):
		...
		if get_datetime(return_posting_datetime) < get_datetime(ref_posting_datetime):
			dataent.throw(_("Posting timestamp must be after {0}").format(format_datetime(ref_posting_datetime)))

		# validate same exchange rate
		if doc.conversion_rate != ref_doc.conversion_rate:
			dataent.throw(_("Exchange Rate must be same as {0} {1} ({2})")
				.format(doc.doctype, doc.return_against, ref_doc.conversion_rate))

		# validate update stock
		if doc.doctype == "Sales Invoice" and doc.update_stock and not ref_doc.update_stock:
				dataent.throw(_("'Update Stock' can not be checked because items are not delivered via {0}")
					.format(doc.return_against))

def validate_returned_items(doc):
	from epaas.stock.doctype.serial_no.serial_no import get_serial_nos

	valid_items = dataent._dict()
	...
	for d in dataent.db.sql("""select {0} from `tab{1} Item` where parent = %s"""
		.format(select_fields, doc.doctype), doc.return_against, as_dict=1):
			valid_items = get_ref_item_dict(valid_items, d)

	if doc.doctype in ("Delivery Note", "Sales Invoice"):
		for d in dataent.db.sql("""select item_code, qty, serial_no, batch_no from `tabPacked Item`
	...
	for d in doc.get("items"):
		if flt(d.qty) < 0 or d.get('received_qty') < 0:
			if d.item_code not in valid_items:
				dataent.throw(_("Row # {0}: Returned Item {1} does not exists in {2} {3}")
					.format(d.idx, d.item_code, doc.doctype, doc.return_against))
			else:
				ref = valid_items.get(d.item_code, dataent._dict())
				validate_quantity(doc, d, ref, valid_items, already_returned_items)

				if ref.rate and doc.doctype in ("Delivery Note", "Sales Invoice") and flt(d.rate) > ref.rate:
					dataent.throw(_("Row # {0}: Rate cannot be greater than the rate used in {1} {2}")
						.format(d.idx, doc.doctype, doc.return_against))

				elif ref.batch_no and d.batch_no not in ref.batch_no:
					dataent.throw(_("Row # {0}: Batch No must be same as {1} {2}")
						.format(d.idx, doc.doctype, doc.return_against))

				elif ref.serial_no:
					if not d.serial_no:
						dataent.throw(_("Row # {0}: Serial No is mandatory").format(d.idx))
					else:
						serial_nos = get_serial_nos(d.serial_no)
						for s in serial_nos:
							if s not in ref.serial_no:
								dataent.throw(_("Row # {0}: Serial No {1} does not match with {2} {3}")
									.format(d.idx, s, doc.doctype, doc.return_against))

				if warehouse_mandatory and dataent.db.get_value("Item", d.item_code, "is_stock_item") \
					and not d.get("warehouse"):
						dataent.throw(_("Warehouse is mandatory"))

			items_returned = True

	if not items_returned:
		dataent.throw(_("Atleast one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, args, ref, valid_items, already_returned_items):
	...
	stock_qty_precision = get_field_precision(dataent.get_meta(doc.doctype + " Item")
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				dataent.throw(_("{0} must be negative in return document").format(label))
			elif returned_qty >= reference_qty and args.get(column):
				dataent.throw(_("Item {0} has already been returned")
					.format(args.item_code), StockOverReturnError)
			elif abs(flt(current_stock_qty, stock_qty_precision)) > max_returnable_qty:
				dataent.throw(_("Row # {0}: Cannot return more than {1} for Item {2}")
					.format(args.idx, max_returnable_qty, args.item_code), StockOverReturnError)

def get_ref_item_dict(valid_items, ref_item_row):
	from epaas.stock.doctype.serial_no.serial_no import get_serial_nos

	valid_items.setdefault(ref_item_row.item_code, dataent._dict({
		"qty": 0,
		"rate": 0,
		"stock_qty": 0,
		"rejected_qty": 0,
...
def get_already_returned_items(doc):
	...
	for d in data:
		items.setdefault(d.item_code, dataent._dict({
			"qty": d.get("qty"),
...
def make_return_doc(doctype, source_name, target_doc=None):
	...
	company = dataent.db.get_value("Delivery Note", source_name, "company")
	default_warehouse_for_sales_return = dataent.db.get_value("Company", company, "default_warehouse_for_sales_return")
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = dataent.db.get_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = dataent.db.get_value("Print Heading", _("Debit Note"))
