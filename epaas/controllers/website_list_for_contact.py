...
from dataent.utils import flt, has_common
from dataent.utils.user import is_website_user

def get_list_context(context=None):
	return {
		"global_number_format": dataent.db.get_default("number_format") or "#,###.##",
		"currency": dataent.db.get_default("currency"),
		"currency_symbols": json.dumps(dict(dataent.db.sql("""select name, symbol
	...
	if txt:
		if meta.get_field('items'):
			if meta.get_field('items').options:
				...
				for item in dataent.get_all(child_doctype, {"item_name": ['like', "%" + txt + "%"]}):
					child = dataent.get_doc(child_doctype, item.name)
					or_filters.append([doctype, "name", "=", child.parent])

	if or_filters:
		for r in dataent.get_list(doctype, fields=fields,filters=filters, or_filters=or_filters,
...
def rfq_transaction_list(parties_doctype, doctype, parties, limit_start, limit_page_length):
	data = dataent.db.sql("""select distinct parent as name, supplier from `tab{doctype}`
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	if customers:
		return dataent.get_all(doctype, filters=[(doctype, "customer", "in", customers),
			(doctype, "name", "=", doc.name)]) and True or False
	elif suppliers:
		fieldname = 'suppliers' if doctype == 'Request for Quotation' else 'supplier'
		return dataent.get_all(doctype, filters=[(doctype, fieldname, "in", suppliers),
			(doctype, "name", "=", doc.name)]) and True or False
	else:
		return False
