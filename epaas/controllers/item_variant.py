...
class ItemVariantExistsError(dataent.ValidationError): pass
class InvalidItemAttributeValueError(dataent.ValidationError): pass
class ItemTemplateCannotHaveStock(dataent.ValidationError): pass
...
def get_variant(template, args=None, variant=None, manufacturer=None,
	...
	"""
	item_template = dataent.get_doc('Item', template)
	...
	if item_template.variant_based_on=='Manufacturer' and manufacturer:
		...
		if not args:
			dataent.throw(_("Please specify at least one attribute in the Attributes table"))
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		dataent.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
	...
	if not (is_in_range and is_incremental):
		dataent.throw(_("Value for Attribute {0} must be within the range of {1} to {2} in the increments of {3} for Item {4}")\
...
def validate_item_attribute_value(attributes_list, attribute, attribute_value, item):
	allow_rename_attribute_value = dataent.db.get_single_value('Item Variant Settings', 'allow_rename_attribute_value')
	if allow_rename_attribute_value:
		pass
	elif attribute_value not in attributes_list:
		dataent.throw(_("Value {0} for Attribute {1} does not exist in the list of valid Item Attribute Values for Item {2}").format(
			attribute_value, attribute, item), InvalidItemAttributeValueError, title=_('Invalid Attribute'))

def get_attribute_values(item):
	if not dataent.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in dataent.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in dataent.get_all('Item Variant Attribute',
		...
		dataent.flags.attribute_values = attribute_values
		dataent.flags.numeric_values = numeric_values

	return dataent.flags.attribute_values, dataent.flags.numeric_values

def find_variant(template, args, variant_item_code=None):
	conditions = ["""(iv_attribute.attribute="{0}" and iv_attribute.attribute_value="{1}")"""\
		.format(dataent.db.escape(key), dataent.db.escape(cstr(value))) for key, value in args.items()]
	...
	possible_variants = dataent.db.sql_list("""select name from `tabItem` item
	...
	for variant in possible_variants:
		variant = dataent.get_doc("Item", variant)
...
def enqueue_multiple_variant_creation(item, args):
	...
	if total_variants >= 600:
		dataent.msgprint("Please do not create more than 500 items at a time", raise_exception=1)
		return
	if total_variants < 10:
		return create_multiple_variants(item, args)
	else:
		dataent.enqueue("epaas.controllers.item_variant.create_multiple_variants",
			item=item, args=args, now=dataent.flags.in_test);
		return 'queued'
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in dataent.get_all("Variant Field", fields = ['field_name'])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		item_attribute = dataent.db.sql("""select i.numeric_values, v.abbr
		...
		if not item_attribute:
			return
			# dataent.throw(_('Invalid attribute {0} {1}').format(dataent.bold(attr.attribute),
			# 	dataent.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template, args):
	variant_based_on = dataent.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
