...
from dataent.utils import cint, flt, round_based_on_smallest_currency_fraction
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				dataent.throw(_("Please select Apply Discount On"))
	...
	def calculate_total_advance(self):
		if self.doc.docstatus < 2:
			...
			if invoice_total > 0 and self.doc.total_advance > invoice_total:
				dataent.throw(_("Advance amount cannot be greater than {0} {1}")
	...
	def calculate_margin(self, item):
		...
		if item.price_list_rate:
			if item.pricing_rule and not self.doc.ignore_pricing_rule:
				pricing_rule = dataent.get_doc('Pricing Rule', item.pricing_rule)
...
def get_itemised_tax_breakup_html(doc):
	if not doc.taxes:
		return
	dataent.flags.company = doc.company
	...
	for tax in taxes:
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				...
				itemised_tax[item_code][tax.description] = dataent._dict(dict(
					tax_rate = tax_rate,
...
def get_itemised_taxable_amount(items):
	itemised_taxable_amount = dataent._dict()
	for item in items:
