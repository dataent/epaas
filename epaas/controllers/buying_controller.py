...
from dataent.utils import flt,cint, cstr, getdate

from epaas.accounts.party import get_party_details
from epaas.stock.get_item_details import get_conversion_factor
from epaas.buying.utils import validate_for_items, update_last_purchase_rate
from epaas.stock.stock_ledger import get_valuation_rate
from epaas.stock.doctype.stock_entry.stock_entry import get_used_alternative_items
from epaas.stock.doctype.serial_no.serial_no import get_auto_serial_nos, auto_make_serial_nos, get_serial_nos
from dataent.contacts.doctype.address.address import get_address_display

from epaas.accounts.doctype.budget.budget import validate_expense_against_budget
from epaas.controllers.stock_controller import StockController

class BuyingController(StockController):
	def __setup__(self):
		if hasattr(self, "taxes"):
			self.flags.print_taxes_with_zero_amount = cint(dataent.db.get_single_value("Print Settings",
	...
	def validate(self):
		...
		if getattr(self, "supplier", None) and not self.supplier_name:
			self.supplier_name = dataent.db.get_value("Supplier", self.supplier, "supplier_name")
	...
	def set_supplier_from_item_default(self):
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				supplier = dataent.db.get_value("Item Default",
					{"parent": d.item_code, "company": self.company}, "default_supplier")
				if supplier:
					self.supplier = supplier
				else:
					item_group = dataent.db.get_value("Item", d.item_code, "item_group")
					supplier = dataent.db.get_value("Item Default",
	...
	def set_landed_cost_voucher_amount(self):
		for d in self.get("items"):
			lc_voucher_data = dataent.db.sql("""select sum(applicable_charges), cost_center
	...
	def validate_for_subcontracting(self):
		...
		if self.is_subcontracted == "Yes":
			if self.doctype in ["Purchase Receipt", "Purchase Invoice"] and not self.supplier_warehouse:
				dataent.throw(_("Supplier Warehouse mandatory for sub-contracted Purchase Receipt"))

			for item in self.get("items"):
				if item in self.sub_contracted_items and not item.bom:
					dataent.throw(_("Please select BOM in BOM field for Item {0}").format(item.item_code))

			if self.doctype == "Purchase Order":
				for supplied_item in self.get("supplied_items"):
					if not supplied_item.reserve_warehouse:
						dataent.throw(_("Reserved Warehouse is mandatory for Item {0} in Raw Materials supplied").format(dataent.bold(supplied_item.rm_item_code)))
	...
	def update_raw_materials_supplied_based_on_bom(self, item, raw_material_table):
		...
		item_wh = dataent._dict(dataent.db.sql("""select i.item_code, id.default_warehouse
		...
		for bom_item in bom_items:
			if self.doctype == "Purchase Order":
				...
				if dataent.db.get_value("Warehouse", reserve_warehouse, "company") != self.company:
	...
	def sub_contracted_items(self):
		if not hasattr(self, "_sub_contracted_items"):
			...
			item_codes = list(set(item.item_code for item in
				self.get("items")))
			if item_codes:
				self._sub_contracted_items = [r[0] for r in dataent.db.sql("""select name
		...
		for d in self.get("items"):
			if d.meta.get_field("stock_qty"):
				if not d.conversion_factor:
					dataent.throw(_("Row {0}: Conversion Factor is mandatory").format(d.idx))
				d.stock_qty = flt(d.qty) * flt(d.conversion_factor)

	def validate_purchase_return(self):
		for d in self.get("items"):
			if self.is_return and flt(d.rejected_qty) != 0:
				dataent.throw(_("Row #{0}: Rejected Qty can not be entered in Purchase Return").format(d.idx))
		...
		for d in self.get("items"):
			if flt(d.rejected_qty) and not d.rejected_warehouse:
				...
				if not d.rejected_warehouse:
					dataent.throw(_("Row #{0}: Rejected Warehouse is mandatory against rejected Item {1}").format(d.idx, d.item_code))
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if (flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty"))):
				dataent.throw(_("Accepted + Rejected Qty must be equal to Received quantity for Item {0}").format(d.item_code))
	...
	def validate_negative_quantity(self, item_row, field_list):
		...
		for fieldname in field_list:
			if flt(item_row[fieldname]) < 0:
				dataent.throw(_("Row #{0}: {1} can not be negative for item {2}".format(item_row['idx'],
					dataent.get_meta(item_row.doctype).get_label(fieldname), item_row['item_code'])))
		...
		for d in self.get('items'):
			if d.item_code in stock_items and d.warehouse:
				...
				if pr_qty:
					...
					if self.is_return:
						original_incoming_rate = dataent.db.get_value("Stock Ledger Entry",
	...
	def update_ordered_and_reserved_qty(self):
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				po_obj = dataent.get_doc("Purchase Order", po)

				if po_obj.status in ["Closed", "Cancelled"]:
					dataent.throw(_("{0} {1} is cancelled or closed").format(_("Purchase Order"), po),
						dataent.InvalidStatusError)
			...
			if d.is_fixed_asset:
				...
				if item_data.get('has_serial_no'):
					...
					if item_data.get('serial_no_series') and not d.serial_no:
						...
						dataent.throw(_("Serial no is mandatory for the item {0}").format(d.item_code))
	...
	def make_asset(self, row):
		if not row.asset_location:
			dataent.throw(_("Row {0}: Enter location for the asset item {1}").format(row.idx, row.item_code))

		item_data = dataent.db.get_value('Item',
			row.item_code, ['asset_naming_series', 'asset_category'], as_dict=1)

		purchase_amount = flt(row.base_net_amount + row.item_tax_amount)
		asset = dataent.get_doc({
			'doctype': 'Asset',
		...
		dataent.msgprint(_("Asset {0} created").format(asset.name))
		return asset.name

	def make_asset_movement(self, row):
		asset_movement = dataent.get_doc({
		...
		for d in self.get("items"):
			if d.is_fixed_asset and d.asset:
				...
				if delete_asset and asset.docstatus == 0:
					dataent.delete_doc("Asset", asset.name)
					d.db_set('asset', None)
					continue
	...
	def delete_linked_asset(self):
		...
		dataent.db.sql("delete from `tabAsset Movement` where reference_name=%s", self.name)
		dataent.db.sql("delete from `tabSerial No` where purchase_document_no=%s", self.name)
		...
		if self.schedule_date:
			for d in self.get('items'):
				...
				if (d.schedule_date and self.transaction_date and
					...
					dataent.throw(_("Row #{0}: Reqd by Date cannot be before Transaction Date").format(d.idx))
		else:
			dataent.throw(_("Please enter Reqd by Date"))
...
def get_items_from_bom(item_code, bom, exploded_item=1):
	...
	bom_items = dataent.db.sql("""select t2.item_code, t2.name,
...
def get_backflushed_subcontracted_raw_materials_from_se(purchase_orders, purchase_receipt):
	return dataent._dict(dataent.db.sql("""
		select
...
def get_asset_item_details(asset_items):
	asset_items_data = {}
	for d in dataent.get_all('Item', fields = ["name", "has_serial_no", "serial_no_series"],
...
def validate_item_type(doc, fieldname, message):
	...
	item_list = ", ".join(["'%s'" % dataent.db.escape(d) for d in items])

	invalid_items = [d[0] for d in dataent.db.sql("""
