...
from dataent.model import mapper
...
class TestMapper(unittest.TestCase):
	def test_map_docs(self):
		...
		items = dataent.get_all("Item", fields = ["name", "item_code"], filters = {'is_sales_item': 1, 'has_variants': 0})
	...
	def make_sales_order(self):
		...
		so = dataent.get_doc(dataent.get_test_records('Sales Order')[0])
