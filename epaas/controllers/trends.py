...
def validate_filters(filters):
	for f in ["Fiscal Year", "Based On", "Period", "Company"]:
		if not filters.get(f.lower().replace(" ", "_")):
			dataent.throw(_("{0} is mandatory").format(f))

	if not dataent.db.exists("Fiscal Year", filters.get("fiscal_year")):
		dataent.throw(_("Fiscal Year: {0} does not exists").format(filters.get("fiscal_year")))

	if filters.get("based_on") == filters.get("group_by"):
		dataent.throw(_("'Based On' and 'Group By' can not be same"))
...
def get_data(filters, conditions):
	...
	year_start_date, year_end_date = dataent.db.get_value("Fiscal Year",
	...
	if filters.get("group_by"):
		...
		if filters.get('based_on') in ['Item','Customer','Supplier']:
			...
			inc = 1
		data1 = dataent.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
		...
		for d in range(len(data1)):
			...
			row = dataent.db.sql("""select DISTINCT(%s) from `tab%s` t1, `tab%s Item` t2 %s
			...
			for i in range(len(row)):
				...
				row1 = dataent.db.sql(""" select %s , %s from `tab%s` t1, `tab%s Item` t2 %s
							where t2.parent = t1.name and t1.company = %s and %s between %s and %s
		...
		data = dataent.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
					where t2.parent = t1.name and t1.company = %s and %s between %s and %s and
					t1.docstatus = 1 %s %s
					group by %s
				""" %
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		...
		if trans in ['Sales Invoice', 'Delivery Note', 'Sales Order']:
			...
			dataent.throw(_("Project-wise data is not available for Quotation"))
