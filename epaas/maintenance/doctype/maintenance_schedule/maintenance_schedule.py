...
from dataent.utils import add_days, getdate, cint, cstr

from dataent import throw, _
from epaas.utilities.transaction_base import TransactionBase, delete_events
from epaas.stock.utils import get_valid_serial_nos
from epaas.hr.doctype.employee.employee import get_holiday_list_for_employee

class MaintenanceSchedule(TransactionBase):
	def generate_schedule(self):
		self.set('schedules', [])
		dataent.db.sql("""delete from `tabMaintenance Schedule Detail`
	...
	def on_submit(self):
		...
		for d in self.get('items'):
			...
			if d.sales_person not in email_map:
				sp = dataent.get_doc("Sales Person", d.sales_person)
				try:
			...
			if no_email_sp:
				dataent.msgprint(
					dataent._("Setting Events to {0}, since the Employee attached to the below Sales Persons does not have a User ID{1}").format(
						self.owner, "<br>" + "<br>".join(no_email_sp)
				))

			scheduled_date = dataent.db.sql("""select scheduled_date from
				`tabMaintenance Schedule Detail` where sales_person=%s and item_code=%s and
				parent=%s""", (d.sales_person, d.item_code, self.name), as_dict=1)

			for key in scheduled_date:
				description =dataent._("Reference: {0}, Item Code: {1} and Customer: {2}").format(self.name, d.item_code, self.customer)	
				dataent.get_doc({
					"doctype": "Event",
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = dataent.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = dataent.get_cached_value('Company',  self.company,  "default_holiday_list")

		holidays = dataent.db.sql_list('''select holiday_date from `tabHoliday` where parent=%s''', holiday_list)
	...
	def validate_sales_order(self):
		for d in self.get('items'):
			if d.sales_order:
				chk = dataent.db.sql("""select ms.name from `tabMaintenance Schedule` ms,
	...
	def on_update(self):
		dataent.db.set(self, 'status', 'Draft')

	def update_amc_date(self, serial_nos, amc_expiry_date=None):
		for serial_no in serial_nos:
			serial_no_doc = dataent.get_doc("Serial No", serial_no)
			serial_no_doc.amc_expiry_date = amc_expiry_date
			serial_no_doc.save()

	def validate_serial_no(self, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			sr_details = dataent.db.get_value("Serial No", serial_no,
				["warranty_expiry_date", "amc_expiry_date", "warehouse", "delivery_date"], as_dict=1)

			if not sr_details:
				dataent.throw(_("Serial No {0} not found").format(serial_no))
