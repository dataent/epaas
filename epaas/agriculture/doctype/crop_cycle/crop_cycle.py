...
class CropCycle(Document):
	...
	def set_missing_values(self):
		crop = dataent.get_doc('Crop', self.crop)
	...
	def create_tasks_for_diseases(self):
		for disease in self.detected_disease:
			if not disease.tasks_created:
				...
				dataent.msgprint(_("Tasks have been created for managing the {0} disease (on row {1})".format(disease.disease, disease.idx)))

	def import_disease_tasks(self, disease, start_date):
		disease_doc = dataent.get_doc('Disease', disease)
		self.create_task(disease_doc.treatment_task, self.name, start_date)

	def create_project(self, period, crop_tasks):
		project = dataent.new_doc("Project")
		project.update({
