...
import dataent
from dataent.utils import datetime
...
class TestCropCycle(unittest.TestCase):
	def test_crop_cycle_creation(self):
		cycle = dataent.get_doc('Crop Cycle', 'Basil from seed 2017')
		self.assertTrue(dataent.db.exists('Crop Cycle', 'Basil from seed 2017'))
...
def check_task_creation():
	...
	all_tasks = dataent.get_all('Task')

	for task in all_tasks:
		sample_task = dataent.get_doc('Task', task.name)
...
def check_project_creation():
	return True if dataent.db.exists('Project', 'Basil from seed 2017') else False
