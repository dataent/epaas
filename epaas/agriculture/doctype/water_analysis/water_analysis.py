...
class WaterAnalysis(Document):
	def load_contents(self):
		docs = dataent.get_all("Agriculture Analysis Criteria", filters={'linked_doctype':'Water Analysis'})
		for doc in docs:
	...
	def validate(self):
		if self.collection_datetime > self.laboratory_testing_datetime:
			dataent.throw(_('Lab testing datetime cannot be before collection datetime'))
		if self.laboratory_testing_datetime > self.result_datetime:
			dataent.throw(_('Lab result datetime cannot be before testing datetime'))