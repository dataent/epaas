...
from dataent.utils import time_diff_in_hours, flt
...
def get_data(filters):
	...
	if(filters.from_date > filters.to_date):
		dataent.msgprint(_(" From Date can not be greater than To Date"))
		return data

	timesheets = get_timesheets(filters)

	filters.from_date = dataent.utils.get_datetime(filters.from_date)
	filters.to_date = dataent.utils.add_to_date(dataent.utils.get_datetime(filters.to_date), days=1, seconds=-1)
...
def get_timesheets(filters):
	...
	timesheets = dataent.get_all("Timesheet", filters=record_filters, fields=["employee", "employee_name", "name"])
	timesheet_map = dataent._dict()
	for d in timesheets:
