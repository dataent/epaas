...
from dataent.utils import (flt, getdate, get_url, now,
	nowtime, get_time, today, get_datetime, add_days)
from epaas.controllers.queries import get_filters_cond
from dataent.desk.reportview import get_match_cond
from epaas.hr.doctype.daily_work_summary.daily_work_summary import get_users_email
from epaas.hr.doctype.daily_work_summary_group.daily_work_summary_group import is_holiday_today
from dataent.model.document import Document

class Project(Document):
	def get_feed(self):
		return '{0}: {1}'.format(_(self.status), dataent.safe_decode(self.project_name))
	...
	def onload(self):
		...
		self.set_onload('activity_summary', dataent.db.sql('''select activity_type,
	...
	def load_tasks(self):
		...
		project_task_custom_fields = dataent.get_all("Custom Field", {"dt": "Project Task"}, "fieldname")
	...
	def get_tasks(self):
		if self.name is None:
			...
			return dataent.get_all("Task", "*", filters, order_by="exp_start_date asc, status asc")
	...
	def validate_project_name(self):
		if self.get("__islocal") and dataent.db.exists("Project", self.project_name):
			dataent.throw(_("Project {0} already exists").format(dataent.safe_decode(self.project_name)))
			...
			for d in self.tasks:
				if self.expected_start_date:
					if d.start_date and getdate(d.start_date) < getdate(self.expected_start_date):
						dataent.throw(_("Start date of task <b>{0}</b> cannot be less than <b>{1}</b> expected start date <b>{2}</b>")
							.format(d.title, self.name, self.expected_start_date))
					if d.end_date and getdate(d.end_date) < getdate(self.expected_start_date):
						dataent.throw(_("End date of task <b>{0}</b> cannot be less than <b>{1}</b> expected start date <b>{2}</b>")
							.format(d.title, self.name, self.expected_start_date))

				if self.expected_end_date:
					if d.start_date and getdate(d.start_date) > getdate(self.expected_end_date):
						dataent.throw(_("Start date of task <b>{0}</b> cannot be greater than <b>{1}</b> expected end date <b>{2}</b>")
							.format(d.title, self.name, self.expected_end_date))
					if d.end_date and getdate(d.end_date) > getdate(self.expected_end_date):
						dataent.throw(_("End date of task <b>{0}</b> cannot be greater than <b>{1}</b> expected end date <b>{2}</b>")
							.format(d.title, self.name, self.expected_end_date))

		if self.expected_start_date and self.expected_end_date:
			if getdate(self.expected_end_date) < getdate(self.expected_start_date):
				dataent.throw(_("Expected End Date can not be less than Expected Start Date"))

	def validate_weights(self):
		for task in self.tasks:
			if task.task_weight is not None:
				if task.task_weight < 0:
					dataent.throw(_("Task weight cannot be negative"))
	...
	def sync_tasks(self):
		...
		custom_fields = dataent.get_all("Custom Field", {"dt": "Project Task",
			"fieldtype": ("not in", exclude_fieldtype)}, "fieldname")

		for d in custom_fields:
			fields.append(d.fieldname)

		for d in dataent.get_all('Project Task',
			fields = fields,
		...
		for t in self.tasks:
			if t.task_id:
				task = dataent.get_doc("Task", t.task_id)
			else:
		...
		# delete
		for t in dataent.get_all("Task", ["name"], {"project": self.name, "name": ("not in", task_names)}):
	...
	def after_insert(self):
		if self.sales_order:
			dataent.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def update_percent_complete(self, from_validate=False):
		if not self.tasks: return
		total = dataent.db.sql("""select count(name) from tabTask where project=%s""", self.name)[0][0]
		if not total and self.percent_complete:
			self.percent_complete = 0
		if (self.percent_complete_method == "Task Completion" and total > 0) or (
			not self.percent_complete_method and total > 0):
			completed = dataent.db.sql("""select count(name) from tabTask where
				project=%s and status in ('Closed', 'Cancelled')""", self.name)[0][0]
			self.percent_complete = flt(flt(completed) / total * 100, 2)

		if (self.percent_complete_method == "Task Progress" and total > 0):
			progress = dataent.db.sql("""select sum(progress) from tabTask where
				project=%s""", self.name)[0][0]
			self.percent_complete = flt(flt(progress) / total, 2)

		if (self.percent_complete_method == "Task Weight" and total > 0):
			weight_sum = dataent.db.sql("""select sum(task_weight) from tabTask where
				project=%s""", self.name)[0][0]
			weighted_progress = dataent.db.sql("""select progress,task_weight from tabTask where
				project=%s""", self.name, as_dict=1)
			pct_complete = 0
			for row in weighted_progress:
				pct_complete += row["progress"] * dataent.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_costing(self):
		from_time_sheet = dataent.db.sql("""select
		...
		from_expense_claim = dataent.db.sql("""select
	...
	def update_purchase_costing(self):
		total_purchase_cost = dataent.db.sql("""select sum(base_net_amount)
			from `tabPurchase Invoice Item` where project = %s and docstatus=1""", self.name)

		self.total_purchase_cost = total_purchase_cost and total_purchase_cost[0][0] or 0

	def update_sales_amount(self):
		total_sales_amount = dataent.db.sql("""select sum(base_net_total)
			from `tabSales Order` where project = %s and docstatus=1""", self.name)

		self.total_sales_amount = total_sales_amount and total_sales_amount[0][0] or 0

	def update_billed_amount(self):
		total_billed_amount = dataent.db.sql("""select sum(base_net_total)
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			dataent.db.set_value('Project', new_name, 'copied_from', new_name)
	...
	def send_welcome_email(self):
		...
		for user in self.users:
			if user.welcome_email_sent == 0:
				dataent.sendmail(user.user, subject=_("Project Collaboration Invitation"),
	...
	def delete_task(self):
		...
		for d in self.get('deleted_task_list'):
			# unlink project
			dataent.db.set_value('Task', d, 'project', '')
	...
	def update_dependencies_on_duplicated_project(self):
		...
		if self.name != self.copied_from and self.get('__unsaved'):
			...
			for task in self.tasks:
				...
				dependency_map[task.title] = [x['subject'] for x in dataent.get_list(
					'Task Depends On', {"parent": name}, ['subject'])]

			for key, value in iteritems(dependency_map):
				task_name = dataent.db.get_value('Task', {"subject": key, "project": self.name })

				task_doc = dataent.get_doc('Task', task_name)

				for dt in value:
					dt_name = dataent.db.get_value('Task', {"subject": dt, "project": self.name})
...
def get_timeline_data(doctype, name):
	...
	return dict(dataent.db.sql('''select unix_timestamp(from_time), count(*)
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	return dataent.db.sql('''select distinct project.*
...
def get_cost_center_name(project):
	return dataent.db.get_value("Project", project, "cost_center")
...
def allow_to_make_project_update(project, time, frequency):
	data = dataent.db.sql(""" SELECT name from `tabProject Update`
...
def get_projects_for_collect_progress(frequency, fields):
	...
	return dataent.get_all("Project", fields = fields,
...
def send_project_update_email_to_users(project):
	...
	incoming_email_account = dataent.db.get_value('Email Account',
...
def collect_project_status():
	for data in dataent.get_all("Project Update",
		...
		for d in replies:
			...
			user_data = dataent.db.get_values("User", {"email": d.sender},
			...
			doc.append("users", {
				...
				'project_status': dataent.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in dataent.get_all("Project Update",
		{'date': yesterday, 'sent': 0}):
		doc = dataent.get_doc("Project Update", d.name)

		project_doc = dataent.get_doc('Project', doc.project)
		...
		dataent.sendmail(recipients=get_users_email(project_doc),
...
def update_project_sales_billing():
	sales_update_frequency = dataent.db.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif (sales_update_frequency == "Monthly" and dataent.utils.now_datetime().day != 1):
		return

	#Else simply fallback to Daily
	exists_query = '(SELECT 1 from `tab{doctype}` where docstatus = 1 and project = `tabProject`.name)'
	project_map = {}
	for project_details in dataent.db.sql('''
		...
		project = project_map.setdefault(project_details.name, dataent.get_doc('Project', project_details.name))
...
def create_kanban_board_if_not_exists(project):
	from dataent.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	if not dataent.db.exists('Kanban Board', project):
