...
dataent.treeview_settings['Task'] = {
	get_tree_nodes: "epaas.projects.doctype.task.task.get_children",
	add_tree_node: "epaas.projects.doctype.task.task.add_node",
	filters: [
		{
			fieldname: "project",
			fieldtype:"Link",
		...
		},
		{
			...
			label: __("Task"),
			get_query: function() {
				var me = dataent.treeview_settings['Task'];
	...
	onload: function(me) {
		dataent.treeview_settings['Task'].page = {};
		$.extend(dataent.treeview_settings['Task'].page, me.page);
		me.make_tree();
	},
	toolbar: [
		{
			...
			click: function(node) {
				...
				const dialog = new dataent.ui.Dialog({
					...
					primary_action: function() {
						dialog.hide();
						return dataent.call({
							method: "epaas.projects.doctype.task.task.add_multiple_tasks",
							args: {
