...
import dataent
import unittest
from dataent.utils import getdate, nowdate, add_days

from epaas.projects.doctype.task.task import CircularReferenceError
...
class TestTask(unittest.TestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(dataent.db.get_value("Task", task2.name, "exp_start_date"),
			getdate(add_days(nowdate(), 21)))
		self.assertEqual(dataent.db.get_value("Task", task2.name, "exp_end_date"),
			getdate(add_days(nowdate(), 25)))

		self.assertEqual(dataent.db.get_value("Task", task3.name, "exp_start_date"),
			getdate(add_days(nowdate(), 26)))
		self.assertEqual(dataent.db.get_value("Task", task3.name, "exp_end_date"),
			getdate(add_days(nowdate(), 30)))

	def test_close_assignment(self):
		if not dataent.db.exists("Task", "Test Close Assignment"):
		...
		def get_owner_and_status():
			return dataent.db.get_value("ToDo",
	...
	def test_overdue(self):
		...
		self.assertEqual(dataent.db.get_value("Task", task.name, "status"), "Overdue")

def create_task(subject, start=None, end=None, depends_on=None, project=None, save=True):
	if not dataent.db.exists("Task", subject):
		task = dataent.new_doc('Task')
		task.status = "Open"
		...
		task = dataent.get_doc("Task", subject)
