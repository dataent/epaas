...
from dataent.utils import flt, time_diff_in_hours, get_datetime, getdate, cint, date_diff, add_to_date
...
class OverWorkLoggedError(dataent.ValidationError): pass

class Timesheet(Document):
	def onload(self):
		self.get("__onload").maintain_bill_work_hours_same = dataent.db.get_single_value('HR Settings', 'maintain_bill_work_hours_same')
	...
	def set_employee_name(self):
		if self.employee and not self.employee_name:
			self.employee_name = dataent.db.get_value('Employee', self.employee, 'employee_name')
	...
	def calculate_std_hours(self):
		std_working_hours = dataent.get_value("Company", self.company, 'standard_working_hours')
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				dataent.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				dataent.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				dataent.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = dataent.get_doc("Task", data.task)
				task.update_time_and_costing()
				task.save()
				tasks.append(data.task)

			elif data.project and data.project not in projects:
				dataent.get_doc("Project", data.project).update_project()
	...
	def validate_overlap_for(self, fieldname, args, value, ignore_validation=False):
		...
		if existing:
			dataent.throw(_("Row {0}: From Time and To Time of {1} is overlapping with {2}")
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		existing = dataent.db.sql("""select ts.name as name, tsd.from_time as from_time, tsd.to_time as to_time from
...
def get_projectwise_timesheet_data(project, parent=None):
	...
	return dataent.db.sql("""select name, parent, billing_hours, billing_amount as billing_amt
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return dataent.db.sql("""select distinct tsd.parent from `tabTimesheet Detail` tsd,
...
def get_timesheet_data(name, project):
	...
	if project and project!='':
		...
		data = dataent.get_all('Timesheet',
...
def make_sales_invoice(source_name, item_code=None, customer=None):
	target = dataent.new_doc("Sales Invoice")
	timesheet = dataent.get_doc('Timesheet', source_name)

	if not timesheet.total_billable_hours:
		dataent.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		dataent.throw(_("Invoice already created for all billing hours"))
...
def make_salary_slip(source_name, target_doc=None):
	target = dataent.new_doc("Salary Slip")
...
def set_missing_values(time_sheet, target):
	doc = dataent.get_doc('Timesheet', time_sheet)
...
def get_activity_cost(employee=None, activity_type=None):
	rate = dataent.db.get_values("Activity Cost", {"employee": employee,
		"activity_type": activity_type}, ["costing_rate", "billing_rate"], as_dict=True)
	if not rate:
		rate = dataent.db.get_values("Activity Type", {"activity_type": activity_type},
...
def get_events(start, end, filters=None):
	...
	return dataent.db.sql("""select `tabTimesheet Detail`.name as name,
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	customer = dataent.db.sql('''SELECT dl.link_name FROM `tabContact` AS c inner join \
	...
	if customer:
		...
		sales_invoice = dataent.db.sql('''SELECT name FROM `tabSales Invoice` WHERE customer = %s''',customer)
		# Return timesheet related data to web portal.
		return dataent. db.sql('''SELECT ts.name, tsd.activity_type, ts.status, ts.total_billable_hours, \
