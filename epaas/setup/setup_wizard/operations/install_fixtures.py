...
from dataent.desk.page.setup_wizard.setup_wizard import make_records
from dataent.utils import cstr, getdate
from epaas.accounts.doctype.account.account import RootNotEditable
...
def install(country=None):
	...
	# path = dataent.get_app_path('epaas', 'regional', dataent.scrub(country))
	# if os.path.exists(path.encode("utf-8")):
	# 	dataent.get_attr("epaas.regional.{0}.setup.setup_company_independent_fixtures".format(dataent.scrub(country)))()
...
def install_defaults(args=None):
	...
	current_fiscal_year = dataent.get_all("Fiscal Year")[0]
	...
	if args.bank_account:
		company_name = args.company_name
		bank_account_group =  dataent.db.get_value("Account",
			{"account_type": "Bank", "is_group": 1, "root_type": "Asset",
				"company": company_name})
		if bank_account_group:
			bank_account = dataent.get_doc({
				"doctype": "Account",
			...
			try:
				...
				dataent.db.set_value("Company", args.company_name, "default_bank_account", bank_account.name, update_modified=False)

				return doc
			except RootNotEditable:
				dataent.throw(_("Bank account cannot be named as {0}").format(args.bank_account))
	...
	records = [
		...
		# Shopping cart: needs price lists
		{
			...
			# uh oh
			'price_list': dataent.db.get_value("Price List", {"selling": 1}),
