...
def make_tax_account(company, account_name, tax_rate):
	...
	if tax_group:
		try:
			...
			if dataent.message_log: dataent.message_log.pop()
			abbr = dataent.get_cached_value('Company',  company,  'abbr')
			account = '{0} - {1}'.format(account_name, abbr)
			return dataent.get_doc('Account', account)
...
def make_sales_and_purchase_tax_templates(accounts, template_name=None):
	...
	# Sales
	dataent.get_doc(copy.deepcopy(sales_tax_template)).insert(ignore_permissions=True)

	# Purchase
	purchase_tax_template = copy.deepcopy(sales_tax_template)
	purchase_tax_template["doctype"] = "Purchase Taxes and Charges Template"

	doc = dataent.get_doc(purchase_tax_template)
	doc.insert(ignore_permissions=True)

def get_tax_account_group(company):
	tax_group = dataent.db.get_value("Account",
		{"account_name": "Duties and Taxes", "is_group": 1, "company": company})
	if not tax_group:
		tax_group = dataent.db.get_value("Account", {"is_group": 1, "root_type": "Liability",
