...
class GlobalDefaults(Document):
	...
	def on_update(self):
		...
		year_start_end_date = dataent.db.sql("""select year_start_date, year_end_date
		...
		if year_start_end_date:
			...
			if ysd and yed:
				dataent.db.set_default('year_start_date', ysd.strftime('%Y-%m-%d'))
				dataent.db.set_default('year_end_date', yed.strftime('%Y-%m-%d'))
