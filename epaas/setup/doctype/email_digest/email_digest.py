...
from dataent.utils import fmt_money, formatdate, format_time, now_datetime, \
	get_url_to_form, get_url_to_list, flt, getdate
from datetime import timedelta
from dateutil.relativedelta import relativedelta
from dataent.core.doctype.user.user import STANDARD_USERS
import dataent.desk.notifications
from epaas.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = dataent.db.get_value('Company',  self.company,  "default_currency")

	def get_users(self):
		"""get list of users"""
		user_list = dataent.db.sql("""
		...
		dataent.response['user_list'] = user_list

	def send(self):
		# send email only to enabled users
		valid_users = [p[0] for p in dataent.db.sql("""select name from `tabUser`
			where enabled=1""")]
	...
	def get_msg_html(self):
		...
		from epaas.setup.doctype.email_digest.quotes import get_random_quote
		...
		# style
		return dataent.render_template("epaas/setup/doctype/email_digest/templates/default.html",
	...
	def get_notifications(self):
		...
		notifications = dataent.desk.notifications.get_notifications()
	...
	def get_todo_list(self, user_id=None):
		...
		todo_list = dataent.db.sql("""select *
	...
	def get_todo_count(self, user_id=None):
		...
		return dataent.db.sql("""select count(*) from `tabToDo`
	...
	def get_issue_list(self, user_id=None):
		...
		meta = dataent.get_meta("Issue")
		role_permissions = dataent.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = dataent.db.sql("""select *
	...
	def get_issue_count(self):
		"""Get count of Issue"""
		return dataent.db.sql("""select count(*) from `tabIssue`
	...
	def get_project_list(self, user_id=None):
		...
		project_list = dataent.db.sql("""select *
	...
	def get_project_count(self):
		...
		return dataent.db.sql("""select count(*) from `tabProject`
		...
		for key in ("income", "expenses_booked", "income_year_to_date","expense_year_to_date",
			...
			if self.get(key):
				...
				if card:
					...
					card = dataent._dict(getattr(self, "get_" + key)())
	...
	def get_type_balance(self, fieldname, account_type, root_type=None):
		...
		if root_type:
			accounts = [d.name for d in \
				dataent.db.get_all("Account", filters={"account_type": account_type,
	...
	def get_roots(self, root_type):
		return [d.name for d in dataent.db.get_all("Account",
	...
	def get_root_type_accounts(self, root_type):
		if not root_type in self._accounts:
			self._accounts[root_type] = [d.name for d in \
				dataent.db.get_all("Account", filters={"root_type": root_type.title(),
					"company": self.company, "is_group": 0})]
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		...
		value, count, billed_value, delivered_value = dataent.db.sql("""select ifnull(sum(grand_total),0), count(*),
	...
	def get_summary_of_pending_quotations(self, fieldname):
		...
		value, count = dataent.db.sql("""select ifnull(sum(grand_total),0), count(*) from `tabQuotation`
			where (transaction_date <= %(to_date)s)
			and company = %(company)s
			and status not in ('Ordered','Cancelled', 'Lost') """,{"to_date": self.future_to_date, "company": self.company})[0]

		last_value = dataent.db.sql("""select ifnull(sum(grand_total),0) from `tabQuotation`
			where (transaction_date <= %(to_date)s)
			and company = %(company)s
	...
	def get_total_on(self, doc_type, from_date, to_date):
		...
		return dataent.db.sql("""select ifnull(sum(grand_total),0), count(*) from `tab{0}`
...
def get_digest_msg(name):
	return dataent.get_doc("Email Digest", name).get_msg_html()
