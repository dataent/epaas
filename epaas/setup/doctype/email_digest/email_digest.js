...
cur_frm.cscript.refresh = function(doc, dt, dn) {
	...
	var err_msg = __("There was an error. One probable reason could be that you haven't saved the form. Please contact support@epaas.xyz if the problem persists.")

	cur_frm.add_custom_button(__('View Now'), function() {
		dataent.call({
			method: 'epaas.setup.doctype.email_digest.email_digest.get_digest_msg',
			args: {
				name: doc.name
			},
			callback: function(r) {
	...
	return $c_obj(doc, 'get_users', '', function(r, rt) {
		if(r.exc) {
			...
			$.each(r.user_list, function(i, v) {
				var fullname = dataent.user.full_name(v.name);
