...
import urllib
import copy
from dataent.utils import nowdate, cint, cstr
...
from epaas.shopping_cart.product_info import set_product_info_for_website
...
class ItemGroup(NestedSet, WebsiteGenerator):
	...
	def make_route(self):
		...
		if not self.route:
			...
			if self.parent_item_group:
				parent_item_group = dataent.get_doc('Item Group', self.parent_item_group)
	...
	def validate_name_with_item(self):
		if dataent.db.exists("Item", self.name):
			dataent.throw(dataent._("An item exists with same name ({0}), please change the item group name or rename the item").format(self.name), dataent.NameError)

	def get_context(self, context):
		context.show_search=True
		context.page_length = cint(dataent.db.get_single_value('Products Settings', 'products_per_page')) or 6
		...
		context.update({
			"items": get_product_list_for_group(product_group = self.name, start=start,
				limit=context.page_length + 1, search=dataent.form_dict.get("search")),
			"parents": get_parent_item_groups(self.parent_item_group),
			"title": self.name,
			"products_as_list": cint(dataent.db.get_single_value('Products Settings', 'products_as_list'))
		})
...
def get_product_list_for_group(product_group=None, start=0, limit=10, search=None):
	...
	child_groups = ", ".join(['"' + dataent.db.escape(i[0]) + '"' for i in get_child_groups(product_group)])
	...
	data = dataent.db.sql(query, {"product_group": product_group,"search": search, "today": nowdate()}, as_dict=1)
...
def get_child_groups_for_list_in_html(item_group, start, limit, search):
	...
	data = dataent.db.get_all('Item Group',
...
def get_child_groups(item_group_name):
	item_group = dataent.get_doc("Item Group", item_group_name)
...
def get_item_for_list_in_html(context):
	...
	context["show_availability_status"] = cint(dataent.db.get_single_value('Products Settings',
		'show_availability_status'))

	products_template = 'templates/includes/products_as_grid.html'
	if cint(dataent.db.get_single_value('Products Settings', 'products_as_list')):
...
def get_group_item_count(item_group):
	...
	return dataent.db.sql("""select count(*) from `tabItem`
...
def get_parent_item_groups(item_group_name):
	if not item_group_name:
		return [{"name": dataent._("Home"), "route":"/"}]
	item_group = dataent.get_doc("Item Group", item_group_name)
	parent_groups = dataent.db.sql("""select name, route from `tabItem Group`
		where lft <= %s and rgt >= %s
		and show_in_website=1
		order by lft asc""", (item_group.lft, item_group.rgt), as_dict=True)

	return 	[{"name": dataent._("Home"), "route":"/"}] + parent_groups
...
def invalidate_cache_for(doc, item_group=None):
	...
	for d in get_parent_item_groups(item_group):
		item_group_name = dataent.db.get_value("Item Group", d.get('name'))
		if item_group_name:
			clear_cache(dataent.db.get_value('Item Group', item_group_name, 'route'))
