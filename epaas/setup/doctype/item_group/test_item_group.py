...
import unittest
import dataent
from dataent.utils.nestedset import NestedSetRecursionError, NestedSetMultipleRootsError, \
	NestedSetChildExistsError, NestedSetInvalidMergeError, rebuild_tree, get_ancestors_of

test_records = dataent.get_test_records('Item Group')

class TestItem(unittest.TestCase):
	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = dataent.db.sql("select max(rgt) from `tabItem Group`")[0][0]

		if not records:
			records = test_records[2:]

		for item_group in records:
			lft, rgt, parent_item_group = dataent.db.get_value("Item Group", item_group["item_group_name"],
				["lft", "rgt", "parent_item_group"])

			if parent_item_group:
				parent_lft, parent_rgt = dataent.db.get_value("Item Group", parent_item_group,
					["lft", "rgt"])
			else:
				# root
	...
	def get_no_of_children(self, item_group):
		def get_no_of_children(item_groups, no_of_children):
			...
			for ig in item_groups:
				children += dataent.db.sql_list("""select name from `tabItem Group`
	...
	def test_recursion(self):
		group_b = dataent.get_doc("Item Group", "_Test Item Group B")
	...
	def test_move_group_into_another(self):
		# before move
		old_lft, old_rgt = dataent.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = dataent.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
	...
	def print_tree(self):
		import json
		print(json.dumps(dataent.db.sql("select name, lft, rgt from `tabItem Group` order by lft"), indent=1))

	def test_move_leaf_into_another_group(self):
		# before move
		old_lft, old_rgt = dataent.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])

		group_b_3 = dataent.get_doc("Item Group", "_Test Item Group B - 3")
	...
	def test_delete_leaf(self):
		# for checking later
		parent_item_group = dataent.db.get_value("Item Group", "_Test Item Group B - 3", "parent_item_group")
		rgt = dataent.db.get_value("Item Group", parent_item_group, "rgt")

		ancestors = get_ancestors_of("Item Group", "_Test Item Group B - 3")
		ancestors = dataent.db.sql("""select name, rgt from `tabItem Group`
			where name in ({})""".format(", ".join(["%s"]*len(ancestors))), tuple(ancestors), as_dict=True)

		dataent.delete_doc("Item Group", "_Test Item Group B - 3")
		...
		for item_group in ancestors:
			new_lft, new_rgt = dataent.db.get_value("Item Group", item_group.name, ["lft", "rgt"])
			self.assertEqual(new_rgt, item_group.rgt - 2)

		# insert it back
		dataent.copy_doc(test_records[6]).insert()

		self.test_basic_tree()

	def test_delete_group(self):
		# cannot delete group with child, but can delete leaf
		self.assertRaises(NestedSetChildExistsError, dataent.delete_doc, "Item Group", "_Test Item Group B")
	...
	def test_merge_leaf_into_group(self):
		self.assertRaises(NestedSetInvalidMergeError, dataent.rename_doc, "Item Group", "_Test Item Group B - 3",
			"_Test Item Group B", merge=True)

	def test_merge_group_into_leaf(self):
		self.assertRaises(NestedSetInvalidMergeError, dataent.rename_doc, "Item Group", "_Test Item Group B",
