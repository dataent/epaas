...
from dataent import msgprint, throw, _

from dataent.model.document import Document
from dataent.model.naming import parse_naming_series
from dataent.permissions import get_doctypes_with_read

class NamingSeriesNotSetError(dataent.ValidationError): pass

class NamingSeries(Document):
	def get_transactions(self, arg=None):
		doctypes = list(set(dataent.db.sql_list("""select parent
		...
		custom_prefixes = dataent.get_all('DocType', fields=["autoname"],
	...
	def validate_series_set(self):
		if self.select_doc_for_series and not self.set_options:
			dataent.throw(_("Please set the series to be used."))
	...
	def set_series_for(self, doctype, ol):
		...
		for prop in prop_dict:
			ps_exists = dataent.db.get_value("Property Setter",
				{"field_name": 'naming_series', 'doc_type': doctype, 'property': prop})

			if ps_exists:
				ps = dataent.get_doc('Property Setter', ps_exists)
	...
	def check_duplicate(self):
		parent = list(set(
			dataent.db.sql_list("""select dt.name
		...
		sr = [[dataent.get_meta(p).get_field("naming_series").options, p] for p in parent]
		dt = dataent.get_doc("DocType", self.select_doc_for_series)
			...
			for i in sr:
				if i[0]:
					...
					if series.split(".")[0] in existing_series:
						dataent.throw(_("Series {0} already used in {1}").format(series,i[1]))
	...
	def get_options(self, arg=None):
		if dataent.get_meta(arg or self.select_doc_for_series).get_field("naming_series"):
			return dataent.get_meta(arg or self.select_doc_for_series).get_field("naming_series").options

	def get_current(self, arg=None):
		"""get series current"""
		if self.prefix:
			prefix = self.parse_naming_series()
			self.current_value = dataent.db.get_value("Series",
				prefix, "current", order_by = "name")

	def insert_series(self, series):
		"""insert series if missing"""
		if not dataent.db.get_value('Series', series, 'name', order_by="name"):
			dataent.db.sql("insert into tabSeries (name, current) values (%s, 0)", (series))

	def update_series_start(self):
		if self.prefix:
			prefix = self.parse_naming_series()
			self.insert_series(prefix)
			dataent.db.sql("update `tabSeries` set current = %s where name = %s",
...
def set_by_naming_series(doctype, fieldname, naming_series, hide_name_field=True):
	...
	if naming_series:
		...
		try:
			dataent.db.sql("""update `tab{doctype}` set naming_series={s} where
		...
		if hide_name_field:
			...
			# set values for mandatory
			dataent.db.sql("""update `tab{doctype}` set `{fieldname}`=`name` where
...
def get_default_naming_series(doctype):
	...
	if not out:
		dataent.throw(_("Please set Naming Series for {0} via Setup > Settings > Naming Series").format(doctype),
