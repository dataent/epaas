...
def get_setup_progress():
	if not getattr(dataent.local, "setup_progress", None):
		dataent.local.setup_progress = dataent.get_doc("Setup Progress", "Setup Progress")

	return dataent.local.setup_progress
...
def update_action(doc):
	...
	if not doc.is_completed:
		if doc.min_doc_count:
			if dataent.db.count(doctype) >= doc.min_doc_count:
				doc.is_completed = 1
				doc.save()
		if docname and field:
			d = dataent.get_doc(doctype, docname)
			if d.get(field):
				doc.is_completed = 1
				doc.save()
