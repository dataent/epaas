...
class AuthorizationRule(Document):
	def check_duplicate_entry(self):
		exists = dataent.db.sql("""select name, docstatus from `tabAuthorization Rule`
		...
		if auth_exists:
			dataent.throw(_("Duplicate Entry. Please check Authorization Rule {0}").format(auth_exists))


	def validate_rule(self):
		if self.transaction != 'Appraisal':
			if not self.approving_role and not self.approving_user:
				dataent.throw(_("Please enter Approving Role or Approving User"))
			elif self.system_user and self.system_user == self.approving_user:
				dataent.throw(_("Approving User cannot be same as user the rule is Applicable To"))
			elif self.system_role and self.system_role == self.approving_role:
				dataent.throw(_("Approving Role cannot be same as role the rule is Applicable To"))
				...
				dataent.throw(_("Cannot set authorization on basis of Discount for {0}").format(self.transaction))
			elif self.based_on == 'Average Discount' and flt(self.value) > 100.00:
				dataent.throw(_("Discount must be less than 100"))
			elif self.based_on == 'Customerwise Discount' and not self.master_name:
				dataent.throw(_("Customer required for 'Customerwise Discount'"))
		else:
			if self.transaction == 'Appraisal':
