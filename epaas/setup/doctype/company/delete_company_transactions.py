...
def delete_company_transactions(company_name):
	...
	if dataent.session.user != doc.owner:
		dataent.throw(_("Transactions can only be deleted by the creator of the Company"),
	...
	if not meta.issingle:
		...
		if naming_series and naming_series.options:
			...
			for prefix in prefixes:
				if prefix:
					last = dataent.db.sql("""select max(name) from `tab{0}`
					...
					dataent.db.sql("""update tabSeries set current = %s
						where name=%s""", (last, prefix))

def delete_bins(company_name):
	dataent.db.sql("""delete from tabBin where warehouse in
			(select name from tabWarehouse where company=%s)""", company_name)

def delete_lead_addresses(company_name):
	"""Delete addresses to which leads are linked"""
	leads = dataent.get_all("Lead", filters={"company": company_name})
	leads = [ "'%s'"%row.get("name") for row in leads ]
	addresses = []
	if leads:
		addresses = dataent.db.sql_list("""select parent from `tabDynamic Link` where link_name 
			in ({leads})""".format(leads=",".join(leads)))

		if addresses:
			addresses = ["'%s'"%dataent.db.escape(addr) for addr in addresses]

			dataent.db.sql("""delete from tabAddress where name in ({addresses}) and 
				name not in (select distinct dl1.parent from `tabDynamic Link` dl1 
				inner join `tabDynamic Link` dl2 on dl1.parent=dl2.parent 
				and dl1.link_doctype<>dl2.link_doctype)""".format(addresses=",".join(addresses)))

			dataent.db.sql("""delete from `tabDynamic Link` where link_doctype='Lead' 
				and parenttype='Address' and link_name in ({leads})""".format(leads=",".join(leads)))

		dataent.db.sql("""update tabCustomer set lead_name=NULL where lead_name in ({leads})""".format(leads=",".join(leads)))
