...
dataent.provide("epaas.company");

dataent.ui.form.on("Company", {
	setup: function(frm) {
		epaas.company.setup_queries(frm);
	...
	date_of_commencement: function(frm) {
		...
		{
			dataent.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function(frm) {
		...
		if(!frm.doc.__islocal) {
			...
			dataent.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Company'}
			...
			frm.add_custom_button(__('Cost Centers'), function() {
				dataent.set_route('Tree', 'Cost Center', {'company': frm.doc.name})
			}, __("View"));

			frm.add_custom_button(__('Chart of Accounts'), function() {
				dataent.set_route('Tree', 'Account', {'company': frm.doc.name})
			}, __("View"));

			frm.add_custom_button(__('Sales Tax Template'), function() {
				dataent.set_route('List', 'Sales Taxes and Charges Template', {'company': frm.doc.name});
			}, __("View"));

			frm.add_custom_button(__('Purchase Tax Template'), function() {
				dataent.set_route('List', 'Purchase Taxes and Charges Template', {'company': frm.doc.name});
			}, __("View"));
		...
		epaas.company.set_chart_of_accounts_options(frm.doc);
	...
	make_default_tax_template: function(frm) {
		frm.call({
			...
			freeze: true,
			callback: function() {
				dataent.msgprint(__("Default tax templates for sales and purchase are created."));
			}
		})
	},
	...
	delete_company_transactions: function(frm) {
		dataent.verify_password(function() {
			var d = dataent.prompt({
				fieldtype:"Data",
			...
			function(data) {
				if(data.company_name !== frm.doc.name) {
					dataent.msgprint(__("Company name not same"));
					return;
				}
				dataent.call({
					method: "epaas.setup.doctype.company.delete_company_transactions.delete_company_transactions",
					args: {
					...
					callback: function(r, rt) {
						if(!r.exc)
							dataent.msgprint(__("Successfully deleted all transactions related to this company!"));
					},
					onerror: function() {
						dataent.msgprint(__("Wrong Password"));
					}
				});
			},
...
epaas.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return dataent.call({
			method: "epaas.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
cur_frm.cscript.change_abbr = function() {
	...
	dialog.fields_dict.update.$input.click(function() {
		...
		if(!args) return;
		dataent.show_alert(__("Update in progress. It might take a while."));
		return dataent.call({
			method: "epaas.setup.doctype.company.company.enqueue_replace_abbr",
			args: {
...
epaas.company.setup_queries = function(frm) {
	$.each([
		...
		epaas.company.set_custom_query(frm, v);
	});
...
epaas.company.set_custom_query = function(frm, v) {
	var filters = {
