dataent.ui.form.on(cur_frm.doctype, {
	...
	refresh: function(frm) {
		...
		cur_frm.fields_dict[table].grid.add_custom_button(__('Add Row'), () => {
			if(!frm.doc.employee){
				dataent.msgprint(__("Please select Employee"));
				return;
			}
			dataent.call({
				method: 'epaas.hr.utils.get_employee_fields_label',
				callback: function(r) {
					if(r.message){
...
var show_dialog = function(frm, table, field_labels) {
	...
	d.fields_dict["property"].df.onchange = () => {
		...
		if(!property){return;}
		dataent.call({
			method: 'epaas.hr.utils.get_employee_field_property',
...
var render_dynamic_field = function(d, fieldtype, options, fieldname) {
	d.data.new = null;
	var dynamic_field = dataent.ui.form.make_control({
		df: {
			"fieldtype": fieldtype,
			"fieldname": fieldname,
			"options": options || ''
...
var add_to_details = function(frm, d, table) {
	...
	if(data.fieldname){
		if(validate_duplicate(frm, table, data.fieldname)){
			dataent.show_alert({message:__("Property already added"), indicator:'orange'});
			return false;
		}
		if(data.current == data.new){
			dataent.show_alert({message:__("Nothing to change"), indicator:'orange'});
			d.get_primary_btn().attr('disabled', false);
			return false;
		}
		frm.add_child(table, {
		...
		d.set_value('current', "");
		dataent.show_alert({message:__("Added to details"),indicator:'green'});
		d.data = {};
	}else {
		dataent.show_alert({message:__("Value missing"),indicator:'red'});
	}
};
