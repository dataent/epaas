...
from epaas.hr.doctype.employee_benefit_application.employee_benefit_application import get_max_benefits
from epaas.hr.utils import get_previous_claimed_amount
from epaas.hr.doctype.payroll_period.payroll_period import get_payroll_period
...
class EmployeeBenefitClaim(Document):
	def validate(self):
		...
		if not max_benefits or max_benefits <= 0:
			dataent.throw(_("Employee {0} has no maximum benefit amount").format(self.employee))
		payroll_period = get_payroll_period(self.claim_date, self.claim_date, dataent.db.get_value("Employee", self.employee, "company"))
		if not payroll_period:
			dataent.throw(_("{0} is not in a valid Payroll Period").format(dataent.format(self.claim_date, dict(fieldtype='Date'))))
	...
	def validate_benefit_claim_amount(self, max_benefits, payroll_period):
		...
		if max_benefits < claimed_amount:
			dataent.throw(_("Maximum benefit of employee {0} exceeds {1} by the sum {2} of previous claimed\
	...
	def validate_max_benefit_for_sal_struct(self, max_benefits):
		if self.claimed_amount > max_benefits:
			dataent.throw(_("Maximum benefit amount of employee {0} exceeds {1}").format(self.employee, max_benefits))
	...
	def validate_max_benefit_for_component(self, payroll_period):
		if self.max_amount_eligible:
			...
			if claimed_amount > self.max_amount_eligible:
				dataent.throw(_("Maximum amount eligible for the component {0} exceeds {1}")
	...
	def validate_non_pro_rata_benefit_claim(self, max_benefits, payroll_period):
		...
		if max_benefits < pro_rata_amount + claimed_amount:
			dataent.throw(_("Maximum benefit of employee {0} exceeds {1} by the sum {2} of benefit application pro-rata component\
	...
	def get_pro_rata_amount_in_application(self, payroll_period):
		...
		if application:
			return dataent.db.get_value("Employee Benefit Application", application, "pro_rata_dispensed_amount")
		return False
...
def get_benefit_pro_rata_ratio_amount(employee, on_date, sal_struct):
	...
	for sal_struct_row in sal_struct.get("earnings"):
		try:
			pay_against_benefit_claim, max_benefit_amount = dataent.db.get_value("Salary Component", sal_struct_row.salary_component, ["pay_against_benefit_claim", "max_benefit_amount"])
		except TypeError:
			# show the error in tests?
			dataent.throw(_("Unable to find Salary Component {0}".format(sal_struct_row.salary_component)))
...
def get_benefit_claim_amount(employee, start_date, end_date, salary_component=None):
	...
	claimed_amount = flt(dataent.db.sql(query, {
		'employee': employee,
...
def get_total_benefit_dispensed(employee, sal_struct, sal_slip_start_date, payroll_period):
	...
	if application:
		application_obj = dataent.get_doc("Employee Benefit Application", application)
	...
	if remaining_benefit > 0:
		...
		for d in sal_struct.get("earnings"):
			if d.is_flexible_benefit == 1:
				salary_component = dataent.get_doc("Salary Component", d.salary_component)
