...
dataent.ui.form.on('Payroll Entry', {
	onload: function (frm) {
		if (!frm.doc.posting_date) {
			frm.doc.posting_date = dataent.datetime.nowdate();
		}
	...
	add_bank_entry_button: function(frm) {
		dataent.call({
			method: 'epaas.hr.doctype.payroll_entry.payroll_entry.payroll_entry_has_bank_entries',
			args: {
				'name': frm.doc.name
			},
			callback: function(r) {
	...
	set_start_end_dates: function (frm) {
		if (!frm.doc.salary_slip_based_on_timesheet) {
			dataent.call({
				method: 'epaas.hr.doctype.payroll_entry.payroll_entry.get_start_end_dates',
				args: {
	...
	set_end_date: function(frm){
		dataent.call({
			method: 'epaas.hr.doctype.payroll_entry.payroll_entry.get_end_date',
			args: {
...
const submit_salary_slip = function (frm) {
	dataent.confirm(__('This will submit Salary Slips and create accrual Journal Entry. Do you want to proceed?'),
		function() {
			dataent.call({
...
let make_bank_entry = function (frm) {
	...
	if (doc.company && doc.start_date && doc.end_date && doc.payment_account) {
		...
		dataent.msgprint(__("Company, Payment Account, From Date and To Date is mandatory"));
	}
};
...
let render_employee_attendance = function(frm, data) {
	frm.fields_dict.attendance_detail_html.html(
		dataent.render_template('employees_to_mark_attendance', {
			data: data
		})
	);
}
