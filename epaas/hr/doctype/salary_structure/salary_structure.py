...
from dataent.utils import flt, cint, cstr
...
class SalaryStructure(Document):
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = dataent.db.get_value("Salary Component", cstr(d.salary_component),
	...
	def validate_amount(self):
		if flt(self.net_pay) < 0 and self.salary_slip_based_on_timesheet:
			dataent.throw(_("Net pay cannot be negative"))
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = dataent.db.get_value("Salary Component", earning_component.salary_component, "max_benefit_amount")
					flexi_amount += max_of_component

			if have_a_flexi and flt(self.max_benefits) == 0:
				dataent.throw(_("Max benefits should be greater than zero to dispense benefits"))
			if have_a_flexi and flexi_amount and flt(self.max_benefits) > flexi_amount:
				dataent.throw(_("Total flexible benefit component amount {0} should not be less than max benefits {1}")
					.format(flexi_amount, self.max_benefits))
		if not have_a_flexi and flt(self.max_benefits) > 0:
			dataent.throw(_("Salary Structure should have flexible benefit component(s) to dispense benefit amount"))
	...
	def get_employees(self, **kwargs):
		...
		employees = dataent.db.sql_list("select name from tabEmployee where status='Active' {condition}"
	...
	def assign_salary_structure(self, grade=None, department=None, designation=None,employee=None,
		...
		if employees:
			if len(employees) > 20:
				dataent.enqueue(assign_salary_structure_for_employees, timeout=600,
			...
			dataent.msgprint(_("No Employee Found"))
...
def assign_salary_structure_for_employees(employees, salary_structure,from_date=None, base=None,variable=None):
	...
	for employee in employees:
		...
		dataent.publish_progress(count*100/len(set(employees) - set(existing_assignments_for)), title = _("Assigning Structures..."))
...
def create_salary_structures_assignment(employee, salary_structure, from_date, base, variable):
	assignment = dataent.new_doc("Salary Structure Assignment")
...
def get_existing_assignments(employees, salary_structure,from_date):
	salary_structures_assignments = dataent.db.sql_list("""
	...
	if salary_structures_assignments:
		dataent.msgprint(_("Skipping Salary Structure Assignment for the following employees, as Salary Structure Assignment records already exists against them. {0}")
...
def make_salary_slip(source_name, target_doc = None, employee = None, as_print = False, print_format = None, for_preview=0):
	...
	if cint(as_print):
		...
		return dataent.get_print(doc.doctype, doc.name, doc = doc, print_format = print_format)
	else:
		return doc
...
def get_employees(salary_structure):
	employees = dataent.get_list('Salary Structure Assignment',
