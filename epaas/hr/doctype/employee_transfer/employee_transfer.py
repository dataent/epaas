...
class EmployeeTransfer(Document):
	def validate(self):
		if dataent.get_value("Employee", self.employee, "status") == "Left":
			dataent.throw(_("Cannot transfer Employee with status Left"))

	def before_submit(self):
		if getdate(self.transfer_date) > getdate():
			dataent.throw(_("Employee Transfer cannot be submitted before Transfer Date "),
	...
	def on_cancel(self):
		...
		if self.create_new_employee_id:
			if self.new_employee_id:
				dataent.throw(_("Please delete the Employee <a href='#Form/Employee/{0}'>{0}</a>\
