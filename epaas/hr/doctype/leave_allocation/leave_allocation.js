...
dataent.ui.form.on("Leave Allocation", {
	onload: function(frm) {
		if(!frm.doc.from_date) frm.set_value("from_date", dataent.datetime.get_today());
	...
	calculate_total_leaves_allocated: function(frm) {
		if (cint(frm.doc.carry_forward) == 1 && frm.doc.leave_type && frm.doc.employee) {
			return dataent.call({
				method: "epaas.hr.doctype.leave_allocation.leave_allocation.get_carry_forwarded_leaves",
				args: {
