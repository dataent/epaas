...
dataent.ui.form.on('Training Result', {
	onload: function(frm) {
	...
	training_event: function(frm) {
		if (frm.doc.training_event && !frm.doc.docstatus && !frm.doc.employees) { 
			dataent.call({
				method: "epaas.hr.doctype.training_result.training_result.get_employees",
				args: {
				...
				callback: function(r) {
					...
					if (r.message) {
						$.each(r.message, function(i, d) {
							var row = dataent.model.add_child(cur_frm.doc, "Training Result Employee", "employees");
