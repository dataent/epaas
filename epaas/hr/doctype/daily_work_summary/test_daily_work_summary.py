...
# test_records = dataent.get_test_records('Daily Work Summary')
...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_email_trigger_failed(self):
		hour = '00:00'
		if dataent.utils.nowtime().split(':')[0] == '00':
			hour = '01:00'
	...
	def test_incoming(self):
		...
		daily_work_summary = dataent.get_doc('Daily Work Summary',
			dataent.get_all('Daily Work Summary')[0].name)
	...
	def setup_and_prepare_test(self, hour=None):
		dataent.db.sql('delete from `tabDaily Work Summary`')
		...
		dataent.db.sql('delete from `tabDaily Work Summary Group`')

		self.users = dataent.get_all('User',
			fields=['email'],
			filters=dict(email=('!=', 'test@example.com')))
		self.setup_groups(hour)

		from epaas.hr.doctype.daily_work_summary_group.daily_work_summary_group \
			import trigger_emails
		trigger_emails()

		# check if emails are created

		self.emails = dataent.db.sql("""select r.recipient, q.message, q.message_id \
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = dataent.utils.nowtime().split(':')[0]
			hour = hour+':00'

		groups = dataent.get_doc(dict(doctype="Daily Work Summary Group",
