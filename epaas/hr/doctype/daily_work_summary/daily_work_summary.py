...
from dataent.utils import global_date_format
...
class DailyWorkSummary(Document):
	...
	def get_message_details(self):
		...
		for d in replies:
			user = dataent.db.get_values("User",
				{"email": d.sender},
		...
		did_not_reply = [(dataent.db.get_value("User", {"email": email}, "full_name") or email)
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, string_types):
		group_doc = dataent.get_doc('Daily Work Summary Group', group)
...
def get_users_email(doc):
	return [d.email for d in doc.users
		if dataent.db.get_value("User", d.user, "enabled")]
