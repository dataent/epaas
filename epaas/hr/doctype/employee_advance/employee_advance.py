...
class EmployeeAdvanceOverPayment(dataent.ValidationError):
	pass
...
class EmployeeAdvance(Document):
	...
	def validate_employee_advance_account(self):
		...
		if (self.advance_account and
			company_currency != dataent.db.get_value('Account', self.advance_account, 'account_currency')):
			dataent.throw(_("Advance account currency should be same as company currency {0}")
	...
	def set_total_advance_paid(self):
		...
		if flt(paid_amount) > self.advance_amount:
			dataent.throw(_("Row {0}# Paid Amount cannot be greater than requested advance amount"),
				EmployeeAdvanceOverPayment)

		self.db_set("paid_amount", paid_amount)
		self.set_status()
		dataent.db.set_value("Employee Advance", self.name , "status", self.status)


	def update_claimed_amount(self):
		claimed_amount = dataent.db.sql("""
		...
		if claimed_amount:
			dataent.db.set_value("Employee Advance", self.name, "claimed_amount", flt(claimed_amount))

@dataent.whitelist()
def get_due_advance_amount(employee, posting_date):
	employee_due_amount = dataent.get_all("Employee Advance", \
