...
import epaas
import unittest
from dataent.utils import nowdate, add_days
...
class TestLoan(unittest.TestCase):
	...
	def test_loan(self):
		loan = dataent.get_doc("Loan", {"applicant":self.applicant})
...
def create_loan_type(loan_name, maximum_loan_amount, rate_of_interest):
	if not dataent.db.exists("Loan Type", loan_name):
		dataent.get_doc({
			"doctype": "Loan Type",
			"loan_name": loan_name,
...
def	create_loan(applicant, loan_type, loan_amount, repayment_method, repayment_periods):
	...
	if not dataent.db.get_value("Loan", {"applicant":applicant}):
		...
		loan.update({
				...
				"payment_account": dataent.db.get_value('Account', {'account_type': 'Cash', 'company': epaas.get_default_company(),'is_group':0}, "name"),
				"loan_account": dataent.db.get_value('Account', {'account_type': 'Cash', 'company': epaas.get_default_company(),'is_group':0}, "name"),
				"interest_income_account": dataent.db.get_value('Account', {'account_type': 'Cash', 'company': epaas.get_default_company(),'is_group':0}, "name")
			})
		loan.insert()
		return loan
	else:
