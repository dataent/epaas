...
import dataent, math, json
import epaas
from dataent import _
from dataent.utils import flt, rounded, add_months, nowdate
...
class Loan(AccountsController):
	def validate(self):
		...
		if self.loan_type and not self.rate_of_interest:
			self.rate_of_interest = dataent.db.get_value("Loan Type", self.loan_type, "rate_of_interest")
		...
		if self.status == 'Disbursed' and self.repayment_start_date < self.disbursement_date:
			dataent.throw(_("Repayment Start Date cannot be before Disbursement Date."))
...
def update_total_amount_paid(doc):
	...
	dataent.db.set_value("Loan", doc.name, "total_amount_paid", total_amount_paid)

def update_disbursement_status(doc):
	disbursement = dataent.db.sql("""select posting_date, ifnull(sum(credit_in_account_currency), 0) as disbursed_amount
	...
	if disbursement.disbursed_amount > doc.loan_amount:
		dataent.throw(_("Disbursed Amount cannot be greater than Loan Amount {0}").format(doc.loan_amount))
	if disbursement.disbursed_amount > 0:
		dataent.db.set_value("Loan", doc.name , "disbursement_date", disbursement.posting_date)
		dataent.db.set_value("Loan", doc.name , "repayment_start_date", disbursement.posting_date)

def check_repayment_method(repayment_method, loan_amount, monthly_repayment_amount, repayment_periods):
	if repayment_method == "Repay Over Number of Periods" and not repayment_periods:
		dataent.throw(_("Please enter Repayment Periods"))

	if repayment_method == "Repay Fixed Amount per Period":
		if not monthly_repayment_amount:
			dataent.throw(_("Please enter repayment Amount"))
		if monthly_repayment_amount > loan_amount:
			dataent.throw(_("Monthly Repayment Amount cannot be greater than Loan Amount"))
...
def get_loan_application(loan_application):
	loan = dataent.get_doc("Loan Application", loan_application)
	if loan:
		return loan.as_dict()
...
def make_repayment_entry(payment_rows, loan, company, loan_account, applicant_type, applicant, \
	...
	if isinstance(payment_rows, dataent.string_types):
		payment_rows_list = json.loads(payment_rows)
	else:
		dataent.throw(_("No repayments available for Journal Entry"))

	if payment_rows_list:
		row_name = list(set(d["name"] for d in payment_rows_list))
	else:
		dataent.throw(_("No repayments selected for Journal Entry"))
	total_payment = 0
	principal_amount = 0
	interest_amount = 0
