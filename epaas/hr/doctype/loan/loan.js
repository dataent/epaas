...
{% include 'epaas/hr/loan_common.js' %};

dataent.ui.form.on('Loan', {
	onload: function (frm) {
...
;		}
	...
	make_jv: function (frm) {
		dataent.call({
			...
			method: "epaas.hr.doctype.loan.loan.make_jv_entry",
			callback: function (r) {
				if (r.message)
	...
	make_repayment_entry: function(frm) {
		...
		if (frm.doc['repayment_schedule']) {
			...
			if (payment.paid == 0 && payment.payment_date <= dataent.datetime.now_date()) {
	...
	loan_application: function (frm) {
	    if(frm.doc.loan_application){
            return dataent.call({
                method: "epaas.hr.doctype.loan.loan.get_loan_application",
                args: {
...
var _make_repayment_entry = function(frm, payment_rows) {
	dataent.call({
		method:"epaas.hr.doctype.loan.loan.make_repayment_entry",
		args: {
		...
		callback: function(r) {
			if (r.message)
				var doc = dataent.model.sync(r.message)[0];
			dataent.set_route("Form", doc.doctype, doc.name, {'payment_rows': payment_rows});
		}
	});
}