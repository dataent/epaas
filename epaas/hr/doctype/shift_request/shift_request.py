...
from dataent.utils import formatdate, getdate
...
class ShiftRequest(Document):
	...
	def on_submit(self):
		...
		for date in date_list:
			assignment_doc = dataent.new_doc("Shift Assignment")
	...
	def on_cancel(self):
		shift_assignment_list = dataent.get_list("Shift Assignment", {'employee': self.employee, 'shift_request': self.name})
		if shift_assignment_list:
			for shift in shift_assignment_list:
				shift_assignment_doc = dataent.get_doc("Shift Assignment", shift['name'])
	...
	def get_working_days(self, start_date, end_date):
		...
		employee_holidays = dataent.db.sql("""select holiday_date from `tabHoliday`
