...
dataent.ui.form.on('Staffing Plan', {
	setup: function(frm) {
...
dataent.ui.form.on('Staffing Plan Detail', {
	designation: function(frm, cdt, cdn) {
		let child = locals[cdt][cdn]
		if(frm.doc.company && child.designation){
			dataent.call({
				"method": "epaas.hr.doctype.staffing_plan.staffing_plan.get_designation_counts",
				args: {
				...
				callback: function (data) {
					if(data.message){
						dataent.model.set_value(cdt, cdn, 'current_count', data.message.employee_count);
						dataent.model.set_value(cdt, cdn, 'current_openings', data.message.job_openings);
						if (child.number_of_positions < (data.message.employee_count +  data.message.job_openings)){
							dataent.model.set_value(cdt, cdn, 'number_of_positions', data.message.employee_count +  data.message.job_openings);
						}
					}
					else{ // No employees for this designation
						dataent.model.set_value(cdt, cdn, 'current_count', 0);
						dataent.model.set_value(cdt, cdn, 'current_openings', 0);
					}
				}
			});
		}
...
var set_vacancies = function(frm, cdt, cdn) {
	...
	if (child.number_of_positions < (child.current_count + child.current_openings)){
		dataent.throw(__("Number of positions cannot be less then current count of employees"))
	}

	if(child.number_of_positions > 0) {
		dataent.model.set_value(cdt, cdn, 'vacancies', child.number_of_positions - (child.current_count + child.current_openings));
	}
	else{
		dataent.model.set_value(cdt, cdn, 'vacancies', 0);
	}
...
var set_total_estimated_cost = function(frm, cdt, cdn) {
	...
	if(child.vacancies > 0 && child.estimated_cost_per_position) {
		dataent.model.set_value(cdt, cdn, 'total_estimated_cost', child.vacancies * child.estimated_cost_per_position);
	}
	else {
		dataent.model.set_value(cdt, cdn, 'total_estimated_cost', 0);
	}
