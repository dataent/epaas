...
from dataent.utils import getdate, nowdate, cint, flt

class SubsidiaryCompanyError(dataent.ValidationError): pass
class ParentCompanyError(dataent.ValidationError): pass
...
class StaffingPlan(Document):
	def validate(self):
		...
		for detail in self.get("staffing_details"):
			...
			if detail.number_of_positions < (detail.current_count + detail.current_openings):
				dataent.throw(_("Number of positions cannot be less then current count of employees"))
	...
	def validate_overlap(self, staffing_plan_detail):
		...
		# and spd.vacancies>0 ?
		overlap = dataent.db.sql("""select spd.parent
		...
		if overlap and overlap [0][0]:
			dataent.throw(_("Staffing Plan {0} already exist for designation {1}"
...
def get_designation_counts(designation, company):
	...
	lft, rgt = dataent.get_cached_value('Company',  company,  ["lft", "rgt"])
	employee_counts_dict["employee_count"] = dataent.db.sql("""select count(*) from `tabEmployee`
	...
	employee_counts_dict['job_openings'] = dataent.db.sql("""select count(*) from `tabJob Opening` \
...
def get_active_staffing_plan_details(company, designation, from_date=getdate(nowdate()), to_date=getdate(nowdate())):
	if not company or not designation:
		dataent.throw(_("Please select Company and Designation"))

	staffing_plan = dataent.db.sql("""
	...
	if not staffing_plan:
		parent_company = dataent.get_cached_value('Company',  company,  "parent_company")
		if parent_company:
