...
import dataent
from dataent.utils import cstr, add_days, date_diff, getdate
...
def get_template():
	...
	dataent.response['doctype'] = "Attendance"

def add_header(w):
	status = ", ".join((dataent.get_meta("Attendance").get_field("status").options or "").strip().split("\n"))
	w.writerow(["Notes:"])
...
def get_active_employees():
	employees = dataent.db.get_all('Employee',
...
def get_existing_attendance_records(args):
	attendance = dataent.db.sql("""select name, attendance_date, employee, status, leave_type, naming_series
...
def get_naming_series():
	series = dataent.get_meta("Attendance").get_field("naming_series").options.strip().split("\n")
	if not series:
		dataent.throw(_("Please setup numbering series for Attendance via Setup > Numbering Series"))
	return series[0]
...
def upload():
	...
	for i, row in enumerate(rows[5:]):
		...
		if d.name:
			d["docstatus"] = dataent.db.get_value("Attendance", d.name, "docstatus")
