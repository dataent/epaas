...
from dataent.utils import get_fullname, flt, cstr
...
class InvalidExpenseApproverError(dataent.ValidationError): pass
class ExpenseApproverIdentityError(dataent.ValidationError): pass

class ExpenseClaim(AccountsController):
	def onload(self):
		self.get("__onload").make_payment_via_journal_entry = dataent.db.get_single_value('Accounts Settings',
	...
	def validate(self):
		...
		if self.task and not self.project:
			self.project = dataent.db.get_value("Task", self.task, "project")
	...
	def set_payable_account(self):
		if not self.payable_account and not self.is_paid:
			self.payable_account = dataent.get_cached_value('Company', self.company, 'default_expense_claim_payable_account')

	def set_cost_center(self):
		if not self.cost_center:
			self.cost_center = dataent.get_cached_value('Company', self.company, 'cost_center')

	def on_submit(self):
		if self.approval_status=="Draft":
			dataent.throw(_("""Approval Status must be 'Approved' or 'Rejected'"""))
	...
	def update_claimed_amount_in_employee_advance(self):
		for d in self.get("advances"):
			dataent.get_doc("Employee Advance", d.employee_advance).update_claimed_amount()

	def update_task_and_project(self):
		if self.task:
			self.update_task()
		elif self.project:
			dataent.get_doc("Project", self.project).update_project()
	...
	def validate_account_details(self):
		if not self.cost_center:
			dataent.throw(_("Cost center is required to book an expense claim"))

		if not self.payable_account:
			dataent.throw(_("Please set default payable account for the company {0}").format(getlink("Company",self.company)))

		if self.is_paid:
			if not self.mode_of_payment:
				dataent.throw(_("Mode of payment is required to make a payment").format(self.employee))
	...
	def update_task(self):
		task = dataent.get_doc("Task", self.task)
	...
	def validate_advances(self):
		...
		for d in self.get("advances"):
			ref_doc = dataent.db.get_value("Employee Advance", d.employee_advance,
			...
			if d.allocated_amount and flt(d.allocated_amount) > flt(d.unclaimed_amount):
				dataent.throw(_("Row {0}# Allocated amount {1} cannot be greater than unclaimed amount {2}")
		...
		if self.total_advance_amount:
			...
			if flt(self.total_advance_amount, precision) > flt(self.total_claimed_amount, precision):
				dataent.throw(_("Total advance amount cannot be greater than total claimed amount"))
			if self.total_sanctioned_amount \
					and flt(self.total_advance_amount, precision) > flt(self.total_sanctioned_amount, precision):
				dataent.throw(_("Total advance amount cannot be greater than total sanctioned amount"))

	def validate_sanctioned_amount(self):
		for d in self.get('expenses'):
			if flt(d.sanctioned_amount) > flt(d.claim_amount):
				dataent.throw(_("Sanctioned Amount cannot be greater than Claim Amount in Row {0}.").format(d.idx))
...
def update_reimbursed_amount(doc):
	amt = dataent.db.sql("""select ifnull(sum(debit_in_account_currency), 0) as amt
		from `tabGL Entry` where against_voucher_type = 'Expense Claim' and against_voucher = %s
		and party = %s """, (doc.name, doc.employee) ,as_dict=1)[0].amt

	doc.total_amount_reimbursed = amt
	dataent.db.set_value("Expense Claim", doc.name , "total_amount_reimbursed", amt)

	doc.set_status()
	dataent.db.set_value("Expense Claim", doc.name , "status", doc.status)

@dataent.whitelist()
def make_bank_entry(dt, dn):
	from epaas.accounts.doctype.journal_entry.journal_entry import get_default_bank_cash_account

	expense_claim = dataent.get_doc(dt, dn)
...
def get_expense_claim_account(expense_claim_type, company):
	account = dataent.db.get_value("Expense Claim Account",
		{"parent": expense_claim_type, "company": company}, "default_account")
	if not account:
		dataent.throw(_("Please set default account in Expense Claim Type {0}")
...
def get_advances(employee, advance_id=None):
	if not advance_id:
		condition = 'docstatus=1 and employee="{0}" and paid_amount > 0 and paid_amount > claimed_amount'.format(dataent.db.escape(employee))
	else:
		condition = 'name="{0}"'.format(dataent.db.escape(advance_id))
...
def get_expense_claim(
	...
	default_payable_account = dataent.get_cached_value('Company',  company,  "default_payable_account")
	default_cost_center = dataent.get_cached_value('Company',  company,  'cost_center')
