...
epaas.hr.ExpenseClaimController = dataent.ui.form.Controller.extend({
	expense_type: function(doc, cdt, cdn) {
		var d = locals[cdt][cdn];
		if(!doc.company) {
			d.expense_type = "";
			dataent.msgprint(__("Please set the Company"));
		...
		return dataent.call({
			method: "epaas.hr.doctype.expense_claim.expense_claim.get_expense_claim_account",
			args: {
...
$.extend(cur_frm.cscript, new epaas.hr.ExpenseClaimController({frm: cur_frm}));
...
cur_frm.cscript.onload = function(doc) {
	if (doc.__islocal) {
		cur_frm.set_value("posting_date", dataent.datetime.get_today());
...
cur_frm.cscript.refresh = function(doc) {
	...
	if(!doc.__islocal) {
		...
		if (doc.docstatus===1) {
			...
			if (cint(doc.total_amount_reimbursed) > 0 && dataent.model.can_read(entry_doctype)) {
				cur_frm.add_custom_button(__('Bank Entries'), function() {
					...
					dataent.set_route("List", entry_doctype);
				}, __("View"));
			}
			/* eslint-enable */
		}
...
cur_frm.cscript.set_help = function(doc) {
	cur_frm.set_intro("");
	if(doc.__islocal && !in_list(dataent.user_roles, "HR User")) {
...
dataent.ui.form.on("Expense Claim", {
	setup: function(frm) {
		...
		frm.set_query("expense_approver", function() {
			return {
				query: "epaas.hr.doctype.department_approver.department_approver.get_approvers",
				filters: {
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return dataent.call({
				method: "epaas.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus===1
				...
				&& dataent.model.can_create("Payment Entry")) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "epaas.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return dataent.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		epaas.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		dataent.model.clear_table(frm.doc, "advances");
		if (frm.doc.employee) {
			return dataent.call({
				method: "epaas.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
				callback: function(r, rt) {
					...
					if(r.message) {
						$.each(r.message, function(i, d) {
							var row = dataent.model.add_child(frm.doc, "Expense Claim Advance", "advances");
...
dataent.ui.form.on("Expense Claim Detail", {
	claim_amount: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		var doc = frm.doc;

		if(!child.sanctioned_amount){
			dataent.model.set_value(cdt, cdn, 'sanctioned_amount', child.claim_amount);
		}
...
dataent.ui.form.on("Expense Claim Advance", {
	employee_advance: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if(!frm.doc.employee){
			dataent.msgprint(__('Select an employee to get the employee advance.'));
			frm.doc.advances = [];
			refresh_field("advances");
		}
		else {
