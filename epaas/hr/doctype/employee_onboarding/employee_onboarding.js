...
dataent.ui.form.on('Employee Onboarding', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.employee) {
			frm.add_custom_button(__('Employee'), function() {
				dataent.set_route("Form", "Employee", frm.doc.employee);
			},__("View"));
		}
		if (frm.doc.project) {
			frm.add_custom_button(__('Project'), function() {
				dataent.set_route("Form", "Project", frm.doc.project);
			},__("View"));
			frm.add_custom_button(__('Task'), function() {
				dataent.set_route('List', 'Task', {project: frm.doc.project});
			},__("View"));
		}
		if ((!frm.doc.employee) && (frm.doc.docstatus === 1)) {
			frm.add_custom_button(__('Employee'), function () {
				dataent.model.open_mapped_doc({
					method: "epaas.hr.doctype.employee_onboarding.employee_onboarding.make_employee",
					frm: frm
				});
			}, __("Make"));
		...
		if (frm.doc.docstatus === 1 && frm.doc.project) {
			dataent.call({
				method: "epaas.hr.utils.get_boarding_status",
				args: {
	...
	employee_onboarding_template: function(frm) {
		...
		if (frm.doc.employee_onboarding_template) {
			dataent.call({
				method: "epaas.hr.utils.get_onboarding_details",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						$.each(r.message, function(i, d) {
							var row = dataent.model.add_child(frm.doc, "Employee Boarding Activity", "activities");
							$.extend(row, d);
						});
					}
