	...
	def validate(self):
		...
		for d in self.get("leave_block_list_dates"):
			...
			if d.block_date in dates:
				dataent.msgprint(_("Date is repeated") + ":" + d.block_date, raise_exception=1)
...
def get_applicable_block_dates(from_date, to_date, employee=None,
	...
	for block_list in get_applicable_block_lists(employee, company, all_lists):
		block_dates.extend(dataent.db.sql("""select block_date, reason
...
def get_applicable_block_lists(employee=None, company=None, all_lists=False):
	...
	if department:
		block_list = dataent.db.get_value("Department", department, "leave_block_list")
		add_block_list(block_list)

	# global
	for block_list in dataent.db.sql_list("""select name from `tabLeave Block List`
...
def is_user_in_allow_list(block_list):
	return dataent.session.user in dataent.db.sql_list("""select allow_user
