dataent.ui.form.on("Employee Attendance Tool", {
	refresh: function(frm) {
		frm.disable_save();
	},

	onload: function(frm) {
		frm.set_value("date", dataent.datetime.get_today());
		epaas.employee_attendance_tool.load_employees(frm);
	},
...
epaas.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			dataent.call({
				method: "epaas.hr.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new epaas.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new epaas.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
epaas.MarkedEmployee = Class.extend({
...
epaas.EmployeeSelector = Class.extend({
	init: function(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make: function(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				dataent.call({
					method: "epaas.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
