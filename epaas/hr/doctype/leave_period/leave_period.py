...
from epaas.hr.utils import validate_overlap, get_employee_leave_policy
...
class LeavePeriod(Document):
	...
	def validate_dates(self):
		if getdate(self.from_date) >= getdate(self.to_date):
			dataent.throw(_("To date can not be equal or less than from date"))
	...
	def grant_leave_allocation(self, grade=None, department=None, designation=None,
		...
		if employees:
			if len(employees) > 20:
				dataent.enqueue(grant_leave_alloc_for_employees, timeout=600,
...
def grant_leave_alloc_for_employees(employees, leave_period, carry_forward_leaves=0):
	...
	for employee in employees:
		...
		dataent.db.commit()
		dataent.publish_progress(count*100/len(set(employees) - set(existing_allocations_for)), title = _("Allocating leaves..."))

	if leave_allocations:
		dataent.msgprint(_("Leaves has been granted sucessfully"))

def get_existing_allocations(employees, leave_period):
	leave_allocations = dataent.db.sql_list("""
	...
	if leave_allocations:
		dataent.msgprint(_("Skipping Leave Allocation for the following employees, as Leave Allocation records already exists against them. {0}")
			.format("\n".join(leave_allocations)))
	return leave_allocations

def get_leave_type_details():
	leave_type_details = dataent._dict()
	leave_types = dataent.get_all("Leave Type", fields=["name", "is_lwp", "is_earned_leave", "is_compensatory", "is_carry_forward"])
