...
from dataent.utils import cint, cstr, flt, nowdate, comma_and, date_diff
...
class LeaveControlPanel(Document):
	def get_employees(self):
		...
		e = dataent.db.sql("select name from tabEmployee where status='Active' {condition}"
	...
	def validate_values(self):
		for f in ["from_date", "to_date", "leave_type", "no_of_days"]:
			if not self.get(f):
				dataent.throw(_("{0} is required").format(self.meta.get_label(f)))
	...
	def allocate_leave(self):
		...
		for d in self.get_employees():
			try:
				...
				la.employee_name = dataent.db.get_value('Employee',cstr(d[0]),'employee_name')
