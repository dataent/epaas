...
from epaas.hr.doctype.employee.employee import EmployeeLeftValidationError

test_records = dataent.get_test_records('Employee')

class TestEmployee(unittest.TestCase):
	def test_birthday_reminders(self):
		employee = dataent.get_doc("Employee", dataent.db.sql_list("select name from tabEmployee limit 1")[0])
		employee.date_of_birth = "1992" + dataent.utils.nowdate()[4:]
		employee.company_email = "test@example.com"
		employee.save()

		from epaas.hr.doctype.employee.employee import get_employees_who_are_born_today, send_birthday_reminders
		...
		email_queue = dataent.db.sql("""select * from `tabEmail Queue`""", as_dict=True)
		self.assertTrue("Subject: Birthday Reminder" in email_queue[0].message)

	def test_employee_status_left(self):
		employee1 = make_employee("test_employee_1@company.com")
		employee2 = make_employee("test_employee_2@company.com")
		employee1_doc = dataent.get_doc("Employee", employee1)
		employee2_doc = dataent.get_doc("Employee", employee2)
...
def make_employee(user):
	...
	if not dataent.db.get_value("Employee", {"user_id": user}):
		employee = dataent.get_doc({
			...
			"department": dataent.get_all("Department", fields="name")[0].name,
			"gender": "Female",
			"company_email": user,
