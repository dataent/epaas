...
QUnit.test("Test: Holiday list [HR]", function (assert) {
	...
	let date = dataent.datetime.add_months(dataent.datetime.nowdate(), -2);		// date 2 months from now

	dataent.run_serially([
		// test holiday list creation
		() => dataent.set_route("List", "Holiday List", "List"),
		() => dataent.new_doc("Holiday List"),
		() => dataent.timeout(1),
		...
		() => dataent.click_button('Get Weekly Off Dates'),
		...
		() => {
			var date_year_later = dataent.datetime.add_days(dataent.datetime.add_months(date, 12), -1);		// date after one year
