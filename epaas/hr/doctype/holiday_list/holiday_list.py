...
import dataent
import json
from dataent.utils import cint, getdate, formatdate
	...
	def validate_days(self):
		...
		for day in self.get("holidays"):
			if not (getdate(self.from_date) <= getdate(day.holiday_date) <= getdate(self.to_date)):
				dataent.throw(_("The holiday on {0} is not between From Date and To Date").format(formatdate(day.holiday_date)))
