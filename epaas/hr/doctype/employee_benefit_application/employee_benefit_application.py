...
from dataent.utils import date_diff, getdate, rounded, add_days, cstr, cint, flt
from dataent.model.document import Document
from epaas.hr.doctype.payroll_period.payroll_period import get_payroll_period_days, get_period_factor
from epaas.hr.doctype.salary_structure_assignment.salary_structure_assignment import get_assigned_salary_structure
from epaas.hr.utils import get_sal_slip_total_benefit_given, get_holidays_for_employee, get_previous_claimed_amount
...
class EmployeeBenefitApplication(Document):
	def validate(self):
		...
		if self.max_benefits and self.max_benefits > 0:
			...
			dataent.throw(_("As per your assigned Salary Structure you cannot apply for benefits").format(self.employee))
		...
		if self.employee_benefits:
			for benefit in self.employee_benefits:
				if benefit.pay_against_benefit_claim == 1:
					payroll_period = dataent.get_doc("Payroll Period", self.payroll_period)
					...
					if benefit_claimed > 0 and benefit_claim_remining > benefit.amount:
						dataent.throw(_("An amount of {0} already claimed for the component {1},\
	...
	def validate_remaining_benefit_amount(self):
		...
		if salary_struct_name:
			...
			pro_rata_amount = 0
			salary_structure = dataent.get_doc("Salary Structure", salary_struct_name)
			if salary_structure.earnings:
				for earnings in salary_structure.earnings:
					if earnings.is_flexible_benefit == 1 and earnings.salary_component not in benefit_components:
						pay_against_benefit_claim, max_benefit_amount = dataent.db.get_value("Salary Component", earnings.salary_component, ["pay_against_benefit_claim", "max_benefit_amount"])
			...
			if pro_rata_amount == 0  and non_pro_rata_amount == 0:
				dataent.throw(_("Please add the remaining benefits {0} to any of the existing component").format(self.remaining_benefit))
			elif non_pro_rata_amount > 0 and non_pro_rata_amount < rounded(self.remaining_benefit):
				dataent.throw(_("You can claim only an amount of {0}, the rest amount {1} should be in the application \
				as pro-rata component").format(non_pro_rata_amount, self.remaining_benefit - non_pro_rata_amount))
			elif non_pro_rata_amount == 0:
				dataent.throw(_("Please add the remaining benefits {0} to the application as \
	...
	def validate_max_benefit_for_component(self):
		if self.employee_benefits:
			...
			if max_benefit_amount > self.max_benefits:
				dataent.throw(_("Maximum benefit amount of employee {0} exceeds {1}").format(self.employee, self.max_benefits))

	def validate_max_benefit(self, earning_component_name):
		max_benefit_amount = dataent.db.get_value("Salary Component", earning_component_name, "max_benefit_amount")
		benefit_amount = 0
		for employee_benefit in self.employee_benefits:
			if employee_benefit.earning_component == earning_component_name:
				benefit_amount += employee_benefit.amount
		prev_sal_slip_flexi_amount = get_sal_slip_total_benefit_given(self.employee, dataent.get_doc("Payroll Period", self.payroll_period), earning_component_name)
	...
	def validate_duplicate_on_payroll_period(self):
		...
		if application:
			dataent.throw(_("Employee {0} already submited an apllication {1} for the payroll period {2}").format(self.employee, application, self.payroll_period))

@dataent.whitelist()
def get_max_benefits(employee, on_date):
	sal_struct = get_assigned_salary_structure(employee, on_date)
	if sal_struct:
		max_benefits = dataent.db.get_value("Salary Structure", sal_struct, "max_benefits")
		if max_benefits > 0:
			return max_benefits
	return False
...
def get_max_benefits_remaining(employee, on_date, payroll_period):
	...
	if max_benefits and max_benefits > 0:
		...
		if prev_sal_slip_flexi_total > 0:
			...
			if sal_struct_name:
				sal_struct = dataent.get_doc("Salary Structure", sal_struct_name)
				for sal_struct_row in sal_struct.get("earnings"):
					salary_component = dataent.get_doc("Salary Component", sal_struct_row.salary_component)
...
def get_benefit_component_amount(employee, start_date, end_date, salary_component, sal_struct, payroll_frequency, payroll_period):
	...
	benefit_application = dataent.db.sql("""
		select name
	...
	component_max_benefit, depends_on_payment_days = dataent.db.get_value("Salary Component",
		salary_component, ["max_benefit_amount", "depends_on_payment_days"])

	if benefit_application:
		benefit_amount = dataent.db.get_value("Employee Benefit Application Detail",
...
def get_benefit_amount_based_on_pro_rata(sal_struct, component_max_benefit):
	...
	for d in sal_struct.get("earnings"):
		if d.is_flexible_benefit == 1:
			component = dataent.db.get_value("Salary Component", d.salary_component, ["max_benefit_amount", "pay_against_benefit_claim"], as_dict=1)
...
def get_earning_components(doctype, txt, searchfield, start, page_len, filters):
	...
	if salary_structure:
		...
		dataent.throw(_("Salary Structure not found for employee {0} and date {1}")
