...
from epaas.hr.doctype.leave_application.leave_application import LeaveDayBlockedError, OverlapError, NotAnOptionalHoliday, get_leave_balance_on
from dataent.permissions import clear_user_permissions_for_doctype
from dataent.utils import add_days, nowdate, now_datetime
...
class TestLeaveApplication(unittest.TestCase):
	...
	def _clear_roles(self):
		dataent.db.sql("""delete from `tabHas Role` where parent in
	...
	def get_application(self, doc):
		application = dataent.copy_doc(doc)
	...
	def test_optional_leave(self):
		...
		if not dataent.db.exists('Holiday List', holiday_list):
		...
		dataent.db.set_value('Leave Period', leave_period.name, 'optional_holiday_list', holiday_list)
	...
	def test_current_leave_on_submit(self):
		...
		allocation = dataent.get_doc(dict(
...
def set_leave_approver():
	employee = get_employee()
	dept_doc = dataent.get_doc("Department", employee.department)
...
def allocate_leaves(employee, leave_period, leave_type, new_leaves_allocated, eligible_leaves=0):
	allocate_leave = dataent.get_doc({
		"doctype": "Leave Allocation",
		"__islocal": 1,
		"employee": employee.name,
