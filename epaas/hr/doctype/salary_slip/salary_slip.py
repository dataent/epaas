...
from dataent.utils import add_days, cint, cstr, flt, getdate, rounded, date_diff, money_in_words
from dataent.model.naming import make_autoname

from dataent import msgprint, _
from epaas.hr.doctype.payroll_entry.payroll_entry import get_start_end_dates
...
from epaas.hr.doctype.additional_salary.additional_salary import get_additional_salary_component
from epaas.hr.doctype.payroll_period.payroll_period import get_period_factor, get_payroll_period
from epaas.hr.doctype.employee_benefit_application.employee_benefit_application import get_benefit_component_amount
from epaas.hr.doctype.employee_benefit_claim.employee_benefit_claim import get_benefit_claim_amount, get_last_payroll_period_benefits
...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		if dataent.db.get_single_value("HR Settings", "max_working_hours_against_timesheet"):
			max_working_hours = dataent.db.get_single_value("HR Settings", "max_working_hours_against_timesheet")
			if self.salary_slip_based_on_timesheet and (self.total_working_hours > int(max_working_hours)):
				dataent.msgprint(_("Total working hours should not be greater than max working hours {0}").
								format(max_working_hours), alert=True)

	def on_submit(self):
		if self.net_pay < 0:
			dataent.throw(_("Net Pay cannot be less than 0"))
		else:
			self.set_status()
			self.update_status(self.name)
			self.update_salary_slip_in_additional_salary()
			if (dataent.db.get_single_value("HR Settings", "email_salary_slip_to_employee")) and not dataent.flags.via_payroll_entry:
	...
	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			ret_exist = dataent.db.sql("""select name from `tabSalary Slip`
						where start_date = %s and end_date = %s and docstatus != 2
						and employee = %s and name != %s""",
						(self.start_date, self.end_date, self.employee, self.name))
			if ret_exist:
				self.employee = ''
				dataent.throw(_("Salary Slip of employee {0} already created for this period").format(self.employee))
		else:
			for data in self.timesheets:
				if dataent.db.get_value('Timesheet', data.time_sheet, 'status') == 'Payrolled':
					dataent.throw(_("Salary Slip of employee {0} already created for time sheet {1}").format(self.employee, data.time_sheet))
	...
	def get_emp_and_leave_details(self):
		...
		if self.employee:
			...
			if not self.salary_slip_based_on_timesheet:
				self.get_date_details()
			self.validate_dates()
			joining_date, relieving_date = dataent.get_cached_value("Employee", self.employee,
			...
			if struct:
				self._salary_structure_doc = dataent.get_doc('Salary Structure', struct)
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			self.set("timesheets", [])
			timesheets = dataent.db.sql(""" select * from `tabTimesheet` where employee = %(employee)s and start_date BETWEEN %(start_date)s AND %(end_date)s and (status = 'Submitted' or
	...
	def check_sal_struct(self, joining_date, relieving_date):
		...
		if st_name:
			...
			dataent.msgprint(_("No active or default Salary Structure found for employee {0} for the given dates")
				.format(self.employee), title=_('Salary Structure Missing'))

	def pull_sal_struct(self):
		from epaas.hr.doctype.salary_structure.salary_structure import make_salary_slip
	...
	def get_leave_details(self, joining_date=None, relieving_date=None, lwp=None, for_preview=0):
		...
		if not cint(dataent.db.get_value("HR Settings", None, "include_holidays_in_total_working_days")):
			working_days -= len(holidays)
			if working_days < 0:
				dataent.throw(_("There are more holidays than working days this month."))

		if not lwp:
			lwp = actual_lwp
		elif lwp != actual_lwp:
			dataent.msgprint(_("Leave Without Pay does not match with approved Leave Application records"))
	...
	def get_payment_days(self, joining_date, relieving_date):
		...
		if relieving_date:
			if getdate(self.start_date) <= relieving_date <= getdate(self.end_date):
				...
				dataent.throw(_("Employee relieved on {0} must be set as 'Left'")
	...
	def get_holidays_for_employee(self, start_date, end_date):
		...
		holidays = dataent.db.sql_list('''select holiday_date from `tabHoliday`
			where
	...
	def add_earning_for_hourly_wages(self, doc, salary_component, amount):
		...
		if not row_exists:
			wages_row = {
				...
				"abbr": dataent.db.get_value("Salary Component", salary_component, "salary_component_abbr"),
	...
	def calculate_component_amounts(self):
		if not getattr(self, '_salary_structure_doc', None):
			self._salary_structure_doc = dataent.get_doc('Salary Structure', self.salary_structure)
	...
	def get_data_for_eval(self):
		...
		data.update(dataent.get_doc("Salary Structure Assignment",
			{"employee": self.employee, "salary_structure": self.salary_structure}).as_dict())

		data.update(dataent.get_doc("Employee", self.employee).as_dict())
		data.update(self.as_dict())

		# set values for components
		salary_components = dataent.get_all("Salary Component", fields=["salary_component_abbr"])
	...
	def eval_condition_and_formula(self, d, data):
		try:
			...
			if condition:
				if not dataent.safe_eval(condition, self.whitelisted_globals, data):
					return None
			amount = d.amount
			if d.amount_based_on_formula:
				formula = d.formula.strip() if d.formula else None
				if formula:
					amount = flt(dataent.safe_eval(formula, self.whitelisted_globals, data), d.precision("amount"))
			if amount:
				data[d.abbr] = amount
			...
			dataent.throw(_("Syntax error in formula or condition: {0}".format(err)))
	...
	def add_employee_benefits(self, payroll_period):
		for struct_row in self._salary_structure_doc.get("earnings"):
			if struct_row.is_flexible_benefit == 1:
				if dataent.db.get_value("Salary Component", struct_row.salary_component, "pay_against_benefit_claim") != 1:
			...
			if (getdate(payroll_period.end_date) <= getdate(self.end_date)):
				...
				if last_benefits:
					for last_benefit in last_benefits:
						last_benefit = dataent._dict(last_benefit)
						amount = last_benefit.amount
						self.update_component_row(dataent._dict(last_benefit.struct_row), amount, "earnings")
	...
	def add_additional_salary_components(self):
		...
		if additional_components:
			for additional_component in additional_components:
				...
				self.update_component_row(dataent._dict(additional_component.struct_row), amount, key, overwrite=overwrite)
	...
	def add_tax_components(self, payroll_period):
		...
		if not tax_components:
			tax_components = [d.name for d in dataent.get_all("Salary Component", filters={"variable_based_on_taxable_salary": 1})
	...
	def calculate_variable_based_on_taxable_salary(self, tax_component, payroll_period):
		if not payroll_period:
			dataent.msgprint(_("Start and end dates not in a valid Payroll Period, cannot calculate {0}.")
	...
	def get_taxable_earnings_for_prev_period(self, start_date, end_date):
		taxable_earnings = dataent.db.sql("""
	...
	def get_tax_paid_in_period(self, start_date, end_date, tax_component):
		...
		total_tax_paid = flt(dataent.db.sql("""
			select
				sum(sd.amount)
			from
				`tabSalary Detail` sd join `tabSalary Slip` ss on sd.parent=ss.name
	...
	def get_taxable_earnings(self, based_on_payment_days=0):
		...
		if not joining_date:
			dataent.throw(_("Please set the Date Of Joining for employee {0}").format(dataent.bold(self.employee_name)))
	...
	def get_amount_based_on_payment_days(self, row, joining_date, relieving_date):
		...
		# apply rounding
		if dataent.get_cached_value("Salary Component", row.salary_component, "round_to_the_nearest_integer"):
	...
	def calculate_unclaimed_taxable_benefits(self, payroll_period):
		...
		total_benefits_paid = flt(dataent.db.sql("""
			select sum(sd.amount)
		...
		total_benefits_claimed = flt(dataent.db.sql("""
	...
	def get_total_exemption_amount_and_other_incomes(self, payroll_period):
		...
		if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
			exemption_proof = dataent.db.get_value("Employee Tax Exemption Proof Submission",
				{"employee": self.employee, "payroll_period": payroll_period.name, "docstatus": 1},
				["exemption_amount", "income_from_other_sources"])
			if exemption_proof:
				total_exemption_amount, other_incomes = exemption_proof
		else:
			declaration = dataent.db.get_value("Employee Tax Exemption Declaration",
				{"employee": self.employee, "payroll_period": payroll_period.name, "docstatus": 1},
	...
	def calculate_tax_by_tax_slab(self, payroll_period, annual_taxable_earning):
		payroll_period_obj = dataent.get_doc("Payroll Period", payroll_period)
	...
	def eval_tax_slab_condition(self, condition, data):
		try:
			...
			if condition:
				return dataent.safe_eval(condition, self.whitelisted_globals, data)
		except NameError as err:
			dataent.throw(_("Name error: {0}".format(err)))
		except SyntaxError as err:
	...
	def get_salary_slip_row(self, salary_component):
		component = dataent.get_doc("Salary Component", salary_component)
		# Data for update_component_row
		struct_row = dataent._dict()
	...
	def get_loan_details(self):
		return dataent.db.sql("""select rps.principal_amount, rps.interest_amount, l.name,
	...
	def email_salary_slip(self):
		receiver = dataent.db.get_value("Employee", self.employee, "prefered_email")
		...
		if receiver:
			email_args = {
				...
				"attachments": [dataent.attach_print(self.doctype, self.name, file_name=self.name)],
				"reference_doctype": self.doctype,
				"reference_name": self.name
				}
			if not dataent.flags.in_test:
				enqueue(method=dataent.sendmail, queue='short', timeout=300, is_async=True, **email_args)
			else:
	...
	def update_status(self, salary_slip=None):
		for data in self.timesheets:
			if data.time_sheet:
				timesheet = dataent.get_doc('Timesheet', data.time_sheet)
	...
	def pull_emp_details(self):
		emp = dataent.db.get_value("Employee", self.employee, ["bank_name", "bank_ac_no"], as_dict=1)
		if emp:
...
def unlink_ref_doc_from_salary_slip(ref_no):
	linked_ss = dataent.db.sql_list("""select name from `tabSalary Slip`
	where journal_entry=%s and docstatus < 2""", (ref_no))
	if linked_ss:
		for ss in linked_ss:
			ss_doc = dataent.get_doc("Salary Slip", ss)
			dataent.db.set_value("Salary Slip", ss_doc.name, "journal_entry", "")
