...
from dataent.utils import getdate, nowdate, add_days, add_months, flt, get_first_day, get_last_day
from epaas.hr.doctype.salary_structure.salary_structure import make_salary_slip
from epaas.hr.doctype.payroll_entry.payroll_entry import get_month_details
from epaas.hr.doctype.employee.test_employee import make_employee
from epaas.hr.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration \
...
class TestSalarySlip(unittest.TestCase):
	def setUp(self):
		...
		dataent.db.set_value("Company", epaas.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List")
		dataent.db.set_value("HR Settings", None, "email_salary_slip_to_employee", 0)

	def tearDown(self):
		dataent.db.set_value("HR Settings", None, "include_holidays_in_total_working_days", 0)
		dataent.set_user("Administrator")

	def test_salary_slip_with_holidays_included(self):
		no_of_days = self.get_no_of_days()
		dataent.db.set_value("HR Settings", None, "include_holidays_in_total_working_days", 1)
		make_employee("test_employee@salary.com")
		dataent.db.set_value("Employee", dataent.get_value("Employee",
	...
	def test_employee_salary_slip_read_permission(self):
		...
		dataent.set_user("test_employee@salary.com")
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=epaas.get_default_company())[0]
	...
	def test_tax_for_payroll_period(self):
		...
		for doc in delete_docs:
			dataent.db.sql("delete from `tab%s` where employee='%s'" % (doc, employee))
		...
		try:
			...
			raise
		dataent.db.sql("""delete from `tabSalary Slip` where employee=%s""", (employee))
		...
		try:
			...
			print("\nSalary Slip - Tax calculation failed on following case\n", data, "\n")
			raise
		dataent.db.sql("""delete from `tabAdditional Salary` where employee=%s""", (employee))

		# undelete fixture data
		dataent.db.rollback()

	def make_holiday_list(self):
		fiscal_year = get_fiscal_year(nowdate(), company=epaas.get_default_company())
		if not dataent.db.get_value("Holiday List", "Salary Slip Test Holiday List"):
	...
	def make_activity_for_employee(self):
		activity_type = dataent.get_doc("Activity Type", "_Test Activity Type")
...
def make_employee_salary_slip(user, payroll_frequency, salary_structure=None):
	...
	employee = dataent.db.get_value("Employee", {"user_id": user})
	salary_structure_doc = make_salary_structure(salary_structure, payroll_frequency, employee)
	salary_slip = dataent.db.get_value("Salary Slip", {"employee": dataent.db.get_value("Employee", {"user_id": user})})

	if not salary_slip:
		salary_slip = make_salary_slip(salary_structure_doc.name, employee = employee)
		salary_slip.employee_name = dataent.get_value("Employee",
			{"name":dataent.db.get_value("Employee", {"user_id": user})}, "employee_name")
...
def make_salary_component(salary_components, test_tax):
	for salary_component in salary_components:
		if not dataent.db.exists('Salary Component', salary_component["salary_component"]):
			if test_tax:
			...
			dataent.get_doc(salary_component).insert()
...
def create_account(company):
	salary_account = dataent.db.get_value("Account", "Salary - " + dataent.get_cached_value('Company',  company,  'abbr'))
	if not salary_account:
		dataent.get_doc({
			"doctype": "Account",
			"account_name": "Salary",
			"parent_account": "Indirect Expenses - " + dataent.get_cached_value('Company',  company,  'abbr'),
			"company": company
		}).insert()
...
def get_tax_paid_in_period(employee):
	tax_paid_amount = dataent.db.sql("""select sum(sd.amount) from `tabSalary Detail`
...
def create_salary_slips_for_payroll_period(employee, salary_structure, payroll_period, deduct_random=True):
	...
	i = 0
	while i < 12:
		slip = dataent.get_doc({"doctype": "Salary Slip", "employee": employee,
