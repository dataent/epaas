...
from dataent.utils import formatdate, format_datetime, getdate, get_datetime, nowdate, flt, cstr
...
class EmployeeBoardingController(Document):
	...
	def on_submit(self):
		...
		for activity in self.activities:
			...
			if activity.role:
				user_list = dataent.db.sql_list('''select distinct(parent) from `tabHas Role`
	...
	def on_cancel(self):
		# delete task project
		for task in dataent.get_all("Task", filters={"project": self.project}):
			dataent.delete_doc("Task", task.name, force=1)
		dataent.delete_doc("Project", self.project, force=1)
		self.db_set('project', '')
		for activity in self.activities:
...
def get_onboarding_details(parent, parenttype):
	return dataent.get_all("Employee Boarding Activity",
...
def set_employee_name(doc):
	if doc.employee and not doc.employee_name:
		doc.employee_name = dataent.db.get_value("Employee", doc.employee, "employee_name")

def update_employee(employee, details, date=None, cancel=False):
	internal_work_history = {}
	for item in details:
		fieldtype = dataent.get_meta("Employee").get_field(item.fieldname).fieldtype
...
def get_employee_fields_label():
	fields = []
	for df in dataent.get_meta("Employee").get("fields"):
...
def validate_dates(doc, from_date, to_date):
	date_of_joining, relieving_date = dataent.db.get_value("Employee", doc.employee, ["date_of_joining", "relieving_date"])
	if getdate(from_date) > getdate(to_date):
		dataent.throw(_("To date can not be less than from date"))
	elif getdate(from_date) > getdate(nowdate()):
		dataent.throw(_("Future dates not allowed"))
	elif date_of_joining and getdate(from_date) < getdate(date_of_joining):
		dataent.throw(_("From date can not be less than employee's joining date"))
	elif relieving_date and getdate(to_date) > getdate(relieving_date):
		dataent.throw(_("To date can not greater than employee's relieving date"))
...
def validate_overlap(doc, from_date, to_date, company = None):
	...
	overlap_doc = dataent.db.sql(query.format(doc.doctype),{
...
def get_employee_leave_policy(employee):
	leave_policy = dataent.db.get_value("Employee", employee, "leave_policy")
	if not leave_policy:
		employee_grade = dataent.db.get_value("Employee", employee, "grade")
		if employee_grade:
			leave_policy = dataent.db.get_value("Employee Grade", employee_grade, "default_leave_policy")
			if not leave_policy:
				dataent.throw(_("Employee {0} of grade {1} have no default leave policy").format(employee, employee_grade))
	if leave_policy:
		return dataent.get_doc("Leave Policy", leave_policy)
	else:
		dataent.throw(_("Please set leave policy for employee {0} in Employee / Grade record").format(employee))

def validate_tax_declaration(declarations):
	subcategories = []
	for d in declarations:
		if d.exemption_sub_category in subcategories:
			dataent.throw(_("More than one selection for {0} not allowed").format(d.exemption_sub_category))
...
def get_total_exemption_amount(declarations):
	...
	for d in declarations:
		...
		if not category_max_amount:
			category_max_amount = dataent.db.get_value("Employee Tax Exemption Category", d.exemption_category, "max_amount")
...
def allocate_earned_leaves():
	...
	e_leave_types = dataent.get_all("Leave Type",
	...
	if e_leave_types:
		for e_leave_type in e_leave_types:
			leave_allocations = dataent.db.sql("""select name, employee, from_date, to_date from `tabLeave Allocation` where '{0}'
			...
			for allocation in leave_allocations:
				...
				if not e_leave_type.earned_leave_frequency == "Monthly":
					if not check_frequency_hit(allocation.from_date, today, e_leave_type.earned_leave_frequency):
						continue
				annual_allocation = dataent.db.sql("""select annual_allocation from `tabLeave Policy Detail`
				...
				if annual_allocation and annual_allocation[0]:
					...
					allocated_leaves = dataent.db.get_value('Leave Allocation', allocation.name, 'total_leaves_allocated')
					...
					dataent.db.set_value('Leave Allocation', allocation.name, 'total_leaves_allocated', new_allocation)
...
def get_sal_slip_total_benefit_given(employee, payroll_period, component=False):
	...
	sum_of_given_benefit = dataent.db.sql(query, {
		'employee': employee,
...
def get_previous_claimed_amount(employee, payroll_period, non_pro_rata=False, component=False):
	...
	sum_of_claimed_amount = dataent.db.sql(query, {
		'employee': employee,
		'start_date': payroll_period.start_date,
		'end_date': payroll_period.end_date,
		'component': component
