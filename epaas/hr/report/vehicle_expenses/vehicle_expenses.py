...
from epaas.accounts.report.financial_statements import get_period_list
...
def get_log_data(filters):
	fy = dataent.db.get_value('Fiscal Year', filters.get('fiscal_year'), ['year_start_date', 'year_end_date'], as_dict=True)
	data = dataent.db.sql("""select
...
def get_service_expense(logname):
	expense_amount = dataent.db.sql("""select sum(expense_amount)
