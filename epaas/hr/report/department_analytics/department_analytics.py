...
def get_employees(filters):
	...
	return dataent.db.sql("""select name, employee_name, date_of_birth,
	branch, department, designation,
	gender, company from `tabEmployee` where status = 'Active' %s""" % conditions, as_list=1)

def get_department(filters):
	return dataent.db.sql("""select name from `tabDepartment`""" , as_list=1)
...
def get_chart_data(departments,employees):
	...
	for department in departments:
		if department:
			total_employee = dataent.db.sql("""select count(*) from \
				`tabEmployee` where \
