...
def get_columns(salary_slips):
	...
	for component in dataent.db.sql("""select distinct sd.salary_component, sc.type
...
def get_salary_slips(filters):
	...
	salary_slips = dataent.db.sql("""select * from `tabSalary Slip` where %s
...
def get_ss_earning_map(salary_slips):
	ss_earnings = dataent.db.sql("""select parent, salary_component, amount
		from `tabSalary Detail` where parent in (%s)""" %
		(', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=1)

	ss_earning_map = {}
	for d in ss_earnings:
		ss_earning_map.setdefault(d.parent, dataent._dict()).setdefault(d.salary_component, [])
		ss_earning_map[d.parent][d.salary_component] = flt(d.amount)

	return ss_earning_map

def get_ss_ded_map(salary_slips):
	ss_deductions = dataent.db.sql("""select parent, salary_component, amount
		from `tabSalary Detail` where parent in (%s)""" %
		(', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=1)

	ss_ded_map = {}
	for d in ss_deductions:
		ss_ded_map.setdefault(d.parent, dataent._dict()).setdefault(d.salary_component, [])
