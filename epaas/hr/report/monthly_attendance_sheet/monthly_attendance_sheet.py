...
import dataent
from dataent.utils import cstr, cint, getdate
...
def execute(filters=None):
	...
	default_holiday_list = dataent.get_cached_value('Company',  filters.get("company"),  "default_holiday_list")
	holiday_list.append(default_holiday_list)
	holiday_list = list(set(holiday_list))
	holiday_map = get_holiday(holiday_list, filters["month"])

	data = []
	leave_types = dataent.db.sql("""select name from `tabLeave Type`""", as_list=True)
	...
	for emp in sorted(att_map):
		...
		leave_details = dataent.db.sql("""select leave_type, status, count(*) as count from `tabAttendance`\
...
def get_attendance_list(conditions, filters):
	attendance_list = dataent.db.sql("""select employee, day(attendance_date) as day_of_month,
		status from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
		conditions, filters, as_dict=1)

	att_map = {}
	for d in attendance_list:
		att_map.setdefault(d.employee, dataent._dict()).setdefault(d.day_of_month, "")
...
def get_employee_details():
	emp_map = dataent._dict()
	for d in dataent.db.sql("""select name, employee_name, designation, department, branch, company,
...
def get_holiday(holiday_list, month):
	holiday_map = dataent._dict()
	for d in holiday_list:
		if d:
			holiday_map.setdefault(d, dataent.db.sql_list('''select day(holiday_date) from `tabHoliday`
...
def get_attendance_years():
	year_list = dataent.db.sql_list("""select distinct YEAR(attendance_date) from tabAttendance ORDER BY YEAR(attendance_date) DESC""")
	if not year_list:
