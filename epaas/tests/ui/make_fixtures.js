...
QUnit.test('Make fixtures', assert => {
	...
	let tasks = [];
	Object.keys(dataent.test_data).forEach(function(doctype) {
		tasks.push(function() {
			return dataent.tests.setup_doctype(doctype, dataent.test_data[doctype]);
		});
	});
	dataent.run_serially(tasks).then(() => done());
});
