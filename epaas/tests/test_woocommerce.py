...
import unittest, dataent, requests, os, time, epaas
from epaas.epaas_integrations.connectors.woocommerce_connection import order

class TestWoocommerce(unittest.TestCase):
	def setUp(self):
		if not dataent.db.exists('Company', 'Woocommerce'):
		...
		woo_settings = dataent.get_doc("Woocommerce Settings")
...
def emulate_request():
	...
	url = "http://{site}:{port}/api/method/epaas.epaas_integrations.connectors.woocommerce_connection.order".format(site=host, port=port)
