...
class RestaurantMenu(Document):
	def validate(self):
		for d in self.items:
			if not d.rate:
				d.rate = dataent.db.get_value('Item', d.item, 'standard_rate')
	...
	def clear_item_price(self, price_list=None):
		...
		dataent.db.sql('delete from `tabItem Price` where price_list = %s', price_list)
	...
	def get_price_list(self):
		...
		price_list_name = dataent.db.get_value('Price List', dict(restaurant_menu=self.name))
		if price_list_name:
			price_list = dataent.get_doc('Price List', price_list_name)
		else:
			price_list = dataent.new_doc('Price List')
