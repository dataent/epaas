...
dataent.ui.form.on('Restaurant Order Entry', {
	setup: function(frm) {
		let get_item_query = () => {
			return {
				query: 'epaas.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				filters: {
	...
	onload_post_render: function(frm) {
		if(!frm.item_selector) {
			frm.item_selector = new epaas.ItemSelector({
				frm: frm,
				item_field: 'item',
				item_query: 'epaas.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				get_filters: () => {
	...
	restaurant_table: function(frm) {
		...
		return dataent.call({
			method: 'epaas.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_invoice',
			args: {
				table: frm.doc.restaurant_table
			},
			callback: (r) => {
	...
	sync: function(frm) {
		return dataent.call({
			method: 'epaas.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.sync',
			args: {
				table: frm.doc.restaurant_table,
				items: frm.doc.items
			},
	...
	make_invoice: function(frm) {
		frm.trigger('sync').then(() => {
			dataent.prompt([
				...
				return dataent.call({
					method: 'epaas.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.make_invoice',
					args: {
						table: frm.doc.restaurant_table,
