...
dataent.pages['bank-reconciliation'].on_page_load = function(wrapper) {
	new epaas.accounts.bankReconciliation(wrapper);
}

epaas.accounts.bankReconciliation = class BankReconciliation {
	constructor(wrapper) {
	...
	make() {
		...
		me.page.add_field({
			...
			get_query: function() {
				if(!me.company) {
					dataent.throw(__("Please select company first"));
					return
				}
	...
	check_plaid_status() {
		const me = this;
		dataent.db.get_value("Plaid Settings", "Plaid Settings", "enabled", (r) => {
			if (r && r.enabled == "1") {
	...
	add_actions() {
		...
		me.page.add_menu_item(__("Upload a statement"), function() {
			...
			new epaas.accounts.bankTransactionUpload(me);
		}, true)

		if (me.plaid_status==="active") {
			me.page.add_menu_item(__("Synchronize this account"), function() {
				me.clear_page_content();
				new epaas.accounts.bankTransactionSync(me);
			}, true)
		}
	...
	clear_page_content() {
		const me = this;
		$(me.page.body).find('.dataent-list').remove();
		me.$main_section.empty();
	}

	make_reconciliation_tool() {
		const me = this;
		dataent.model.with_doctype("Bank Transaction", () => {
			epaas.accounts.ReconciliationList = new epaas.accounts.ReconciliationTool({
				parent: me.parent,
...
epaas.accounts.bankTransactionUpload = class bankTransactionUpload {
	constructor(parent) {
		this.parent = parent;
		this.data = [];

		const assets = [
			"/assets/dataent/css/dataent-datatable.css",
			"/assets/dataent/js/lib/clusterize.min.js",
			"/assets/dataent/js/lib/Sortable.min.js",
			"/assets/dataent/js/lib/dataent-datatable.js"
		];
	...
	make() {
		...
		dataent.upload.make({
			args: {
				method: 'epaas.accounts.doctype.bank_transaction.bank_transaction_upload.upload_bank_statement',
				allow_multiple: 0
			},
			no_socketio: true,
	...
	create_datatable() {
		try {
			...
			let msg = __(`Your file could not be processed by EPAAS.
	...
	add_bank_entries() {
		const me = this;
		dataent.xcall('epaas.accounts.doctype.bank_transaction.bank_transaction_upload.create_bank_entries',
			...
			if (result.errors == 0) {
				dataent.show_alert({message:__("All bank transactions have been created"), indicator:'green'});
			} else {
				dataent.show_alert({message:__("Please check the error log for details about the import errors"), indicator:'red'});
			}
		})
	}
}

epaas.accounts.bankTransactionSync = class bankTransactionSync {
	constructor(parent) {
		this.parent = parent;
		this.data = [];
	...
	sync_transactions() {
		const me = this;
		dataent.db.get_value("Bank Account", me.parent.bank_account, "bank", (v) => {
			dataent.xcall('epaas.epaas_integrations.doctype.plaid_settings.plaid_settings.sync_transactions', {
				bank: v['bank'],
			...
			.then((result) => {
				...
				dataent.show_alert({message:__("Bank account '{0}' has been synchronized", [me.parent.bank_account]), indicator:'green'});
			})
		})
	}
}
...
epaas.accounts.ReconciliationTool = class ReconciliationTool extends dataent.views.BaseList {
	constructor(opts) {
		super(opts);
		this.show();
	}
	...
	render() {
		...
		me.data.map((value) => {
			...
			new epaas.accounts.ReconciliationRow(row, value);
		})
	}
	...
	render_header() {
		...
		if ($(this.wrapper).find('.transaction-header').length === 0) {
			me.$result.append(dataent.render_template("bank_transaction_header"));
		}
	}
}

epaas.accounts.ReconciliationRow = class ReconciliationRow {
	...
	make() {
		$(this.row).append(dataent.render_template("bank_transaction_row", this.data))
	}
	...
	new_payment() {
		...
		dataent.new_doc("Payment Entry", {"payment_type": payment_type, "paid_amount": paid_amount,
	...
	new_invoice() {
		...
		dataent.new_doc(invoice_type)
	}

	new_expense() {
		dataent.new_doc("Expense Claim")
	}
	...
	show_dialog(data) {
		...
		dataent.db.get_value("Bank Account", me.data.bank_account, "account", (r) => {
			me.gl_account = r.account;
		})

		dataent.xcall('epaas.accounts.page.bank_reconciliation.bank_reconciliation.get_linked_payments',
	...
	make_dialog(data) {
		...
		const fields = [
			...
			},
			{
				...
				get_query: () => {
					...
					if (dt === "Payment Entry") {
						return {
							query: "epaas.accounts.page.bank_reconciliation.bank_reconciliation.payment_entry_query",
							filters : {
						...
						return {
							query: "epaas.accounts.page.bank_reconciliation.bank_reconciliation.journal_entry_query",
							filters : {
								"bank_account": this.data.bank_account,
								"company": this.data.company
							}
						...
						return {
							query: "epaas.accounts.page.bank_reconciliation.bank_reconciliation.sales_invoices_query"
						}
		...
		if (data && data.length > 0) {
			proposals_wrapper.append(dataent.render_template("linked_payment_header"));
			data.map(value => {
				proposals_wrapper.append(dataent.render_template("linked_payment_row", value))
			})
		} else {
			const empty_data_msg = __("EPAAS could not find any matching payment entry")
		...
		$(me.dialog.body).on('click', '.reconciliation-btn', (e) => {
			...
			dataent.xcall('epaas.accounts.page.bank_reconciliation.bank_reconciliation.reconcile',
				{bank_transaction: me.bank_entry, payment_doctype: payment_doctype, payment_name: payment_entry})
			.then((result) => {
				setTimeout(function(){
					epaas.accounts.ReconciliationList.refresh();
				}, 2000);
				me.dialog.hide();
			})
		})
	...
	display_payment_details(event) {
		...
		if (event.value) {
			...
			dataent.db.get_doc(dt, event.value)
			.then(doc => {
				let displayed_docs = []
				if (dt === "Payment Entry") {
				...
				details_wrapper.append(dataent.render_template("linked_payment_header"));
				displayed_docs.forEach(values => {
					details_wrapper.append(dataent.render_template("linked_payment_row", values));
				})
			})
		}
