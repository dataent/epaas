...
def reconcile(bank_transaction, payment_doctype, payment_name):
	transaction = dataent.get_doc("Bank Transaction", bank_transaction)
	payment_entry = dataent.get_doc(payment_doctype, payment_name)

	account = dataent.db.get_value("Bank Account", transaction.bank_account, "account")
	gl_entry = dataent.get_doc("GL Entry", dict(account=account, voucher_type=payment_doctype, voucher_no=payment_name))

	if transaction.unallocated_amount == 0:
		dataent.throw(_("This bank transaction is already fully reconciled"))

	if transaction.credit > 0 and gl_entry.credit > 0:
		dataent.throw(_("The selected payment entry should be linked with a debtor bank transaction"))

	if transaction.debit > 0 and gl_entry.debit > 0:
		dataent.throw(_("The selected payment entry should be linked with a creditor bank transaction"))
...
def get_linked_payments(bank_transaction):
	transaction = dataent.get_doc("Bank Transaction", bank_transaction)
	bank_account = dataent.db.get_values("Bank Account", transaction.bank_account, ["account", "company"], as_dict=True)
...
def check_matching_amount(bank_account, company, transaction):
	...
	payment_entries = dataent.get_all("Payment Entry", fields=["'Payment Entry' as doctype", "name", "paid_amount", "payment_type", "reference_no", "reference_date",
	...
	if transaction.debit > 0:
		purchase_invoices = dataent.get_all("Purchase Invoice",
		...
		mode_of_payments = [x["parent"] for x in dataent.db.get_list("Mode of Payment Account",
			filters={"default_account": bank_account}, fields=["parent"])]

		company_currency = get_company_currency(company)

		expense_claims = dataent.get_all("Expense Claim",
...
def get_matching_descriptions_data(company, transaction):
	...
	for key, value in iteritems(links):
		if key == "Payment Entry":
			data.extend(dataent.get_all("Payment Entry", filters=[["name", "in", value]],
		...
		if key == "Journal Entry":
			journal_entries = dataent.get_all("Journal Entry", filters=[["name", "in", value]],
			...
			for journal_entry in journal_entries:
				journal_entry_accounts = dataent.get_all("Journal Entry Account", filters={"parenttype": journal_entry["doctype"], "parent": journal_entry["name"]}, fields=["account_currency"])
				journal_entry["currency"] = journal_entry_accounts[0]["account_currency"] if journal_entry_accounts else company_currency
			data.extend(journal_entries)
		if key == "Sales Invoice":
			data.extend(dataent.get_all("Sales Invoice", filters=[["name", "in", value]], fields=["'Sales Invoice' as doctype", "posting_date", "customer_name as party", "paid_amount", "currency"]))
		if key == "Purchase Invoice":
			data.extend(dataent.get_all("Purchase Invoice", filters=[["name", "in", value]], fields=["'Purchase Invoice' as doctype", "posting_date", "supplier_name as party", "paid_amount", "currency"]))
		if key == "Expense Claim":
			expense_claims = dataent.get_all("Expense Claim", filters=[["name", "in", value]], fields=["'Expense Claim' as doctype", "posting_date", "employee_name as party", "total_amount_reimbursed as paid_amount"])
...
def get_matching_transactions_payments(description_matching):
	...
	if payments:
		reference_payment_list = dataent.get_all("Payment Entry", fields=["name", "paid_amount", "payment_type", "reference_no", "reference_date",
...
def payment_entry_query(doctype, txt, searchfield, start, page_len, filters):
	account = dataent.db.get_value("Bank Account", filters.get("bank_account"), "account")
	if not account:
		return
