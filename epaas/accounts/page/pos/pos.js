dataent.provide("epaas.pos");
{% include "epaas/public/js/controllers/taxes_and_totals.js" %}

dataent.pages['pos'].on_page_load = function (wrapper) {
	...
	dataent.db.get_value('POS Settings', {name: 'POS Settings'}, 'is_online', (r) => {
		if (r && r.use_pos_in_offline_mode && cint(r.use_pos_in_offline_mode)) {
			// offline
			wrapper.pos = new epaas.pos.PointOfSale(wrapper);
			cur_pos = wrapper.pos;
		} else {
			// online
			dataent.flags.is_online = true
			dataent.set_route('point-of-sale');
		}
	});
}

dataent.pages['pos'].refresh = function (wrapper) {
	window.onbeforeunload = function () {
		return wrapper.pos.beforeunload()
	}

	if (dataent.flags.is_online) {
		dataent.set_route('point-of-sale');
	}
}

epaas.pos.PointOfSale = epaas.taxes_and_totals.extend({
	...
	beforeunload: function (e) {
		if (this.connection_status == false && dataent.get_route()[0] == "pos") {
			e = e || window.event;
	...
	make_menu_list: function () {
		...
		this.page.add_menu_item(__("Cashier Closing"), function () {
			dataent.set_route('List', 'Cashier Closing');
		});

		this.page.add_menu_item(__("POS Profile"), function () {
			dataent.set_route('List', 'POS Profile');
		});
	},
	...
	email_prompt: function() {
		...
		this.email_dialog = new dataent.ui.Dialog({
			title: "Email",
			fields: fields,
	...
	update_email_queue: function () {
		try {
			...
			dataent.throw(__("LocalStorage is full, did not save"))
		}
	},
		...
		$.each(this.removed_items, function(index, pos_name){
			$.each(me.si_docs, function(key, data){
				if(me.si_docs[key][pos_name] && me.si_docs[key][pos_name].offline_pos_name == pos_name ){
					dataent.throw(__("Submitted orders can not be deleted"))
				}
			})
		})
	},
	...
	get_data_from_server: function (callback) {
		var me = this;
		dataent.call({
			method: "epaas.accounts.doctype.sales_invoice.pos.get_pos_data",
			freeze: true,
			...
			error: () => {
				setTimeout(() => dataent.set_route('List', 'POS Profile'), 2000);
			}
		})
	},
	...
	init_master_data: function (r) {
		...
		this.letter_head = (this.pos_profile_data.length > 0) ? dataent.boot.letter_heads[this.pos_profile_data[letter_head]] : {};
	},
	...
	load_data: function (load_doc) {
		...
		$.each(this.meta, function (i, data) {
			dataent.meta.sync(data)
			locals["DocType"][data.name] = data;
		})

		this.print_template_data = dataent.render_template("print_template", {
			content: this.print_template,
			title: "POS",
			base_url: dataent.urllib.get_base_url(),
			print_css: dataent.boot.print_css,
	...
	make_control: function() {
		...
		this.wrapper.html(dataent.render_template("pos", this.frm.doc));
		this.make_search();
		this.make_customer();
	...
	make_search: function () {
		var me = this;
		this.search_item = dataent.ui.form.make_control({
			df: {
				"fieldtype": "Data",
				"label": __("Item"),
				"fieldname": "pos_item",
	...
	render_list_customers: function () {
		...
		if(this.si_docs.length) {
			this.si_docs.forEach(function (data, i) {
				for (var key in data) {
					html += dataent.render_template("pos_invoice_list", {
						sr: i + 1,
						name: key,
	...
	bind_delete_event: function() {
		...
		$(this.page.wrapper).on('click', '.btn-danger', function(){
			dataent.confirm(__("Delete permanently?"), function () {
				me.delete_records();
	...
	make_customer: function () {
		...
		if(!this.party_field) {
			if(this.page.wrapper.find('.pos-bill-toolbar').length === 0) {
				$(dataent.render_template('customer_toolbar', {
					allow_delete: this.pos_profile_data["allow_delete"]
				})).insertAfter(this.page.$title_area.hide());
			}

			this.party_field = dataent.ui.form.make_control({
				df: {
					"fieldtype": "Data",
					"options": this.party,
					"label": this.party,
	...
	update_customer: function (new_customer) {
		...
		this.customer_doc = new dataent.ui.Dialog({
			'title': 'Customer',
			fields: [
				{
					"label": __("Full Name"),
	...
	render_address_data: function() {
		...
		if(!this.customer_doc.fields_dict.customer_pos_id.value) {
			this.customer_doc.set_value("customer_pos_id", dataent.datetime.now_datetime())
		}
	},
	...
	make_item_list: function (customer) {
		var me = this;
		if (!this.price_list) {
			dataent.msgprint(__("Price List not found or disabled"));
			return;
		}
		...
		if (this.items.length > 0) {
			$.each(this.items, function(index, obj) {
				...
				if(index < me.page_len) {
					$(dataent.render_template("pos_item", {
						item_code: obj.name,
						...
						color: dataent.get_palette(obj.item_name),
						abbr: dataent.get_abbr(obj.item_name)
	...
	render_selected_item: function() {
		...
		if(this.child_doc.length) {
			...
			this.selected_row = $(dataent.render_template("pos_selected_item", this.child_doc[0]))
	...
	set_item_details: function (item_code, field, value, remove_zero_qty_items) {
		var me = this;
		if (value < 0) {
			dataent.throw(__("Enter value must be positive"));
		}
	...
	customer_validate: function () {
		...
		if (!this.frm.doc.customer || this.party_field.get_value() == "") {
			dataent.throw(__("Please select customer"))
		}
	},
	...
	add_new_item_to_grid: function () {
		var me = this;
		this.child = dataent.model.add_child(this.frm.doc, this.frm.doc.doctype + " Item", "items");
	...
	show_items_in_item_cart: function () {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			$(dataent.render_template("pos_bill_item_new", {
	...
	set_taxes: function () {
		...
		$.each(taxes, function (i, d) {
			if (d.tax_amount && cint(d.included_in_print_rate) == 0) {
				$(dataent.render_template("pos_tax_row", {
	...
	set_primary_action: function () {
		...
		if (this.frm.doc.docstatus == 1 || this.pos_profile_data["allow_print_before_pay"]) {
			this.page.set_secondary_action(__("Print"), function () {
				me.create_invoice();
				var html = dataent.render(me.print_template_data, me.frm.doc)
	...
	print_dialog: function () {
		...
		this.msgprint = dataent.msgprint(
	...
	create_invoice: function () {
		...
		if (this.frm.doc.offline_pos_name
			...
			this.frm.doc.offline_pos_name = dataent.datetime.now_datetime();
			this.frm.doc.posting_date = dataent.datetime.get_today();
			this.frm.doc.posting_time = dataent.datetime.now_time();
	...
	sync_sales_invoice: function () {
		...
		if ((this.si_docs.length || this.email_queue_list || this.customers_list) && !this.freeze) {
			...
			dataent.call({
				method: "epaas.accounts.doctype.sales_invoice.pos.make_invoice",
	...
	item_validate: function () {
		if (this.frm.doc.items.length == 0) {
			dataent.throw(__("Select items to save the invoice"))
		}
	},

	validate_mode_of_payments: function () {
		if (this.frm.doc.payments.length === 0) {
			dataent.throw(__("Payment Mode is not configured. Please check, whether account has been set on Mode of Payments or on POS Profile."))
		}
	},
	...
	validate_serial_no: function () {
		...
		if (this.items[0].has_serial_no && serial_no == "") {
			this.refresh();
			dataent.throw(__(repl("Error: Serial no is mandatory for item %(item)s", {
			...
			$.each(this.frm.doc.items, function (index, data) {
				if (data.item_code == item_code) {
					if (in_list(data.serial_no.split('\n'), serial_no)) {
						dataent.throw(__(repl("Serial no %(serial_no)s is already taken", {
							'serial_no': serial_no
						})))
					}
				}
	...
	validate_serial_no_qty: function (args, item_code, field, value) {
		...
		if (args.item_code == item_code && args.serial_no
			...
			args.qty = 0.0;
			this.refresh();
			dataent.throw(__("Serial no item cannot be a fraction"))
		}
		...
		if (args.item_code == item_code && args.serial_no && args.serial_no.split('\n').length != cint(value)) {
			...
			args.serial_no = ''
			this.refresh();
			dataent.throw(__(repl("Total nos of serial no is not equal to quantity for item %(item)s.", {
				'item': item_code
			})))
		}
	},
	...
	validate_item_condition: function (data, item) {
		var apply_on = dataent.model.scrub(data.apply_on);
	...
	validate_pricing_rule: function (pricing_rule) {
		...
		if (pricing_rule.length > 1) {
			...
			if (priority == 0 || count > 1) {
				dataent.throw(__(repl("Multiple Price Rules exists with same criteria, please resolve conflict by assigning priority. Price Rules: %(pricing_rule)s", {
	...
	validate_warehouse: function () {
		if (this.items[0].is_stock_item && !this.items[0].default_warehouse && !this.pos_profile_data['warehouse']) {
			dataent.throw(__("Default warehouse is required for selected item"))
		}
	},
