...
from epaas.accounts.doctype.payment_entry.payment_entry import get_payment_entry, InvalidPaymentEntry
from epaas.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice, create_sales_invoice_against_cost_center
from epaas.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice, make_purchase_invoice_against_cost_center
from epaas.hr.doctype.expense_claim.test_expense_claim import make_expense_claim
...
class TestPaymentEntry(unittest.TestCase):
	...
	def test_payment_entry_against_pi(self):
		...
		outstanding_amount = flt(dataent.db.get_value("Sales Invoice", pi.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)

	def test_payment_entry_against_ec(self):

		payable = dataent.get_cached_value('Company',  "_Test Company",  'default_payable_account')
		...
		outstanding_amount = flt(dataent.db.get_value("Expense Claim", ec.name, "total_sanctioned_amount")) - \
			flt(dataent.db.get_value("Expense Claim", ec.name, "total_amount_reimbursed"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_payment_entry_retrieves_last_exchange_rate(self):
		from epaas.setup.doctype.currency_exchange.test_currency_exchange import test_records, save_new_records
	...
	def get_gle(self, voucher_no):
		return dataent.db.sql("""select account, debit, credit, against_voucher
