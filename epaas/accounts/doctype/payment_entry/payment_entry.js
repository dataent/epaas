...
{% include "epaas/public/js/controllers/accounts.js" %}

dataent.ui.form.on('Payment Entry', {
	onload: function(frm) {
	...
	setup: function(frm) {
		frm.set_query("paid_from", function() {
			var account_types = in_list(["Pay", "Internal Transfer"], frm.doc.payment_type) ?
				["Bank", "Cash"] : [dataent.boot.party_account_types[frm.doc.party_type]];
		...
		frm.set_query("party_type", function() {
			return{
				"filters": {
					"name": ["in", Object.keys(dataent.boot.party_account_types)],
				}
			}
		});
	...
	hide_unhide_fields: function(frm) {
		var company_currency = frm.doc.company? dataent.get_doc(":Company", frm.doc.company).default_currency: "";
	...
	party_type: function(frm) {
		...
		let party_types = Object.keys(dataent.boot.party_account_types);
		if(frm.doc.party_type && !party_types.includes(frm.doc.party_type)){
			frm.set_value("party_type", "");
			dataent.throw(__("Party can only be one of "+ party_types.join(", ")));
		}
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party) {
			if(!frm.doc.posting_date) {
				dataent.msgprint(__("Please select Posting Date before selecting Party"))
				frm.set_value("party", "");
				return ;
			}
			...
			return dataent.call({
				method: "epaas.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			dataent.call({
				method: "epaas.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	source_exchange_rate: function(frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", epaas.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function(frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", epaas.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_documents: function(frm) {
		...
		return  dataent.call({
			method: 'epaas.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents',
			args: {
				args: {
	...
	allocate_party_amount_against_ref_docs: function(frm, paid_amount) {
		...
		var total_negative_outstanding = 0;
		var total_deductions = dataent.utils.sum($.map(frm.doc.deductions || [],
		...
		if (
			...
			if(paid_amount > total_negative_outstanding) {
				if(total_negative_outstanding == 0) {
					dataent.msgprint(__("Cannot {0} {1} {2} without any negative outstanding invoice",
						[frm.doc.payment_type,
							(frm.doc.party_type=="Customer" ? "to" : "from"), frm.doc.party_type]));
					return false
				} else {
					dataent.msgprint(__("Paid Amount cannot be greater than total negative outstanding amount {0}", [total_negative_outstanding]));
					return false;
				}
			} else {
	...
	check_mandatory_to_fetch: function(frm) {
		$.each(["Company", "Party Type", "Party", "payment_type"], function(i, field) {
			if(!frm.doc[dataent.model.scrub(field)]) {
				dataent.msgprint(__("Please select {0} first", [field]));
				return false;
			}
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Customer" &&
				...
				dataent.model.set_value(row.doctype, row.name, "reference_doctype", null);
				dataent.msgprint(__("Row #{0}: Reference Document Type must be one of Sales Order, Sales Invoice or Journal Entry", [row.idx]));
				return false;
			}
			...
			if(frm.doc.party_type=="Supplier" &&
				...
				dataent.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				dataent.msgprint(__("Row #{0}: Reference Document Type must be one of Purchase Order, Purchase Invoice or Journal Entry", [row.idx]));
				return false;
			}
			...
			if(frm.doc.party_type=="Employee" &&
				...
				dataent.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				dataent.msgprint(__("Row #{0}: Reference Document Type must be one of Expense Claim or Journal Entry", [row.idx]));
				return false;
			}
		}
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			dataent.call({
				method: "epaas.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
					if(r.message) {
						...
						if (row) {
							...
							dataent.msgprint(__("No gain or loss in the exchange rate"))
						}
	...
	bank_account: function(frm) {
		...
		if (frm.doc.bank_account && in_list(['Pay', 'Receive'], frm.doc.payment_type)) {
			dataent.call({
				method: "epaas.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
dataent.ui.form.on('Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return dataent.call({
				method: "epaas.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
...
dataent.ui.form.on('Payment Entry Deduction', {
	amount: function(frm) {
		frm.events.set_unallocated_amount(frm);
	},
...
dataent.ui.form.on('Payment Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return dataent.call({
				method: "epaas.accounts.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
