...
class AccountingPeriod(Document):
	...
	def validate_overlap(self):
		existing_accounting_period = dataent.db.sql("""select name from `tabAccounting Period`
			where (
		...
		if len(existing_accounting_period) > 0:
			dataent.throw(_("Accounting Period overlaps with {0}".format(existing_accounting_period[0].get("name"))))
