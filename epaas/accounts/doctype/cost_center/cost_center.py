...
class CostCenter(NestedSet):
	...
	def validate_mandatory(self):
		if self.cost_center_name != self.company and not self.parent_cost_center:
			dataent.throw(_("Please enter parent cost center"))
		elif self.cost_center_name == self.company and self.parent_cost_center:
			dataent.throw(_("Root cannot have a parent cost center"))

	def convert_group_to_ledger(self):
		if self.check_if_child_exists():
			dataent.throw(_("Cannot convert Cost Center to ledger as it has child nodes"))
		elif self.check_gle_exists():
			dataent.throw(_("Cost Center with existing transactions can not be converted to ledger"))
		else:
			self.is_group = 0
			self.save()
			return 1
	...
	def convert_ledger_to_group(self):
		if self.check_gle_exists():
			dataent.throw(_("Cost Center with existing transactions can not be converted to group"))
		else:
			self.is_group = 1
			self.save()
			return 1
	...
	def check_gle_exists(self):
		return dataent.db.get_value("GL Entry", {"cost_center": self.name})

	def check_if_child_exists(self):
		return dataent.db.sql("select name from `tabCost Center` where \
			parent_cost_center = %s and docstatus != 2", self.name)

	def before_rename(self, olddn, newdn, merge=False):
		# Add company abbr if not provided
		from epaas.setup.doctype.company.company import get_name_with_abbr
	...
	def after_rename(self, olddn, newdn, merge=False):
		...
		if not merge:
			new_cost_center = dataent.db.get_value("Cost Center", newdn, ["cost_center_name", "cost_center_number"], as_dict=1)
...
def on_doctype_update():
	dataent.db.add_index("Cost Center", ["lft", "rgt"])
