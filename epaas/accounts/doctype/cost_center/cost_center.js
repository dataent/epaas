...
dataent.ui.form.on('Cost Center', {
	onload: function(frm) {
	...
	update_cost_center_number: function(frm) {
		var d = new dataent.ui.Dialog({
			...
			primary_action: function() {
				...
				dataent.call({
					method: "epaas.accounts.utils.update_number_field",
					args: {
					...
					callback: function(r) {
						if(!r.exc) {
							if(r.message) {
								dataent.set_route("Form", "Cost Center", r.message);
							} else {
...
cur_frm.cscript.refresh = function(doc, cdt, cdn) {
	...
	if(!cur_frm.doc.__islocal) {
		cur_frm.add_custom_button(__('Chart of Cost Centers'),
			function() { dataent.set_route("Tree", "Cost Center"); });

		cur_frm.add_custom_button(__('Budget'),
			function() { dataent.set_route("List", "Budget", {'cost_center': cur_frm.doc.name}); });
	}
}

cur_frm.cscript.parent_cost_center = function(doc, cdt, cdn) {
	if(!doc.company){
		dataent.msgprint(__('Please enter company name first'));
	}
}
