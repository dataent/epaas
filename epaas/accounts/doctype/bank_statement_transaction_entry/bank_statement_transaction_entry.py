...
class BankStatementTransactionEntry(Document):
	def autoname(self):
		...
		if self.bank:
			...
			if not dataent.db.exists("Bank Statement Settings", mapper_name):
				self.create_settings(self.bank)
			self.bank_settings = mapper_name

	def create_settings(self, bank):
		mapper = dataent.new_doc("Bank Statement Settings")
		mapper.bank = bank
	...
	def get_statement_headers(self):
		if not self.bank_settings:
			dataent.throw(_("Bank Data mapper doesn't exist"))
		mapper_doc = dataent.get_doc("Bank Statement Settings", self.bank_settings)
	...
	def get_matching_payments(self, party, amount, pay_date):
		...
		matching_payments = dataent.db.sql(query, as_dict=True)
	...
	def map_unknown_transactions(self):
		for entry in self.new_transaction_items:
			...
			invoices = dataent.db.sql(query, as_dict = True)
			if(len(invoices) > 0):
	...
	def create_payment_entry(self, pe):
		payment = dataent.new_doc("Payment Entry")
	...
	def submit_payment_entries(self):
		for payment in self.new_transaction_items:
			...
			doc = dataent.get_doc(payment.reference_type, payment.reference_name)
			if doc.docstatus == 1:
...
def get_matching_journal_entries(from_date, to_date, account, against, amount):
	...
	jv_entries = dataent.db.sql(query, as_dict=True)
...
def get_payments_matching_invoice(invoice, amount, pay_date):
	...
	payments = dataent.db.sql(query, as_dict=True)
...
def get_transaction_entries(filename, headers):
	...
	if (filename.lower().endswith("xlsx")):
		...
		dataent.throw(_("Only .csv and .xlsx files are supported currently"))
