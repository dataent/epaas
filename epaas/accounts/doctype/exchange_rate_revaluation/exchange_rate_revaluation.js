...
dataent.ui.form.on('Exchange Rate Revaluation', {
	setup: function(frm) {
	...
	get_entries: function(frm) {
		dataent.call({
			...
			doc: cur_frm.doc,
			callback: function(r){
				dataent.model.clear_table(frm.doc, "accounts");
				if(r.message) {
					r.message.forEach((d) => {
...
dataent.ui.form.on("Exchange Rate Revaluation Account", {
...
var get_account_details = function(frm, cdt, cdn) {
	...
	if(!frm.doc.company || !frm.doc.posting_date) {
		dataent.throw(__("Please select Company and Posting Date to getting entries"));
	}
	dataent.call({
		method: "epaas.accounts.doctype.exchange_rate_revaluation.exchange_rate_revaluation.get_account_details",
		args:{
			account: row.account,
