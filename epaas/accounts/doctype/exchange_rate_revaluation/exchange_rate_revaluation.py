...
from epaas.accounts.doctype.journal_entry.journal_entry import get_balance_on
...
class ExchangeRateRevaluation(Document):
	...
	def validate_mandatory(self):
		if not (self.company and self.posting_date):
			dataent.throw(_("Please select Company and Posting Date to getting entries"))
	...
	def make_jv_entry(self):
		...
		unrealized_exchange_gain_loss_account = dataent.get_cached_value('Company',  self.company,
			"unrealized_exchange_gain_loss_account")
		if not unrealized_exchange_gain_loss_account:
			dataent.throw(_("Please set Unrealized Exchange Gain/Loss Account in Company {0}")
				.format(self.company))
