...
from dataent.contacts.doctype.address.address import get_default_address
from dataent.utils.nestedset import get_root_of
from epaas.setup.doctype.customer_group.customer_group import get_parent_customer_groups

import functools

from six import iteritems

class IncorrectCustomerGroup(dataent.ValidationError): pass
class IncorrectSupplierType(dataent.ValidationError): pass
class ConflictingTaxRule(dataent.ValidationError): pass
...
class TaxRule(Document):
	...
	def validate_tax_template(self):
		...
		if not (self.sales_tax_template or self.purchase_tax_template):
			dataent.throw(_("Tax Template is mandatory."))

	def validate_date(self):
		if self.from_date and self.to_date and self.from_date > self.to_date:
			dataent.throw(_("From Date cannot be greater than To Date"))
	...
	def validate_filters(self):
		...
		for d in filters:
			if conds:
				conds += " and "
			conds += """ifnull({0}, '') = '{1}'""".format(d, dataent.db.escape(cstr(filters[d])))
		...
		tax_rule = dataent.db.sql("select name, priority \
			from `tabTax Rule` where {0} and name != '{1}'".format(conds, self.name), as_dict=1)

		if tax_rule:
			if tax_rule[0].priority == self.priority:
				dataent.throw(_("Tax Rule Conflicts with {0}".format(tax_rule[0].name)), ConflictingTaxRule)

	def validate_use_for_shopping_cart(self):
		'''If shopping cart is enabled and no tax rule exists for shopping cart, enable this one'''
		if (not self.use_for_shopping_cart
			and cint(dataent.db.get_single_value('Shopping Cart Settings', 'enabled'))
			and not dataent.db.get_value('Tax Rule', {'use_for_shopping_cart': 1, 'name': ['!=', self.name]})):

			self.use_for_shopping_cart = 1
			dataent.msgprint(_("Enabling 'Use for Shopping Cart', as Shopping Cart is enabled and there should be at least one Tax Rule for Shopping Cart"))
...
def get_party_details(party, party_type, args=None):
	...
	if args:
		if args.get('billing_address'):
			billing_address = dataent.get_doc('Address', args.get('billing_address'))
		if args.get('shipping_address'):
			shipping_address = dataent.get_doc('Address', args.get('shipping_address'))
	else:
		...
		if billing_address_name:
			billing_address = dataent.get_doc('Address', billing_address_name)
		if shipping_address_name:
			shipping_address = dataent.get_doc('Address', shipping_address_name)
...
def get_tax_template(posting_date, args):
	...
	for key, value in iteritems(args):
		...
		if key == 'customer_group':
			...
			conditions.append("ifnull({0}, '') in ('', '{1}')".format(key, dataent.db.escape(cstr(value))))
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = ["'%s'"%(dataent.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)]
	if customer_groups:
