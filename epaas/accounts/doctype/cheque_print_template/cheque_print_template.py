...
def create_or_update_cheque_print_format(template_name):
	if not dataent.db.exists("Print Format", template_name):
		cheque_print = dataent.new_doc("Print Format")
		cheque_print.update({
		...
		cheque_print = dataent.get_doc("Print Format", template_name)
	
	doc = dataent.get_doc("Cheque Print Template", template_name)
...
<div style="position: relative; top:%(starting_position_from_top_edge)scm">
	<div style="width:%(cheque_width)scm;height:%(cheque_height)scm;">
		...
		<span style="top:%(date_dist_from_top_edge)scm; left:%(date_dist_from_left_edge)scm;
			position: absolute;">
			{{ dataent.utils.formatdate(doc.reference_date) or '' }}
		</span>
		...
		<span style="top:%(amt_in_words_from_top_edge)scm; left:%(amt_in_words_from_left_edge)scm;
			...
			line-height:%(amt_in_words_line_spacing)scm; word-wrap: break-word;">
				{{dataent.utils.money_in_words(doc.base_paid_amount or doc.base_received_amount)}}
		</span>
	...
	dataent.db.set_value("Cheque Print Template", template_name, "has_print_format", 1)
