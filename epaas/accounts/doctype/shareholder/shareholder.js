...
dataent.ui.form.on('Shareholder', {
	refresh: function(frm) {
		dataent.dynamic_link = { doc: frm.doc, fieldname: 'name', doctype: 'Shareholder' };
		...
		if (frm.doc.folio_no != undefined){
			frm.add_custom_button(__("Share Balance"), function(){
				...
				dataent.set_route("query-report", "Share Balance");
			});
			frm.add_custom_button(__("Share Ledger"), function(){
				dataent.route_options = {
					"shareholder": frm.doc.name,
				};
				dataent.set_route("query-report", "Share Ledger");
			});
