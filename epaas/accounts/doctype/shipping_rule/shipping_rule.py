...
from dataent.utils import flt, fmt_money
from dataent.model.document import Document

class OverlappingConditionError(dataent.ValidationError): pass
class FromGreaterThanToError(dataent.ValidationError): pass
class ManyBlankToValuesError(dataent.ValidationError): pass
	...
	def validate_countries(self, doc):
		...
		if self.countries:
			...
			if not shipping_country:
				dataent.throw(_('Shipping Address does not have country, which is required for this Shipping Rule'))
			if shipping_country not in [d.country for d in self.countries]:
				dataent.throw(_('Shipping rule not applicable for country {0}'.format(shipping_country)))
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if not doc.meta.get_field('taxes').options == 'Sales Taxes and Charges':
				dataent.throw(_('Shipping rule only applicable for Selling'))
			...
			if not doc.meta.get_field('taxes').options == 'Purchase Taxes and Charges':
				dataent.throw(_('Shipping rule only applicable for Buying'))
