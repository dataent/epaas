...
dataent.ui.form.on("Payment Request", "onload", function(frm, dt, dn){
	if (frm.doc.reference_doctype) {
		dataent.call({
			method:"epaas.accounts.doctype.payment_request.payment_request.get_print_format_list",
...
dataent.ui.form.on("Payment Request", "refresh", function(frm) {
	if(frm.doc.payment_request_type == 'Inward' &&
		...
		frm.add_custom_button(__('Resend Payment Email'), function(){
			dataent.call({
				method: "epaas.accounts.doctype.payment_request.payment_request.resend_payment_email",
				args: {"docname": frm.doc.name},
				freeze: true,
				freeze_message: __("Sending"),
				callback: function(r){
					if(!r.exc) {
						dataent.msgprint(__("Message Sent"));
					}
				}
			});
		});
	...
	if(!frm.doc.payment_gateway_account && frm.doc.status == "Initiated") {
		frm.add_custom_button(__('Make Payment Entry'), function(){
			dataent.call({
				method: "epaas.accounts.doctype.payment_request.payment_request.make_payment_entry",
				args: {"docname": frm.doc.name},
				freeze: true,
				callback: function(r){
					if(!r.exc) {
...
dataent.ui.form.on("Payment Request", "is_a_subscription", function(frm) {
	...
	if (frm.doc.is_a_subscription && frm.doc.reference_doctype && frm.doc.reference_name) {
		dataent.call({
			method: "epaas.accounts.doctype.payment_request.payment_request.get_subscription_details",
			args: {"reference_doctype": frm.doc.reference_doctype, "reference_name": frm.doc.reference_name},
			freeze: true,
			callback: function(data){
				if(!data.exc) {
					$.each(data.message || [], function(i, v){
						var d = dataent.model.add_child(frm.doc, "Subscription Plan Detail", "subscription_plans");
						d.qty = v.qty;
						d.plan = v.plan;
					});
