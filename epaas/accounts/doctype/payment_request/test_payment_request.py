...
from epaas.accounts.doctype.payment_request.payment_request import make_payment_request
...
class TestPaymentRequest(unittest.TestCase):
	def setUp(self):
		if not dataent.db.get_value("Payment Gateway", payment_gateway["gateway"], "name"):
			dataent.get_doc(payment_gateway).insert(ignore_permissions=True)
			
		for method in payment_method:
			if not dataent.db.get_value("Payment Gateway Account", {"payment_gateway": method["payment_gateway"], 
				"currency": method["currency"]}, "name"):
				dataent.get_doc(method).insert(ignore_permissions=True)
			
	def test_payment_request_linkings(self):
		so_inr = make_sales_order(currency="INR")
		pr = make_payment_request(dt="Sales Order", dn=so_inr.name, recipient_id="saurabh@epaas.xyz")
		...
		pr = make_payment_request(dt="Sales Invoice", dn=si_usd.name, recipient_id="saurabh@epaas.xyz")
