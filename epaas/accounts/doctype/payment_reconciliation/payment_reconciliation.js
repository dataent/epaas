...
dataent.ui.form.on("Payment Reconciliation Payment", {
	invoice_number: function(frm, cdt, cdn) {
		...
		if(row.invoice_number) {
			...
			dataent.model.set_value(cdt, cdn, "allocated_amount", Math.min(invoice_amount, row.amount));
		}
	}
});

epaas.accounts.PaymentReconciliationController = dataent.ui.form.Controller.extend({
	onload: function() {
		var me = this;
		...
		this.frm.set_query('receivable_payable_account', function() {
			if(!me.frm.doc.company || !me.frm.doc.party_type) {
				dataent.msgprint(__("Please select Company and Party Type first"));
			} else {
				return{
					filters: {
						"company": me.frm.doc.company,
						"is_group": 0,
						"account_type": dataent.boot.party_account_types[me.frm.doc.party_type]
					}
				};
			}
		...
		this.frm.set_query('bank_cash_account', function() {
			if(!me.frm.doc.company) {
				dataent.msgprint(__("Please select Company first"));
			} else {
				return{
					filters:[
	...
	set_invoice_options: function() {
		...
		if (invoices) {
			dataent.meta.get_docfield("Payment Reconciliation Payment", "invoice_number",
...
$.extend(cur_frm.cscript, new epaas.accounts.PaymentReconciliationController({frm: cur_frm}));
