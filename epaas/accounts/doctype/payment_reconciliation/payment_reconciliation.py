...
from epaas.accounts.utils import get_outstanding_invoices
...
class PaymentReconciliation(Document):
	...
	def get_jv_entries(self):
		dr_or_cr = ("credit_in_account_currency" if epaas.get_party_account_type(self.party_type) == 'Receivable'
	...
	def reconcile(self, args):
		...
		dr_or_cr = ("credit_in_account_currency"
			if epaas.get_party_account_type(self.party_type) == 'Receivable' else "debit_in_account_currency")
	...
	def check_mandatory_to_fetch(self):
		for fieldname in ["company", "party_type", "party", "receivable_payable_account"]:
			if not self.get(fieldname):
				dataent.throw(_("Please select {0} first").format(self.meta.get_label(fieldname)))


	def validate_invoice(self):
		if not self.get("invoices"):
			dataent.throw(_("No records found in the Invoice table"))

		if not self.get("payments"):
			dataent.throw(_("No records found in the Payment table"))

		unreconciled_invoices = dataent._dict()
		...
		for p in self.get("payments"):
			if p.invoice_type and p.invoice_number and p.allocated_amount:
				...
				if p.invoice_number not in unreconciled_invoices.get(p.invoice_type, {}):
					dataent.throw(_("{0}: {1} not found in Invoice Details table")
						.format(p.invoice_type, p.invoice_number))

				if flt(p.allocated_amount) > flt(p.amount):
					dataent.throw(_("Row {0}: Allocated amount {1} must be less than or equals to Payment Entry amount {2}")
				...
				if flt(p.allocated_amount) - invoice_outstanding > 0.009:
					dataent.throw(_("Row {0}: Allocated amount {1} must be less than or equals to invoice outstanding amount {2}")
						.format(p.idx, p.allocated_amount, invoice_outstanding))

		if not invoices_to_reconcile:
			dataent.throw(_("Please select Allocated Amount, Invoice Type and Invoice Number in atleast one row"))

	def check_condition(self):
		cond = " and posting_date >= '{0}'".format(dataent.db.escape(self.from_date)) if self.from_date else ""
		cond += " and posting_date <= '{0}'".format(dataent.db.escape(self.to_date)) if self.to_date else ""
		dr_or_cr = ("debit_in_account_currency" if epaas.get_party_account_type(self.party_type) == 'Receivable'
