...
from epaas.accounts.doctype.budget.budget import get_actual_expense, BudgetError
from epaas.accounts.doctype.journal_entry.test_journal_entry import make_journal_entry
...
class TestBudget(unittest.TestCase):		
	def test_monthly_budget_crossed_ignore(self):
		...
		self.assertTrue(dataent.db.get_value("GL Entry",
	...
	def test_monthly_budget_crossed_stop1(self):
		...
		dataent.db.set_value("Budget", budget.name, "action_if_accumulated_monthly_budget_exceeded", "Stop")
	...
	def test_exception_approver_role(self):
		...
		dataent.db.set_value('Company', budget.company, 'exception_budget_approver_role', 'Accounts User')

		jv.submit()
		self.assertEqual(dataent.db.get_value('Journal Entry', jv.name, 'docstatus'), 1)
		jv.cancel()

		dataent.db.set_value('Company', budget.company, 'exception_budget_approver_role', '')
	...
	def test_monthly_budget_crossed_for_mr(self):
		...
		dataent.db.set_value("Budget", budget.name, "action_if_accumulated_monthly_budget_exceeded", "Stop")
		dataent.db.set_value("Budget", budget.name, "fiscal_year", fiscal_year)
	...
	def test_monthly_budget_against_parent_group_cost_center(self):
		...
		if not dataent.db.exists("Cost Center", cost_center):
			dataent.get_doc({
				'doctype': 'Cost Center',
		...
		dataent.delete_doc('Journal Entry', jv.name)
		dataent.delete_doc('Cost Center', cost_center)
...
def set_total_expense_zero(posting_date, budget_against_field=None, budget_against_CC=None):
	...
	existing_expense = get_actual_expense(dataent._dict({
...
def make_budget(**args):
	...
	if budget_against == "Project":
		budget_list = dataent.get_all("Budget", fields=["name"], filters = {"name": ("like", "_Test Project/_Test Fiscal Year 2013%")})
	else:
		cost_center_name = "{0}%".format(cost_center or "_Test Cost Center - _TC/_Test Fiscal Year 2013")
		budget_list = dataent.get_all("Budget", fields=["name"], filters = {"name": ("like", cost_center_name)})
	for d in budget_list:
		dataent.db.sql("delete from `tabBudget` where name = %(name)s", d)
		dataent.db.sql("delete from `tabBudget Account` where parent = %(name)s", d)
