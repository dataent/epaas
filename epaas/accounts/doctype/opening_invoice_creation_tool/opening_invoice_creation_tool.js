...
dataent.ui.form.on('Opening Invoice Creation Tool', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		frm.page.set_primary_action(__("Make Invoices"), () => {
			...
			return frm.call({
				...
				callback: (r) => {
					if(!r.exc){
						dataent.msgprint(__("Opening {0} Invoice created", [frm.doc.invoice_type]));
	...
	company: function(frm) {
		dataent.call({
			method: 'epaas.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.get_temporary_opening_account',
			args: {
	...
	make_dashboard: function(frm) {
		...
		if(!$.isEmptyObject(opening_invoices_summary)) {
			let section = frm.dashboard.add_section(
				dataent.render_template('opening_invoice_creation_tool_dashboard', {
...
dataent.ui.form.on('Opening Invoice Creation Tool Item', {
	invoices_add: (frm) => {
		frm.trigger('update_invoice_table');
	}
});