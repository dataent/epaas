...
class TestTaxWithholdingCategory(unittest.TestCase):
	...
	def test_cumulative_threshold_tds(self):
		dataent.db.set_value("Supplier", "Test TDS Supplier", "tax_withholding_category", "Cumulative Threshold TDS")
		invoices = []
		...
		for d in invoices:
			d.cancel()
			dataent.delete_doc("Purchase Invoice", d.name)

	def test_single_threshold_tds(self):
		invoices = []
		dataent.db.set_value("Supplier", "Test TDS Supplier1", "tax_withholding_category", "Single Threshold TDS")
		...
		gl_entries = dataent.db.get_all('GL Entry', filters={'voucher_no': pi.name}, fields=["*"])
	...
	def test_single_threshold_tds_with_previous_vouchers(self):
		invoices = []
		dataent.db.set_value("Supplier", "Test TDS Supplier2", "tax_withholding_category", "Single Threshold TDS")
...
def create_purchase_invoice(**args):
	...
	item = dataent.get_doc('Item', {'item_name': 'TDS Item'})
...
def create_records():
	...
	for name in ['Test TDS Supplier', 'Test TDS Supplier1', 'Test TDS Supplier2']:
		if dataent.db.exists('Supplier', name):
			continue
	...
	# create an item
	if not dataent.db.exists('Item', "TDS Item"):
		dataent.get_doc({
			"doctype": "Item",
			"item_code": "TDS Item",
			"item_name": "TDS Item",
	...
	# create an account
	if not dataent.db.exists("Account", "TDS - _TC"):
		dataent.get_doc({
			'doctype': 'Account',
			'company': '_Test Company',
			'account_name': 'TDS',
...
def create_tax_with_holding_category():
	...
	if not dataent.db.exists("Tax Withholding Category", "Cumulative Threshold TDS"):
		dataent.get_doc({
	...
	# Single thresold
	if not dataent.db.exists("Tax Withholding Category", "Single Threshold TDS"):
		dataent.get_doc({
			"doctype": "Tax Withholding Category",
