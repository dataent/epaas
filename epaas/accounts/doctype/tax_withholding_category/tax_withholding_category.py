...
def get_party_tax_withholding_details(ref_doc):
	tax_withholding_category = dataent.db.get_value('Supplier', ref_doc.supplier, 'tax_withholding_category')
	...
	if not tax_details:
		dataent.throw(_('Please set associated account in Tax Withholding Category {0} against Company {1}')
...
def get_tax_withholding_details(tax_withholding_category, fiscal_year, company):
	tax_withholding = dataent.get_doc("Tax Withholding Category", tax_withholding_category)
...
def get_tax_withholding_rates(tax_withholding, fiscal_year):
	...
	dataent.throw(_("No Tax Withholding data found for the current Fiscal Year."))
...
def get_tds_amount(ref_doc, tax_details, fiscal_year_details):
	...
	if tds_deducted:
		...
		supplier_credit_amount = dataent.get_all('Purchase Invoice Item',
		...
		jv_supplier_credit_amt = dataent.get_all('Journal Entry Account',
