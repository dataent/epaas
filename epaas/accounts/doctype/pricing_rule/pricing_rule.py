...
class MultiplePricingRuleConflict(dataent.ValidationError): pass
...
class PricingRule(Document):
	...
	def validate_mandatory(self):
		for field in ["apply_on", "applicable_for"]:
			tocheck = dataent.scrub(self.get(field) or "")
			if tocheck and not self.get(tocheck):
				throw(_("{0} is required").format(self.meta.get_label(tocheck)), dataent.MandatoryError)
	...
	def cleanup_fields_value(self):
		for logic_field in ["apply_on", "applicable_for", "rate_or_discount"]:
			fieldname = dataent.scrub(self.get(logic_field) or "")
	...
	def validate_rate_or_discount(self):
		for field in ["Rate"]:
			if flt(self.get(dataent.scrub(field))) < 0:
				throw(_("{0} can not be negative").format(field))

	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.item_code:
			max_discount = dataent.get_cached_value("Item", self.item_code, "max_discount")
			if max_discount and flt(self.discount_percentage) > flt(max_discount):
				throw(_("Max discount allowed for item: {0} is {1}%").format(self.item_code, max_discount))

	def validate_price_list_with_currency(self):
		if self.currency and self.for_price_list:
			price_list_currency = dataent.db.get_value("Price List", self.for_price_list, "currency", True)
...
def apply_pricing_rule(args):
	...
	set_serial_nos_based_on_fifo = dataent.db.get_single_value("Stock Settings",
...
def get_serial_no_for_item(args):
	from epaas.stock.get_item_details import get_serial_no
...
def get_pricing_rule_for_item(args):
	...
	if args.ignore_pricing_rule or not args.item_code:
		if dataent.db.exists(args.doctype, args.name) and args.get("pricing_rule"):
			item_details = remove_pricing_rule_for_item(args.get("pricing_rule"), item_details)
		return item_details

	if not (args.item_group and args.brand):
		try:
			args.item_group, args.brand = dataent.get_cached_value("Item", args.item_code, ["item_group", "brand"])
		except TypeError:
			# invalid item_code
			return item_details
		if not args.item_group:
			dataent.throw(_("Item Group not mentioned in item master for item {0}").format(args.item_code))

	if args.transaction_type=="selling":
		if args.customer and not (args.customer_group and args.territory):

			if args.quotation_to and args.quotation_to != 'Customer':
				customer = dataent._dict()
			else:
				customer = dataent.get_cached_value("Customer", args.customer, ["customer_group", "territory"])
		...
		args.supplier_group = dataent.get_cached_value("Supplier", args.supplier, "supplier_group")
...
def remove_pricing_rule_for_item(pricing_rule, item_details):
	pricing_rule = dataent.get_cached_value('Pricing Rule', pricing_rule,
...
def get_pricing_rules(args):
	def _get_tree_conditions(parenttype, allow_blank=True):
		field = dataent.scrub(parenttype)
		condition = ""
		if args.get(field):
			if not dataent.flags.tree_conditions:
				dataent.flags.tree_conditions = {}
			key = (parenttype, args[field], )
			if key in dataent.flags.tree_conditions:
				return dataent.flags.tree_conditions[key]

			try:
				lft, rgt = dataent.db.get_value(parenttype, args[field], ["lft", "rgt"])
			except TypeError:
				dataent.throw(_("Invalid {0}").format(args[field]))

			parent_groups = dataent.db.sql_list("""select name from `tab%s`
				where lft<=%s and rgt>=%s""" % (parenttype, '%s', '%s'), (lft, rgt))

			if parent_groups:
				if allow_blank: parent_groups.append('')
				condition = " ifnull("+field+", '') in ('" + \
					"', '".join([dataent.db.escape(d) for d in parent_groups])+"')"
			dataent.flags.tree_conditions[key] = condition
	...
	if "variant_of" not in args:
		args.variant_of = dataent.get_cached_value("Item", args.item_code, "variant_of")

	if args.variant_of:
		item_variant_condition = ' or item_code=%(variant_of)s '
		values['variant_of'] = args.variant_of

	return dataent.db.sql("""select * from `tabPricing Rule`
...
def filter_pricing_rules(args, pricing_rules):
	...
	if len(pricing_rules) > 1 and not args.for_shopping_cart:
		dataent.throw(_("Multiple Price Rules exists with same criteria, please resolve conflict by assigning priority. Price Rules: {0}")
...
def make_pricing_rule(doctype, docname):
	doc = dataent.new_doc("Pricing Rule")
	doc.applicable_for = doctype
	doc.set(dataent.scrub(doctype), docname)
