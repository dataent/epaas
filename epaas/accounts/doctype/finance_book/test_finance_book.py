...
class TestFinanceBook(unittest.TestCase):
	def create_finance_book(self):
		if not dataent.db.exists("Finance Book", "_Test Finance Book"):
			finance_book = dataent.get_doc({
				"doctype": "Finance Book",
				"finance_book_name": "_Test Finance Book"
			}).insert()
		else:
			finance_book = dataent.get_doc("Finance Book", "_Test Finance Book")
	...
	def test_finance_book(self):
		...
		gl_entries = dataent.get_all("GL Entry", fields=["name", "finance_book"],
