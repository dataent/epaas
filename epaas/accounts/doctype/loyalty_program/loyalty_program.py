...
from dataent.utils import today, flt
...
def get_loyalty_details(customer, loyalty_program, expiry_date=None, company=None, include_expired_entry=False):
	...
	condition = ''
	if company:
		condition = " and company='%s' " % dataent.db.escape(company)
	if not include_expired_entry:
		condition += " and expiry_date>='%s' " % expiry_date

	loyalty_point_details = dataent.db.sql('''select sum(loyalty_points) as loyalty_points,
...
def get_loyalty_program_details_with_points(customer, loyalty_program=None, expiry_date=None, company=None, silent=False, include_expired_entry=False, current_transaction_amount=0):
	...
	loyalty_program = dataent.get_doc("Loyalty Program", loyalty_program)
...
def get_loyalty_program_details(customer, loyalty_program=None, expiry_date=None, company=None, silent=False, include_expired_entry=False):
	lp_details = dataent._dict()

	if not loyalty_program:
		loyalty_program = dataent.db.get_value("Customer", customer, "loyalty_program")

		if not (loyalty_program or silent):
			dataent.throw(_("Customer isn't enrolled in any Loyalty Program"))
		elif silent and not loyalty_program:
			return dataent._dict({"loyalty_program": None})

	if not company:
		company = dataent.db.get_default("company") or dataent.get_all("Company")[0].name
...
def get_redeemption_factor(loyalty_program=None, customer=None):
	...
	if not loyalty_program:
		customer_loyalty_program = dataent.db.get_value("Customer", customer, "loyalty_program")
		loyalty_program = customer_loyalty_program
	if loyalty_program:
		return dataent.db.get_value("Loyalty Program", loyalty_program, "conversion_factor")
	else:
		dataent.throw(_("Customer isn't enrolled in any Loyalty Program"))
...
def validate_loyalty_points(ref_doc, points_to_redeem):
	...
	if hasattr(ref_doc, "loyalty_program") and ref_doc.loyalty_program:
		...
		loyalty_program = dataent.db.get_value("Customer", ref_doc.customer, ["loyalty_program"])

	if loyalty_program and dataent.db.get_value("Loyalty Program", loyalty_program, ["company"]) !=\
		ref_doc.company:
		dataent.throw(_("The Loyalty Program isn't valid for the selected company"))
	...
	if loyalty_program and points_to_redeem:
		...
		if points_to_redeem > loyalty_program_details.loyalty_points:
			dataent.throw(_("You don't have enought Loyalty Points to redeem"))

		loyalty_amount = flt(points_to_redeem * loyalty_program_details.conversion_factor)

		if loyalty_amount > ref_doc.grand_total:
			dataent.throw(_("You can't redeem Loyalty Points having more value than the Grand Total."))
