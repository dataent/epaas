...
class TestLoyaltyProgram(unittest.TestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		dataent.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Single Loyalty")
		# create a new sales invoice
		si_original = create_sales_invoice_record()
		si_original.insert()
		si_original.submit()

		customer = dataent.get_doc('Customer', {"customer_name": "Test Loyalty Customer"})
		earned_points = get_points_earned(si_original)

		lpe = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si_original.name, 'customer': si_original.customer})
		...
		lpe_redeem = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si_redeem.name, 'redeem_against': lpe.name})
		lpe_earn = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si_redeem.name, 'name': ['!=', lpe_redeem.name]})
		...
		for d in [si_redeem, si_original]:
			d.cancel()
			dataent.delete_doc('Sales Invoice', d.name)

	def test_loyalty_points_earned_multiple_tier(self):
		dataent.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = dataent.get_doc('Customer', {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = dataent.get_doc('Loyalty Program', {'loyalty_program_name': 'Test Multiple Loyalty'}).name
		customer.save()
	...
	def test_cancel_sales_invoice(self):
		...
		lpe = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si.name, 'customer': si.customer})
		self.assertEqual(True, not (lpe is None))

		# cancelling sales invoice
		si.cancel()
		lpe = dataent.db.exists('Loyalty Point Entry', lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		earned_points = get_points_earned(si_original)
		lpe_original = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si_original.name, 'customer': si_original.customer})
		...
		si_original = dataent.get_doc('Sales Invoice', lpe_original.sales_invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = dataent.get_doc('Loyalty Point Entry', {'sales_invoice': si_original.name, 'customer': si_original.customer})
		...
		for d in [si_return, si_original]:
			try:
				d.cancel()
			except dataent.TimestampMismatchError:
				dataent.get_doc('Sales Invoice', d.name).cancel()
			dataent.delete_doc('Sales Invoice', d.name)

	def test_loyalty_points_for_dashboard(self):
		doc = dataent.get_doc('Customer', 'Test Loyalty Customer')
...
def create_sales_invoice_record(qty=1):
	...
	return dataent.get_doc({
		...
		"customer": dataent.get_doc('Customer', {"customer_name": "Test Loyalty Customer"}).name,
		...
		"items": [{
			...
			'item_code': dataent.get_doc('Item', {'item_name': 'Loyal Item'}).name,
			'qty': qty,
			"rate": 10000,
...
def create_records():
	...
	if dataent.db.exists("Account", "Loyalty - _TC"):
		return
