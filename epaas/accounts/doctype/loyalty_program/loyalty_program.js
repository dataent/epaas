...
dataent.ui.form.on('Loyalty Program', {
	setup: function(frm) {
		var help_content =
	...
	onload: function(frm) {
		...
		frm.set_value("company", dataent.defaults.get_user_default("Company"));
	},

	refresh: function(frm) {
		if (frm.doc.loyalty_program_type === "Single Tier Program" && frm.doc.collection_rules.length > 1) {
			dataent.throw(__("Please select the Multiple Tier Program type for more than one collection rules."));
		}
	}
});
