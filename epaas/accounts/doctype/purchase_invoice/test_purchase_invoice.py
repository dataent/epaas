...
from dataent.utils import cint, flt, today, nowdate, add_days
...
from epaas.controllers.accounts_controller import get_payment_terms
from epaas.exceptions import InvalidCurrency
from epaas.stock.doctype.stock_entry.test_stock_entry import get_qty_after_transaction
...
class TestPurchaseInvoice(unittest.TestCase):
	...
	def test_purchase_invoice_for_blocked_supplier(self):
		...
		self.assertRaises(dataent.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_for_blocked_supplier_payment(self):
		...
		self.assertRaises(
			dataent.ValidationError, get_payment_entry, dt='Purchase Invoice', dn=pi.name, bank_account="_Test Bank - _TC")
	...
	def test_purchase_invoice_for_blocked_supplier_payment_today_date(self):
		...
		self.assertRaises(
			dataent.ValidationError, get_payment_entry, dt='Purchase Invoice', dn=pi.name,
	...
	def test_purchase_invoice_with_advance(self):
		...
		self.assertFalse(dataent.db.sql("""select name from `tabJournal Entry Account`
	...
	def test_total_purchase_cost_for_project(self):
		existing_purchase_cost = dataent.db.sql("""select sum(base_net_amount)
			from `tabPurchase Invoice Item` where project = '_Test Project' and docstatus=1""")
		existing_purchase_cost = existing_purchase_cost and existing_purchase_cost[0][0] or 0

		pi = make_purchase_invoice(currency="USD", conversion_rate=60, project="_Test Project")
		self.assertEqual(dataent.db.get_value("Project", "_Test Project", "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(dataent.db.get_value("Project", "_Test Project", "total_purchase_cost"), existing_purchase_cost)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		...
		pi = make_purchase_invoice(update_stock=1, posting_date=dataent.utils.nowdate(),
			posting_time=dataent.utils.nowtime())
	...
	def test_purchase_invoice_for_is_paid_and_update_stock_gl_entry_with_perpetual_inventory(self):
		set_perpetual_inventory()
		pi = make_purchase_invoice(update_stock=1, posting_date=dataent.utils.nowdate(),
			posting_time=dataent.utils.nowtime(), cash_bank_account="Cash - _TC", is_paid=1)

		gl_entries = dataent.db.sql("""select account, account_currency, sum(debit) as debit,
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(dataent.db.get_value("Serial No", pi.get("items")[0].serial_no, "warehouse"),
			pi.get("items")[0].warehouse)

		self.assertEqual(dataent.db.get_value("Serial No", pi.get("items")[0].rejected_serial_no,
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(dataent.ValidationError, pi.insert)

	def test_debit_note(self):
		from epaas.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from epaas.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
...
test_records = dataent.get_test_records('Purchase Invoice')
