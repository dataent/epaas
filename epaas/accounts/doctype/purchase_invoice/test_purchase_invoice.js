...
QUnit.test("test purchase invoice", function(assert) {
	...
	dataent.run_serially([
		...
		() => {
			...
			dataent.tests.set_control('due_date', dataent.datetime.add_days(date, 1));
			dataent.timeout(0.5);
		...
		() => dataent.timeout(0.5),
		() => dataent.tests.set_form_values(cur_frm, [{'payment_terms_schedule': ''}]),
		() => {
			let date = cur_frm.doc.due_date;
			dataent.tests.set_control('due_date', dataent.datetime.add_days(date, 1));
			dataent.timeout(0.5);
		...
		() => dataent.tests.click_button('Close'),
		() => dataent.timeout(0.5),
		() => dataent.tests.set_form_values(cur_frm, [{'payment_schedule': []}]),
		() => {
			let date = cur_frm.doc.due_date;
			dataent.tests.set_control('due_date', dataent.datetime.add_days(date, 1));
			dataent.timeout(0.5);
