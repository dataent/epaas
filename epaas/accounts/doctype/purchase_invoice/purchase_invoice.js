...
epaas.accounts.PurchaseInvoice = epaas.buying.BuyingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc) {
		...
		if(doc.docstatus===0) {
			this.frm.add_custom_button(__('Purchase Order'), function() {
				epaas.utils.map_current_doc({
					method: "epaas.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			dataent.model.with_doc("Supplier", me.frm.doc.supplier, function() {
				var supplier = dataent.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice: function() {
		const me = this;
		dataent.call({
			'method': 'epaas.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	can_change_release_date: function(date) {
		const diff = dataent.datetime.get_diff(date, dataent.datetime.nowdate());
		if (diff < 0) {
			dataent.throw(__('New release date should be in the future'));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice: function(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			dataent.call({
				'method': 'epaas.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice',
	...
	set_release_date: function(data) {
		return dataent.call({
			'method': 'epaas.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier: function() {
		...
		if(this.frm.updating_party_details)
			return;
		epaas.utils.get_party_details(this.frm, "epaas.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice: function(frm) {
		dataent.model.open_mapped_doc({
			method: "epaas.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	},
	...
	on_submit: function() {
		$.each(this.frm.doc["items"] || [], function(i, row) {
			if(row.purchase_receipt) dataent.model.clear_doc("Purchase Receipt", row.purchase_receipt)
		})
	},

	make_debit_note: function() {
		dataent.model.open_mapped_doc({
			method: "epaas.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	},
	...
	asset: function(frm, cdt, cdn) {
		...
		if(row.asset) {
			dataent.call({
				method: "epaas.assets.doctype.asset_category.asset_category.get_asset_category_account",
				args: {
					"asset": row.asset,
				...
				callback: function(r, rt) {
					dataent.model.set_value(cdt, cdn, "expense_account", r.message);
				}
			})
		}
	}
...
cur_frm.script_manager.make(epaas.accounts.PurchaseInvoice);
