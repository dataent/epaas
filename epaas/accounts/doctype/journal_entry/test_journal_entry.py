...
class TestJournalEntry(unittest.TestCase):
	def test_journal_entry_with_against_jv(self):
		jv_invoice = dataent.copy_doc(test_records[2])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = dataent.copy_doc(test_records[1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = dataent.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def advance_paid_testcase(self, base_jv, test_voucher, dr_or_cr):
		...
		advance_paid = dataent.db.sql("""select advance_paid from `tab%s`
	...
	def test_jv_against_stock_account(self):
		...
		from epaas.accounts.general_ledger import StockAccountInvalidTransaction
	...
	def test_inter_company_jv(self):
		dataent.db.set_value("Account", "Sales Expenses - _TC", "inter_company_account", 1)
		dataent.db.set_value("Account", "Buildings - _TC", "inter_company_account", 1)
		dataent.db.set_value("Account", "Sales Expenses - _TC1", "inter_company_account", 1)
		dataent.db.set_value("Account", "Buildings - _TC1", "inter_company_account", 1)
	...
	def test_jv_for_enable_allow_cost_center_in_entry_of_bs_account(self):
		...
		gl_entries = dataent.db.sql("""select account, cost_center, debit, credit
			from `tabGL Entry` where voucher_type='Journal Entry' and voucher_no=%s
			order by account asc""", jv.name, as_dict=1)
...
def make_journal_entry(account1, account2, amount, cost_center=None, posting_date=None, exchange_rate=1, save=True, submit=False, project=None):
	...
	jv = dataent.new_doc("Journal Entry")
