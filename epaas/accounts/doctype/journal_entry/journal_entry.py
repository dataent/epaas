...
import dataent, epaas, json
from dataent.utils import cstr, flt, fmt_money, formatdate, getdate, nowdate, cint
from dataent import msgprint, _, scrub
from epaas.controllers.accounts_controller import AccountsController
from epaas.accounts.utils import get_balance_on, get_account_currency
from epaas.accounts.party import get_party_account
from epaas.hr.doctype.expense_claim.expense_claim import update_reimbursed_amount
from epaas.hr.doctype.loan.loan import update_disbursement_status, update_total_amount_paid
...
class JournalEntry(AccountsController):
	...
	def update_advance_paid(self):
		...
		for voucher_type, order_list in iteritems(advance_paid):
			for voucher_no in list(set(order_list)):
				dataent.get_doc(voucher_type, voucher_no).set_total_advance_paid()

	def validate_inter_company_accounts(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			doc = dataent.get_doc("Journal Entry", self.inter_company_journal_entry_reference)
			account_currency = dataent.get_cached_value('Company',  self.company,  "default_currency")
			previous_account_currency = dataent.get_cached_value('Company',  doc.company,  "default_currency")
			if account_currency == previous_account_currency:
				if self.total_credit != doc.total_debit or self.total_debit != doc.total_credit:
					dataent.throw(_("Total Credit/ Debit Amount should be same as linked Journal Entry"))

	def update_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			dataent.db.set_value("Journal Entry", self.inter_company_journal_entry_reference,\
				"inter_company_journal_entry_reference", self.name)

	def on_cancel(self):
		from epaas.accounts.utils import unlink_ref_doc_from_payment_entries
		from epaas.hr.doctype.salary_slip.salary_slip import unlink_ref_doc_from_salary_slip
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = dataent.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_asset_reference(self):
		for d in self.get("accounts"):
			if d.reference_type=="Asset" and d.reference_name:
				asset = dataent.get_doc("Asset", d.reference_name)
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			dataent.db.set_value("Journal Entry", self.name,\
				"inter_company_journal_entry_reference", "")

	def unlink_asset_adjustment_entry(self):
		dataent.db.sql(""" update `tabAsset Value Adjustment`
			set journal_entry = null where journal_entry = %s""", self.name)

	def validate_party(self):
		for d in self.get("accounts"):
			account_type = dataent.db.get_value("Account", d.account, "account_type")
			if account_type in ["Receivable", "Payable"]:
				if not (d.party_type and d.party):
					dataent.throw(_("Row {0}: Party Type and Party is required for Receivable / Payable account {1}").format(d.idx, d.account))
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				if (d.party_type == 'Customer' and flt(d.credit) > 0) or \
					...
					if d.is_advance=="No":
						...
						dataent.throw(_("Row {0}: Payment against Sales/Purchase Order should always be marked as advance").format(d.idx))

				if d.is_advance == "Yes":
					if d.party_type == 'Customer' and flt(d.debit) > 0:
						dataent.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == 'Supplier' and flt(d.credit) > 0:
						dataent.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))

	def validate_against_jv(self):
		for d in self.get('accounts'):
			if d.reference_type=="Journal Entry":
				account_root_type = dataent.db.get_value("Account", d.account, "root_type")
				if account_root_type == "Asset" and flt(d.debit) > 0:
					dataent.throw(_("For {0}, only credit accounts can be linked against another debit entry")
						.format(d.account))
				elif account_root_type == "Liability" and flt(d.credit) > 0:
					dataent.throw(_("For {0}, only debit accounts can be linked against another credit entry")
						.format(d.account))

				if d.reference_name == self.name:
					dataent.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = dataent.db.sql("""select * from `tabJournal Entry Account`
					where account = %s and docstatus = 1 and parent = %s
					and (reference_type is null or reference_type in ("", "Sales Order", "Purchase Order"))
					""", (d.account, d.reference_name), as_dict=True)

				if not against_entries:
					dataent.throw(_("Journal Entry {0} does not have account {1} or already matched against other voucher")
					...
					for jvd in against_entries:
						if flt(jvd[dr_or_cr]) > 0:
							valid = True
					if not valid:
						dataent.throw(_("Against Journal Entry {0} does not have any unmatched {1} entry")
		...
		for d in self.get("accounts"):
			...
			if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
				...
				if d.reference_type=="Sales Order" and flt(d.debit) > 0:
					dataent.throw(_("Row {0}: Debit entry can not be linked with a {1}").format(d.idx, d.reference_type))

				if d.reference_type == "Purchase Order" and flt(d.credit) > 0:
					dataent.throw(_("Row {0}: Credit entry can not be linked with a {1}").format(d.idx, d.reference_type))
				...
				against_voucher = dataent.db.get_value(d.reference_type, d.reference_name,
					[scrub(dt) for dt in field_dict.get(d.reference_type)])

				if not against_voucher:
					dataent.throw(_("Row {0}: Invalid reference {1}").format(d.idx, d.reference_name))

				# check if party and account match
				if d.reference_type in ("Sales Invoice", "Purchase Invoice"):
					if (against_voucher[0] != d.party or against_voucher[1] != d.account):
						dataent.throw(_("Row {0}: Party / Account does not match with {1} / {2} in {3} {4}")
				...
				if d.reference_type in ("Sales Order", "Purchase Order"):
					...
					if against_voucher != d.party:
						dataent.throw(_("Row {0}: {1} {2} does not match with {3}") \
	...
	def validate_orders(self):
		...
		for reference_name, total in iteritems(self.reference_totals):
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				order = dataent.get_doc(reference_type, reference_name)

				if order.docstatus != 1:
					dataent.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))

				if flt(order.per_billed) >= 100:
					dataent.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))

				if cstr(order.status) == "Closed":
					dataent.throw(_("{0} {1} is closed").format(reference_type, reference_name))
				...
				if flt(voucher_total) < (flt(order.advance_paid) + total):
					dataent.throw(_("Advance paid against {0} {1} cannot be greater \
	...
	def validate_invoices(self):
		...
		for reference_name, total in iteritems(self.reference_totals):
			...
			if reference_type in ("Sales Invoice", "Purchase Invoice"):
				invoice = dataent.db.get_value(reference_type, reference_name,
				...
				if total and flt(invoice.outstanding_amount) < total:
					dataent.throw(_("Payment against {0} {1} cannot be greater than Outstanding Amount {2}")
	...
	def validate_total_debit_and_credit(self):
		...
		if self.difference:
			dataent.throw(_("Total Debit must be equal to Total Credit. The difference is {0}")
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				dataent.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		alternate_currency = []
		for d in self.get("accounts"):
			account = dataent.db.get_value("Account", d.account, ["account_currency", "account_type"], as_dict=1)
			if account:
		...
		if alternate_currency:
			if not self.multi_currency:
				dataent.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def set_exchange_rate(self):
		for d in self.get("accounts"):
			...
			if not d.exchange_rate:
				dataent.throw(_("Row {0}: Exchange Rate is mandatory").format(d.idx))
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				...
				msgprint(_("Please enter Reference date"), raise_exception=dataent.MandatoryError)
		...
		for d in self.get('accounts'):
			...
			if d.reference_type == "Purchase Invoice" and d.debit:
				bill_no = dataent.db.sql("""select bill_no, bill_date
		...
		for d in self.get('accounts'):
			if d.party_type in ['Customer', 'Supplier'] and d.party:
				if not pay_to_recd_from:
					pay_to_recd_from = dataent.db.get_value(d.party_type, d.party,
			...
			elif dataent.db.get_value("Account", d.account, "account_type") in ["Bank", "Cash"]:
	...
	def get_values(self):
		...
		if self.write_off_based_on == 'Accounts Receivable':
			return dataent.db.sql("""select name, debit_to as account, customer as party, outstanding_amount
				from `tabSales Invoice` where docstatus = 1 and company = %s
				and outstanding_amount > 0 %s""" % ('%s', cond), self.company, as_dict=True)
		elif self.write_off_based_on == 'Accounts Payable':
			return dataent.db.sql("""select name, credit_to as account, supplier as party, outstanding_amount
	...
	def update_expense_claim(self):
		for d in self.accounts:
			if d.reference_type=="Expense Claim" and d.reference_name:
				doc = dataent.get_doc("Expense Claim", d.reference_name)
	...
	def update_loan(self):
		if self.paid_loan:
			...
			for name in paid_loan:
				dataent.db.set_value("Repayment Schedule", name, "paid", value)
		for d in self.accounts:
			if d.reference_type=="Loan" and flt(d.debit) > 0:
				doc = dataent.get_doc("Loan", d.reference_name)
	...
	def validate_expense_claim(self):
		for d in self.accounts:
			if d.reference_type=="Expense Claim":
				sanctioned_amount, reimbursed_amount = dataent.db.get_value("Expense Claim",
					d.reference_name, ("total_sanctioned_amount", "total_amount_reimbursed"))
				pending_amount = flt(sanctioned_amount) - flt(reimbursed_amount)
				if d.debit > pending_amount:
					dataent.throw(_("Row No {0}: Amount cannot be greater than Pending Amount against Expense Claim {1}. Pending Amount is {2}".format(d.idx, d.reference_name, pending_amount)))

	def validate_credit_debit_note(self):
		if self.stock_entry:
			if dataent.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				dataent.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))

			if dataent.db.exists({"doctype": "Journal Entry", "stock_entry": self.stock_entry, "docstatus":1}):
				dataent.msgprint(_("Warning: Another {0} # {1} exists against stock entry {2}".format(self.voucher_type, self.name, self.stock_entry)))

	def validate_empty_accounts_table(self):
		if not self.get('accounts'):
			dataent.throw(_("Accounts table cannot be blank."))
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	from epaas.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
	if mode_of_payment:
	...
	if not account:
		...
		if account_type=="Bank":
			account = dataent.get_cached_value('Company',  company,  "default_bank_account")
			if not account:
				account_list = dataent.get_all("Account", filters = {"company": company,
					"account_type": "Bank", "is_group": 0})
				if len(account_list) == 1:
					account = account_list[0].name

		elif account_type=="Cash":
			account = dataent.get_cached_value('Company',  company,  "default_cash_account")
			if not account:
				account_list = dataent.get_all("Account", filters = {"company": company,
	...
	if account:
		account_details = dataent.db.get_value("Account", account,
...
def get_payment_entry_against_order(dt, dn, amount=None, debit_in_account_currency=None, journal_entry=False, bank_account=None):
	ref_doc = dataent.get_doc(dt, dn)

	if flt(ref_doc.per_billed, 2) > 0:
		dataent.throw(_("Can only make payment against unbilled {0}").format(dt))
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or dataent.get_cached_value('Company',  ref_doc.company,  "cost_center")
	exchange_rate = 1
	...
	party_row = je.append("accounts", {
		...
		"account_type": dataent.db.get_value("Account", args.get("party_account"), "account_type"),
...
def get_opening_accounts(company):
	...
	accounts = dataent.db.sql_list("""select
			name from tabAccount
		where
			is_group=0 and report_type='Balance Sheet' and company=%s and
			name not in(select distinct account from tabWarehouse where
			account is not null and account != '')
		order by name asc""", dataent.db.escape(company))

	return [{"account": a, "balance": get_balance_on(a)} for a in accounts]


def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	return dataent.db.sql("""select jv.name, jv.posting_date, jv.user_remark
		from `tabJournal Entry` jv, `tabJournal Entry Account` jv_detail
		where jv_detail.parent = jv.name and jv_detail.account = %s and ifnull(jv_detail.party, '') = %s
		and (jv_detail.reference_type is null or jv_detail.reference_type = '')
		and jv.docstatus = 1 and jv.`{0}` like %s order by jv.name desc limit %s, %s""".format(dataent.db.escape(searchfield)),
		(filters.get("account"), cstr(filters.get("party")), "%{0}%".format(txt), start, page_len))


@dataent.whitelist()
def get_outstanding(args):
	if not dataent.has_permission("Account"):
		dataent.msgprint(_("No Permission"), raise_exception=1)

	if isinstance(args, string_types):
		args = json.loads(args)

	company_currency = epaas.get_company_currency(args.get("company"))

	if args.get("doctype") == "Journal Entry":
		condition = " and party=%(party)s" if args.get("party") else ""

		against_jv_amount = dataent.db.sql("""
		...
		invoice = dataent.db.get_value(args["doctype"], args["docname"],
...
def get_party_account_and_balance(company, party_type, party, cost_center=None):
	...
	return {
		...
		"account_currency": dataent.db.get_value("Account", account, "account_currency")
	}
...
def get_account_balance_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None, cost_center=None):
	...
	account_details = dataent.db.get_value("Account", account, ["account_type", "account_currency"], as_dict=1)
...
def get_exchange_rate(posting_date, account=None, account_currency=None, company=None,
	...
	if not account_details:
		dataent.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = dataent.db.get_value(reference_type, reference_name, "conversion_rate")
