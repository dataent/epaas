...
epaas.accounts.SalesInvoiceController = epaas.selling.SellingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc, dt, dn) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			dataent.model.with_doc("Customer", me.frm.doc.customer, function() {
				var customer = dataent.model.get_doc("Customer", me.frm.doc.customer);
	...
	on_submit: function(doc, dt, dn) {
		...
		if (dataent.get_route()[0] != 'Form') {
			return
		}

		$.each(doc["items"], function(i, row) {
			if(row.delivery_note) dataent.model.clear_doc("Delivery Note", row.delivery_note)
		})
	},
	...
	quotation_btn: function() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				epaas.utils.map_current_doc({
					method: "epaas.selling.doctype.quotation.quotation.make_sales_invoice",
	...
	delivery_note_btn: function() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				epaas.utils.map_current_doc({
					method: "epaas.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					...
					get_query: function() {
						...
						return {
							query: "epaas.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer: function() {
		...
		epaas.utils.get_party_details(this.frm,
			"epaas.accounts.party.get_party_details", {
		...
		if(this.frm.doc.customer) {
			dataent.call({
				"method": "epaas.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				"args": {
	...
	make_inter_company_invoice: function() {
		dataent.model.open_mapped_doc({
			method: "epaas.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm
		});
	},
	...
	write_off_outstanding_amount_automatically: function() {
		if(cint(this.frm.doc.write_off_outstanding_amount_automatically)) {
			dataent.model.round_floats_in(this.frm.doc, ["grand_total", "paid_amount"]);
	...
	make_sales_return: function() {
		dataent.model.open_mapped_doc({
			method: "epaas.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm
		})
	},
	...
	asset: function(frm, cdt, cdn) {
		...
		if(row.asset) {
			dataent.call({
				method: epaas.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					"company": frm.doc.company
				},
				callback: function(r, rt) {
					dataent.model.set_value(cdt, cdn, "income_account", r.message[0]);
					dataent.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				}
			})
		}
	},
	...
	set_pos_data: function() {
		if(this.frm.doc.is_pos) {
			...
			if(!this.frm.doc.company) {
				...
				dataent.msgprint(__("Please specify Company to proceed"));
			} else {
				var me = this;
				return this.frm.call({
					...
					callback: function(r) {
						if(!r.exc) {
							...
							dataent.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
...
$.extend(cur_frm.cscript, new epaas.accounts.SalesInvoiceController({frm: cur_frm}));
...
cur_frm.cscript.hide_fields = function(doc) {
	...
	if(cint(doc.is_pos) == 1) {
		...
		for (var i in parent_fields) {
			var docfield = dataent.meta.docfield_map[doc.doctype][parent_fields[i]];
			if(!docfield.hidden) unhide_field(parent_fields[i]);
		}
	}

	// India related fields
	if (dataent.boot.sysdefaults.country == 'India') unhide_field(['c_form_applicable', 'c_form_no']);
...
cur_frm.cscript['Make Delivery Note'] = function() {
	dataent.model.open_mapped_doc({
		method: "epaas.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm
	})
}
...
cur_frm.set_query("income_account", "items", function(doc) {
	return{
		query: "epaas.controllers.queries.get_income_account",
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	epaas.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	epaas.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	epaas.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
dataent.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "epaas.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.fields_dict['items'].grid.get_field('expense_account').get_query = function(doc) {
			if (epaas.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
						'report_type': 'Profit and Loss',
						'company': doc.company,
		...
		frm.set_query('pos_profile', function(doc) {
			if(!doc.company) {
				dataent.throw(_('Please set Company'));
			}

			return {
				query: 'epaas.accounts.doctype.pos_profile.pos_profile.pos_profile_query',
				filters: {
					company: doc.company
				}
			};
	...
	company:function(frm){
		...
		{
			dataent.call({
				method:"dataent.contacts.doctype.address.address.get_default_address",
	...
	loyalty_points: function(frm) {
		if (frm.redemption_conversion_factor) {
			...
			dataent.call({
				method: "epaas.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function(frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			dataent.call({
				method: "epaas.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	set_loyalty_points: function(frm) {
		if (frm.redemption_conversion_factor) {
			...
			if (frm.doc.grand_total && (remaining_amount < loyalty_amount)) {
				...
				dataent.throw(__("You can only redeem max {0} points in this order.",[redeemable_points]));
			}
	...
	patient: function(frm) {
		if (dataent.boot.active_domains.includes("Healthcare")){
			if(frm.doc.patient){
				dataent.call({
					method: "dataent.client.get_value",
					args:{
...
dataent.ui.form.on('Sales Invoice Timesheet', {
	time_sheet: function(frm, cdt, cdn){
		var d = locals[cdt][cdn];
		if(d.time_sheet) {
			dataent.call({
				method: "epaas.projects.doctype.timesheet.timesheet.get_timesheet_data",
				args: {
					'name': d.time_sheet,
				...
				callback: function(r, rt) {
					if(r.message){
						data = r.message;
						dataent.model.set_value(cdt, cdn, "billing_hours", data.billing_hours);
						dataent.model.set_value(cdt, cdn, "billing_amount", data.billing_amount);
						dataent.model.set_value(cdt, cdn, "timesheet_detail", data.timesheet_detail);
...
var get_healthcare_services_to_invoice = function(frm) {
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "epaas.healthcare.utils.get_healthcare_services_to_invoice";
...
var set_primary_action= function(frm, dialog, $results, invoice_healthcare_services) {
	...
	dialog.set_primary_action(__('Add'), function() {
		...
		if(checked_values.length > 0){
			...
			if(invoice_healthcare_services){
				dataent.msgprint(__("Please select Healthcare Service"));
			}
			else{
				dataent.msgprint(__("Please select Drug"));
			}
		}
	});
};
...
var get_drugs_to_invoice = function(frm) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "epaas.healthcare.utils.get_drugs_to_invoice";
...
var add_to_item_line = function(frm, checked_values, invoice_healthcare_services){
	if(invoice_healthcare_services){
		...
		for(let i=0; i<checked_values.length; i++){
			var si_item = dataent.model.add_child(frm.doc, 'Sales Invoice Item', 'items');
			dataent.model.set_value(si_item.doctype, si_item.name, 'item_code', checked_values[i]['item']);
			dataent.model.set_value(si_item.doctype, si_item.name, 'qty', 1);
			if(checked_values[i]['qty'] > 1){
				dataent.model.set_value(si_item.doctype, si_item.name, 'qty', parseFloat(checked_values[i]['qty']));
			}
		}
		frm.refresh_fields();
	}
