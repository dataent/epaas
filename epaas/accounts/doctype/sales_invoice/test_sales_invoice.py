...
from dataent.utils import nowdate, flt, getdate, cint
from dataent.model.dynamic_links import get_dynamic_link_map
from epaas.stock.doctype.stock_entry.test_stock_entry import make_stock_entry, get_qty_after_transaction
from epaas.accounts.doctype.purchase_invoice.test_purchase_invoice import unlink_payment_on_cancel_of_invoice
from epaas.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from epaas.stock.doctype.purchase_receipt.test_purchase_receipt import set_perpetual_inventory
from epaas.exceptions import InvalidAccountCurrency, InvalidCurrency
from epaas.stock.doctype.serial_no.serial_no import SerialNoWarehouseError
...
from epaas.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from epaas.stock.doctype.item.test_item import create_item
from six import iteritems
from epaas.regional.india.utils import get_ewb_data

class TestSalesInvoice(unittest.TestCase):
	def make(self):
		w = dataent.copy_doc(test_records[0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = dataent.get_doc(w.doctype, w.name)
		...
		import time
		time.sleep(1)
		self.assertRaises(dataent.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = dataent.copy_doc(test_records[2])
		si.insert()
		si.naming_series = 'TEST-'

		self.assertRaises(dataent.CannotChangeConstantError, si.save)

		si = dataent.copy_doc(test_records[1])
		si.insert()
		si.naming_series = 'TEST-'
	...
	def test_payment_entry_unlink_against_invoice(self):
		...
		si = dataent.copy_doc(test_records[0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		unlink_payment_on_cancel_of_invoice(0)
		si = dataent.get_doc('Sales Invoice', si.name)
		self.assertRaises(dataent.LinkExistsError, si.cancel)
	...
	def test_sales_invoice_discount_amount(self):
		si = dataent.copy_doc(test_records[3])
	...
	def test_discount_amount_gl_entry(self):
		...
		gle = dataent.db.sql("""select * from `tabGL Entry`
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(dataent.ValidationError, si.insert)
	...
	def test_payment(self):
		...
		jv = dataent.get_doc(dataent.copy_doc(jv_test_records[0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(dataent.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(dataent.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)
	...
	def test_make_pos_invoice(self):
		from epaas.accounts.doctype.sales_invoice.pos import make_invoice
		...
		sales_invoice = dataent.get_all('Sales Invoice', fields =["*"], filters = {'offline_pos_name': '09052016142', 'docstatus': 1})
		si = dataent.get_doc('Sales Invoice', sales_invoice[0].name)
		self.assertEqual(si.grand_total, 630.0)
	...
	def test_make_pos_invoice_in_draft(self):
		...
		allow_negative_stock = dataent.db.get_single_value('Stock Settings', 'allow_negative_stock')
		if allow_negative_stock:
			dataent.db.set_value('Stock Settings', None, 'allow_negative_stock', 0)
		...
		sales_invoice = dataent.get_all('Sales Invoice', fields =["*"], filters = {'offline_pos_name': timestamp})
		...
		sales_invoice1 = dataent.get_all('Sales Invoice', fields =["*"], filters = {'offline_pos_name': timestamp})
	...
	def pos_gl_entry(self, si, pos, cash_amount):
		...
		sle = dataent.db.sql("""select * from `tabStock Ledger Entry`
	...
	def test_sales_invoice_gl_entry_with_perpetual_inventory_no_item_code(self):
		...
		si = dataent.get_doc(test_records[1])
	...
	def _insert_purchase_receipt(self):
		from epaas.stock.doctype.purchase_receipt.test_purchase_receipt import test_records \
			as pr_test_records
	...
	def _insert_delivery_note(self):
		from epaas.stock.doctype.delivery_note.test_delivery_note import test_records \
			as dn_test_records
		dn = dataent.copy_doc(dn_test_records[0])
	...
	def test_sales_invoice_with_advance(self):
		...
		jv = dataent.copy_doc(jv_test_records[0])
		jv.insert()
		jv.submit()
		...
		self.assertTrue(dataent.db.sql("""select name from `tabJournal Entry Account`
		...
		self.assertTrue(not dataent.db.sql("""select name from `tabJournal Entry Account`
			where reference_name=%s""", si.name))
	...
	def test_serialized(self):
		...
		self.assertFalse(dataent.db.get_value("Serial No", serial_nos[0], "warehouse"))
		self.assertEqual(dataent.db.get_value("Serial No", serial_nos[0],
			"delivery_document_no"), si.name)
		self.assertEqual(dataent.db.get_value("Serial No", serial_nos[0], "sales_invoice"),
			si.name)

		# check if the serial number is already linked with any other Sales Invoice
		_si = dataent.copy_doc(si.as_dict())
		self.assertRaises(dataent.ValidationError, _si.insert)
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(dataent.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC")
		self.assertFalse(dataent.db.get_value("Serial No", serial_nos[0],
			"delivery_document_no"))
		self.assertFalse(dataent.db.get_value("Serial No", serial_nos[0], "sales_invoice"))
	...
	def test_return_sales_invoice(self):
		...
		# outgoing_rate
		outgoing_rate = dataent.db.get_value("Stock Ledger Entry", {"voucher_type": "Sales Invoice",
		...
		incoming_rate, stock_value_difference = dataent.db.get_value("Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = dataent.db.get_value("GL Entry", {"voucher_type": "Sales Invoice",
			"voucher_no": si1.name, "account": stock_in_hand_account}, "debit")

		self.assertEqual(gle_warehouse_amount, stock_value_difference)

		party_credited = dataent.db.get_value("GL Entry", {"voucher_type": "Sales Invoice",
		...
		self.assertEqual(dataent.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 1500)
	...
	def test_multi_currency_gle(self):
		...
		gl_entries = dataent.db.sql("""select account, account_currency, debit, credit,
		...
		gle = dataent.db.sql("""select name from `tabGL Entry`
			where voucher_type='Sales Invoice' and voucher_no=%s""", si.name)
	...
	def test_outstanding_amount_after_advance_payment_entry_cancelation(self):
		...
		#added to avoid Document has been modified exception
		pe = dataent.get_doc("Payment Entry", pe.name)
		pe.cancel()
	...
	def test_item_wise_tax_breakup_india(self):
		...
		dataent.flags.country = None

	def test_item_wise_tax_breakup_outside_india(self):
		dataent.flags.country = "United States"
	...
	def test_company_monthly_sales(self):
		existing_current_month_sales = dataent.get_cached_value('Company',  "_Test Company",  "total_monthly_sales")

		si = create_sales_invoice()
		current_month_sales = dataent.get_cached_value('Company',  "_Test Company",  "total_monthly_sales")
	...
	def test_deferred_revenue(self):
		...
		from epaas.accounts.deferred_revenue import convert_deferred_revenue_to_income
	...
	def check_gl_entries(self, voucher_no, expected_gle, posting_date):
		gl_entries = dataent.db.sql("""select account, debit, credit, posting_date
			from `tabGL Entry`
	...
	def test_eway_bill_json(self):
		if not dataent.db.exists('Address', '_Test Address for Eway bill-Billing'):
		...
		if not dataent.db.exists('Address', '_Test Customer-Address for Eway bill-Shipping'):
			address = dataent.get_doc({
				"address_line1": "_Test Address Line 1",
...
def create_sales_invoice(**args):
	si = dataent.new_doc("Sales Invoice")
...
test_records = dataent.get_test_records('Sales Invoice')
