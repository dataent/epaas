...
import dataent
from epaas.accounts.party import get_party_account_currency
...
from dataent.utils import nowdate, cint
...
@dataent.whitelist()
def get_pos_data():
	doc = dataent.new_doc('Sales Invoice')
	doc.is_pos = 1
	pos_profile = get_pos_profile(doc.company) or {}
	if not pos_profile:
		dataent.throw(_("POS Profile is required to use Point-of-Sale"))
	...
	if pos_profile.get('name'):
		pos_profile = dataent.get_doc('POS Profile', pos_profile.get('name'))
	...
	print_template = dataent.db.get_value('Print Format', default_print_format, 'html')
...
def get_meta():
	doctype_meta = {
		'customer': dataent.get_meta('Customer'),
		'invoice': dataent.get_meta('Sales Invoice')
	}

	for row in dataent.get_all('DocField', fields=['fieldname', 'options'],
            filters={'parent': 'Sales Invoice', 'fieldtype': 'Table'}):
		doctype_meta[row.fieldname] = dataent.get_meta(row.options)

	return doctype_meta


def get_company_data(company):
	return dataent.get_all('Company', fields=["*"], filters={'name': company})[0]
...
def update_pos_profile_data(doc, pos_profile, company_data):
	...
	doc.selling_price_list = pos_profile.get('selling_price_list') or \
		dataent.db.get_value('Selling Settings', None, 'selling_price_list')
	...
	doc.terms = dataent.db.get_value('Terms and Conditions', pos_profile.get('tc_name'), 'terms') or doc.terms or ''
	doc.offline_pos_name = ''


def get_root(table):
	root = dataent.db.sql(""" select name from `tab%(table)s` having
...
def get_mode_of_payment(doc):
	return dataent.db.sql(""" select mpa.default_account, mpa.parent, mp.type as type from `tabMode of Payment Account` mpa, \
...
def get_item_groups(pos_profile):
	item_group_dict = {}
	item_groups = dataent.db.sql("""Select name,
...
def get_customers_list(pos_profile={}):
	...
	return dataent.db.sql(""" select name, customer_name, customer_group,
...
def get_customers_address(customers):
	...
	if isinstance(customers, string_types):
		customers = [dataent._dict({'name': customers})]

	for data in customers:
		address = dataent.db.sql(""" select name, address_line1, address_line2, city, state,
...
def get_contacts(customers):
	...
	for data in customers:
		contact = dataent.db.sql(""" select email_id, phone, mobile_no from `tabContact`
...
def get_child_nodes(group_type, root):
	lft, rgt = dataent.db.get_value(group_type, root, ["lft", "rgt"])
	return dataent.db.sql(""" Select name, lft, rgt from `tab{tab}` where
...
def get_serial_no_data(pos_profile, company):
	...
	serial_nos = dataent.db.sql("""select name, warehouse, item_code
		from `tabSerial No` where {0} and company = %(company)s """.format(cond),{
			'company': company, 'warehouse': dataent.db.escape(pos_profile.get('warehouse'))
		}, as_dict=1)
...
def get_batch_no_data():
	...
	itemwise_batch = {}
	batches = dataent.db.sql("""select name, item from `tabBatch`
...
def get_item_tax_data():
	...
	itemwise_tax = {}
	taxes = dataent.db.sql(""" select parent, tax_type, tax_rate from `tabItem Tax`""", as_dict=1)
...
def get_price_list_data(selling_price_list):
	itemwise_price_list = {}
	price_lists = dataent.db.sql("""Select ifnull(price_list_rate, 0) as price_list_rate,
...
def get_customer_wise_price_list():
	customer_wise_price = {}
	customer_price_list_mapping = dataent._dict(dataent.get_all('Customer',fields = ['default_price_list', 'name'], as_list=1))
...
def get_bin_data(pos_profile):
	...
	bin_data = dataent.db.sql(""" select item_code, warehouse, actual_qty from `tabBin`
		where actual_qty > 0 and {cond}""".format(cond=cond), {
			'warehouse': dataent.db.escape(pos_profile.get('warehouse'))
		}, as_dict=1)
...
def get_pricing_rule_data(doc):
	...
	if doc.ignore_pricing_rule == 0:
		pricing_rules = dataent.db.sql(""" Select * from `tabPricing Rule` where docstatus < 2
...
def make_invoice(doc_list={}, email_queue_list={}, customers_list={}):
	...
	for docs in doc_list:
		for name, doc in iteritems(docs):
			if not dataent.db.exists('Sales Invoice', {'offline_pos_name': name}):
				validate_records(doc)
				si_doc = dataent.new_doc('Sales Invoice')
...
def get_customer_id(doc, customer=None):
	...
	if doc.get('customer_pos_id'):
		cust_id = dataent.db.get_value('Customer',{'customer_pos_id': doc.get('customer_pos_id')}, 'name')

	if not cust_id:
		customer = customer or doc.get('customer')
		if dataent.db.exists('Customer', customer):
			cust_id = customer
		else:
...
def make_customer_and_address(customers):
	...
	for customer, data in iteritems(customers):
		...
		if not cust_id:
			...
			dataent.db.set_value("Customer", cust_id, "customer_name", data.get('full_name'))
...
def add_customer(data):
	customer_doc = dataent.new_doc('Customer')
...
def get_territory(data):
	...
	return dataent.db.get_single_value('Selling Settings','territory') or _('All Territories')
...
def get_customer_group(data):
	...
	return dataent.db.get_single_value('Selling Settings', 'customer_group') or dataent.db.get_value('Customer Group', {'is_group': 0}, 'name')


def make_contact(args, customer):
	if args.get('email_id') or args.get('phone'):
		name = dataent.db.get_value('Dynamic Link',
		...
		doc = dataent.new_doc('Contact')
		if name:
			doc = dataent.get_doc('Contact', name)
...
def make_address(args, customer):
	...
	if name:
		address = dataent.get_doc('Address', name)
	else:
		address = dataent.new_doc('Address')
		address.country = dataent.get_cached_value('Company',  args.get('company'),  'country')
...
def validate_item(doc):
	for item in doc.get('items'):
		if not dataent.db.exists('Item', item.get('item_code')):
			item_doc = dataent.new_doc('Item')
