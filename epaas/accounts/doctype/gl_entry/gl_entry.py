...
from dataent.utils import flt, fmt_money, getdate, formatdate
from dataent.model.document import Document
from dataent.model.meta import get_field_precision
from epaas.accounts.party import validate_party_gle_currency, validate_party_frozen_disabled
from epaas.accounts.utils import get_account_currency
from epaas.accounts.utils import get_fiscal_year
from epaas.exceptions import InvalidAccountCurrency
...
class GLEntry(Document):
	...
	def check_mandatory(self):
		...
		for k in mandatory:
			if not self.get(k):
				dataent.throw(_("{0} is required").format(_(self.meta.get_label(k))))

		account_type = dataent.db.get_value("Account", self.account, "account_type")
		if not (self.party_type and self.party):
			if account_type == "Receivable":
				dataent.throw(_("{0} {1}: Customer is required against Receivable account {2}")
					.format(self.voucher_type, self.voucher_no, self.account))
			elif account_type == "Payable":
				dataent.throw(_("{0} {1}: Supplier is required against Payable account {2}")
					.format(self.voucher_type, self.voucher_no, self.account))

		# Zero value transaction is not allowed
		if not (flt(self.debit, self.precision("debit")) or flt(self.credit, self.precision("credit"))):
			dataent.throw(_("{0} {1}: Either debit or credit amount is required for {2}")
				.format(self.voucher_type, self.voucher_no, self.account))

	def pl_must_have_cost_center(self):
		if dataent.db.get_value("Account", self.account, "report_type") == "Profit and Loss":
			if not self.cost_center and self.voucher_type != 'Period Closing Voucher':
				dataent.throw(_("{0} {1}: Cost Center is required for 'Profit and Loss' account {2}. Please set up a default Cost Center for the Company.")
					.format(self.voucher_type, self.voucher_no, self.account))
		else:
			from epaas.accounts.utils import get_allow_cost_center_in_entry_of_bs_account
	...
	def check_pl_account(self):
		if self.is_opening=='Yes' and \
				dataent.db.get_value("Account", self.account, "report_type")=="Profit and Loss" and \
				self.voucher_type not in ['Purchase Invoice', 'Sales Invoice']:
			dataent.throw(_("{0} {1}: 'Profit and Loss' type account {2} not allowed in Opening Entry")
				.format(self.voucher_type, self.voucher_no, self.account))

	def validate_account_details(self, adv_adj):
		"""Account must be ledger, active and not freezed"""

		ret = dataent.db.sql("""select is_group, docstatus, company
			from tabAccount where name=%s""", self.account, as_dict=1)[0]

		if ret.is_group==1:
			dataent.throw(_("{0} {1}: Account {2} cannot be a Group")
				.format(self.voucher_type, self.voucher_no, self.account))

		if ret.docstatus==2:
			dataent.throw(_("{0} {1}: Account {2} is inactive")
				.format(self.voucher_type, self.voucher_no, self.account))

		if ret.company != self.company:
			dataent.throw(_("{0} {1}: Account {2} does not belong to Company {3}")
	...
	def validate_cost_center(self):
		...
		def _get_cost_center_company():
			if not self.cost_center_company.get(self.cost_center):
				self.cost_center_company[self.cost_center] = dataent.db.get_value(
					"Cost Center", self.cost_center, "company")

			return self.cost_center_company[self.cost_center]

		if self.cost_center and _get_cost_center_company() != self.company:
			dataent.throw(_("{0} {1}: Cost Center {2} does not belong to Company {3}")
...
def validate_balance_type(account, adv_adj=False):
	if not adv_adj and account:
		balance_must_be = dataent.db.get_value("Account", account, "balance_must_be")
		if balance_must_be:
			balance = dataent.db.sql("""select sum(debit) - sum(credit)
				from `tabGL Entry` where account = %s""", account)[0][0]

			if (balance_must_be=="Debit" and flt(balance) < 0) or \
				(balance_must_be=="Credit" and flt(balance) > 0):
				dataent.throw(_("Balance for Account {0} must always be {1}").format(account, _(balance_must_be)))
