...
from dataent.exceptions import ValidationError
...
class ShareTransfer(Document):
	def before_submit(self):
		if self.transfer_type == 'Issue':
			...
			doc = dataent.get_doc('Shareholder', self.to_shareholder)
	...
	def validate(self):
		...
		if self.transfer_type == 'Issue':
			...
			if ret_val != False:
				dataent.throw(_('The shares already exist'), dataent.DuplicateEntryError)
		else:
			# validate share exists with from_shareholder
			ret_val = self.share_exists(self.from_shareholder)
			if ret_val != True:
				dataent.throw(_("The shares don't exist with the {0}")
	...
	def basic_validations(self):
		if self.transfer_type == 'Purchase':
			...
			if self.from_shareholder is None or self.from_shareholder is '':
				dataent.throw(_('The field From Shareholder cannot be blank'))
			...
			if self.to_shareholder is None or self.to_shareholder == '':
				dataent.throw(_('The field To Shareholder cannot be blank'))
			if self.to_folio_no is None or self.to_folio_no is '':
				self.to_folio_no = self.autoname_folio(self.to_shareholder)
		else:
			if self.from_shareholder is None or self.to_shareholder is None:
				dataent.throw(_('The fields From Shareholder and To Shareholder cannot be blank'))
			if self.to_folio_no is None or self.to_folio_no is '':
				self.to_folio_no = self.autoname_folio(self.to_shareholder)
		if self.from_shareholder == self.to_shareholder:
			dataent.throw(_('The seller and the buyer cannot be the same'))
		if self.no_of_shares != self.to_no - self.from_no + 1:
			dataent.throw(_('The number of shares and the share numbers are inconsistent'))
		if self.amount is None:
			self.amount = self.rate * self.no_of_shares
		if self.amount != self.rate * self.no_of_shares:
			dataent.throw(_('There are inconsistencies between the rate, no of shares and the amount calculated'))
	...
	def folio_no_validation(self):
		...
		for shareholder in shareholders:
			...
			if doc.company != self.company:
				dataent.throw(_('The shareholder does not belong to this company'))
