...
from dataent.utils.data import nowdate, getdate, cint, add_days, date_diff, get_last_day, add_to_date, flt
...
class Subscription(Document):
	...
	def validate_plans_billing_cycle(billing_cycle_data):
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			dataent.throw(_('You can only have Plans with the same billing cycle in a Subscription'))
	...
	def get_current_invoice(self):
		...
		if len(self.invoices):
			...
			if dataent.db.exists('Sales Invoice', current.invoice):
				doc = dataent.get_doc('Sales Invoice', current.invoice)
				return doc
			else:
				dataent.throw(_('Invoice {0} no longer exists'.format(current.invoice)))
	...
	def validate_trial_period(self):
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				dataent.throw(_('Trial Period End Date Cannot be before Trial Period Start Date'))

		elif self.trial_period_start or self.trial_period_end:
			dataent.throw(_('Both Trial Period Start Date and Trial Period End Date must be set'))
	...
	def get_items_from_plans(self, plans, prorate=0):
		...
		for plan in plans:
			item_code = dataent.db.get_value("Subscription Plan", plan.plan, "item")
			if not prorate:
	...
	def process_for_past_due_date(self):
		...
		current_invoice = self.get_current_invoice()
		if not current_invoice:
			dataent.throw(_('Current invoice {0} is missing'.format(current_invoice.invoice)))
		else:
	...
	def cancel_subscription(self):
		...
		if self.status != 'Cancelled':
			...
			to_prorate = dataent.db.get_single_value('Subscription Settings', 'prorate')
			self.status = 'Cancelled'
			self.cancelation_date = nowdate()
	...
	def restart_subscription(self):
		...
		if self.status == 'Cancelled':
			...
			dataent.throw(_('You cannot restart a Subscription that is not cancelled.'))
...
def cancel_subscription(name):
	...
	"""
	subscription = dataent.get_doc('Subscription', name)
