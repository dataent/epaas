...
dataent.ui.form.on('Account', {
	...
	add_toolbar_buttons: function(frm) {
		frm.add_custom_button(__('Chart of Accounts'),
			function () { dataent.set_route("Tree", "Account"); });
		...
		if (frm.doc.is_group == 1) {
			...
			&& dataent.boot.user.can_read.indexOf("GL Entry") !== -1) {
	...
	merge_account: function(frm) {
		var d = new dataent.ui.Dialog({
			...
			primary_action: function() {
				...
				dataent.call({
					method: "epaas.accounts.doctype.account.account.merge_account",
					args: {
						old: frm.doc.name,
						new: data.name,
					...
					callback: function(r) {
						if(!r.exc) {
							if(r.message) {
								dataent.set_route("Form", "Account", r.message);
							}
							d.hide();
						}
					}
	...
	update_account_number: function(frm) {
		var d = new dataent.ui.Dialog({
			...
			primary_action: function() {
				...
				dataent.call({
					method: "epaas.accounts.doctype.account.account.update_account_number",
					args: {
