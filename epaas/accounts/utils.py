...
from dataent.utils import nowdate, cstr, flt, cint, now, getdate
from dataent import throw, _
from dataent.utils import formatdate, get_number_format_info
from six import iteritems
# imported to enable epaas.accounts.utils.get_account_currency
from epaas.accounts.doctype.account.account import get_account_currency

class FiscalYearError(dataent.ValidationError): pass
...
def get_fiscal_years(transaction_date=None, fiscal_year=None, label="Date", verbose=1, company=None, as_dict=False):
	fiscal_years = dataent.cache().hget("fiscal_years", company) or []

	if not fiscal_years:
		# if year start date is 2012-04-01, year end date should be 2013-03-31 (hence subdate)
		cond = ""
		if fiscal_year:
			cond += " and fy.name = {0}".format(dataent.db.escape(fiscal_year))
		if company:
			cond += """
		...
		dataent.cache().hset("fiscal_years", company, fiscal_years)
	...
	if verbose==1: dataent.msgprint(error_msg)
...
def get_balance_on(account=None, date=None, party_type=None, party=None, company=None, in_account_currency=True, cost_center=None):
	if not account and dataent.form_dict.get("account"):
		account = dataent.form_dict.get("account")
	if not date and dataent.form_dict.get("date"):
		date = dataent.form_dict.get("date")
	if not party_type and dataent.form_dict.get("party_type"):
		party_type = dataent.form_dict.get("party_type")
	if not party and dataent.form_dict.get("party"):
		party = dataent.form_dict.get("party")
	if not cost_center and dataent.form_dict.get("cost_center"):
		cost_center = dataent.form_dict.get("cost_center")


	cond = []
	if date:
		cond.append("posting_date <= '%s'" % dataent.db.escape(cstr(date)))
	else:
	...
	if cost_center and allow_cost_center_in_entry_of_bs_account:
		cc = dataent.get_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(""" exists (
			...
			cond.append("""gle.cost_center = "%s" """ % (dataent.db.escape(cost_center, percent=False), ))


	if account:

		acc = dataent.get_doc("Account", account)

		if not dataent.flags.ignore_account_permission:
		...
		if acc.is_group:
			...
			if acc.account_currency == dataent.get_cached_value('Company',  acc.company,  "default_currency"):
				in_account_currency = False
		else:
			cond.append("""gle.account = "%s" """ % (dataent.db.escape(account, percent=False), ))

	if party_type and party:
		cond.append("""gle.party_type = "%s" and gle.party = "%s" """ %
			(dataent.db.escape(party_type), dataent.db.escape(party, percent=False)))

	if company:
		cond.append("""gle.company = "%s" """ % (dataent.db.escape(company, percent=False)))
...
def reconcile_against_document(args):
	...
	for d in args:
		...
		doc = dataent.get_doc(d.voucher_type, d.voucher_no)
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = ("paid_from"
			if epaas.get_party_account_type(args.party_type) == 'Receivable' else "paid_to")

		if args.voucher_detail_no:
			ret = dataent.db.sql("""select t1.name
			...
			ret = dataent.db.sql("""select name from `tabPayment Entry`
				where
...
def get_company_default(company, fieldname):
	value = dataent.get_cached_value('Company',  company,  fieldname)
...
def fix_total_debit_credit():
	vouchers = dataent.db.sql("""select voucher_type, voucher_no,
	...
	for d in vouchers:
		if abs(d.diff) > 0:
			...
			dataent.db.sql("""update `tabGL Entry` set %s = %s + %s
...
def get_stock_rbnb_difference(posting_date, company):
	stock_items = dataent.db.sql_list("""select distinct item_code
		from `tabStock Ledger Entry` where company=%s""", company)

	pr_valuation_amount = dataent.db.sql("""
	...
	pi_valuation_amount = dataent.db.sql("""
	...
	stock_rbnb_account = "Stock Received But Not Billed - " + dataent.get_cached_value('Company',  company,  "abbr")
...
def get_outstanding_invoices(party_type, party, account, condition=None):
	...
	precision = dataent.get_precision("Sales Invoice", "outstanding_amount") or 2

	if epaas.get_party_account_type(party_type) == 'Receivable':
	...
	invoice = 'Sales Invoice' if epaas.get_party_account_type(party_type) == 'Receivable' else 'Purchase Invoice'
	held_invoices = get_held_invoices(party_type, party)

	invoice_list = dataent.db.sql("""
		select
...
def get_account_name(account_type=None, root_type=None, is_group=None, account_currency=None, company=None):
	...
	return dataent.db.get_value("Account", {
		...
		"account_currency": account_currency or dataent.defaults.get_defaults().currency,
		"company": company or dataent.defaults.get_defaults().company
	}, "name")

@dataent.whitelist()
def get_companies():
	"""get a list of companies based on permission"""
	return [d.name for d in dataent.get_list("Company", fields=["name"],
		order_by="name")]
...
def create_payment_gateway_account(gateway):
	from epaas.setup.setup_wizard.operations.company_setup import create_bank_account
	...
	bank_account = dataent.db.get_value("Account", {"account_name": _(gateway), "company": company},
		["name", 'account_currency'], as_dict=1)

	if not bank_account:
		# check for untranslated one
		bank_account = dataent.db.get_value("Account", {"account_name": gateway, "company": company},
			["name", 'account_currency'], as_dict=1)
	...
	if not bank_account:
		dataent.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def validate_field_number(doctype_name, name, number_value, company, field_name):
	...
	if number_value:
		if company:
			doctype_with_same_number = dataent.db.get_value(doctype_name,
				{field_name: number_value, "company": company, "name": ["!=", name]})
		else:
			doctype_with_same_number = dataent.db.get_value(doctype_name,
				{field_name: number_value, "name": ["!=", name]})
		if doctype_with_same_number:
			dataent.throw(_("{0} Number {1} already used in account {2}")
...
def get_autoname_with_number(number_value, doc_title, name, company):
	...
	if name:
		...
		abbr = dataent.get_cached_value('Company',  company,  ["abbr"], as_dict=True)
...
def get_coa(doctype, parent, is_root, chart=None):
	from epaas.accounts.doctype.account.chart_of_accounts.chart_of_accounts import build_tree_from_json

	# add chart to flags to retrieve when called from expand all function
	chart = chart if chart else dataent.flags.chart
	dataent.flags.chart = chart
