...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	if not filters: filters = dataent._dict({})
	...
	company_currency = dataent.get_cached_value('Company',  filters.get("company"),  "default_currency")
...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		income_accounts = dataent.db.sql_list("""select distinct income_account
			from `tabSales Invoice Item` where docstatus = 1 and parent in (%s)
			order by income_account""" %
			', '.join(['%s']*len(invoice_list)), tuple([inv.name for inv in invoice_list]))

		tax_accounts = 	dataent.db.sql_list("""select distinct account_head
...
def get_invoice_income_map(invoice_list):
	income_details = dataent.db.sql("""select parent, income_account, sum(base_net_amount) as amount
		from `tabSales Invoice Item` where parent in (%s) group by parent, income_account""" %
		', '.join(['%s']*len(invoice_list)), tuple([inv.name for inv in invoice_list]), as_dict=1)

	invoice_income_map = {}
	for d in income_details:
		invoice_income_map.setdefault(d.parent, dataent._dict()).setdefault(d.income_account, [])
...
def get_invoice_tax_map(invoice_list, invoice_income_map, income_accounts):
	...
	for d in tax_details:
		if d.account_head in income_accounts:
			...
			invoice_tax_map.setdefault(d.parent, dataent._dict()).setdefault(d.account_head, [])
			invoice_tax_map[d.parent][d.account_head] = flt(d.tax_amount)

	return invoice_income_map, invoice_tax_map

def get_invoice_so_dn_map(invoice_list):
	si_items = dataent.db.sql("""select parent, sales_order, delivery_note, so_detail
	...
	for d in si_items:
		if d.sales_order:
			invoice_so_dn_map.setdefault(d.parent, dataent._dict()).setdefault(
		...
		if d.delivery_note:
			...
			delivery_note_list = dataent.db.sql_list("""select distinct parent from `tabDelivery Note Item`
				where docstatus=1 and so_detail=%s""", d.so_detail)

		if delivery_note_list:
			invoice_so_dn_map.setdefault(d.parent, dataent._dict()).setdefault("delivery_note", delivery_note_list)

	return invoice_so_dn_map

def get_invoice_cc_wh_map(invoice_list):
	si_items = dataent.db.sql("""select parent, cost_center, warehouse
		from `tabSales Invoice Item` where parent in (%s)
	...
	for d in si_items:
		if d.cost_center:
			invoice_cc_wh_map.setdefault(d.parent, dataent._dict()).setdefault(
...
def get_mode_of_payments(invoice_list):
	mode_of_payments = {}
	if invoice_list:
		inv_mop = dataent.db.sql("""select parent, mode_of_payment
