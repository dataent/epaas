...
from epaas.controllers.trends import get_period_date_ranges, get_period_month_ranges
...
def validate_filters(filters):
	if filters.get("budget_against")=="Project" and filters.get("cost_center"):
		dataent.throw(_("Filter based on Cost Center is only applicable if Budget Against is selected as Cost Center"))
...
def get_cost_centers(filters):
	...
	return dataent.db.sql_list("""select name from `tab{tab}` where company=%s
...
def get_cost_center_target_details(filters):
	cond = ""
	if filters.get("cost_center"):
		cond += " and b.cost_center='%s'" % dataent.db.escape(filters.get("cost_center"))
...
def get_target_distribution_details(filters):
	target_details = {}
	for d in dataent.db.sql("""select md.name, mdp.month, mdp.percentage_allocation
...
def get_actual_details(name, filters):
	...
	if filters.get("budget_against") == "Cost Center":
		cc_lft, cc_rgt = dataent.db.get_value("Cost Center", name, ["lft", "rgt"])
		cond = "lft>='{lft}' and rgt<='{rgt}'".format(lft = cc_lft, rgt=cc_rgt)

	ac_details = dataent.db.sql("""select gl.account, gl.debit, gl.credit,gl.fiscal_year,
...
def get_fiscal_years(filters):
	...
	fiscal_year = dataent.db.sql("""select name from `tabFiscal Year` where
