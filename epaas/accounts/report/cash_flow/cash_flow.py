...
from epaas.accounts.report.financial_statements import (get_period_list, get_columns, get_data)
...
def get_account_type_based_gl_data(company, start_date, end_date, account_type, filters):
	...
	if filters.finance_book:
		cond = " and finance_book = '%s'" %(dataent.db.escape(filters.finance_book))
		if filters.include_default_book_entries:
			company_fb = dataent.db.get_value("Company", company, 'default_finance_book')

			cond = """ and finance_book in ('%s', '%s')
				""" %(dataent.db.escape(filters.finance_book), dataent.db.escape(company_fb))

	gl_sum = dataent.db.sql_list("""
