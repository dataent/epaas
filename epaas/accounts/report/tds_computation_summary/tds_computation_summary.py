...
from epaas.accounts.doctype.tax_withholding_category.tax_withholding_category \
...
def validate_filters(filters):
	...
	if from_year != to_year:
		dataent.throw(_("From Date and To Date lie in different Fiscal Year"))
...
def get_result(filters):
	...
	if filters.supplier:
		filters.supplier = dataent.db.get_list('Supplier',
	...
	for supplier in filters.supplier:
		tds = dataent.get_doc("Tax Withholding Category", supplier.tax_withholding_category)
...
def get_invoice_and_tds_amount(supplier, account, company, from_date, to_date):
	...
	tds_deducted = 0
	if vouchers:
		tds_deducted = flt(dataent.db.sql("""
			select sum(credit)
			from `tabGL Entry`
