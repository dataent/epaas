...
class PartyLedgerSummaryReport(object):
	...
	def run(self, args):
		...
		self.party_naming_by = dataent.db.get_value(args.get("naming_by")[0], None, args.get("naming_by")[1])
	...
	def get_data(self):
		company_currency = dataent.get_cached_value('Company',  self.filters.get("company"),  "default_currency")
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"

		self.party_data = dataent._dict({})
		for gle in self.gl_entries:
			self.party_data.setdefault(gle.party, dataent._dict({
				"party": gle.party,
	...
	def get_return_invoices(self):
		...
		self.return_invoices = [d.name for d in dataent.get_all(doctype, filters={"is_return": 1, "docstatus": 1,
	...
	def get_party_adjustment_amounts(self):
		...
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"
		round_off_account = dataent.get_cached_value('Company', self.filters.company, "round_off_account")
		...
		for voucher_gl_entries in itervalues(adjustment_voucher_entries):
			...
			for gle in voucher_gl_entries:
				...
				elif dataent.get_cached_value("Account", gle.account, "account_type") == income_or_expense:
