...
from epaas.accounts.report.utils import get_currency, convert_to_presentation_currency
from epaas.accounts.report.financial_statements import get_fiscal_year_data, sort_accounts
from epaas.accounts.report.balance_sheet.balance_sheet import (get_provisional_profit_loss,
	check_opening_balance, get_chart_data)
from epaas.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (get_net_profit_loss,
	get_chart_data as get_pl_chart_data)
from epaas.accounts.report.cash_flow.cash_flow import (get_cash_flow_accounts, get_account_type_based_gl_data,
...
def execute(filters=None):
	...
	if filters.get('report') == "Balance Sheet":
		...
		if cint(dataent.db.get_single_value('Accounts Settings', 'use_custom_cash_flow')):
			from epaas.accounts.report.cash_flow.custom_cash_flow import execute as execute_custom
...
def get_data(companies, root_type, balance_must_be, fiscal_year, filters=None, ignore_closing_entries=False):
	...
	for root in dataent.db.sql("""select lft, rgt from tabAccount
...
def get_company_currency(filters=None):
	return (filters.get('presentation_currency')
		or dataent.get_cached_value('Company',  filters.company,  "default_currency"))
...
def get_subsidiary_companies(company):
	lft, rgt = dataent.db.get_value('Company', company,  ["lft", "rgt"])

	return dataent.db.sql_list("""select name from `tabCompany`
		where lft >= {0} and rgt <= {1} order by lft, rgt""".format(lft, rgt))

def get_accounts(root_type, filters):
	return dataent.db.sql(""" select name, is_group, company,
...
def set_gl_entries_by_account(from_date, to_date, root_lft, root_rgt, filters, gl_entries_by_account,
	...
	company_lft, company_rgt = dataent.get_cached_value('Company',
		filters.get('company'),  ["lft", "rgt"])

	additional_conditions = get_additional_conditions(from_date, ignore_closing_entries, filters)
	companies = dataent.db.sql(""" select name, default_currency from `tabCompany`
		where lft >= %(company_lft)s and rgt <= %(company_rgt)s""", {
			"company_lft": company_lft,
			"company_rgt": company_rgt,
		}, as_dict=1)

	currency_info = dataent._dict({
		'report_date': to_date,
		'presentation_currency': filters.get('presentation_currency')
	})

	for d in companies:
		gl_entries = dataent.db.sql("""select gl.posting_date, gl.account, gl.debit, gl.credit, gl.is_opening, gl.company,
			...
			order by gl.account, gl.posting_date""".format(additional_conditions=additional_conditions),
			{
				...
				"company_fb": dataent.db.get_value("Company", d.name, 'default_finance_book')
			},
			as_dict=True)
...
def validate_entries(key, entry, accounts_by_name):
	if key not in accounts_by_name:
		...
		dataent.throw(_("{0} {1} is not present in the parent company").format(field, key))
