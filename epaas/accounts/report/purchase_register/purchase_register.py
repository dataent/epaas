...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		expense_accounts = dataent.db.sql_list("""select distinct expense_account
...
def get_invoice_expense_map(invoice_list):
	expense_details = dataent.db.sql("""
	...
	for d in expense_details:
		invoice_expense_map.setdefault(d.parent, dataent._dict()).setdefault(d.expense_account, [])
...
def get_invoice_po_pr_map(invoice_list):
	...
	for d in pi_items:
		if d.purchase_order:
			invoice_po_pr_map.setdefault(d.parent, dataent._dict()).setdefault(
		...
		if d.purchase_receipt:
			...
			pr_list = dataent.db.sql_list("""select distinct parent from `tabPurchase Receipt Item`
				where docstatus=1 and purchase_order_item=%s""", d.po_detail)

		if pr_list:
			invoice_po_pr_map.setdefault(d.parent, dataent._dict()).setdefault("purchase_receipt", pr_list)
...
def get_account_details(invoice_list):
	...
	for acc in dataent.db.sql("""select name, parent_account from tabAccount
...
def get_supplier_details(suppliers):
	supplier_details = {}
	for supp in dataent.db.sql("""select name, supplier_group from `tabSupplier`
