...
from dataent.utils import formatdate, getdate, flt, add_days
...
def get_asset_costs(assets, filters):
	asset_costs = dataent._dict()
	for d in assets:
		asset_costs.setdefault(d.asset_category, dataent._dict({
...
def get_accumulated_depreciations(assets, filters):
	asset_depreciations = dataent._dict()
	for d in assets:
		asset = dataent.get_doc("Asset", d.name)
		...
		if d.asset_category in asset_depreciations:
			...
			asset_depreciations.setdefault(d.asset_category, dataent._dict({
