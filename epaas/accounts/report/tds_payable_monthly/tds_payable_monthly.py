...
def execute(filters=None):
	filters["invoices"] = dataent.cache().hget("invoices", dataent.session.user)
...
def set_filters(filters):
	...
	filters.naming_series = dataent.db.get_single_value('Buying Settings', 'supp_master_name')
...
def get_supplier_map(filters):
	...
	pan = "pan" if dataent.db.has_column("Supplier", "pan") else "tax_id"
	supplier_detail = dataent.db.get_all('Supplier',
	...
	for d in supplier_detail:
		if d.get("tax_withholding_category") not in tds_docs:
			tds_docs[d.get("tax_withholding_category")] = \
				dataent.get_doc("Tax Withholding Category", d.get("tax_withholding_category"))
...
def get_gle_map(filters):
	...
	gle_map = {}
	gle = dataent.db.get_all('GL Entry',\
...
def get_tds_invoices():
	...
	suppliers = [d.name for d in dataent.db.get_list("Supplier",
		{"tax_withholding_category": ["!=", ""]}, ["name"])]

	invoices = dataent.db.get_list("Purchase Invoice",
		{"supplier": ["in", suppliers]}, ["name", "supplier"])

	invoices = [d for d in invoices if d.supplier]
	dataent.cache().hset("invoices", dataent.session.user, invoices)
