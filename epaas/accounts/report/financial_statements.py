...
from dataent.utils import (flt, getdate, get_first_day, add_months, add_days, formatdate)
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
	fiscal_year = dataent.db.sql("""select min(year_start_date) as year_start_date,
...
def set_gl_entries_by_account(
	...
	accounts = dataent.db.sql_list("""select name from `tabAccount`
		where lft >= %s and rgt <= %s""", (root_lft, root_rgt))
	additional_conditions += " and account in ('{}')"\
		.format("', '".join([dataent.db.escape(d) for d in accounts]))

	gl_entries = dataent.db.sql("""select posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency from `tabGL Entry`
		...
		order by account, posting_date""".format(additional_conditions=additional_conditions),
		{
			...
			"company_fb": dataent.db.get_value("Company", company, 'default_finance_book')
		},
		as_dict=True)
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		lft, rgt = dataent.db.get_value("Cost Center", d, ["lft", "rgt"])
		children = dataent.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
