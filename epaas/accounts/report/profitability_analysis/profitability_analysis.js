...
dataent.require("assets/epaas/js/financial_statements.js", function() {
	dataent.query_reports["Profitability Analysis"] = {
		"filters": [
			{
				"fieldname": "company",
				"label": __("Company"),
		...
		"formatter": function(value, row, column, data, default_formatter) {
			if (column.fieldname=="account") {
				...
				column.link_onclick =
					"dataent.query_reports['Profitability Analysis'].open_profit_and_loss_statement(" + JSON.stringify(data) + ")";
				column.is_tree = true;
			}
		...
		"open_profit_and_loss_statement": function(data) {
			...
			if(data.based_on == 'cost_center'){
				dataent.route_options["cost_center"] = data.account
			} else {
				dataent.route_options["project"] = data.account
			}

			dataent.set_route("query-report", "Profit and Loss Statement");
		},
		"tree": true,
		"name_field": "account",
