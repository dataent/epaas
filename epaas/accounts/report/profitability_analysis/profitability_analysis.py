...
from epaas.accounts.report.financial_statements import filter_accounts, filter_out_zero_value_rows
from epaas.accounts.report.trial_balance.trial_balance import validate_filters
...
def get_accounts_data(based_on, company):
	if based_on == 'cost_center':
		return dataent.db.sql("""select name, parent_cost_center as parent_account, cost_center_name as account_name, lft, rgt
			from `tabCost Center` where company=%s order by name""", company, as_dict=True)
	else:
		return dataent.get_all('Project', fields = ["name"], filters = {'company': company}, order_by = 'name')
...
def set_gl_entries_by_account(company, from_date, to_date, based_on, gl_entries_by_account,
	...
	gl_entries = dataent.db.sql("""select posting_date, {based_on} as based_on, debit, credit, 
