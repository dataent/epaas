...
def execute(filters=None):
	...
	filters.currency = dataent.get_cached_value('Company',  filters.company,  "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = dataent._dict({
...
class GrossProfitGenerator(object):
	def __init__(self, filters=None):
		...
		self.filters = dataent._dict(filters)
	...
	def get_returned_invoice_items(self):
		returned_invoices = dataent.db.sql("""
			select
		...
		self.returned_invoices = dataent._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, dataent._dict())\
	...
	def get_average_buying_rate(self, row, item_code):
		...
		if not item_code in self.average_buying_rate:
			if item_code in self.non_stock_items:
				self.average_buying_rate[item_code] = flt(dataent.db.sql("""
	...
	def get_last_purchase_rate(self, item_code):
		if self.filters.to_date:
			last_purchase_rate = dataent.db.sql("""
	...
	def load_stock_ledger_entries(self):
		res = dataent.db.sql("""select item_code, voucher_type, voucher_no,
	...
	def load_product_bundle(self):
		...
		for d in dataent.db.sql("""select parenttype, parent, parent_item,
			item_code, warehouse, -1*qty as total_qty, parent_detail_docname
			from `tabPacked Item` where docstatus=1""", as_dict=True):
			self.product_bundles.setdefault(d.parenttype, dataent._dict()).setdefault(d.parent,
				dataent._dict()).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = dataent.db.sql_list("""select name from tabItem
