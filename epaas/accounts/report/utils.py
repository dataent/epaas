...
from epaas.accounts.doctype.fiscal_year.fiscal_year import get_from_and_to_date
from dataent.utils import cint, get_datetime_str, formatdate, flt

__exchange_rates = {}
P_OR_L_ACCOUNTS = list(
	sum(dataent.get_list('Account', fields=['name'], or_filters=[{'root_type': 'Income'}, {'root_type': 'Expense'}], as_list=True), ())
)
...
def get_invoiced_item_gross_margin(sales_invoice=None, item_code=None, company=None, with_item_data=False):
	from epaas.accounts.report.gross_profit.gross_profit import GrossProfitGenerator

	sales_invoice = sales_invoice or dataent.form_dict.get('sales_invoice')
	item_code = item_code or dataent.form_dict.get('item_code')
	company = company or dataent.get_cached_value("Sales Invoice", sales_invoice, 'company')
