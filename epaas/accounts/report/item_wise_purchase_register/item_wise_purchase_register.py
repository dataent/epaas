...
from epaas.accounts.report.item_wise_sales_register.item_wise_sales_register import get_tax_accounts
...
def get_items(filters, additional_query_columns):
	...
	match_conditions = dataent.build_match_conditions("Purchase Invoice")
...
def get_aii_accounts():
	return dict(dataent.db.sql("select name, stock_received_but_not_billed from tabCompany"))

def get_purchase_receipts_against_purchase_order(item_list):
	po_pr_map = dataent._dict()
	po_item_rows = list(set([d.po_detail for d in item_list]))

	if po_item_rows:
		purchase_receipts = dataent.db.sql("""
