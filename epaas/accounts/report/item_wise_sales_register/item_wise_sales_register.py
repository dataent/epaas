...
from epaas.accounts.report.sales_register.sales_register import get_mode_of_payments
...
def get_items(filters, additional_query_columns):
	...
	match_conditions = dataent.build_match_conditions("Sales Invoice")
...
def get_delivery_notes_against_sales_order(item_list):
	so_dn_map = dataent._dict()
	so_item_rows = list(set([d.so_detail for d in item_list]))

	if so_item_rows:
		delivery_notes = dataent.db.sql("""
...
def get_deducted_taxes():
	return dataent.db.sql_list("select name from `tabPurchase Taxes and Charges` where add_deduct_tax = 'Deduct'")
