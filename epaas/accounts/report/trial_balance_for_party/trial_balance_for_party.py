...
def get_data(filters, show_party_name):
	party_name_field = "{0}_name".format(dataent.scrub(filters.get('party_type')))
	...
	parties = dataent.get_all(filters.get("party_type"), fields = ["name", party_name_field], 
...
def get_balances_within_period(filters):
	...
	balances_within_period = dataent._dict()
	for d in gle:
...
def is_party_name_visible(filters):
	...
	if filters.get('party_type') in ['Customer', 'Supplier']:
		if filters.get("party_type") == "Customer":
			party_naming_by = dataent.db.get_single_value("Selling Settings", "cust_master_name")
		else:
			party_naming_by = dataent.db.get_single_value("Buying Settings", "supp_master_name")
