...
from dataent.utils import getdate, nowdate, flt, cint, formatdate, cstr

class ReceivablePayableReport(object):
	def __init__(self, filters=None):
		self.filters = dataent._dict(filters or {})
	...
	def run(self, args):
		party_naming_by = dataent.db.get_value(args.get("naming_by")[0], None, args.get("naming_by")[1])
	...
	def get_data(self, party_naming_by, args):
		...
		if not self.filters.get("company"):
			self.filters["company"] = dataent.db.get_single_value('Global Defaults', 'default_company')

		self.company_currency = dataent.get_cached_value('Company',  self.filters.get("company"), "default_currency")
	...
	def get_return_entries(self, party_type):
		...
		return_entries = dataent._dict(dataent.get_all(doctype,
	...
	def get_party_map(self, party_type):
		if not hasattr(self, "party_map"):
			...
			self.party_map = dict(((r.name, r) for r in dataent.db.sql("select {0} from `tab{1}`"
	...
	def get_gl_entries(self, party_type, date=None, for_future=False):
		...
		self.gl_entries = dataent.db.sql("""
			select
	...
	def prepare_conditions(self, party_type):
		...
		if party_type_field=="customer":
			...
			if self.filters.get("customer_group"):
				lft, rgt = dataent.db.get_value("Customer Group",
			...
			if self.filters.get("territory"):
				lft, rgt = dataent.db.get_value("Territory",
			...
			if self.filters.get("sales_person"):
				lft, rgt = dataent.db.get_value("Sales Person",
		...
		accounts = [d.name for d in dataent.get_all("Account",
	...
	def get_payment_term_detail(self, voucher_nos):
		payment_term_map = dataent._dict()
		payment_terms_details = dataent.db.sql(""" select si.name,
		...
		"""	% (dataent.db.escape(self.filters.company), ','.join(['%s'] *len(voucher_nos))),
		...
		for d in payment_terms_details:
			...
			payment_term_map.setdefault(d.name, []).append(dataent._dict({
				"due_date": d.due_date,
...
def get_pdc_details(party_type, report_date):
	pdc_details = dataent._dict()
	pdc_via_pe = dataent.db.sql("""
		select
...
def get_voucher_details(party_type, voucher_nos, dn_details):
	...
	if party_type == "Supplier":
		for pi in dataent.db.sql("""select name, due_date, bill_no, bill_date
			from `tabPurchase Invoice` where docstatus = 1 and name in (%s)
			""" %(','.join(['%s'] *len(voucher_nos))), (tuple(voucher_nos)), as_dict=1):
			voucher_details.setdefault(pi.name, pi)

	for pi in dataent.db.sql("""select name, due_date, bill_no, bill_date from
