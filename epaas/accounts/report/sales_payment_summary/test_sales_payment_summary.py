...
import unittest
import dataent
from epaas.accounts.report.sales_payment_summary.sales_payment_summary import get_mode_of_payments, get_mode_of_payment_details
...
class TestSalesPaymentSummary(unittest.TestCase):
	...
	def setUpClass(self):
		create_records()
		pes = dataent.get_all("Payment Entry")
		jes = dataent.get_all("Journal Entry")
		sis = dataent.get_all("Sales Invoice")
		for pe in pes:
			dataent.db.set_value("Payment Entry", pe.name, "docstatus", 2)
		for je in jes:
			dataent.db.set_value("Journal Entry", je.name, "docstatus", 2)
		for si in sis:
			dataent.db.set_value("Sales Invoice", si.name, "docstatus", 2)
	...
	def test_get_mode_of_payments(self):
		...
		payment_entries = dataent.get_all("Payment Entry", filters={"mode_of_payment": "Cash", "docstatus": 1}, fields=["name", "docstatus"])
		for payment_entry in payment_entries:
			pe = dataent.get_doc("Payment Entry", payment_entry.name)
			pe.cancel()
	...
	def test_get_mode_of_payments_details(self):
		...
		payment_entries = dataent.get_all("Payment Entry", filters={"mode_of_payment": "Credit Card", "docstatus": 1}, fields=["name", "docstatus"])
...
def create_sales_invoice_record(qty=1):
	...
	return dataent.get_doc({
		...
		"customer": dataent.get_doc('Customer', {"customer_name": "Prestiga-Biz"}).name,
		"company": '_Test Company',
		"due_date": today(),
		"posting_date": today(),
		"currency": "INR",
		...
		"items": [{
			...
			'item_code': dataent.get_doc('Item', {'item_name': 'Consulting'}).name,
			'qty': qty,
			"rate": 10000,
...
def create_records():
	if dataent.db.exists("Customer", "Prestiga-Biz"):
		return
