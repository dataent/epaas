...
from epaas.accounts.report.accounts_receivable.accounts_receivable import get_ageing_data
...
def validate_filters(filters):
	if (filters.get("payment_type") == "Incoming" and filters.get("party_type") == "Supplier") or \
		(filters.get("payment_type") == "Outgoing" and filters.get("party_type") == "Customer"):
			dataent.throw(_("{0} payment entries can not be filtered by {1}")\
...
def get_invoice_posting_date_map(filters):
	...
	for t in dataent.db.sql("select name, posting_date, due_date from `tab{0}`".format(dt), as_dict=1):
