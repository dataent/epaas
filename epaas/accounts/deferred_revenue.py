...
from dataent.utils import date_diff, add_months, today, getdate, add_days, flt, get_last_day
...
def validate_service_stop_date(doc):
	...
	old_stop_dates = {}
	old_doc = dataent.db.get_all("{0} Item".format(doc.doctype),
	...
	for item in doc.items:
		...
		if item.service_stop_date:
			if date_diff(item.service_stop_date, item.service_start_date) < 0:
				dataent.throw(_("Service Stop Date cannot be before Service Start Date"))

			if date_diff(item.service_stop_date, item.service_end_date) > 0:
				dataent.throw(_("Service Stop Date cannot be after Service End Date"))

		if old_stop_dates and old_stop_dates.get(item.name) and item.service_stop_date!=old_stop_dates[item.name]:
			dataent.throw(_("Cannot change Service Stop Date for item in row {0}".format(item.idx)))
...
def convert_deferred_expense_to_expense(start_date=None, end_date=None):
	...
	for invoice in invoices:
		doc = dataent.get_doc("Purchase Invoice", invoice)
...
def calculate_amount(doc, item, last_gl_entry, total_days, total_booking_days, account_currency):
	...
	if not last_gl_entry:
		...
		gl_entries_details = dataent.db.sql('''
