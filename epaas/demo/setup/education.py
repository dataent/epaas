...
from epaas.demo.setup.setup_data import import_json
import random
...
def setup_item():
	items = json.loads(open(dataent.get_app_path('epaas', 'demo', 'data', 'item_education.json')).read())
	for i in items:
		item = dataent.new_doc('Item')
		item.update(i)
		item.min_order_qty = random.randint(10, 30)
		item.item_defaults[0].default_warehouse = dataent.get_all('Warehouse',
...
def make_student_group():
	for term in dataent.db.get_list("Academic Term"):
		for program in dataent.db.get_list("Program"):
			sg_tool = dataent.new_doc("Student Group Creation Tool")
...
def make_fees_category():
	...
	for i in fee_type:
		fee_category = dataent.new_doc("Fee Category")
...
def make_fees_structure():
	for d in dataent.db.get_list("Program"):
		program = dataent.get_doc("Program", d.name)
		for academic_term in ["2017-18 (Semester 1)", "2017-18 (Semester 2)", "2017-18 (Semester 3)"]:
			fee_structure = dataent.new_doc("Fee Structure")
			fee_structure.program = d.name
			fee_structure.academic_term = random.choice(dataent.db.get_list("Academic Term")).name
			for j in range(1,4):
				temp = {"fees_category": random.choice(dataent.db.get_list("Fee Category")).name , "amount" : random.randint(500,1000)}
...
def make_assessment_groups():
	for year in dataent.db.get_list("Academic Year"):
		ag = dataent.new_doc('Assessment Group')
		ag.assessment_group_name = year.name
		ag.parent_assessment_group = "All Assessment Groups"
		ag.is_group = 1
		ag.insert()
		for term in dataent.db.get_list("Academic Term", filters = {"academic_year": year.name}):
			ag1 = dataent.new_doc('Assessment Group')
			ag1.assessment_group_name = term.name
			ag1.parent_assessment_group = ag.name
			ag1.is_group = 1
			ag1.insert()
			for assessment_group in ['Term I', 'Term II']:
				ag2 = dataent.new_doc('Assessment Group')
...
def get_json_path(doctype):
		return dataent.get_app_path('epaas', 'demo', 'data', dataent.scrub(doctype) + '.json')
