...
from epaas.healthcare.doctype.lab_test.lab_test import create_test_from_template
...
def make_appointment():
	...
	while i <= 4:
		...
		department = dataent.get_value("Healthcare Practitioner", practitioner, "department")
...
def lab_test_on_encounter():
	...
	while i <= 2:
		...
		test_rx = dataent.get_doc("Lab Prescription", test_rx)
		encounter = dataent.get_doc("Patient Encounter", test_rx.parent)
...
def append_drug_rx(encounter):
	...
	while i <= 3:
		...
		drug = dataent.get_doc("Item", drug)
...
def import_drug():
	...
	data = json.loads(open(dataent.get_app_path('epaas', 'demo', 'data', 'drug_list.json')).read())
	for d in data:
