...
def work(domain="Manufacturing"):
	dataent.set_user(dataent.db.get_global('demo_sales_user_2'))
	...
	if random.random() < 0.5:
		...
		try:
			if sales_order_name:
				so = dataent.get_doc("Sales Order", sales_order_name)
...
def make_quotation(domain):
	...
	if opportunity:
		...
		party_account_currency = get_party_account_currency("Customer", customer, epaas.get_default_company())
...
def make_sales_order():
	...
	if q:
		...
		so.delivery_date = dataent.utils.add_days(dataent.flags.current_date, 10)
		so.insert()
		dataent.db.commit()
		so.submit()
		dataent.db.commit()
