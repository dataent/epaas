...
import random
import datetime
from dataent.utils import random_string, add_days, get_last_day, getdate
from epaas.projects.doctype.timesheet.test_timesheet import make_timesheet
from epaas.projects.doctype.timesheet.timesheet import make_salary_slip, make_sales_invoice
from dataent.utils.make_random import get_random
from epaas.hr.doctype.expense_claim.test_expense_claim import get_payable_account
from epaas.hr.doctype.expense_claim.expense_claim import make_bank_entry
...
def work():
	...
	# payroll entry
	if not dataent.db.sql('select name from `tabSalary Slip` where month(adddate(start_date, interval 1 month))=month(curdate())'):
		# based on frequency
	...
	if dataent.db.get_global('demo_hr_user'):
		...
		if rand < 0.4:
			...
			if random.randint(0, 1):
				...
				je = dataent.get_doc(make_bank_entry("Expense Claim", expense_claim.name))
...
def get_payroll_entry():
	...
	payroll_entry.company = dataent.flags.company
	payroll_entry.payroll_frequency = 'Monthly'

	# select a posting date from the previous month
	payroll_entry.posting_date = get_last_day(getdate(dataent.flags.current_date) - datetime.timedelta(days=10))
	payroll_entry.payment_account = dataent.get_value('Account', {'account_type': 'Cash', 'company': epaas.get_default_company(),'is_group':0}, "name")
...
def get_expenses():
	expenses = []
	expese_types = dataent.db.sql("""select ect.name, eca.default_account from `tabExpense Claim Type` ect,
		`tabExpense Claim Account` eca where eca.parent=ect.name
		and eca.company=%s """, dataent.flags.company,as_dict=1)

	for expense_type in expese_types[:random.randint(1,4)]:
		claim_amount = random.randint(1,20)*10

		expenses.append({
			"expense_date": dataent.flags.current_date,
...
def get_timesheet_based_salary_slip_employee():
	sal_struct = dataent.db.sql("""
			select name from `tabSalary Structure`
			where salary_slip_based_on_timesheet = 1
			and docstatus != 2""")
	if sal_struct:
		employees = dataent.db.sql("""
...
def make_timesheet_records():
	...
	for e in employees:
		ts = make_timesheet(e.employee, simulate = True, billable = 1, activity_type=get_random("Activity Type"), company=dataent.flags.company)
		dataent.db.commit()
...
def make_leave_application():
	allocated_leaves = dataent.get_all("Leave Allocation", fields=['employee', 'leave_type'])

	for allocated_leave in allocated_leaves:
		leave_balance = get_leave_balance_on(allocated_leave.employee, allocated_leave.leave_type, dataent.flags.current_date,
			consider_all_leaves_in_the_allocation_period=True)
		if leave_balance != 0:
			if leave_balance == 1:
				to_date = dataent.flags.current_date
			else:
				to_date = add_days(dataent.flags.current_date, random.randint(0, leave_balance-1))

			leave_application = dataent.get_doc({
				"doctype": "Leave Application",
				"employee": allocated_leave.employee,
				"from_date": dataent.flags.current_date,
				"to_date": to_date,
...
def mark_attendance():
	attendance_date = dataent.flags.current_date
	for employee in dataent.get_all('Employee', fields=['name'], filters = {'status': 'Active'}):

		if not dataent.db.get_value("Attendance", {"employee": employee.name, "attendance_date": attendance_date}):
			attendance = dataent.get_doc({
				"doctype": "Attendance",
				"employee": employee.name,
				"attendance_date": attendance_date
			})

			leave = dataent.db.sql("""select name from `tabLeave Application`
...
def setup_department_approvers():
	for d in dataent.get_all('Department', filters={'department_name': ['!=', 'All Departments']}):
		doc = dataent.get_doc('Department', d.name)
		doc.append("leave_approvers", {'approver': dataent.session.user})
		doc.append("expense_approvers", {'approver': dataent.session.user})
