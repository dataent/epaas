...
import dataent, random, json, epaas
from dataent.utils.make_random import how_many, get_random
...
from epaas.stock.doctype.material_request.material_request import make_request_for_quotation
from epaas.buying.doctype.request_for_quotation.request_for_quotation import \
...
def work():
	...
	company_currency = dataent.get_cached_value('Company', epaas.get_default_company(), "default_currency")
	party_account_currency = get_party_account_currency("Supplier", supplier, epaas.get_default_company())
...
def make_material_request(item_code, qty):
	...
	variant_of = dataent.db.get_value('Item', item_code, 'variant_of') or item_code

	if dataent.db.get_value('BOM', {'item': variant_of, 'is_default': 1, 'is_active': 1}):
...
def make_subcontract():
	...
	if item_code:
		...
		po.schedule_date = dataent.utils.add_days(dataent.flags.current_date, 7)

		item_code = get_random("Item", {"is_sub_contracted_item": 1})

		po.append("items", {
			"item_code": item_code,
			"schedule_date": dataent.utils.add_days(dataent.flags.current_date, 7),
		...
		stock_entry = dataent.get_doc(make_rm_stock_entry(po.name, json.dumps([rm_items])))
