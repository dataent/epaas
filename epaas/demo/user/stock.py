...
from epaas.stock.doctype.serial_no.serial_no import SerialNoRequiredError, SerialNoQtyError
from epaas.stock.doctype.batch.batch import UnableToSelectBatchError
from epaas.stock.doctype.delivery_note.delivery_note import make_sales_return
from epaas.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return
...
def make_stock_reconciliation():
	...
	from epaas.stock.doctype.stock_reconciliation.stock_reconciliation \
		import OpeningEntryAccountError, EmptyStockReconciliationItemsError

	if random.random() < 0.4:
		stock_reco = dataent.new_doc("Stock Reconciliation")
		stock_reco.posting_date = dataent.flags.current_date
		stock_reco.company = epaas.get_default_company()
...
def submit_draft_stock_entries():
	...
	dataent.db.commit()
	for st in dataent.db.get_values("Stock Entry", {"docstatus":0}, "name"):
		try:
			ste = dataent.get_doc("Stock Entry", st[0])
			ste.posting_date = dataent.flags.current_date
			ste.save()
			ste.submit()
			dataent.db.commit()
...
def make_sales_return_records():
	if random.random() < 0.1:
		for data in dataent.get_all('Delivery Note', fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
...
def make_purchase_return_records():
	if random.random() < 0.1:
		for data in dataent.get_all('Purchase Receipt', fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
