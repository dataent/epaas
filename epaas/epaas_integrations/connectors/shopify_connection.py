...
import json
from dataent.utils import cstr, cint, nowdate, flt
from epaas.epaas_integrations.utils import validate_webhooks_request
from epaas.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
from epaas.epaas_integrations.doctype.shopify_settings.sync_product import sync_item_from_shopify
from epaas.epaas_integrations.doctype.shopify_settings.sync_customer import create_customer
from epaas.epaas_integrations.doctype.shopify_log.shopify_log import make_shopify_log, dump_request_data
...
def store_request_data(order=None, event=None):
	if dataent.request:
		order = json.loads(dataent.request.data)
		event = dataent.request.headers.get('X-Shopify-Topic')

	dump_request_data(order, event)

def sync_sales_order(order, request_id=None):
	shopify_settings = dataent.get_doc("Shopify Settings")
	dataent.flags.request_id = request_id

	if not dataent.db.get_value("Sales Order", filters={"shopify_order_id": cstr(order['id'])}):
		try:
...
def get_sales_order(shopify_order_id):
	sales_order = dataent.db.get_value("Sales Order", filters={"shopify_order_id": shopify_order_id})
	if sales_order:
		so = dataent.get_doc("Sales Order", sales_order)
		return so

def validate_customer(order, shopify_settings):
	customer_id = order.get("customer", {}).get("id")
	if customer_id:
		if not dataent.db.get_value("Customer", {"shopify_customer_id": customer_id}, "name"):
			create_customer(order.get("customer"), shopify_settings)

def validate_item(order, shopify_settings):
	for item in order.get("line_items"):
		if item.get("product_id") and not dataent.db.get_value("Item", {"shopify_product_id": item.get("product_id")}, "name"):
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	product_not_exists = []
	customer = dataent.db.get_value("Customer", {"shopify_customer_id": shopify_order.get("customer", {}).get("id")}, "name")
	so = dataent.db.get_value("Sales Order", {"shopify_order_id": shopify_order.get("id")}, "name")
	...
	if not so:
		...
		so = dataent.get_doc("Sales Order", so)
...
def get_item_code(shopify_item):
	item_code = dataent.db.get_value("Item", {"shopify_variant_id": shopify_item.get("variant_id")}, "item_code")
	if not item_code:
		item_code = dataent.db.get_value("Item", {"shopify_product_id": shopify_item.get("product_id")}, "item_code")
	if not item_code:
		item_code = dataent.db.get_value("Item", {"item_name": shopify_item.get("title")}, "item_code")
...
def get_tax_account_head(tax):
	...
	tax_account =  dataent.db.get_value("Shopify Tax Account", \
		{"parent": "Shopify Settings", "shopify_tax": tax_title}, "tax_account")

	if not tax_account:
		dataent.throw(_("Tax Account not specified for Shopify Tax {0}".format(tax.get("title"))))
