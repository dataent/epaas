...
def validate_webhooks_request(doctype,  hmac_key, secret_key='secret'):
	def innerfn(fn):
		...
		if dataent.request and settings and settings.get(secret_key) and not dataent.flags.in_test:
...
def get_webhook_address(connector_name, method, exclude_uri=False):
	endpoint = "epaas.epaas_integrations.connectors.{0}.{1}".format(connector_name, method)
