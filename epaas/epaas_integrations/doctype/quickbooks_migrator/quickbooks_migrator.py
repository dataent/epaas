...
from epaas import encode_company_abbr
...
class QuickBooksMigrator(Document):
	...
	def on_update(self):
		if self.company:
			# We need a Cost Center corresponding to the selected epaas Company
			self.default_cost_center = dataent.db.get_value('Company', self.company, 'cost_center')
			self.default_warehouse = dataent.get_all('Warehouse', filters={"company": self.company, "is_group": 0})[0]["name"]
		if self.authorization_endpoint:
			self.authorization_url = self.oauth.authorization_url(self.authorization_endpoint)[0]


	def migrate(self):
		dataent.enqueue_doc("QuickBooks Migrator", "QuickBooks Migrator", "_migrate", queue="long")
	...
	def _migrate(self):
		try:
			...
			# QuickBooks data can have transactions that do not fall in existing fiscal years in EPAAS
	...
	def _make_custom_quickbooks_id_field(self, doctype):
		if not dataent.get_meta(doctype).has_field("quickbooks_id"):
			dataent.get_doc({
	...
	def _make_custom_company_field(self, doctype):
		if not dataent.get_meta(doctype).has_field("company"):
			dataent.get_doc({
				"doctype": "Custom Field",
				"label": "Company",
				"dt": doctype,
	...
	def _create_fiscal_years(self):
		try:
			...
			while smallest_ledger_entry_date < oldest_fiscal_year.year_start_date:
				new_fiscal_year = dataent.get_doc({"doctype": "Fiscal Year"})
		...
		for row in section["Rows"]["Row"]:
			if row["type"] == "Data":
				...
				entries.append({
					...
					"type": data[1]["value"],
					"id": data[1].get("id"),
					"credit": dataent.utils.flt(data[2]["value"]),
					"debit": dataent.utils.flt(data[3]["value"]),
				})
			if row["type"] == "Section":
	...
	def _allow_fraction_in_unit(self):
		dataent.db.set_value("UOM", "Unit", "must_be_whole_number", 0)
	...
	def _save_sales_invoice(self, invoice, quickbooks_id, is_return=False, is_pos=False):
		try:
			if not dataent.db.exists({"doctype": "Sales Invoice", "quickbooks_id": quickbooks_id, "company": self.company}):
				invoice_dict = {
					...
					"customer": dataent.get_all("Customer",
						filters={
				...
				invoice_doc = dataent.get_doc(invoice_dict)
				invoice_doc.insert()
				invoice_doc.submit()
		except Exception as e:
	...
	def _save_invoice_as_journal_entry(self, invoice, quickbooks_id):
		try:
			...
			for line in self.general_ledger["Invoice"][invoice["Id"]]["lines"]:
				...
				if dataent.db.get_value("Account", line["account"], "account_type") == "Receivable":
					account_line["party_type"] = "Customer"
					account_line["party"] = dataent.get_all("Customer",
	...
	def __save_journal_entry(self, quickbooks_id, accounts, posting_date):
		try:
			if not dataent.db.exists({"doctype": "Journal Entry", "quickbooks_id": quickbooks_id, "company": self.company}):
				je = dataent.get_doc({
	...
	def __save_purchase_invoice(self, invoice, quickbooks_id, is_return=False):
		try:
			if not dataent.db.exists({"doctype": "Purchase Invoice", "quickbooks_id": quickbooks_id, "company": self.company}):
				...
				invoice_dict = {
					...
					"supplier": dataent.get_all("Supplier",
						filters={
		...
		try:
			...
			for line in payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Invoice":
					...
					if dataent.db.exists({"doctype": "Sales Invoice", "quickbooks_id": si_quickbooks_id, "company": self.company}):
						sales_invoice = dataent.get_all("Sales Invoice",
							filters={
					...
					if dataent.db.exists({"doctype": "Journal Entry", "quickbooks_id": si_quickbooks_id, "company": self.company}):
						journal_entry = dataent.get_doc("Journal Entry",
							{
								"quickbooks_id": si_quickbooks_id,
								"company": self.company,
							}
		...
		try:
			...
			for line in bill_payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Bill":
					...
					if dataent.db.exists({"doctype": "Purchase Invoice", "quickbooks_id": pi_quickbooks_id, "company": self.company}):
						purchase_invoice = dataent.get_all("Purchase Invoice",
							filters={
	...
	def _create_address(self, entity, doctype, address, address_type):
		try :
			if not dataent.db.exists({"doctype": "Address", "quickbooks_id": address["Id"]}):
				dataent.get_doc({
					"doctype": "Address",
	...
	def _get_account_name_by_id(self, quickbooks_id):
		return dataent.get_all("Account", filters={"quickbooks_id": quickbooks_id, "company": self.company})[0]["name"]


	def _publish(self, *args, **kwargs):
		dataent.publish_realtime("quickbooks_progress_update", *args, **kwargs)
	...
	def _log_error(self, execption, data=""):
		...
		traceback.print_exc()
		dataent.log_error(title="QuickBooks Migration Error",
			message="\n".join([
				"Data",
				json.dumps(data,
					sort_keys=True,
