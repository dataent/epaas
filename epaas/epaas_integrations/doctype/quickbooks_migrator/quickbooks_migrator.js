...
dataent.ui.form.on('QuickBooks Migrator', {
	...
	onload: function(frm) {
		...
		var domain = dataent.urllib.get_base_url()
		var redirect_url = `${domain}/api/method/epaas.epaas_integrations.doctype.quickbooks_migrator.quickbooks_migrator.callback`
		...
		frm.trigger("set_indicator")
 		dataent.realtime.on("quickbooks_progress_update", function (data) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.access_token) {
			...
			// All we need now is a Company from epaas
