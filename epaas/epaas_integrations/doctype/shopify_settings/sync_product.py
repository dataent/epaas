...
from dataent.utils import cstr, cint, get_request_session
from epaas.epaas_integrations.doctype.shopify_settings.shopify_settings import get_shopify_url, get_header
...
def create_attribute(shopify_item):
	...
	for attr in shopify_item.get('options'):
		if not dataent.db.get_value("Item Attribute", attr.get("name"), "name"):
			dataent.get_doc({
			...
			item_attr = dataent.get_doc("Item Attribute", attr.get("name"))
...
def create_item(shopify_item, warehouse, has_variant=0, attributes=None,variant_of=None):
	...
	if not is_item_exists(item_dict, attributes, variant_of=variant_of):
		...
		if not item_details:
			new_item = dataent.get_doc(item_dict)
			new_item.insert()
			name = new_item.name
...
def create_item_variants(shopify_item, warehouse, attributes, shopify_variants_attr_list):
	template_item = dataent.db.get_value("Item", filters={"shopify_product_id": shopify_item.get("id")},
...
def get_attribute_value(variant_attr_val, attribute):
	attribute_value = dataent.db.sql("""select attribute_value from `tabItem Attribute Value`
...
def get_item_group(product_type=None):
	import dataent.utils.nestedset
	parent_item_group = dataent.utils.nestedset.get_root_of("Item Group")

	if product_type:
		if not dataent.db.get_value("Item Group", product_type, "name"):
			item_group = dataent.get_doc({
				"doctype": "Item Group",
...
def add_to_price_list(item, name):
	shopify_settings = dataent.db.get_value("Shopify Settings", None, ["price_list", "update_price_in_epaas_price_list"], as_dict=1)
	if not shopify_settings.update_price_in_epaas_price_list:
		return

	item_price_name = dataent.db.get_value("Item Price",
	...
	if not item_price_name:
		...
		item_rate = dataent.get_doc("Item Price", item_price_name)
...
def get_supplier_group():
	supplier_group = dataent.db.get_value("Supplier Group", _("Shopify Supplier"))
	if not supplier_group:
		supplier_group = dataent.get_doc({
...
def get_item_details(shopify_item):
	...
	item_details = dataent.db.get_value("Item", {"shopify_product_id": shopify_item.get("id")},
	...
	if item_details:
		...
		item_details = dataent.db.get_value("Item", {"shopify_variant_id": shopify_item.get("id")},
			["name", "stock_uom", "item_name"], as_dict=1)
		return item_details
...
def is_item_exists(shopify_item, attributes=None, variant_of=None):
	if variant_of:
		...
		name = dataent.db.get_value("Item", {"item_name": shopify_item.get("item_name")})

	if name:
		item = dataent.get_doc("Item", name)
		...
		if item.shopify_product_id and attributes and attributes[0].get("attribute_value"):
			if not variant_of:
				variant_of = dataent.db.get_value("Item",
			...
			parent = dataent.db.sql(""" select * from tabItem it where
			...
			if parent:
				variant = dataent.get_doc("Item", parent[0][0])
