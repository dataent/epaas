...
from epaas.epaas_integrations.doctype.shopify_log.shopify_log import make_shopify_log
	...
	def validate_access_credentials(self):
		if self.app_type == "Private":
			if not (self.get_password(raise_exception=False) and self.api_key and self.shopify_url):
				dataent.msgprint(_("Missing value for Password, API Key or Shopify URL"), raise_exception=dataent.ValidationError)

		else:
			if not (self.access_token and self.shopify_url):
				dataent.msgprint(_("Access token or Shopify URL missing"), raise_exception=dataent.ValidationError)

	def validate_app_type(self):
		if self.app_type == "Public":
			dataent.throw(_("Support for public app is deprecated. Please setup private app, for more details refer user manual"))
	...
	def unregister_webhooks(self):
		...
		for d in self.webhooks:
			...
			try:
				...
				dataent.log_error(message=dataent.get_traceback(), title=e.message[:140])
...
def get_series():
	return {
		"sales_order_series" : dataent.get_meta("Sales Order").get_options("naming_series") or "SO-Shopify-",
		"sales_invoice_series" : dataent.get_meta("Sales Invoice").get_options("naming_series")  or "SI-Shopify-",
		"delivery_note_series" : dataent.get_meta("Delivery Note").get_options("naming_series")  or "DN-Shopify-"
	}
