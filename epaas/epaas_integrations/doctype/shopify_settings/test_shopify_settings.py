...
from epaas.epaas_integrations.connectors.shopify_connection import create_order
from epaas.epaas_integrations.doctype.shopify_settings.sync_product import make_item
from epaas.epaas_integrations.doctype.shopify_settings.sync_customer import create_customer
from dataent.core.doctype.data_import.data_import import import_doc
...
class ShopifySettings(unittest.TestCase):
	...
	def test_order(self):
		...
		sales_order = dataent.get_doc("Sales Order", {"shopify_order_id": cstr(shopify_order.get("order").get("id"))})
		...
		sales_order_customer_id = dataent.get_value("Customer", sales_order.customer, "shopify_customer_id")

		self.assertEqual(shopify_order_customer_id, sales_order_customer_id)

		#check sales invoice
		sales_invoice = dataent.get_doc("Sales Invoice", {"shopify_order_id": sales_order.shopify_order_id})
		self.assertEqual(sales_invoice.rounded_total, sales_order.rounded_total)

		#check delivery note
		delivery_note_count = dataent.db.sql("""select count(*) from `tabDelivery Note`
