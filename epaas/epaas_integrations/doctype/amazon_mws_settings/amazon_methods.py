...
import dataent, time, dateutil, math, csv
try:
    from StringIO import StringIO
except ImportError:
    from io import StringIO
import epaas.epaas_integrations.doctype.amazon_mws_settings.amazon_mws_api as mws
...
def get_products_details():
	...
	mws_settings = dataent.get_doc("Amazon MWS Settings")
...
def call_mws_method(mws_method, *args, **kwargs):
	...
	for x in range(0, max_retries):
		try:
			...
			dataent.log_error(message=e, title=str(mws_method))
			time.sleep(delay)
			continue
	...
	dataent.throw(_("Sync has been temporarily disabled because maximum retries have been exceeded"))

def create_item_code(amazon_item_json, sku):
	if dataent.db.get_value("Item", sku):
		return
	...
	item_group = dataent.db.get_value("Item Group",filters={"item_group_name": temp_item_group})

	if not item_group:
		igroup = dataent.new_doc("Item Group")
...
def create_manufacturer(amazon_item_json):
	existing_manufacturer = dataent.db.get_value("Manufacturer",
		filters={"short_name":amazon_item_json.Product.AttributeSets.ItemAttributes.Manufacturer})

	if not existing_manufacturer:
		manufacturer = dataent.new_doc("Manufacturer")
...
def create_item_price(amazon_item_json, item_code):
	...
	item_price.price_list = dataent.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "price_list")
...
def get_orders(after_date):
	try:
		...
		dataent.log_error(title="get_orders", message=e)
...
def create_sales_order(order_json,after_date):
	...
	so = dataent.db.get_value("Sales Order",
			filters={"amazon_order_id": market_place_order_id},
			fieldname="name")

	taxes_and_charges = dataent.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "taxes_charges")
	...
	if not so:
		...
		so = dataent.get_doc({
				...
				"items": items,
				"company": dataent.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "company")
			})
		...
		try:
			...
			dataent.log_error(message=e, title="Create Sales Order")
...
def create_customer(order_json):
	...
	existing_customer_name = dataent.db.get_value("Customer",
	...
	if existing_customer_name:
		...
		existing_contacts = dataent.get_list("Contact", filters)

		if existing_contacts:
			pass
		else:
			new_contact = dataent.new_doc("Contact")
		...
		mws_customer_settings = dataent.get_doc("Amazon MWS Settings")
		new_customer = dataent.new_doc("Customer")
...
def create_address(amazon_order_item_json, customer_name):
	...
	existing_address = dataent.get_list("Address", filters)

	if not("ShippingAddress" in amazon_order_item_json):
		return None
	else:
		make_address = dataent.new_doc("Address")
		...
		for address in existing_address:
			address_doc = dataent.get_doc("Address", address["name"])
...
def get_order_items(market_place_order_id):
	...
	warehouse = dataent.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "warehouse")
...
def get_item_code(order_item):
	...
	item_code = dataent.db.get_value("Item", {"item_code": sku}, "item_code")
	if item_code:
		return item_code
...
def get_account(name):
	existing_account = dataent.db.get_value("Account", {"account_name": "Amazon {0}".format(name)})
	account_name = existing_account
	mws_settings = dataent.get_doc("Amazon MWS Settings")

	if not existing_account:
		try:
			new_account = dataent.new_doc("Account")
			...
			dataent.log_error(message=e, title="Create Account")
