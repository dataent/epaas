...
def set_mandate_status(event):
	...
	for mandate in mandates:
		dataent.db.set_value("GoCardless Mandate", mandate, "disabled", disabled)
...
def get_webhook_keys():
	def _get_webhook_keys():
		webhook_keys = [d.webhooks_secret for d in dataent.get_all("GoCardless Settings", fields=["webhooks_secret"],) if d.webhooks_secret]

		return webhook_keys

	return dataent.cache().get_value("gocardless_webhooks_secret", _get_webhook_keys)

def clear_cache():
	dataent.cache().delete_value("gocardless_webhooks_secret")
