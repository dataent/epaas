...
dataent.ui.form.on('Woocommerce Settings', {
	refresh (frm) {
		...
		frm.set_query("tax_account", ()=>{
			return {
				"filters": {
					"company": dataent.defaults.get_default("company"),
					"is_group": 0
				}
			};
		});
		...
		frm.add_custom_button(__('Generate Secret'), () => {
			dataent.confirm(
				...
				() => {
					dataent.call({
						type:"POST",
						method:"epaas.epaas_integrations.doctype.woocommerce_settings.woocommerce_settings.generate_secret",
					}).done(() => {
						frm.reload_doc();
					}).fail(() => {
						dataent.msgprint(__("Could not generate Secret"));
					});
				}
			);
		});
...
dataent.ui.form.on("Woocommerce Settings", "onload", function () {
	dataent.call({
		method: "epaas.epaas_integrations.doctype.woocommerce_settings.woocommerce_settings.get_series",
		callback: function (r) {
