...
from dataent.utils import add_days, add_months, add_years, getdate, nowdate
...
class AssetMaintenance(Document):
	...
	def sync_maintenance_tasks(self):
		...
		asset_maintenance_logs = dataent.get_all("Asset Maintenance Log", fields=["name"], filters = {"asset_maintenance": self.name,
			"task": ("not in", tasks_names)})
		if asset_maintenance_logs:
			for asset_maintenance_log in asset_maintenance_logs:
				maintenance_log = dataent.get_doc('Asset Maintenance Log', asset_maintenance_log.name)
				maintenance_log.db_set('maintenance_status', 'Cancelled')

@dataent.whitelist()
def assign_tasks(asset_maintenance_name, assign_to_member, maintenance_task, next_due_date):
	team_member = dataent.get_doc('User', assign_to_member).email
	args = {
	...
	if not dataent.db.sql("""select owner from `tabToDo`
...
def update_maintenance_log(asset_maintenance, item_code, item_name, task):
	asset_maintenance_log = dataent.get_value("Asset Maintenance Log", {"asset_maintenance": asset_maintenance,
	...
	if not asset_maintenance_log:
		...
		maintenance_log = dataent.get_doc('Asset Maintenance Log', asset_maintenance_log)
...
def get_team_members(doctype, txt, searchfield, start, page_len, filters):
	return dataent.db.get_values('Maintenance Team Member', {'parent':filters.get("maintenance_team")})
