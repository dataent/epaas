...
import dataent, epaas, math, json
from dataent import _
from six import string_types
from dataent.utils import flt, add_months, cint, nowdate, getdate, today, date_diff
from dataent.model.document import Document
from epaas.assets.doctype.asset_category.asset_category import get_asset_category_account
from epaas.assets.doctype.asset.depreciation \
	import get_disposal_account_and_cost_center, get_depreciation_accounts
from epaas.accounts.general_ledger import make_gl_entries, delete_gl_entries
...
class Asset(AccountsController):
	...
	def validate_item(self):
		item = dataent.get_cached_value("Item", self.item_code,
			["is_fixed_asset", "is_stock_item", "disabled"], as_dict=1)
		if not item:
			dataent.throw(_("Item {0} does not exist").format(self.item_code))
		elif item.disabled:
			dataent.throw(_("Item {0} has been disabled").format(self.item_code))
		elif not item.is_fixed_asset:
			dataent.throw(_("Item {0} must be a Fixed Asset Item").format(self.item_code))
		elif item.is_stock_item:
			dataent.throw(_("Item {0} must be a non-stock item").format(self.item_code))

	def validate_in_use_date(self):
		if not self.available_for_use_date:
			dataent.throw(_("Available for use date is required"))

	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = dataent.get_cached_value("Item", self.item_code, "asset_category")
	...
	def validate_asset_values(self):
		if not flt(self.gross_purchase_amount):
			dataent.throw(_("Gross Purchase Amount is mandatory"), dataent.MandatoryError)

		if not is_cwip_accounting_disabled():
			if not self.is_existing_asset and not (self.purchase_receipt or self.purchase_invoice):
				dataent.throw(_("Please create purchase receipt or purchase invoice for the item {0}").
					format(self.item_code))

			if (not self.purchase_receipt and self.purchase_invoice
				and not dataent.db.get_value('Purchase Invoice', self.purchase_invoice, 'update_stock')):
				dataent.throw(_("Update stock must be enable for the purchase invoice {0}").
					format(self.purchase_invoice))

		if not self.calculate_depreciation:
			return
		elif not self.finance_books:
			dataent.throw(_("Enter depreciation details"))
		...
		if docname:
			...
			date = dataent.db.get_value(doctype, docname, 'posting_date')

		if self.available_for_use_date and getdate(self.available_for_use_date) < getdate(self.purchase_date):
			dataent.throw(_("Available-for-use Date should be after purchase date"))
		...
		if not self.get("schedules") and self.available_for_use_date:
			...
			for d in self.get('finance_books'):
				...
				if number_of_pending_depreciations:
					...
					if  (cint(dataent.db.get_value("Asset Settings", None, "schedule_based_on_fiscal_year")) == 1
	...
	def validate_asset_finance_books(self, row):
		if flt(row.expected_value_after_useful_life) >= flt(self.gross_purchase_amount):
			dataent.throw(_("Row {0}: Expected Value After Useful Life must be less than Gross Purchase Amount")
				.format(row.idx))

		if not row.depreciation_start_date:
			dataent.throw(_("Row {0}: Depreciation Start Date is required").format(row.idx))
		...
		if not self.is_existing_asset:
			...
			if flt(self.opening_accumulated_depreciation) > depreciable_amount:
					dataent.throw(_("Opening Accumulated Depreciation must be less than equal to {0}")
						.format(depreciable_amount))

			if self.opening_accumulated_depreciation:
				if not self.number_of_depreciations_booked:
					dataent.throw(_("Please set Number of Depreciations Booked"))
			else:
				self.number_of_depreciations_booked = 0

			if cint(self.number_of_depreciations_booked) > cint(row.total_number_of_depreciations):
				dataent.throw(_("Number of Depreciations Booked cannot be greater than Total Number of Depreciations"))

		if row.depreciation_start_date and getdate(row.depreciation_start_date) < getdate(nowdate()):
			dataent.msgprint(_("Depreciation Row {0}: Depreciation Start Date is entered as past date")
				.format(row.idx), title=_('Warning'), indicator='red')

		if row.depreciation_start_date and getdate(row.depreciation_start_date) < getdate(self.purchase_date):
			dataent.throw(_("Depreciation Row {0}: Next Depreciation Date cannot be before Purchase Date")
				.format(row.idx))

		if row.depreciation_start_date and getdate(row.depreciation_start_date) < getdate(self.available_for_use_date):
			dataent.throw(_("Depreciation Row {0}: Next Depreciation Date cannot be before Available-for-use Date")
				.format(row.idx))
	...
	def get_depreciation_amount_prorata_temporis(self, depreciable_value, row, start_date=None, end_date=None):
		if start_date and end_date:
			prorata_temporis =  min(abs(flt(date_diff(str(end_date), str(start_date)))) / flt(dataent.db.get_value("Asset Settings", None, "number_of_days_in_fiscal_year")), 1)
		else:
			prorata_temporis = 1
		...
		for row in self.get('finance_books'):
			...
			if accumulated_depreciation_after_full_schedule:
				...
				if row.expected_value_after_useful_life < asset_value_after_full_schedule:
					dataent.throw(_("Depreciation Row {0}: Expected value after useful life must be greater than or equal to {1}")
						.format(row.idx, asset_value_after_full_schedule))

	def validate_cancellation(self):
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			dataent.throw(_("Asset cannot be cancelled, as it is already {0}").format(self.status))

		if self.purchase_invoice:
			dataent.throw(_("Please cancel Purchase Invoice {0} first").format(self.purchase_invoice))

		if self.purchase_receipt:
			dataent.throw(_("Please cancel Purchase Receipt {0} first").format(self.purchase_receipt))

	def delete_depreciation_entries(self):
		for d in self.get("schedules"):
			if d.journal_entry:
				dataent.get_doc("Journal Entry", d.journal_entry).cancel()
	...
	def get_default_finance_book_idx(self):
		if not self.get('default_finance_book') and self.company:
			self.default_finance_book = epaas.get_default_finance_book(self.company)
	...
	def update_stock_movement(self):
		...
		if asset_movement:
			doc = dataent.get_doc('Asset Movement', asset_movement)
			doc.submit()
	...
	def make_gl_entries(self):
		...
		if ((self.purchase_receipt or (self.purchase_invoice and
			dataent.db.get_value('Purchase Invoice', self.purchase_invoice, 'update_stock')))
	...
	def get_depreciation_rate(self, args, on_validate=False):
		...
		float_precision = cint(dataent.db.get_default("float_precision")) or 2
...
def update_maintenance_status():
	assets = dataent.get_all('Asset', filters = {'docstatus': 1, 'maintenance_required': 1})

	for asset in assets:
		asset = dataent.get_doc("Asset", asset.name)
		if dataent.db.exists('Asset Maintenance Task', {'parent': asset.name, 'next_due_date': today()}):
			asset.set_status('In Maintenance')
		if dataent.db.exists('Asset Repair', {'asset_name': asset.name, 'repair_status': 'Pending'}):
...
def make_post_gl_entry():
	...
	assets = dataent.db.sql_list(""" select name from `tabAsset`
		where ifnull(booked_fixed_asset, 0) = 0 and available_for_use_date = %s""", nowdate())

	for asset in assets:
		doc = dataent.get_doc('Asset', asset)
		doc.make_gl_entries()

def get_asset_naming_series():
	meta = dataent.get_meta('Asset')
...
def make_purchase_invoice(asset, item_code, gross_purchase_amount, company, posting_date):
	pi = dataent.new_doc("Purchase Invoice")
	pi.company = company
	pi.currency = dataent.get_cached_value('Company',  company,  "default_currency")
...
def make_sales_invoice(asset, item_code, company, serial_no=None):
	...
	si.company = company
	si.currency = dataent.get_cached_value('Company',  company,  "default_currency")
...
def create_asset_maintenance(asset, item_code, item_name, asset_category, company):
	asset_maintenance = dataent.new_doc("Asset Maintenance")
...
def create_asset_adjustment(asset, asset_category, company):
	asset_maintenance = dataent.new_doc("Asset Value Adjustment")
	asset_maintenance.update({
		"asset": asset,
		"company": company,
		"asset_category": asset_category
...
def transfer_asset(args):
	...
	movement_entry = dataent.new_doc("Asset Movement")
	...
	dataent.msgprint(_("Asset Movement record {0} created").format("<a href='#Form/Asset Movement/{0}'>{0}</a>".format(movement_entry.name)))

@dataent.whitelist()
def get_item_details(item_code, asset_category):
	asset_category_doc = dataent.get_doc('Asset Category', asset_category)
	books = []
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
	...
	if not account:
		account = dataent.get_cached_value('Company',  company,  account_name)

	if not account:
		dataent.throw(_("Set {0} in asset category {1} or company {2}")
...
def make_journal_entry(asset_name):
	...
	depreciation_cost_center, depreciation_series = dataent.db.get_value("Company", asset.company,
...
def is_cwip_accounting_disabled():
	return cint(dataent.db.get_single_value("Asset Settings", "disable_cwip_accounting"))
