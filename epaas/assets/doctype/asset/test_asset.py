...
import dataent
import unittest
from dataent.utils import cstr, nowdate, getdate, flt, get_last_day, add_days, add_months
from epaas.assets.doctype.asset.depreciation import post_depreciation_entries, scrap_asset, restore_asset
from epaas.assets.doctype.asset.asset import make_sales_invoice, make_purchase_invoice
from epaas.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt
from epaas.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice as make_invoice
...
class TestAsset(unittest.TestCase):
	...
	def test_purchase_asset(self):
		...
		gle = dataent.db.sql("""select account, debit, credit from `tabGL Entry`
		...
		self.assertFalse(dataent.db.get_value("GL Entry",
			{"voucher_type": "Purchase Invoice", "voucher_no": pi.name}))

	def test_is_fixed_asset_set(self):
		doc = dataent.new_doc('Purchase Invoice')
	...
	def test_schedule_for_straight_line_method_for_existing_asset(self):
		...
		asset = dataent.get_doc("Asset", {"asset_name": "Macbook Pro 1"})
		asset.calculate_depreciation = 1
	...
	def test_depreciation(self):
		...
		dataent.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
	...
	def test_asset_sale(self):
		...
		self.assertEqual(dataent.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		self.assertEqual(dataent.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
...
def create_asset_data():
	if not dataent.db.exists("Asset Category", "Computers"):
		create_asset_category()

	if not dataent.db.exists("Item", "Macbook Pro"):
...
def remove_prorated_depreciation_schedule():
	asset_settings = dataent.get_doc("Asset Settings", "Asset Settings")
