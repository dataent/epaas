...
from dataent.utils import flt, today, getdate, cint
...
def get_depreciable_assets(date):
	return dataent.db.sql_list("""select a.name
...
def get_depreciation_accounts(asset):
	...
	accounts = dataent.db.get_value("Asset Category Account",
	...
	if not accumulated_depreciation_account or not depreciation_expense_account:
		accounts = dataent.get_cached_value('Company',  asset.company,
	...
	if not fixed_asset_account or not accumulated_depreciation_account or not depreciation_expense_account:
		dataent.throw(_("Please set Depreciation related Accounts in Asset Category {0} or Company {1}")
...
def restore_asset(asset_name):
	...
	dataent.get_doc("Journal Entry", je).cancel()
...
def get_disposal_account_and_cost_center(company):
	disposal_account, depreciation_cost_center = dataent.get_cached_value('Company',  company,
		["disposal_account", "depreciation_cost_center"])

	if not disposal_account:
		dataent.throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		dataent.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
