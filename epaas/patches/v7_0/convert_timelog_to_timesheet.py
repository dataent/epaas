...
def execute():
	dataent.reload_doc('projects', 'doctype', 'task')
	dataent.reload_doc('projects', 'doctype', 'timesheet')
	if not dataent.db.table_exists("Time Log"):
		return

	from epaas.manufacturing.doctype.work_order.work_order \
		import make_timesheet, add_timesheet_detail

	for data in dataent.db.sql("select * from `tabTime Log`", as_dict=1):
		if data.task:
			company = dataent.db.get_value("Task", data.task, "company")
		elif data.work_order:
			company = dataent.db.get_value("Work Order", data.work_order, "company")
		else:
