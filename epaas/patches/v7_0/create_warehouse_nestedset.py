...
def execute():
	...
	if check_is_warehouse_associated_with_company():
		for company in dataent.get_all("Company", fields=["name", "abbr"]):
			make_warehouse_nestedset(company)
	else:
		sle_against_companies = dataent.db.sql_list("""select distinct company from `tabStock Ledger Entry`""")

		if len(sle_against_companies) == 1:
			company = dataent.get_cached_value('Company',  sle_against_companies[0],  
...
def check_is_warehouse_associated_with_company():
	...
	for warehouse in dataent.get_all("Warehouse", fields=["name", "company"]):
...
def make_warehouse_nestedset(company=None):
	...
	enable_perpetual_inventory = cint(epaas.is_perpetual_inventory_enabled(company.name)) or 0
...
def validate_parent_account_for_warehouse(company=None):
	...
	if cint(epaas.is_perpetual_inventory_enabled(company.name)):
		parent_account = dataent.db.sql("""select name from tabAccount
			where account_type='Stock' and company=%s and is_group=1
			and (warehouse is null or warehouse = '')""", company.name)

		if not parent_account:
			current_parent_accounts_for_warehouse = dataent.db.sql("""select parent_account from tabAccount
				where account_type='Warehouse' and (warehouse is not null or warehouse != '') """)

			if current_parent_accounts_for_warehouse:
				dataent.db.set_value("Account", current_parent_accounts_for_warehouse[0][0], "account_type", "Stock")
...
def set_parent_to_warehouse(warehouse_group, company=None):
	dataent.db.sql(""" update tabWarehouse set parent_warehouse = %s, is_group = 0
...
def set_parent_to_warehouse_account(company):
	dataent.db.sql(""" update tabAccount set parent_account = %s
...
def set_company_to_warehouse(company):
	dataent.db.sql("update tabWahouse set company=%s", company)

def get_stock_account_group(company):
	stock_account_group = dataent.db.get_all('Account', filters = {'company': company, 'is_group': 1,
