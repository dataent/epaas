...
def execute():
	if dataent.db.table_exists("Time Log") and "employee" in dataent.db.get_table_columns("Time Log"):
		timesheet = dataent.db.sql("""select tl.employee as employee, ts.name as name,
		...
		for data in timesheet:
			ts_doc = dataent.get_doc('Timesheet', data.name)
			if len(ts_doc.time_logs) == 1:
				dataent.db.sql(""" update `tabTimesheet` set creation = %(creation)s,
