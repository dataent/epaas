...
def execute():
	if not dataent.db.sql("""select name from `tabPatch Log`
		where patch = 'epaas.patches.v5_0.taxes_and_totals_in_party_currency'"""):
			return
...
def update_values(dt, tax_table):
	rate_field_precision = get_field_precision(dataent.get_meta(dt + " Item").get_field("rate"))
	tax_amount_precision = get_field_precision(dataent.get_meta(tax_table).get_field("tax_amount"))
