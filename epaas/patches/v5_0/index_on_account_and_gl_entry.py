...
def execute():
	...
	for dt, indexes in index_map.items():
		existing_indexes = [(d.Key_name, d.Column_name) for d in dataent.db.sql("""show index from `tab{0}` 
		...
		for old, column in existing_indexes:
			if column in ("parent", "group_or_ledger", "is_group", "is_pl_account", "debit_or_credit", 
				...
				dataent.db.sql("alter table `tab{0}` drop index {1}".format(dt, old))
		...
		for new in indexes:
			if new not in existing_indexed_columns:
				dataent.db.sql("alter table `tab{0}` add index ({1})".format(dt, new))