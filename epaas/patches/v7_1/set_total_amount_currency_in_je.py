...
def execute():
	...
	dataent.db.sql(""" update `tabJournal Entry` set total_amount_currency = %s
		where ifnull(multi_currency, 0) = 0
		and (pay_to_recd_from is not null or pay_to_recd_from != "") """, get_default_currency())

	for je in dataent.db.sql(""" select name from `tabJournal Entry` where multi_currency = 1
		and (pay_to_recd_from is not null or pay_to_recd_from != "")""", as_dict=1):

		doc = dataent.get_doc("Journal Entry", je.name)
		...
		dataent.db.set_value("Journal Entry", je.name, "total_amount_currency",
