...
def execute():
	...
	dataent.reload_doctype("Stock Entry Detail")
	dataent.reload_doctype("Landed Cost Taxes and Charges")

	stock_entry_db_columns = dataent.db.get_table_columns("Stock Entry")
	...
	dataent.db.sql("""update `tabStock Entry Detail` sed, `tabStock Entry` se
	...
	stock_entries = dataent.db.sql_list("""select name from `tabStock Entry`
	...
	for d in stock_entries:
		stock_entry = dataent.get_doc("Stock Entry", d)
