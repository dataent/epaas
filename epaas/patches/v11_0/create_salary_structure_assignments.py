...
from epaas.hr.doctype.salary_structure_assignment.salary_structure_assignment import DuplicateAssignment

def execute():
	dataent.reload_doc('hr', 'doctype', 'salary_structure')
	dataent.reload_doc("hr", "doctype", "salary_structure_assignment")
	dataent.db.sql("""
	...
	""")
	if dataent.db.table_exists('Salary Structure Employee'):
		ss_details = dataent.db.sql("""
		...
		cols = ""
		if "base" in dataent.db.get_table_columns("Salary Structure"):
			cols = ", base, variable"
	...
	for d in ss_details:
		try:
			joining_date, relieving_date = dataent.db.get_value("Employee", d.employee,
			...
			s = dataent.new_doc("Salary Structure Assignment")
	...
	dataent.db.sql("update `tabSalary Structure` set docstatus=1")