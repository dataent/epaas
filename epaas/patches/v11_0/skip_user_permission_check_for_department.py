...
# https://github.com/dataent/epaas/pull/14121
...
def execute():
	...
	for doctype in ['Appraisal', 'Leave Allocation', 'Expense Claim', 'Instructor', 'Salary Slip',
		...
		if dataent.db.exists('Custom Field', { 'dt': doctype, 'fieldname': 'department'}): continue
		doctypes_to_skip.append(doctype)

	dataent.reload_doctype('User Permission')

	user_permissions = dataent.get_all("User Permission",
	...
	if user_permissions_to_delete:
		dataent.db.sql('DELETE FROM `tabUser Permission` WHERE `name` IN ({})'.format( # nosec
