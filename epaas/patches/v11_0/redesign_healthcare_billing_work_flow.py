...
from epaas.domains.healthcare import data
...
def execute():
	...
	for si_ref_doc in sales_invoice_referenced_doc:
		if dataent.db.exists('DocType', si_ref_doc):
			dataent.reload_doc(get_doctype_module(si_ref_doc), 'doctype', scrub(si_ref_doc))
			...
			and dataent.db.has_column(si_ref_doc, 'invoiced'):
				...
				if doc_list:
					dataent.reload_doc(get_doctype_module("Sales Invoice"), 'doctype', 'sales_invoice')
					for doc_id in doc_list:
						invoice_id = dataent.db.get_value(si_ref_doc, doc_id[0], sales_invoice_referenced_doc[si_ref_doc])
						if dataent.db.exists("Sales Invoice", invoice_id):
							if si_ref_doc == "Lab Test":
								template = dataent.db.get_value("Lab Test", doc_id[0], "template")
								if template:
									item = dataent.db.get_value("Lab Test Template", template, "item")
									if item:
								...
								invoice = dataent.get_doc("Sales Invoice", invoice_id)
...
def healthcare_custom_field_in_sales_invoice():
	dataent.reload_doc('healthcare', 'doctype', 'patient')
