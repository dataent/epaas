...
def execute():
	...
	'''
	if not dataent.db.has_column('Item', 'default_warehouse'):
		return

	dataent.reload_doc('stock', 'doctype', 'item_default')
	dataent.reload_doc('stock', 'doctype', 'item')

	companies = dataent.get_all("Company")
	if len(companies) == 1 and not dataent.get_all("Item Default", limit=1):
		try:
			dataent.db.sql('''
		...
		item_details = dataent.db.sql(""" SELECT name, default_warehouse,
		...
		for item_data in item_details:
			for d in [["default_warehouse", "Warehouse"], ["expense_account", "Account"],
				...
				if item_data.get(d[0]):
					company = dataent.get_value(d[1], item_data.get(d[0]), "company", cache=True)
