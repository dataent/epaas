...
def execute():
	company = dataent.db.sql_list("select name from tabCompany where country = 'India'")
	if not company:
		return
	...
	for dt in doctypes:
		...
		for t in list(parent):
			trans_doc = dataent.get_doc(dt, t)
			hsnwise_tax = get_itemised_tax_breakup_html(trans_doc)
			dataent.db.set_value(dt, t, "other_charges_calculation", hsnwise_tax, update_modified=False)