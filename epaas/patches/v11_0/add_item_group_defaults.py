...
def execute():
	...
	item_groups = dataent.db.sql("""select name, default_income_account, default_expense_account,\
		default_cost_center from `tabItem Group`""", as_dict=True)

	if len(companies) == 1:
		for item_group in item_groups:
			doc = dataent.get_doc("Item Group", item_group.get("name"))
			item_group_defaults = []
		...
		for item_group in item_groups:
			...
			for d in [
				...
				if item_group.get(d[0]):
					company = dataent.get_value(d[1], item_group.get(d[0]), "company", cache=True)
