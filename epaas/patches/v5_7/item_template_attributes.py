...
def execute():
	...
	variant_templates = dataent.get_all("Item", filters={"has_variants": 1}, limit_page_length=1)
	if not variant_templates:
		# database does not have items that have variants
		# so no point in running the patch
		return

	variant_attributes = dataent.get_all("Item Variant Attribute", fields=["*"], limit_page_length=1)
...
def migrate_manage_variants():
	item_attribute = {}
	for d in  dataent.db.sql("""select DISTINCT va.attribute, i.variant_of
...
def migrate_item_variants():
	for item in dataent.get_all("Item", filters={"has_variants": 1}):
		all_variants = dataent.get_all("Item", filters={"variant_of": item.name}, fields=["name", "description"])
		item_attributes = dataent.db.sql("""select distinct item_attribute, item_attribute_value
			from `tabItem Variant` where parent=%s""", item.name)

		if not item_attributes and not all_variants:
			item = dataent.get_doc("Item", item.name)
			item.has_variants = 0
			item.save()
			continue
	...
	dataent.delete_doc("DocType", "Item Variant")
...
def save_attributes_in_variant(variant, combination):
	...
	variant_item = dataent.get_doc("Item", variant.name)
