...
def execute():
	...
	if not dataent.get_value('Role', dict(role_name='Guardian')):
		dataent.get_doc({
			'doctype': 'Role',
			'role_name': 'Guardian',
			'desk_access': 0,
	...
	if dataent.db.exists("Doctype", "Guardian"):
		for user in dataent.db.sql_list("""select u.name from `tabUser` u , `tabGuardian` g where g.email_address = u.name"""):
