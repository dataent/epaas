...
import dataent
from epaas.patches.v8_10.change_default_customer_credit_days import make_payment_term, make_template

def execute():
	for dt in ("Company", "Customer Group"):
		dataent.reload_doc("setup", "doctype", dataent.scrub(dt))

		credit_records = dataent.db.sql("""
...
def create_payment_terms_template(data):
	...
	if not dataent.db.exists("Payment Terms Template", pyt_template_name):
		payment_term = make_payment_term(data.credit_days, data.credit_days_based_on)
		template = make_template(payment_term)
	else:
		template = dataent.get_doc("Payment Terms Template", pyt_template_name)
	return template