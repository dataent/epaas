...
import dataent
from dataent.model.rename_doc import get_fetch_fields
	...
	for i in range(0, len(customers), batch_size):
		...
		for d in customer_group_fetch:
			...
			for customer in batch_customers:
				value = dataent.db.escape(dataent.as_unicode(customer.get("customer_group")))

				when_then.append('''
					WHEN `%s` = "%s" and %s != "%s"
					THEN "%s"
				'''%(d["master_fieldname"], dataent.db.escape(dataent.as_unicode(customer.name)),
