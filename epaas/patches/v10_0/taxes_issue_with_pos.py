...
def execute():
	for d in dataent.get_all('Sales Invoice', fields=["name"],
		filters = {'is_pos':1, 'docstatus': 1, 'creation': ('>', '2018-04-23')}):
		doc = dataent.get_doc('Sales Invoice', d.name)
		if (not doc.taxes and doc.taxes_and_charges and doc.pos_profile and doc.outstanding_amount != 0 and
			dataent.db.get_value('POS Profile', doc.pos_profile, 'taxes_and_charges', cache=True) == doc.taxes_and_charges):
