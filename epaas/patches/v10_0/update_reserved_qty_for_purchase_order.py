...
def execute():
	...
	dataent.reload_doc("buying", "doctype", "purchase_order_item_supplied")
	company_warehouse = dataent._dict(dataent.db.sql("""select company, min(name) from `tabWarehouse`
		where is_group = 0 group by company"""))

	items = list(set([d.rm_item_code for d in po_item]))
	item_wh = dataent._dict(dataent.db.sql("""select item_code, default_warehouse
...
def get_warehouse(item_code, company, company_warehouse, item_wh):
	...
	if dataent.db.get_value("Warehouse", reserve_warehouse, "company") != company:
