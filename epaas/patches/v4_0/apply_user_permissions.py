...
import dataent
from epaas.hr.doctype.employee.employee import EmployeeUserDisabledError
...
def update_hr_permissions():
	...
	dataent.db.sql("""update `tabDocPerm` set `set_user_permissions`=1 where parent in ('Employee', 'Leave Application')
	...
	if docperm_meta.get_field('apply_user_permissions'):
		...
		dataent.db.sql("""update `tabDocPerm` set `apply_user_permissions`=1 where parent in ('Employee', 'Leave Application')
			and role in ('Employee', 'Leave Approver') and permlevel=0 and `read`=1""")

	dataent.clear_cache()

	# save employees to run on_update events
	for employee in dataent.db.sql_list("""select name from `tabEmployee` where docstatus < 2"""):
		try:
...
def remove_duplicate_user_permissions():
	...
	for d in dataent.db.sql("""select parent, defkey, defvalue,
		...
		if d.cnt > 1:
			...
			dataent.db.sql("""delete from tabDefaultValue where `parent`=%s and `defkey`=%s and
