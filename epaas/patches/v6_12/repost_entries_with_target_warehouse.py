...
To execute this patch run following commands from dataent-bench directory:
```
	bench --site [your-site-name] execute epaas.patches.v6_12.repost_entries_with_target_warehouse.check
	bench --site [your-site-name] backup
	bench --site [your-site-name] execute epaas.patches.v6_12.repost_entries_with_target_warehouse.repost
```
...
def repost_so():
	...
	dataent.db.sql("update `tabSales Order Item` set target_warehouse=''")
...
def get_affected_delivery_notes():
	return dataent.db.sql("""select distinct dn.name, dn.docstatus
...
def get_affected_sales_invoice():
	return dataent.db.sql("""select distinct si.name, si.docstatus
...
def get_affected_sales_order():
	return dataent.db.sql_list("""select distinct parent from `tabSales Order Item` 
