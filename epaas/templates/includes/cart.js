...
$.extend(shopping_cart, {
	...
	bind_address_select: function() {
		$(".cart-addresses").find('input[data-address-name]').on("click", function() {
			if($(this).prop("checked")) {
				...
				return dataent.call({
					type: "POST",
					method: "epaas.shopping_cart.cart.update_cart_address",
					freeze: true,
					args: {
	...
	apply_shipping_rule: function(rule, btn) {
		return dataent.call({
			btn: btn,
			type: "POST",
			method: "epaas.shopping_cart.cart.apply_shipping_rule",
	...
	place_order: function(btn) {
		return dataent.call({
			type: "POST",
			method: "epaas.shopping_cart.cart.place_order",
			btn: btn,
			callback: function(r) {
				if(r.exc) {
					var msg = "";
					...
					$("#cart-error")
						.empty()
						.html(msg || dataent._("Something went wrong!"))
						.toggle(true);
				} else {
