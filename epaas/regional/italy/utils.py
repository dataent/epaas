...
from epaas.controllers.taxes_and_totals import get_itemised_tax
from dataent import _
from six import string_types
from dataent.utils.file_manager import save_file, remove_file
...
def export_invoices(filters=None):
	...
	invoices = dataent.get_all("Sales Invoice", filters=get_conditions(filters), fields=["*"])

	for invoice in invoices:
		attachments = get_e_invoice_attachments(invoice)
		saved_xmls += [attachment.file_name for attachment in attachments]

	zip_filename = "{0}-einvoices.zip".format(dataent.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = dataent.get_doc("Company", invoice.company)
	...
	company_address = dataent.get_doc("Address", invoice.company_address)
	invoice.company_address_data = company_address

	#Set invoice type
	if invoice.is_return and invoice.return_against:
		invoice.type_of_document = "TD04" #Credit Note (Nota di Credito)
		invoice.return_against_unamended =  get_unamended_name(dataent.get_doc("Sales Invoice", invoice.return_against))
	else:
		invoice.type_of_document = "TD01" #Sales Invoice (Fattura)

	#set customer information
	invoice.customer_data = dataent.get_doc("Customer", invoice.customer)
	customer_address = dataent.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = dataent.get_doc("Address", invoice.shipping_address_name)
...
#TODO: Use function from dataent once PR #6853 is merged.
def download_zip(files, output_filename):
	from zipfile import ZipFile

	input_files = [dataent.get_site_path('private', 'files', filename) for filename in files]
	output_path = dataent.get_site_path('private', 'files', output_filename)
			...
			if reference_row:
				items.append(
					dataent._dict(
						...
						stock_uom=dataent.db.get_single_value("Stock Settings", "stock_uom") or _("Nos"),
						tax_rate=tax.rate,
...
def sales_invoice_validate(doc):
	...
	if not doc.company_address:
		dataent.throw(_("Please set an Address on the Company '%s'" % doc.company), title=_("E-Invoicing Information Missing"))
	else:
		validate_address(doc.company_address)

	company_fiscal_regime = dataent.get_cached_value("Company", doc.company, 'fiscal_regime')
	if not company_fiscal_regime:
		dataent.throw(_("Fiscal Regime is mandatory, kindly set the fiscal regime in the company {0}")
			.format(doc.company))
	else:
		doc.company_fiscal_regime = company_fiscal_regime

	doc.company_tax_id = dataent.get_cached_value("Company", doc.company, 'tax_id')
	doc.company_fiscal_code = dataent.get_cached_value("Company", doc.company, 'fiscal_code')
	if not doc.company_tax_id and not doc.company_fiscal_code:
		dataent.throw(_("Please set either the Tax ID or Fiscal Code on Company '%s'" % doc.company), title=_("E-Invoicing Information Missing"))
	...
	if customer.customer_type == "Individual":
		...
		if not doc.customer_fiscal_code:
			dataent.throw(_("Please set Fiscal Code for the customer '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))
	else:
		if customer.is_public_administration:
			doc.customer_fiscal_code = customer.fiscal_code
			if not doc.customer_fiscal_code:
				dataent.throw(_("Please set Fiscal Code for the public administration '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))
		else:
			doc.tax_id = customer.tax_id
			if not doc.tax_id:
				dataent.throw(_("Please set Tax ID for the customer '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))

	if not doc.customer_address:
	 	dataent.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
		validate_address(doc.customer_address)

	if not len(doc.taxes):
		dataent.throw(_("Please set at least one row in the Taxes and Charges Table"), title=_("E-Invoicing Information Missing"))
	else:
		for row in doc.taxes:
			if row.rate == 0 and row.tax_amount == 0 and not row.tax_exemption_reason:
				dataent.throw(_("Row {0}: Please set at Tax Exemption Reason in Sales Taxes and Charges".format(row.idx)),
					title=_("E-Invoicing Information Missing"))

	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = dataent.get_cached_value('Mode of Payment',
...
def sales_invoice_on_submit(doc, method):
	...
	if not len(doc.payment_schedule):
		dataent.throw(_("Please set the Payment Schedule"), title=_("E-Invoicing Information Missing"))
	else:
		for schedule in doc.payment_schedule:
			if not schedule.mode_of_payment:
				dataent.throw(_("Row {0}: Please set the Mode of Payment in Payment Schedule".format(schedule.idx)),
					title=_("E-Invoicing Information Missing"))
			elif not dataent.db.get_value("Mode of Payment", schedule.mode_of_payment, "mode_of_payment_code"):
				dataent.throw(_("Row {0}: Please set the correct code on Mode of Payment {1}".format(schedule.idx, schedule.mode_of_payment)),
					title=_("E-Invoicing Information Missing"))
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	invoice_xml = dataent.render_template('epaas/regional/italy/e-invoice.xml', context={"doc": invoice}, is_path=True)
...
def generate_single_invoice(docname):
	doc = dataent.get_doc("Sales Invoice", docname)
...
def download_e_invoice_file(file_name):
	content = None
	with open(dataent.get_site_path('private', 'files', file_name), "r") as f:
		content = f.read()

	dataent.local.response.filename = file_name
	dataent.local.response.filecontent = content
	dataent.local.response.type = "download"
...
def get_company_country(company):
	return dataent.get_cached_value('Company', company, 'country')
...
def validate_address(address_name):
	...
	data = dataent.get_cached_value("Address", address_name, fields, as_dict=1) or {}

	for field in fields:
		if not data.get(field):
			dataent.throw(_("Please set {0} for address {1}".format(field.replace('-',''), address_name)),
				title=_("E-Invoicing Information Missing"))
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = dataent.model.naming.make_autoname(company_tax_id + "_.#####")
