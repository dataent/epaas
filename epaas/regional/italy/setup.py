...
from epaas.regional.italy import fiscal_regimes, tax_exemption_reasons, mode_of_payment_codes, vat_collectability_options
...
def make_custom_fields(update=True):
	...
	custom_fields = {
		'Company': [
			...
			dict(fieldname='fiscal_regime', label='Fiscal Regime',
				...
				options="\n".join(map(lambda x: dataent.safe_decode(x, encoding='utf-8'), fiscal_regimes))),
			...
			dict(fieldname='vat_collectability', label='VAT Collectability',
				...
				options="\n".join(map(lambda x: dataent.safe_decode(x, encoding='utf-8'), vat_collectability_options))),
		...
		'Sales Taxes and Charges': [
			dict(fieldname='tax_exemption_reason', label='Tax Exemption Reason',
				...
				options="\n" + "\n".join(map(lambda x: dataent.safe_decode(x, encoding='utf-8'), tax_exemption_reasons))),
		...
		'Mode of Payment': [
			...
			fieldtype='Select', insert_after='included_in_print_rate', print_hide=1,
			options="\n".join(map(lambda x: dataent.safe_decode(x, encoding='utf-8'), mode_of_payment_codes)))
		],
		'Payment Schedule': [
			dict(fieldname='mode_of_payment_code', label='Code',
	...
	create_custom_fields(custom_fields, ignore_validate = dataent.flags.in_patch, update=update)

def setup_report():
	report_name = 'Electronic Invoice Register'

	dataent.db.sql(""" update `tabReport` set disabled = 0 where
		name = %s """, report_name)
