...
dataent.query_reports["Fichier des Ecritures Comptables [FEC]"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
...
let fec_export = function(query_report) {
	...
	dataent.db.get_value("Company", company, "siren_number", (value) => {
		const company_data = value.siren_number;
		if (company_data === null || company_data === undefined) {
			dataent.msgprint(__("Please register the SIREN number in the company information file"));
		} else {
			dataent.db.get_value("Fiscal Year", fiscal_year, "year_end_date", (r) => {
