...
from dataent.utils import flt, formatdate
from datetime import date
from six import iteritems
from epaas.regional.india.utils import get_gst_accounts
...
class Gstr1Report(object):
	...
	def get_invoice_data(self):
		self.invoices = dataent._dict()
		conditions = self.get_conditions()
		invoice_data = dataent.db.sql("""
			select
				{select_columns}
			from `tab{doctype}`
	...
	def get_conditions(self):
		...
		customers = dataent.get_all("Customer", filters={"disabled": 0})
		...
		if self.filters.get("type_of_business") ==  "B2B":
			conditions += """ and ifnull(invoice_type, '') != 'Export' and is_return != 1
				and customer in ('{0}') and (customer_gstin IS NOT NULL AND customer_gstin NOT IN ('', 'NA'))""".\
					format("', '".join([dataent.db.escape(c.name) for c in customers]))

		if self.filters.get("type_of_business") in ("B2C Large", "B2C Small"):
			b2c_limit = dataent.db.get_single_value('GST Settings', 'b2c_limit')
			if not b2c_limit:
				dataent.throw(_("Please set B2C Limit in GST Settings."))

		if self.filters.get("type_of_business") ==  "B2C Large":
			conditions += """ and SUBSTR(place_of_supply, 1, 2) != SUBSTR(company_gstin, 1, 2)
				and grand_total > {0} and is_return != 1 and customer in ('{1}')""".\
					format(flt(b2c_limit), "', '".join([dataent.db.escape(c.name) for c in customers]))
	...
	def get_invoice_items(self):
		self.invoice_items = dataent._dict()
	...
	def get_items_based_on_tax_rate(self):
		self.tax_details = dataent.db.sql("""
			select
		...
		self.invoice_cess = dataent._dict()
		...
		if unidentified_gst_accounts:
			dataent.msgprint(_("Following accounts might be selected in GST Settings:")
				+ "<br>" + "<br>".join(unidentified_gst_accounts), alert=True)

		# Build itemised tax for export invoices where tax table is blank
		for invoice, items in iteritems(self.invoice_items):
			if invoice not in self.items_based_on_tax_rate \
				and dataent.db.get_value(self.doctype, invoice, "export_type") == "Without Payment of Tax":
