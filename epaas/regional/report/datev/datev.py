...
def validate_filters(filters):
	...
	if not filters.get('from_date'):
		dataent.throw(_('{0} is mandatory').format(_('From Date')))

	if not filters.get('to_date'):
		dataent.throw(_('{0} is mandatory').format(_('To Date')))
...
def download_datev_csv(filters=None):
	...
	GET /api/method/epaas.regional.report.datev.datev.download_datev_csv
	...
	dataent.response['result'] = get_datev_csv(data)
	dataent.response['doctype'] = filename
