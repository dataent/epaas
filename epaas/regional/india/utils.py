...
import dataent, re, json
from dataent import _
from dataent.utils import cstr, flt, date_diff, nowdate
from epaas.regional.india import states, state_numbers
from epaas.controllers.taxes_and_totals import get_itemised_tax, get_itemised_taxable_amount
from epaas.controllers.accounts_controller import get_taxes_and_charges
from epaas.hr.utils import get_salary_assignment
...
def validate_gstin_for_india(doc, method):
	...
	if len(doc.gstin) != 15:
		dataent.throw(_("Invalid GSTIN! A GSTIN must have 15 characters."))

	p = re.compile("^[0-9]{2}[A-Z]{4}[0-9A-Z]{1}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[1-9A-Z]{1}[0-9A-Z]{1}$")
	if not p.match(doc.gstin):
		dataent.throw(_("Invalid GSTIN! The input you've entered doesn't match the format of GSTIN."))

	validate_gstin_check_digit(doc.gstin)

	if doc.gst_state_number != doc.gstin[:2]:
		dataent.throw(_("Invalid GSTIN! First 2 digits of GSTIN should match with State number {0}.")
...
def validate_gstin_check_digit(gstin, label='GSTIN'):
	...
	if gstin[-1] != code_point_chars[((mod - (total % mod)) % mod)]:
		dataent.throw(_("Invalid {0}! The check digit validation has failed. " +
			"Please ensure you've typed the {0} correctly.".format(label)))

def get_itemised_tax_breakup_header(item_doctype, tax_accounts):
	if dataent.get_meta(item_doctype).has_field('gst_hsn_code'):
...
def get_itemised_tax_breakup_data(doc, account_wise=False):
	...
	if not dataent.get_meta(doc.doctype + " Item").has_field('gst_hsn_code'):
		return itemised_tax, itemised_taxable_amount

	item_hsn_map = dataent._dict()
	for d in doc.items:
	...
	for item, taxes in itemised_tax.items():
		...
		hsn_tax.setdefault(hsn_code, dataent._dict())
	...
	# set taxable amount
	hsn_taxable_amount = dataent._dict()
...
def get_place_of_supply(out, doctype):
	if not dataent.get_meta('Address').has_field('gst_state'): return
	...
	if address_name:
		address = dataent.db.get_value("Address", address_name, ["gst_state", "gst_state_number"], as_dict=1)
...
def get_regional_address_details(out, doctype, company):
	...
	if ((doctype in ("Sales Invoice", "Delivery Note") and out.company_gstin
		...
		default_tax = dataent.db.get_value(master_doctype, {"company": company, "is_inter_state":1, "disabled":0})
	else:
		default_tax = dataent.db.get_value(master_doctype, {"company": company, "disabled":0, "is_default": 1})
...
def calculate_annual_eligible_hra_exemption(doc):
	basic_component = dataent.get_cached_value('Company',  doc.company,  "basic_component")
	hra_component = dataent.get_cached_value('Company',  doc.company,  "hra_component")
	if not (basic_component and hra_component):
		dataent.throw(_("Please mention Basic and HRA component in Company"))
	annual_exemption, monthly_exemption, hra_amount = 0, 0, 0
	if hra_component and basic_component:
		assignment = get_salary_assignment(doc.employee, nowdate())
		if assignment:
			hra_component_exists = dataent.db.exists("Salary Detail", {
			...
			dataent.throw(_("Salary Structure must be submitted before submission of Tax Ememption Declaration"))
...
def calculate_hra_exemption(salary_structure, basic, monthly_hra, monthly_house_rent, rented_in_metro_city):
	...
	exemptions = []
	frequency = dataent.get_value("Salary Structure", salary_structure, "payroll_frequency")
...
def validate_house_rent_dates(doc):
	if not doc.rented_to_date or not doc.rented_from_date:
		dataent.throw(_("House rented dates required for exemption calculation"))

	if date_diff(doc.rented_to_date, doc.rented_from_date) < 14:
		dataent.throw(_("House rented dates should be atleast 15 days apart"))

	proofs = dataent.db.sql("""
		select name
	...
	if proofs:
		dataent.throw(_("House rent paid days overlapping with {0}").format(proofs[0][0]))
...
def get_ewb_data(dt, dn):
	if dt != 'Sales Invoice':
		dataent.throw(_('e-Way Bill JSON can only be generated from Sales Invoice'))

	dn = dn.split(',')

	ewaybills = []
	for doc_name in dn:
		doc = dataent.get_doc(dt, doc_name)
		...
		if doc.invoice_type in ['Regular', 'SEZ']:
			...
			dataent.throw(_('Unsupported Invoice Type for e-Way Bill JSON generation'))

		data.docType = 'INV'
		data.docDate = dataent.utils.formatdate(doc.posting_date, 'dd/mm/yyyy')

		company_address = dataent.get_doc('Address', doc.company_address)
		billing_address = dataent.get_doc('Address', doc.customer_address)

		shipping_address = dataent.get_doc('Address', doc.shipping_address_name)
		...
		disable_rounded = dataent.db.get_single_value('Global Defaults', 'disable_rounded_total')
...
def generate_ewb_json(dt, dn):
	...
	dataent.local.response.filecontent = json.dumps(data, indent=4, sort_keys=True)
	...
	dataent.local.response.filename = '{0}_e-WayBill_Data_{1}.json'.format(doc_name, dataent.utils.random_string(5))

@dataent.whitelist()
def get_gstins_for_company(company):
	company_gstins =[]
	if company:
		company_gstins = dataent.db.sql("""select
...
def get_item_list(data, doc):
	...
	for hsn_code, taxable_amount in hsn_taxable_amount.items():
		item_data = dataent._dict()
		if not hsn_code:
			dataent.throw(_('GST HSN Code does not exist for one or more items'))
...
def validate_sales_invoice(doc):
	if doc.docstatus != 1:
		dataent.throw(_('e-Way Bill JSON can only be generated from submitted document'))

	if doc.is_return:
		dataent.throw(_('e-Way Bill JSON cannot be generated for Sales Return as of now'))

	if doc.ewaybill:
		dataent.throw(_('e-Way Bill already exists for this document'))
	...
	for fieldname in reqd_fields:
		if not doc.get(fieldname):
			dataent.throw(_('{} is required to generate e-Way Bill JSON'.format(
				doc.meta.get_label(fieldname)
			)))

	if len(doc.company_gstin) < 15:
		dataent.throw(_('You must be a registered supplier to generate e-Way Bill'))

def get_transport_details(data, doc):
	if doc.distance > 4000:
		dataent.throw(_('Distance cannot be greater than 4000 kms'))
	...
	if doc.mode_of_transport == 'Road':
		if not doc.gst_transporter_id and not doc.vehicle_no:
			dataent.throw(_('Either GST Transporter ID or Vehicle No is required if Mode of Transport is Road'))
		if doc.vehicle_no:
			data.vehicleNo = doc.vehicle_no.replace(' ', '')
		if not doc.gst_vehicle_type:
			dataent.throw(_('Vehicle Type is required if Mode of Transport is Road'))
		else:
			data.vehicleType = vehicle_types.get(doc.gst_vehicle_type)
	else:
		if not doc.lr_no or not doc.lr_date:
			dataent.throw(_('Transport Receipt No and Date are mandatory for your chosen Mode of Transport'))

	if doc.lr_no:
		data.transDocNo = doc.lr_no

	if doc.lr_date:
		data.transDocDate = dataent.utils.formatdate(doc.lr_date, 'dd/mm/yyyy')
...
def validate_pincode(pincode, address):
	...
	if not pincode:
		dataent.throw(_(pin_not_found.format(address)))

	pincode = pincode.replace(' ', '')
	if not pincode.isdigit() or len(pincode) != 6:
		dataent.throw(_(incorrect_pin.format(address)))
	else:
		return int(pincode)
...
def validate_state_code(state_code, address):
	...
	if not state_code:
		dataent.throw(_(no_state_code.format(address)))
	else:
