...
from dataent.permissions import add_permission, update_permission_property
...
def setup_company_independent_fixtures():
	...
	dataent.enqueue('epaas.regional.india.setup.add_hsn_sac_codes', now=dataent.flags.in_test)
	add_print_formats()
...
def update_address_template():
	...
	address_template = dataent.db.get_value('Address Template', 'India')
	if address_template:
		dataent.db.set_value('Address Template', 'India', 'template', html)
	else:
...
def create_hsn_codes(data, code_field):
	for d in data:
		hsn_code = dataent.new_doc('GST HSN Code')
...
def add_print_formats():
	dataent.reload_doc("regional", "print_format", "gst_tax_invoice")
	dataent.reload_doc("accounts", "print_format", "gst_pos_invoice")

	dataent.db.sql(""" update `tabPrint Format` set disabled = 0 where
...
def make_fixtures(company=None):
	docs = []
	company = company.name if company else dataent.db.get_value("Global Defaults", None, "default_company")
