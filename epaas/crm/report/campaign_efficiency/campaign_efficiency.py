...
def get_columns(based_on):
	return [
		{
			"fieldname": dataent.scrub(based_on),
			"label": _(based_on),
			"fieldtype": "Data",
			"width": 150
		},
...
def get_lead_data(filters, based_on):
	based_on_field = dataent.scrub(based_on)
	conditions = get_filter_conditions(filters)

	lead_details = dataent.db.sql("""
...
def get_order_amount(leads):
	return dataent.db.sql("""select sum(base_net_amount)
