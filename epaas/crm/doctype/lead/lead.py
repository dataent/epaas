...
from dataent.utils import (cstr, validate_email_add, cint, comma_and, has_gravatar, now, getdate, nowdate)
...
class Lead(SellingController):
	...
	def onload(self):
		customer = dataent.db.get_value("Customer", {"lead_name": self.name})
	...
	def validate(self):
		...
		if self.email_id:
			...
			if self.email_id == self.contact_by:
				dataent.throw(_("Next Contact By cannot be same as the Lead Email Address"))
		...
		if self.ends_on and self.contact_date and\
			...
			dataent.throw(_("Ends On date cannot be before Next Contact Date."))
	...
	def check_email_id_is_unique(self):
		if self.email_id:
			...
			duplicate_leads = dataent.db.sql_list("""select name from tabLead
				where email_id=%s and name!=%s""", (self.email_id, self.name))

			if duplicate_leads:
				dataent.throw(_("Email Address must be unique, already exists for {0}")
					.format(comma_and(duplicate_leads)), dataent.DuplicateEntryError)

	def on_trash(self):
		dataent.db.sql("""update `tabIssue` set lead='' where lead=%s""",
			self.name)

		self.delete_events()

	def has_customer(self):
		return dataent.db.get_value("Customer", {"lead_name": self.name})

	def has_opportunity(self):
		return dataent.db.get_value("Opportunity", {"party_name": self.name, "status": ["!=", "Lost"]})

	def has_quotation(self):
		return dataent.db.get_value("Quotation", {
	...
	def set_lead_name(self):
		if not self.lead_name:
			...
			if not self.company_name and not self.flags.ignore_mandatory:
				dataent.throw(_("A Lead requires either a person's name or an organization's name"))
...
def make_lead_from_communication(communication, ignore_communication_links=False):
	...
	lead_name = None
	if doc.sender:
		lead_name = dataent.db.get_value("Lead", {"email_id": doc.sender})
	if not lead_name and doc.phone_no:
		lead_name = dataent.db.get_value("Lead", {"mobile_no": doc.phone_no})
	if not lead_name:
