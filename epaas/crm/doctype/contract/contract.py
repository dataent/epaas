...
from dataent.utils import getdate, now_datetime, nowdate
...
class Contract(Document):
	def autoname(self):
		...
		if dataent.db.exists("Contract", name):
			count = len(dataent.get_all("Contract", filters={"name": ["like", "%{}%".format(name)]}))
...
def update_status_for_contracts():
	...
	contracts = dataent.get_all("Contract",
	...
	for contract in contracts:
		...
		dataent.db.set_value("Contract", contract.get("name"), "status", status)
