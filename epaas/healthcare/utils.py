...
import math
from dataent.utils import time_diff_in_hours, rounded, getdate, add_days
from epaas.healthcare.doctype.healthcare_settings.healthcare_settings import get_income_account
from epaas.healthcare.doctype.fee_validity.fee_validity import create_fee_validity, update_fee_validity
from epaas.healthcare.doctype.lab_test.lab_test import create_multiple
...
def get_healthcare_services_to_invoice(patient):
	...
	if patient:
		if patient.customer:
			item_to_invoice = []
			patient_appointments = dataent.get_list("Patient Appointment",{'patient': patient.name, 'invoiced': False},
			...
			if patient_appointments:
				...
				for patient_appointment in patient_appointments:
					patient_appointment_obj = dataent.get_doc("Patient Appointment", patient_appointment['name'])

					if patient_appointment_obj.procedure_template:
						if dataent.db.get_value("Clinical Procedure Template", patient_appointment_obj.procedure_template, "is_billable") == 1:
						...
						if not practitioner_exist_in_list:
							...
							if validity_exist:
								fee_validity = dataent.get_doc("Fee Validity", validity_exist[0][0])
			...
			encounters = dataent.get_list("Patient Encounter", {'patient': patient.name, 'invoiced': False, 'docstatus': 1})
			if encounters:
				for encounter in encounters:
					encounter_obj = dataent.get_doc("Patient Encounter", encounter['name'])
			...
			lab_tests = dataent.get_list("Lab Test", {'patient': patient.name, 'invoiced': False, 'docstatus': 1})
			if lab_tests:
				for lab_test in lab_tests:
					lab_test_obj = dataent.get_doc("Lab Test", lab_test['name'])
					if dataent.db.get_value("Lab Test Template", lab_test_obj.template, "is_billable") == 1:
						item_to_invoice.append({'reference_type': 'Lab Test', 'reference_name': lab_test_obj.name,
						'service': dataent.db.get_value("Lab Test Template", lab_test_obj.template, "item")})

			lab_rxs = dataent.db.sql("""select lp.name from `tabPatient Encounter` et, `tabLab Prescription` lp
			...
			if lab_rxs:
				for lab_rx in lab_rxs:
					...
					if rx_obj.lab_test_code and (dataent.db.get_value("Lab Test Template", rx_obj.lab_test_code, "is_billable") == 1):
						item_to_invoice.append({'reference_type': 'Lab Prescription', 'reference_name': rx_obj.name,
						'service': dataent.db.get_value("Lab Test Template", rx_obj.lab_test_code, "item")})

			procedures = dataent.get_list("Clinical Procedure", {'patient': patient.name, 'invoiced': False})
			if procedures:
				for procedure in procedures:
					procedure_obj = dataent.get_doc("Clinical Procedure", procedure['name'])
					if not procedure_obj.appointment:
						if procedure_obj.procedure_template and (dataent.db.get_value("Clinical Procedure Template", procedure_obj.procedure_template, "is_billable") == 1):
							item_to_invoice.append({'reference_type': 'Clinical Procedure', 'reference_name': procedure_obj.name,
							'service': dataent.db.get_value("Clinical Procedure Template", procedure_obj.procedure_template, "item")})

			procedure_rxs = dataent.db.sql("""select pp.name from `tabPatient Encounter` et,
			...
			if procedure_rxs:
				for procedure_rx in procedure_rxs:
					rx_obj = dataent.get_doc("Procedure Prescription", procedure_rx[0])
					if dataent.db.get_value("Clinical Procedure Template", rx_obj.procedure, "is_billable") == 1:
						item_to_invoice.append({'reference_type': 'Procedure Prescription', 'reference_name': rx_obj.name,
						'service': dataent.db.get_value("Clinical Procedure Template", rx_obj.procedure, "item")})
			...
			inpatient_services = dataent.db.sql("""select io.name, io.parent from `tabInpatient Record` ip,
			...
			if inpatient_services:
				for inpatient_service in inpatient_services:
					inpatient_occupancy = dataent.get_doc("Inpatient Occupancy", inpatient_service[0])
					service_unit_type = dataent.get_doc("Healthcare Service Unit Type", dataent.db.get_value("Healthcare Service Unit", inpatient_occupancy.service_unit, "service_unit_type"))
			...
			dataent.throw(_("The Patient {0} do not have customer refrence to invoice").format(patient.name))
...
def get_practitioner_service_item(practitioner, service_item_field):
	return dataent.db.get_value("Healthcare Practitioner", practitioner, service_item_field)

def get_healthcare_service_item(service_item_field):
	return dataent.db.get_value("Healthcare Settings", None, service_item_field)
...
def get_practitioner_charge(practitioner, is_ip):
	if is_ip:
		practitioner_charge = dataent.db.get_value("Healthcare Practitioner", practitioner, "inpatient_visit_charge")
	else:
		practitioner_charge = dataent.db.get_value("Healthcare Practitioner", practitioner, "op_consulting_charge")
	...
	if doc.items:
		for item in doc.items:
			if item.get("reference_dt") and item.get("reference_dn"):
				if dataent.get_meta(item.reference_dt).has_field("invoiced"):
					set_invoiced(item, method, doc.name)

	if method=="on_submit" and dataent.db.get_value("Healthcare Settings", None, "create_test_on_si_submit") == '1':
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == 'Clinical Procedure':
		if get_healthcare_service_item('clinical_procedure_consumable_item') == item.item_code:
			dataent.db.set_value(item.reference_dt, item.reference_dn, "consumption_invoiced", invoiced)
		else:
			dataent.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)
	else:
		dataent.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)

	if item.reference_dt == 'Patient Appointment':
		if dataent.db.get_value('Patient Appointment', item.reference_dn, 'procedure_template'):
...
def validate_invoiced_on_submit(item):
	if item.reference_dt == 'Clinical Procedure' and get_healthcare_service_item('clinical_procedure_consumable_item') == item.item_code:
			is_invoiced = dataent.db.get_value(item.reference_dt, item.reference_dn, "consumption_invoiced")
	else:
		is_invoiced = dataent.db.get_value(item.reference_dt, item.reference_dn, "invoiced")
	if is_invoiced == 1:
		dataent.throw(_("The item referenced by {0} - {1} is already invoiced"\
		).format(item.reference_dt, item.reference_dn))

def manage_prescriptions(invoiced, ref_dt, ref_dn, dt, created_check_field):
	created = dataent.db.get_value(ref_dt, ref_dn, created_check_field)
	if created == 1:
		# Fetch the doc created for the prescription
		doc_created = dataent.db.get_value(dt, {'prescription': ref_dn})
		dataent.db.set_value(dt, doc_created, 'invoiced', invoiced)
...
def manage_fee_validity(appointment_name, method, ref_invoice=None):
	appointment_doc = dataent.get_doc("Patient Appointment", appointment_name)
		...
		if (fee_validity.valid_till >= appointment_doc.appointment_date):
			if (method == "on_cancel" and appointment_doc.status != "Closed"):
				if ref_invoice == fee_validity.ref_invoice:
					...
					if visited < 0:
						visited = 0
					dataent.db.set_value("Fee Validity", fee_validity.name, "visited", visited)
				do_not_update = True
			...
			if (method == "on_cancel" and appointment.status != "Closed"):
				if ref_invoice == fee_validity.ref_invoice:
					...
					if visited < 0:
						visited = 0
					dataent.db.set_value("Fee Validity", fee_validity.name, "visited", visited)
				dataent.db.set_value("Patient Appointment", appointment.name, "invoiced", False)
				...
				if ref_invoice == fee_validity.ref_invoice:
					visited = visited + 1
					dataent.db.set_value("Fee Validity", fee_validity.name, "visited", visited)
				dataent.db.set_value("Patient Appointment", appointment.name, "invoiced", True)
				manage_doc_for_appoitnment("Patient Encounter", appointment.name, True)
			if ref_invoice == fee_validity.ref_invoice:
				visit = visit + 1

	if method == "on_cancel":
		ref_invoice_in_fee_validity = dataent.db.get_value("Fee Validity", fee_validity.name, 'ref_invoice')
		if ref_invoice_in_fee_validity == ref_invoice:
			dataent.delete_doc("Fee Validity", fee_validity.name)
...
def appointments_valid_in_fee_validity(appointment, invoiced):
	...
	return dataent.get_list("Patient Appointment",{'patient': appointment.patient, 'invoiced': invoiced,
...
def manage_doc_for_appoitnment(dt_from_appointment, appointment, invoiced):
	dn_from_appointment = dataent.db.exists(
		dt_from_appointment,
		{
	...
	if dn_from_appointment:
		dataent.db.set_value(dt_from_appointment, dn_from_appointment, "invoiced", invoiced)

@dataent.whitelist()
def get_drugs_to_invoice(encounter):
	encounter = dataent.get_doc("Patient Encounter", encounter)
	if encounter:
		patient = dataent.get_doc("Patient", encounter.patient)
		if patient and patient.customer:
				item_to_invoice = []
				for drug_line in encounter.drug_prescription:
					if drug_line.drug_code:
						qty = 1
						if dataent.db.get_value("Item", drug_line.drug_code, "stock_uom") == "Nos":
...
def get_children(doctype, parent, company, is_root=False):
	...
	hc_service_units = dataent.get_list(doctype, fields=fields, filters=filters)
