dataent.pages['appointment-analytic'].on_page_load = function(wrapper) {
	dataent.ui.make_app_page({
		parent: wrapper,
		title: 'Appointment Analytics',
		single_column: true
	});
	new epaas.AppointmentAnalytics(wrapper);
	dataent.breadcrumbs.add("Medical");
};

epaas.AppointmentAnalytics = dataent.views.TreeGridReport.extend({
	...
	prepare_data: function() {
		var me = this;
		if (!this.tl) {
			this.tl = dataent.report_dump.data["Patient Appointment"];
		}
		if(!this.data || me.item_type != me.tree_type) {
			var items = null;
			if(me.tree_type=='Healthcare Practitioner') {
				items = dataent.report_dump.data["Healthcare Practitioner"];
	...
	prepare_balances: function() {
		var me = this;
		var from_date = dataent.datetime.str_to_obj(this.from_date);
		var status = this.status;
		var type = this.type;
		var to_date = dataent.datetime.str_to_obj(this.to_date);
		$.each(this.tl, function(i, tl) {
			if (me.is_default('company') ? true : tl.company === me.company) {

				var date = dataent.datetime.str_to_obj(tl.appointment_date);
