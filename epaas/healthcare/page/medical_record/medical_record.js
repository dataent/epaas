dataent.provide("dataent.medical_record");
dataent.pages['medical_record'].on_page_load = function(wrapper) {
	var me = this;
	...
	dataent.breadcrumbs.add("Medical");

	page.main.html(dataent.render_template("patient_select", {}));
	var patient = dataent.ui.form.make_control({
		...
		df: {
			...
			fieldname: "patient",
			change: function(){
				page.main.find(".dataent-list").html("");
	...
	this.page.sidebar.on("click", ".edit-details", function() {
		...
		if (patient) {
			dataent.set_route(["Form", "Patient", patient]);
		}
	});

};

dataent.pages['medical_record'].refresh = function() {
	var me = this;

	if(dataent.route_options) {
		if(dataent.route_options.patient){
			me.page.main.find(".dataent-list").html("");
			var patient = dataent.route_options.patient;
...
var draw_page = function(patient, me){
	dataent.model.with_doctype("Patient Medical Record", function() {
		me.page.list = new dataent.ui.BaseList({
			hide_refresh: true,
			page: me.page,
			method: 'epaas.healthcare.page.medical_record.medical_record.get_feed',
			args: {name: patient},
			...
			render_view: function(values) {
				...
				values.map(function (value) {
					...
					new dataent.medical_record.Feed(row, value);
				});
			},
			show_filters: true,
...
dataent.medical_record.last_feed_date = false;
dataent.medical_record.Feed = Class.extend({
	init: function(row, data) {
		...
		$(row)
			.append(dataent.render_template("medical_record_row", data))
			.find("a").addClass("grey");
	},
	scrub_data: function(data) {
		data.by = dataent.user.full_name(data.owner);
		data.imgsrc = dataent.utils.get_file_link(dataent.user_info(data.owner).image);

		data.icon = "icon-flag";
	},
	add_date_separator: function(row, data) {
		var date = dataent.datetime.str_to_obj(data.creation);
		var last = dataent.medical_record.last_feed_date;
		...
		dataent.medical_record.last_feed_date = date;
	}
});
