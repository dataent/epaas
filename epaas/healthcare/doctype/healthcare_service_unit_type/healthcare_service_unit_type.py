	...
	def validate(self):
		if self.is_billable == 1:
			if not self.uom or not self.item_group or not self.description or not self.no_of_hours > 0:
				dataent.throw(_("Configure Item Fields like UOM, Item Group, Description and No of Hours."))
	...
	def on_trash(self):
		if(self.item):
			try:
				...
				dataent.throw(_("""Not permitted. Please disable the Service Unit Type"""))
	...
	def on_update(self):
		if(self.change_in_item and self.is_billable == 1 and self.item):
			...
			if not item_price:
				...
				dataent.db.set_value("Item Price", item_price, "price_list_rate", self.rate)
...
def updating_item(doc):
	dataent.db.sql("""update `tabItem` set item_name=%s, item_group=%s, disabled=0, standard_rate=%s,
...
def create_item(doc):
	...
	#Set item to the Doc
	dataent.db.set_value("Healthcare Service Unit Type", doc.name, "item", item.name)
...
def change_item_code(item, item_code, doc_name):
	...
	if(item_exist):
		...
		dataent.db.set_value("Healthcare Service Unit Type", doc_name, "item_code", item_code)

@dataent.whitelist()
def disable_enable(status, doc_name, item=None,  is_billable=None):
	dataent.db.set_value("Healthcare Service Unit Type", doc_name, "disabled", status)
	if(is_billable == 1):
		dataent.db.set_value("Item", item, "disabled", status)
