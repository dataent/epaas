...
from dataent.utils import cint, flt, nowdate, nowtime, cstr
from epaas.healthcare.doctype.healthcare_settings.healthcare_settings import get_account
from epaas.healthcare.doctype.lab_test.lab_test import create_sample_doc
...
class ClinicalProcedure(Document):
	def validate(self):
		if self.consume_stock and not self.status == 'Draft':
			if not self.warehouse:
				dataent.throw(_("Set warehouse for Procedure {0} ").format(self.name))
			self.set_actual_qty()
	...
	def after_insert(self):
		if self.prescription:
			dataent.db.set_value("Procedure Prescription", self.prescription, "procedure_created", 1)
		if self.appointment:
			dataent.db.set_value("Patient Appointment", self.appointment, "status", "Closed")
		template = dataent.get_doc("Clinical Procedure Template", self.procedure_template)
		if template.sample:
			patient = dataent.get_doc("Patient", self.patient)
			sample_collection = create_sample_doc(template, patient, None)
			dataent.db.set_value("Clinical Procedure", self.name, "sample", sample_collection.name)
		self.reload()
...
def get_item_dict(table, parent, parenttype):
	...
	return dataent.db.sql(query.format(table=table, parent=parent, parenttype=parenttype), as_dict=True)
...
def create_stock_entry(doc):
	...
	for item_line in stock_entry.items:
		cost_center = dataent.get_cached_value('Company',  doc.company,  'cost_center')
		...
		#	expense_account = dataent.db.get_value("Item", item_line.item_code, "expense_account")
...
def create_procedure(appointment):
	...
	procedure = dataent.new_doc("Clinical Procedure")
	...
	consume_stock = dataent.db.get_value("Clinical Procedure Template", appointment.procedure_template, "consume_stock")
	if consume_stock == 1:
		procedure.consume_stock = True
		warehouse = False
		if appointment.service_unit:
			warehouse = dataent.db.get_value("Healthcare Service Unit", appointment.service_unit, "warehouse")
		if not warehouse:
			warehouse = dataent.db.get_value("Stock Settings", None, "default_warehouse")
		if warehouse:
