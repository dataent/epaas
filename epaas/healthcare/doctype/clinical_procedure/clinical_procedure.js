...
dataent.ui.form.on('Clinical Procedure', {
	setup: function(frm) {
			...
			if(!item.item_code) {
				...
				if (frm.doc.status == 'In Progress') {
					var filters = {
						...
						'posting_date': frm.doc.start_date || dataent.datetime.nowdate()
					};
	...
	practitioner: function(frm) {
		if(frm.doc.practitioner){
			dataent.call({
				...
				callback: function (data) {
					dataent.model.set_value(frm.doctype,frm.docname, "medical_department",data.message.department);
				}
			});
		}
	}
...
dataent.ui.form.on('Clinical Procedure Item', {
	qty: function(frm, cdt, cdn){
		var d = locals[cdt][cdn];
		dataent.model.set_value(cdt, cdn, "transfer_qty", d.qty*d.conversion_factor);
	},
	...
	item_code: function(frm, cdt, cdn) {
		...
		if(d.item_code) {
			...
			return dataent.call({
				...
				callback: function(r) {
					if(r.message) {
						dataent.model.set_value(cdt, cdn, "item_name", r.message.item_name);
						dataent.model.set_value(cdt, cdn, "stock_uom", r.message.stock_uom);
						dataent.model.set_value(cdt, cdn, "conversion_factor", r.message.conversion_factor);
						dataent.model.set_value(cdt, cdn, "actual_qty", r.message.actual_qty);
						refresh_field("items");
					}
				}
			});
