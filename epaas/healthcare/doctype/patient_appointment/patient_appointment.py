...
import json
from dataent.utils import getdate, add_days, get_time
from dataent import _
import datetime
from dataent.core.doctype.sms_settings.sms_settings import send_sms
from epaas.hr.doctype.employee.employee import is_holiday
from epaas.healthcare.doctype.healthcare_settings.healthcare_settings import get_receivable_account,get_income_account
from epaas.healthcare.utils import validity_exists, service_item_and_practitioner_charge
...
class PatientAppointment(Document):
	def on_update(self):
		...
		if today == appointment_date:
			dataent.db.set_value("Patient Appointment", self.name, "status", "Open")
			self.reload()
	...
	def after_insert(self):
		if self.procedure_prescription:
			dataent.db.set_value("Procedure Prescription", self.procedure_prescription, "appointment_booked", True)
			if self.procedure_template:
				comments = dataent.db.get_value("Procedure Prescription", self.procedure_prescription, "comments")
				if comments:
					dataent.db.set_value("Patient Appointment", self.name, "notes", comments)
		...
		if validity_exist:
			...
			if (fee_validity.valid_till >= appointment_date) and (fee_validity.visited < fee_validity.max_visit):
				...
				dataent.msgprint(_("{0} has fee validity till {1}").format(appointment.patient, fee_validity.valid_till))
		confirm_sms(self)

		if dataent.db.get_value("Healthcare Settings", None, "manage_appointment_invoice_automatically") == '1' and \
			dataent.db.get_value("Patient Appointment", self.name, "invoiced") != 1:
...
def appointment_cancel(appointment_id):
	appointment = dataent.get_doc("Patient Appointment", appointment_id)
	...
	if appointment.invoiced:
		...
		if sales_invoice and cancel_sales_invoice(sales_invoice):
			...
			if validity:
				fee_validity = dataent.get_doc("Fee Validity", validity[0][0])
				if appointment_valid_in_fee_validity(appointment, fee_validity.valid_till, True, fee_validity.ref_invoice):
					...
					dataent.msgprint(_("Appointment cancelled"))
			else:
				dataent.msgprint(_("Appointment cancelled"))
	else:
		dataent.msgprint(_("Appointment cancelled"))
...
def appointment_valid_in_fee_validity(appointment, valid_end_date, invoiced, ref_invoice):
	...
	appointments = dataent.get_list("Patient Appointment",{'patient': appointment.patient, 'invoiced': invoiced,
...
def cancel_sales_invoice(sales_invoice):
	if dataent.db.get_value("Healthcare Settings", None, "manage_appointment_invoice_automatically") == '1':
...
def exists_sales_invoice(appointment):
	...
	if sales_item_exist:
		sales_invoice = dataent.get_doc("Sales Invoice", dataent.db.get_value("Sales Invoice Item", sales_item_exist, "parent"))
...
def get_availability_data(date, practitioner):
	...
	practitioner_obj = dataent.get_doc("Healthcare Practitioner", practitioner)
	...
	if practitioner_obj.employee:
		...
		if dataent.db.exists({
			...
			employee = dataent.get_doc("Employee", {"user_id": practitioner_obj.user_id}).name

	if employee:
		# Check if it is Holiday
		if is_holiday(employee, date):
			dataent.throw(_("{0} is a company holiday".format(date)))

		# Check if He/She on Leave
		leave_record = dataent.db.sql("""select half_day from `tabLeave Application`
			where employee = %s and %s between from_date and to_date
			and docstatus = 1""", (employee, date), as_dict=True)
		if leave_record:
			if leave_record[0].half_day:
				dataent.throw(_("{0} on Half day Leave on {1}").format(practitioner, date))
			else:
				dataent.throw(_("{0} on Leave on {1}").format(practitioner, date))
	...
	if practitioner_obj.practitioner_schedules:
		for schedule in practitioner_obj.practitioner_schedules:
			if schedule.schedule:
				practitioner_schedule = dataent.get_doc("Practitioner Schedule", schedule.schedule)
			else:
				dataent.throw(_("{0} does not have a Healthcare Practitioner Schedule. Add it in Healthcare Practitioner master".format(practitioner)))
			...
			if practitioner_schedule:
				...
				if available_slots:
					...
					if schedule.service_unit:
						...
						allow_overlap = dataent.get_value('Healthcare Service Unit', schedule.service_unit, 'overlap_appointments')
						if allow_overlap:
							# fetch all appointments to practitioner by service unit
							appointments = dataent.get_all(
								"Patient Appointment",
	...
	if not available_slots and not slot_details:
		...
		dataent.throw(_("Healthcare Practitioner not available on {0}").format(weekday))
...
def update_status(appointment_id, status):
	dataent.db.set_value("Patient Appointment", appointment_id, "status", status)
	appointment_booked = True
	if status == "Cancelled":
		appointment_booked = False
		appointment_cancel(appointment_id)

	procedure_prescription = dataent.db.get_value("Patient Appointment", appointment_id, "procedure_prescription")
	if procedure_prescription:
		dataent.db.set_value("Procedure Prescription", procedure_prescription, "appointment_booked", appointment_booked)
...
def confirm_sms(doc):
	if dataent.db.get_value("Healthcare Settings", None, "app_con") == '1':
		message = dataent.db.get_value("Healthcare Settings", None, "app_con_msg")
		send_message(doc, message)

@dataent.whitelist()
def create_encounter(appointment):
	appointment = dataent.get_doc("Patient Appointment", appointment)
	encounter = dataent.new_doc("Patient Encounter")
...
def remind_appointment():
	if dataent.db.get_value("Healthcare Settings", None, "app_rem") == '1':
		rem_before = datetime.datetime.strptime(dataent.get_value("Healthcare Settings", None, "rem_before"), "%H:%M:%S")
		rem_dt = datetime.datetime.now() + datetime.timedelta(
			hours=rem_before.hour, minutes=rem_before.minute, seconds=rem_before.second)

		appointment_list = dataent.db.sql(
		...
		for i in range(0, len(appointment_list)):
			doc = dataent.get_doc("Patient Appointment", appointment_list[i][0])
			message = dataent.db.get_value("Healthcare Settings", None, "app_rem_msg")
			send_message(doc, message)
			dataent.db.set_value("Patient Appointment", doc.name, "reminded",1)


def send_message(doc, message):
	patient = dataent.get_doc("Patient", doc.patient)
	if patient.mobile:
		...
		message = dataent.render_template(message, context)
...
def get_procedure_prescribed(patient):
	return dataent.db.sql("""select pp.name, pp.procedure, pp.parent, ct.practitioner,
