...
dataent.ui.form.on('Patient Appointment', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		dataent.db.get_value('Healthcare Settings', {name: 'Healthcare Settings'}, 'manage_appointment_invoice_automatically', (r) => {
...
var check_and_set_availability = function(frm) {
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')){
			...
			dataent.call({
				method: 'epaas.healthcare.doctype.patient_appointment.patient_appointment.get_availability_data',
				args: {
...
var get_procedure_prescribed = function(frm){
	if(frm.doc.patient){
		dataent.call({
			method:"epaas.healthcare.doctype.patient_appointment.patient_appointment.get_procedure_prescribed",
		...
		dataent.msgprint("Please select Patient to get prescribed procedure");
	}
};
...
var btn_create_procedure = function(frm){
	var doc = frm.doc;
	dataent.call({
		method:"epaas.healthcare.doctype.clinical_procedure.clinical_procedure.create_procedure",
		args: {appointment: doc.name},
		callback: function(data){
			if(!data.exc){
				var doclist = dataent.model.sync(data.message);
...
var btn_create_encounter = function(frm){
	var doc = frm.doc;
	dataent.call({
		method:"epaas.healthcare.doctype.patient_appointment.patient_appointment.create_encounter",
		args: {appointment: doc.name},
		callback: function(data){
			if(!data.exc){
				var doclist = dataent.model.sync(data.message);
...
var btn_update_status = function(frm, status){
	var doc = frm.doc;
	dataent.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			dataent.call({
				method:
				"epaas.healthcare.doctype.patient_appointment.patient_appointment.update_status",
...
dataent.ui.form.on("Patient Appointment", "practitioner", function(frm) {
	if(frm.doc.practitioner){
		dataent.call({
			...
			callback: function (data) {
				dataent.model.set_value(frm.doctype,frm.docname, "department",data.message.department);
				dataent.model.set_value(frm.doctype,frm.docname, "paid_amount",data.message.op_consulting_charge);
			}
		});
	}
});

dataent.ui.form.on("Patient Appointment", "patient", function(frm) {
	if(frm.doc.patient){
		dataent.call({
			"method": "dataent.client.get",
			args: {
...
dataent.ui.form.on("Patient Appointment", "appointment_type", function(frm) {
	if(frm.doc.appointment_type) {
		dataent.call({
			...
			callback: function (data) {
				dataent.model.set_value(frm.doctype,frm.docname, "duration",data.message.default_duration);
			}
		});
	}
});
