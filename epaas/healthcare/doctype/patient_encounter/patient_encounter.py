...
class PatientEncounter(Document):
	...
	def on_cancel(self):
		if(self.appointment):
			dataent.db.set_value("Patient Appointment", self.appointment, "status", "Open")
...
def update_encounter_to_medical_record(encounter):
	medical_record_id = dataent.db.sql("select name from `tabPatient Medical Record` where reference_name=%s", (encounter.name))
	if medical_record_id and medical_record_id[0][0]:
		subject = set_subject_field(encounter)
		dataent.db.set_value("Patient Medical Record", medical_record_id[0][0], "subject", subject)
	else:
		insert_encounter_to_medical_record(encounter)

def delete_medical_record(encounter):
	dataent.db.sql("""delete from `tabPatient Medical Record` where reference_name = %s""", (encounter.name))
