...
dataent.ui.form.on('Lab Test', {
	...
	refresh :  function(frm){
		...
		if(frm.doc.docstatus==1	&&	frm.doc.status!='Approved'	&&	frm.doc.status!='Rejected'	&&	dataent.defaults.get_default("require_test_result_approval")	&&	dataent.user.has_role("LabTest Approver")){
		...
		if(frm.doc.docstatus==1 && frm.doc.sms_sent==0){
			frm.add_custom_button(__('Send SMS'), function() {
				dataent.call({
					method: "epaas.healthcare.doctype.healthcare_settings.healthcare_settings.get_sms_text",
		...
		if(frm.doc.employee){
			dataent.call({
				...
				callback: function(arg){
					dataent.model.set_value(frm.doctype,frm.docname,"employee_name", arg.message.employee_name);
					dataent.model.set_value(frm.doctype,frm.docname,"employee_designation", arg.message.designation);
				}
			});
		}
	}
...
dataent.ui.form.on("Lab Test", "patient", function(frm) {
	if(frm.doc.patient){
		dataent.call({
			...
			callback: function (data) {
				...
				dataent.model.set_value(frm.doctype,frm.docname, "email", data.message.email);
				dataent.model.set_value(frm.doctype,frm.docname, "mobile", data.message.mobile);
				dataent.model.set_value(frm.doctype,frm.docname, "report_preference", data.message.report_preference);
			}
		});
	}
});

dataent.ui.form.on('Normal Test Items', {
	normal_test_items_remove: function() {
		dataent.msgprint(__("Not permitted, configure Lab Test Template as required"));
		cur_frm.reload_doc();
	}
});

dataent.ui.form.on('Special Test Items', {
...
var status_update = function(approve,frm){
	...
	dataent.call({
		method: "epaas.healthcare.doctype.lab_test.lab_test.update_status",
...
var get_lab_test_prescribed = function(frm){
	if(frm.doc.patient){
		dataent.call({
			method:	"epaas.healthcare.doctype.lab_test.lab_test.get_lab_test_prescribed",
		...
		dataent.msgprint(__("Please select Patient to get Lab Tests"));
	}
};
	...
	if(doc.normal_test_items){
		for(let result in doc.normal_test_items){
			if(!doc.normal_test_items[result].result_value	&&	doc.normal_test_items[result].require_result_value == 1){
				dataent.msgprint(__("Please input all required Result Value(s)"));
				throw("Error");
			}
		}
	}
...
var send_sms = function(v,frm){
	...
	dataent.call({
		method: "dataent.core.doctype.sms_settings.sms_settings.send_sms",
		args: {
			receiver_list: [number],
			msg: messages
		},
		callback: function(r) {
			if(r.exc) {dataent.msgprint(r.exc); return; }
			else{
				dataent.call({
					method: "epaas.healthcare.doctype.lab_test.lab_test.update_lab_test_print_sms_email_status",
