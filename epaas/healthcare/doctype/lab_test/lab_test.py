...
from dataent.utils import getdate, cstr

class LabTest(Document):
	def on_submit(self):
		dataent.db.set_value(self.doctype,self.name,"submitted_date", getdate())
		insert_lab_test_to_medical_record(self)
		dataent.db.set_value("Lab Test", self.name, "status", "Completed")

	def on_cancel(self):
		delete_lab_test_from_medical_record(self)
		dataent.db.set_value("Lab Test", self.name, "status", "Cancelled")
		self.reload()
	...
	def after_insert(self):
		if(self.prescription):
			dataent.db.set_value("Lab Prescription", self.prescription, "lab_test_created", 1)
			if dataent.db.get_value("Lab Prescription", self.prescription, 'invoiced') == 1:
				self.invoiced = True
...
def create_test_from_template(lab_test):
	template = dataent.get_doc("Lab Test Template", lab_test.template)
	patient = dataent.get_doc("Patient", lab_test.patient)
...
def update_status(status, name):
	dataent.db.sql("""update `tabLab Test` set status=%s, approved_date=%s where name = %s""", (status, getdate(), name))

@dataent.whitelist()
def update_lab_test_print_sms_email_status(print_sms_email, name):
	dataent.db.set_value("Lab Test",name,print_sms_email,1)
...
def create_multiple(doctype, docname):
	...
	if lab_test_created:
		dataent.msgprint(_("Lab Test(s) "+lab_test_created+" created."))
	else:
		dataent.msgprint(_("No Lab Test created"))

def create_lab_test_from_encounter(encounter_id):
	lab_test_created = False
	encounter = dataent.get_doc("Patient Encounter", encounter_id)

	lab_test_ids = dataent.db.sql("""select lp.name, lp.lab_test_code, lp.invoiced
	...
	if lab_test_ids:
		...
		for lab_test_id in lab_test_ids:
			...
			if template:
				...
				dataent.db.set_value("Lab Prescription", lab_test_id[0], "lab_test_created", 1)
...
def create_lab_test_from_invoice(invoice_name):
	...
	if invoice.patient:
		patient = dataent.get_doc("Patient", invoice.patient)
		for item in invoice.items:
			lab_test_created = 0
			if item.reference_dt == "Lab Prescription":
				lab_test_created = dataent.db.get_value("Lab Prescription", item.reference_dn, "lab_test_created")
			...
			if lab_test_created != 1:
				...
				if template:
					...
					lab_test.save(ignore_permissions = True)
					if item.reference_dt != "Lab Prescription":
						dataent.db.set_value("Sales Invoice Item", item.name, "reference_dt", "Lab Test")
						dataent.db.set_value("Sales Invoice Item", item.name, "reference_dn", lab_test.name)
...
def get_lab_test_template(item):
	...
	if template_id:
		return dataent.get_doc("Lab Test Template", template_id)
	return False

def check_template_exists(item):
	template_exists = dataent.db.exists(
		"Lab Test Template",
		{
			'item': item
		}
...
def create_lab_test_doc(invoiced, practitioner, patient, template):
	lab_test = dataent.new_doc("Lab Test")
...
def create_sample_doc(template, patient, invoice):
	if(template.sample):
		sample_exist = dataent.db.exists({
		...
		if sample_exist :
			...
			sample_collection = dataent.get_doc("Sample Collection",sample_exist[0][0])
			...
			if(template.sample_collection_details):
				...
				dataent.db.set_value("Sample Collection", sample_collection.name, "sample_collection_details",sample_collection_details)
			dataent.db.set_value("Sample Collection", sample_collection.name, "sample_quantity",quantity)

		else:
			#create Sample Collection for template, copy vals from Invoice
			sample_collection = dataent.new_doc("Sample Collection")
			if(invoice):
...
def create_sample_collection(lab_test, template, patient, invoice):
	if(dataent.db.get_value("Healthcare Settings", None, "require_sample_collection") == "1"):
...
def load_result_format(lab_test, template, prescription, invoice):
	if(template.lab_test_template_type == 'Single'):
		...
		for lab_test_group in template.lab_test_groups:
			...
			if(lab_test_group.lab_test_template):
				template_in_group = dataent.get_doc("Lab Test Template",
	...
	if(template.lab_test_template_type != 'No Result'):
		if(prescription):
			...
			if(invoice):
				dataent.db.set_value("Lab Prescription", prescription, "invoiced", True)
		lab_test.save(ignore_permissions=True) # insert the result
		return lab_test

@dataent.whitelist()
def get_employee_by_user_id(user_id):
	emp_id = dataent.db.get_value("Employee",{"user_id":user_id})
	employee = dataent.get_doc("Employee",emp_id)
	return employee
...
def delete_lab_test_from_medical_record(self):
	medical_record_id = dataent.db.sql("select name from `tabPatient Medical Record` where reference_name=%s",(self.name))
...
def get_lab_test_prescribed(patient):
	return dataent.db.sql("""select cp.name, cp.lab_test_code, cp.parent, cp.invoiced, ct.practitioner, ct.encounter_date from `tabPatient Encounter` ct,
