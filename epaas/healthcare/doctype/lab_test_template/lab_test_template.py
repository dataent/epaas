...
class LabTestTemplate(Document):
	def on_update(self):
		...
		if(self.change_in_item and self.is_billable == 1 and self.item):
			...
			if not item_price:
				...
				dataent.db.set_value("Item Price", item_price, "price_list_rate", self.lab_test_rate)
	...
	def on_trash(self):
		...
		if(self.item):
			try:
				dataent.delete_doc("Item",self.item, force=True)
			except Exception:
				dataent.throw(_("""Not permitted. Please disable the Test Template"""))
...
def create_item_from_template(doc):
	...
	dataent.db.set_value("Lab Test Template", doc.name, "item", item.name)
...
def disable_enable_test_template(status, name,  is_billable):
	dataent.db.set_value("Lab Test Template",name,"disabled",status)
	if(is_billable == 1):
		dataent.db.set_value("Item",name,"disabled",status)
