...
dataent.ui.form.on("Lab Test Template",{
...
var disable_template = function(frm){
	var doc = frm.doc;
	dataent.call({
		method: 		"epaas.healthcare.doctype.lab_test_template.lab_test_template.disable_enable_test_template",
...
var change_template_code = function(frm,doc){
	...
	var change_test_code_from_template = function(lab_test_code,doc){
		dataent.call({
			"method": "epaas.healthcare.doctype.lab_test_template.lab_test_template.change_test_code_from_template",
			"args": {lab_test_code: lab_test_code, doc: doc},
			callback: function (data) {
				dataent.set_route("Form", "Lab Test Template", data.message);
			}
		});
	};
};

dataent.ui.form.on("Lab Test Template", "lab_test_name", function(frm){

	frm.doc.change_in_item = 1;

});
dataent.ui.form.on("Lab Test Template", "lab_test_rate", function(frm){

	frm.doc.change_in_item = 1;

});
dataent.ui.form.on("Lab Test Template", "lab_test_group", function(frm){

	frm.doc.change_in_item = 1;

});
dataent.ui.form.on("Lab Test Template", "lab_test_description", function(frm){

	frm.doc.change_in_item = 1;

});

dataent.ui.form.on("Lab Test Groups", "template_or_new_line", function (frm, cdt, cdn) {
	var child = locals[cdt][cdn];
	if(child.template_or_new_line =="Add new line"){
		dataent.model.set_value(cdt, cdn, 'lab_test_template', "");
		dataent.model.set_value(cdt, cdn, 'lab_test_description', "");
	}
});
