...
from dataent.utils import today, now_datetime
from dataent.model.document import Document
from dataent.desk.reportview import get_match_cond

class InpatientRecord(Document):
	def after_insert(self):
		dataent.db.set_value("Patient", self.patient, "inpatient_status", "Admission Scheduled")
		dataent.db.set_value("Patient", self.patient, "inpatient_record", self.name)

	def validate(self):
		self.validate_already_scheduled_or_admitted()
		if self.status == "Discharged":
			dataent.db.set_value("Patient", self.patient, "inpatient_status", None)
			dataent.db.set_value("Patient", self.patient, "inpatient_record", None)
	...
	def validate_already_scheduled_or_admitted(self):
		...
		ip_record = dataent.db.sql(query,{
				"name": self.name,
...
def schedule_discharge(patient, encounter_id=None, practitioner=None):
	inpatient_record_id = dataent.db.get_value('Patient', patient, 'inpatient_record')
	if inpatient_record_id:
		inpatient_record = dataent.get_doc("Inpatient Record", inpatient_record_id)
		...
		inpatient_record.save(ignore_permissions = True)
	dataent.db.set_value("Patient", patient, "inpatient_status", "Discharge Scheduled")
	...
	if inpatient_record.inpatient_occupancies:
		for inpatient_occupancy in inpatient_record.inpatient_occupancies:
			if inpatient_occupancy.left != 1:
				...
				dataent.db.set_value("Healthcare Service Unit", inpatient_occupancy.service_unit, "occupancy_status", "Vacant")
...
def validate_invoiced_inpatient(inpatient_record):
	...
	if pending_invoices:
		dataent.throw(_("Can not mark Inpatient Record Discharged, there are Unbilled Invoices {0}").format(", "
...
def get_inpatient_docs_not_invoiced(doc, inpatient_record):
	return dataent.db.get_list(doc, filters = {"patient": inpatient_record.patient,
...
def admit_patient(inpatient_record, service_unit, check_in, expected_discharge=None):
	...
	dataent.db.set_value("Patient", inpatient_record.patient, "inpatient_status", "Admitted")
	dataent.db.set_value("Patient", inpatient_record.patient, "inpatient_record", inpatient_record.name)
...
def transfer_patient(inpatient_record, service_unit, check_in):
	...
	dataent.db.set_value("Healthcare Service Unit", service_unit, "occupancy_status", "Occupied")
...
def get_leave_from(doctype, txt, searchfield, start, page_len, filters):
	...
	return dataent.db.sql(query.format(**{
		"docname":	docname,
