...
{% include 'epaas/accounts/doctype/sales_taxes_and_charges_template/sales_taxes_and_charges_template.js' %}


cur_frm.email_field = "contact_email";

dataent.provide("epaas.selling");
epaas.selling.SellingController = epaas.TransactionController.extend({
	setup: function() {
		this._super();
		...
		$.each([["customer", "customer"],
			...
			function(i, opts) {
				if(me.frm.fields_dict[opts[0]])
					me.frm.set_query(opts[0], epaas.queries[opts[1]]);
			});

		me.frm.set_query('contact_person', epaas.queries.contact_query);
		me.frm.set_query('customer_address', epaas.queries.address_query);
		me.frm.set_query('shipping_address_name', epaas.queries.address_query);
	...
	customer_address: function() {
		epaas.utils.get_address_display(this.frm, "customer_address");
	},

	shipping_address_name: function() {
		epaas.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
	},
	...
	total_commission: function() {
		if(this.frm.doc.base_net_total) {
			dataent.model.round_floats_in(this.frm.doc, ["base_net_total", "total_commission"]);

			if(this.frm.doc.base_net_total < this.frm.doc.total_commission) {
				var msg = (__("[Error]") + " " +
					__(dataent.meta.get_label(this.frm.doc.doctype, "total_commission",
						this.frm.doc.name)) + " > " +
					__(dataent.meta.get_label(this.frm.doc.doctype, "base_net_total", this.frm.doc.name)));
	...
	allocated_percentage: function(doc, cdt, cdn) {
		var sales_person = dataent.get_doc(cdt, cdn);
	...
	warehouse: function(doc, cdt, cdn) {
		...
		var has_batch_no;
		dataent.db.get_value('Item', {'item_code': item.item_code}, 'has_batch_no', (r) => {
			has_batch_no = r && r.has_batch_no;
			if(item.item_code && item.warehouse) {
				return this.frm.call({
					method: "epaas.stock.get_item_details.get_bin_details_and_serial_nos",
					child: item,
					args: {
	...
	toggle_editable_price_list_rate: function() {
		var df = dataent.meta.get_docfield(this.frm.doc.doctype + " Item", "price_list_rate", this.frm.doc.name);
		var editable_price_list_rate = cint(dataent.defaults.get_default("editable_price_list_rate"));
	...
	calculate_commission: function() {
		if(this.frm.fields_dict.commission_rate) {
			if(this.frm.doc.commission_rate > 100) {
				var msg = __(dataent.meta.get_label(this.frm.doc.doctype, "commission_rate", this.frm.doc.name)) +
	...
	calculate_contribution: function() {
		...
		$.each(this.frm.doc.doctype.sales_team || [], function(i, sales_person) {
			dataent.model.round_floats_in(sales_person);
			if(sales_person.allocated_percentage) {
	...
	batch_no: function(doc, cdt, cdn) {
		...
		item.serial_no = null;
		var has_serial_no;
		dataent.db.get_value('Item', {'item_code': item.item_code}, 'has_serial_no', (r) => {
			has_serial_no = r && r.has_serial_no;
			if(item.warehouse && item.item_code && item.batch_no) {
				return this.frm.call({
					method: "epaas.stock.get_item_details.get_batch_qty_and_serial_no",
					child: item,
					args: {
	...
	set_product_bundle_help: function(doc) {
		...
		if ((doc.packed_items || []).length) {
			...
			if (in_list(['Delivery Note', 'Sales Invoice'], doc.doctype)) {
				...
				"</div>";
				dataent.meta.get_docfield(doc.doctype, 'product_bundle_help', doc.name).options = help_msg;
			}
		} else {
			$(cur_frm.fields_dict.packing_list.row.wrapper).toggle(false);
			if (in_list(['Delivery Note', 'Sales Invoice'], doc.doctype)) {
				dataent.meta.get_docfield(doc.doctype, 'product_bundle_help', doc.name).options = '';
			}
		}
	...
	margin_type: function(doc, cdt, cdn){
		...
		var item = locals[cdt][cdn];
		if(!item.margin_type) {
			dataent.model.set_value(cdt, cdn, "margin_rate_or_amount", 0);
		} else {
	...
	*/
	set_batch_number: function(cdt, cdn) {
		const doc = dataent.get_doc(cdt, cdn);
	...
	_set_batch_number: function(doc) {
		return dataent.call({
			method: 'epaas.stock.doctype.batch.batch.get_batch_no',
			args: {'item_code': doc.item_code, 'warehouse': doc.warehouse, 'qty': flt(doc.qty) * flt(doc.conversion_factor)},
			callback: function(r) {
				if(r.message) {
					dataent.model.set_value(doc.doctype, doc.name, 'batch_no', r.message);
				} else {
				    dataent.model.set_value(doc.doctype, doc.name, 'batch_no', r.message);
				}
			}
...
dataent.ui.form.on(cur_frm.doctype,"project", function(frm) {
	if(in_list(["Delivery Note", "Sales Invoice"], frm.doc.doctype)) {
		if(frm.doc.project) {
			dataent.call({
				method:'epaas.projects.doctype.project.project.get_cost_center_name' ,
				args: {	project: frm.doc.project	},
				callback: function(r, rt) {
					if(!r.exc) {
						$.each(frm.doc["items"] || [], function(i, row) {
							if(r.message) {
								dataent.model.set_value(row.doctype, row.name, "cost_center", r.message);
								dataent.msgprint(__("Cost Center For Item with Item Code '"+row.item_name+"' has been Changed to "+ r.message));
							}
						})
					}
				}
