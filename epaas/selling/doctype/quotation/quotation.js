...
epaas.selling.QuotationController = epaas.selling.SellingController.extend({
	...
	refresh: function(doc, dt, dn) {
		...
		dataent.dynamic_link = {doc: this.frm.doc, fieldname: 'party_name', doctype: doctype}

		var me = this;

		if (doc.__islocal && !doc.valid_till) {
			if(dataent.boot.sysdefaults.quotation_valid_till){
				this.frm.set_value('valid_till', dataent.datetime.add_days(doc.transaction_date, dataent.boot.sysdefaults.quotation_valid_till));
			} else {
				this.frm.set_value('valid_till', dataent.datetime.add_months(doc.transaction_date, 1));
			}
		}

		if(doc.docstatus == 1 && doc.status!=='Lost') {
			if(!doc.valid_till || dataent.datetime.get_diff(doc.valid_till, dataent.datetime.get_today()) > 0) {
	...
	validate_company_and_party: function(party_field) {
		if(!this.frm.doc.quotation_to) {
			dataent.msgprint(__("Please select a value for {0} quotation_to {1}", [this.frm.doc.doctype, this.frm.doc.name]));
			return false;
	...
	get_lead_details: function() {
		...
		dataent.call({
			method: "epaas.crm.doctype.lead.lead.get_lead_details",
			args: {
...
cur_frm.script_manager.make(epaas.selling.QuotationController);
...
dataent.ui.form.on("Quotation Item", "items_on_form_rendered", "packed_items_on_form_rendered", function(frm, cdt, cdn) {
	// enable tax_amount field if Actual
})

dataent.ui.form.on("Quotation Item", "stock_balance", function(frm, cdt, cdn) {
	var d = dataent.model.get_doc(cdt, cdn);
	dataent.route_options = {"item_code": d.item_code};
