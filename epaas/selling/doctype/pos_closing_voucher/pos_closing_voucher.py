...
class POSClosingVoucher(Document):
	...
	def validate(self):
		user = dataent.get_all('POS Closing Voucher',
			filters = {
				'user': self.user,
				'docstatus': 1
			},
		...
		if user:
			dataent.throw(_("POS Closing Voucher alreday exists for {0} between date {1} and {2}"
	...
	def get_payment_reconciliation_details(self):
		...
		return dataent.render_template("epaas/selling/doctype/pos_closing_voucher/closing_voucher_details.html",
			{"data": self, "currency": currency})

@dataent.whitelist()
def get_cashiers(doctype, txt, searchfield, start, page_len, filters):
	cashiers_list = dataent.get_all("POS Profile User", filters=filters, fields=['user'])
...
def get_mode_of_payment_details(invoice_list):
	...
	if invoice_list:
		inv_mop_detail = dataent.db.sql("""select a.owner, a.posting_date,
		...
		inv_change_amount = dataent.db.sql("""select a.owner, a.posting_date,
...
def get_tax_details(invoice_list):
	...
	for invoice in invoice_list:
		doc = dataent.get_doc("Sales Invoice", invoice.name)
...
def get_company_currency(doc):
	...
	return dataent.get_doc('Currency', currency)

def get_invoices(filters):
	return dataent.db.sql("""select a.name, a.base_grand_total as grand_total,
