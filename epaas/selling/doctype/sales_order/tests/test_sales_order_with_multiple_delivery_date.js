...
QUnit.test("test: Sales Order", function (assert) {
	...
	let delivery_date = dataent.datetime.add_days(dataent.defaults.get_default("year_end_date"), 1);
	...
	dataent.run_serially([
			...
			return dataent.tests.make('Sales Order', [
				...
				{items: [
					...
					[
						...
						{"qty": 5},
						{'rate': 100},
						{'delivery_date': dataent.datetime.add_days(delivery_date, 5)}
					]]
				}
			])
		},
		...
		() => dataent.timeout(1),
		() => {
			assert.ok(cur_frm.doc.delivery_date == dataent.datetime.add_days(delivery_date, 5));
		},
		() => done()
	]);
});