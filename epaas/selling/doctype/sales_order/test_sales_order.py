...
from epaas.selling.doctype.sales_order.sales_order import make_work_orders
from epaas.controllers.accounts_controller import update_child_qty_rate
import json
from epaas.selling.doctype.sales_order.sales_order import make_raw_material_request
class TestSalesOrder(unittest.TestCase):
	...
	def test_make_material_request(self):
		...
		self.assertRaises(dataent.ValidationError, make_material_request, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery tolerance
		dataent.db.set_value('Item', "_Test Product Bundle Item", 'tolerance', 50)
	...
	def test_update_child_qty_rate(self):
		...
		self.assertRaises(dataent.ValidationError, update_child_qty_rate,'Sales Order', trans_item, so.name)

	def test_warehouse_user(self):
		dataent.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", "test@example.com")
		dataent.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", "test2@example.com")
		dataent.permissions.add_user_permission("Company", "_Test Company 1", "test2@example.com")
		...
		test_user_2 = dataent.get_doc("User", "test2@example.com")
		...
		self.assertRaises(dataent.PermissionError, so.insert)

		dataent.set_user("test2@example.com")
		so.insert()

		dataent.set_user("Administrator")
		dataent.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", "test@example.com")
		dataent.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", "test2@example.com")
		dataent.permissions.remove_user_permission("Company", "_Test Company 1", "test2@example.com")
	...
	def test_auto_insert_price(self):
		...
		dataent.db.set_value("Stock Settings", None, "auto_insert_price_list_rate_if_missing", 1)

		item_price = dataent.db.get_value("Item Price", {"price_list": "_Test Price List",
		...
		self.assertEqual(dataent.db.get_value("Item Price",
	...
	def test_drop_shipping(self):
		...
		if dataent.db.get_value("Item", "_Test Regular Item", "is_stock_item")==1:
			make_stock_entry(item="_Test Regular Item", target="_Test Warehouse - _TC", qty=10, rate=100)

		#setuo existing qty from bin
		bin = dataent.get_all("Bin", filters={"item_code": po_item.item_code, "warehouse": "_Test Warehouse - _TC"},
			fields=["ordered_qty", "reserved_qty"])

		existing_ordered_qty = bin[0].ordered_qty if bin else 0.0
		existing_reserved_qty = bin[0].reserved_qty if bin else 0.0

		bin = dataent.get_all("Bin", filters={"item_code": dn_item.item_code,
		...
		reserved_qty = dataent.db.get_value("Bin",
		...
		self.assertEqual(flt(dataent.db.get_value("Sales Order", so.name, "per_delivered"), 2), 75.00)
	...
	def test_reserved_qty_for_closing_so(self):
		bin = dataent.get_all("Bin", filters={"item_code": "_Test Item", "warehouse": "_Test Warehouse - _TC"},
			fields=["reserved_qty"])
	...
	def test_make_work_order(self):
		...
		for item in so_item_name:
			wo_qty = dataent.db.sql("select sum(qty) from `tabWork Order` where sales_order=%s and sales_order_item=%s", (so.name, item))
			self.assertEquals(wo_qty[0][0], so_item_name.get(item))

	def test_serial_no_based_delivery(self):
		dataent.set_value("Stock Settings", None, "automatically_set_serial_nos_based_on_fifo", 1)
		from epaas.stock.doctype.item.test_item import make_item
		...
		dataent.db.sql("""delete from `tabSerial No` where item_code=%s""", (item.item_code))
		...
		item_serial_no = dataent.get_doc("Serial No", {"item_code": item.item_code})
		from epaas.manufacturing.doctype.work_order.work_order import \
			make_stock_entry as make_production_stock_entry
		se = dataent.get_doc(make_production_stock_entry(work_order.name, "Manufacture", 1))
		se.submit()
		reserved_serial_no = se.get("items")[2].serial_no
		serial_no_so = dataent.get_value("Serial No", reserved_serial_no, "sales_order")
	...
	def test_request_for_raw_materials(self):
		...
		mr = dataent.db.sql("""select name from `tabMaterial Request` ORDER BY creation DESC LIMIT 1""", as_dict=1)[0]
		mr_doc = dataent.get_doc('Material Request',mr.get('name'))
		self.assertEqual(mr_doc.items[0].sales_order, so.name)

def make_sales_order(**args):
	so = dataent.new_doc("Sales Order")
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(dataent.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse},
		"reserved_qty"))
