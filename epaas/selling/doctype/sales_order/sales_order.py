...
from dataent.utils import cstr, flt, getdate, comma_and, cint, nowdate, add_days
from dataent import _
from six import string_types
from dataent.model.utils import get_fetch_values
from dataent.model.mapper import get_mapped_doc
from epaas.stock.stock_balance import update_bin_qty, get_reserved_qty
from dataent.desk.notifications import clear_doctype_notifications
from dataent.contacts.doctype.address.address import get_company_address
...
class WarehouseRequired(dataent.ValidationError): pass
...
class SalesOrder(SellingController):
		...
		if self.po_date:
			for d in self.get("items"):
				if d.delivery_date and getdate(self.po_date) > getdate(d.delivery_date):
					dataent.throw(_("Row #{0}: Expected Delivery Date cannot be before Purchase Order Date")
						.format(d.idx))

		if self.po_no and self.customer:
			so = dataent.db.sql("select name from `tabSales Order` \
				where ifnull(po_no, '') = %s and name != %s and docstatus < 2\
				and customer = %s", (self.po_no, self.name, self.customer))
			if so and so[0][0] and not cint(dataent.db.get_single_value("Selling Settings",
				"allow_against_multiple_purchase_orders")):
				dataent.msgprint(_("Warning: Sales Order {0} already exists against Customer's Purchase Order {1}").format(so[0][0], self.po_no))
	...
	def validate_for_items(self):
		...
		for d in self.get('items'):
			...
			tot_avail_qty = dataent.db.sql("select projected_qty from `tabBin` \
		...
		if len(unique_chk_list) != len(check_list) and \
			not cint(dataent.db.get_single_value("Selling Settings", "allow_multiple_items")):
			dataent.msgprint(_("Same item has been entered multiple times"),
				title=_("Warning"), indicator='orange')

	def product_bundle_has_stock_item(self, product_bundle):
		"""Returns true if product bundle has stock item"""
		ret = len(dataent.db.sql("""select i.name from tabItem i, `tabProduct Bundle Item` pbi
	...
	def validate_sales_mntc_quotation(self):
		for d in self.get('items'):
			if d.prevdoc_docname:
				res = dataent.db.sql("select name from `tabQuotation` where name=%s and order_type = %s",
					(d.prevdoc_docname, self.order_type))
				if not res:
					dataent.msgprint(_("Quotation {0} not of type {1}")
			...
			if self.delivery_date:
				for d in self.get("items"):
					...
					if getdate(self.transaction_date) > getdate(d.delivery_date):
						dataent.msgprint(_("Expected Delivery Date should be after Sales Order Date"),
							indicator='orange', title=_('Warning'))
				if getdate(self.delivery_date) != getdate(max_delivery_date):
					self.delivery_date = max_delivery_date
			else:
				dataent.throw(_("Please enter Delivery Date"))

		self.validate_sales_mntc_quotation()

	def validate_proj_cust(self):
		if self.project and self.customer_name:
			res = dataent.db.sql("""select name from `tabProject` where name = %s
				and (customer = %s or ifnull(customer,'')='')""",
					(self.project, self.customer))
			if not res:
				dataent.throw(_("Customer {0} does not belong to project {1}").format(self.customer, self.project))

	def validate_warehouse(self):
		super(SalesOrder, self).validate_warehouse()

		for d in self.get("items"):
			if (dataent.get_cached_value("Item", d.item_code, "is_stock_item") == 1 or
				(self.has_product_bundle(d.item_code) and self.product_bundle_has_stock_item(d.item_code))) \
				and not d.warehouse and not cint(d.delivered_by_supplier):
				dataent.throw(_("Delivery warehouse required for stock item {0}").format(d.item_code),
					WarehouseRequired)
	...
	def update_enquiry_status(self, prevdoc, flag):
		enq = dataent.db.sql("select t2.prevdoc_docname from `tabQuotation` t1, `tabQuotation Item` t2 where t2.parent = t1.name and t1.name=%s", prevdoc)
		if enq:
			dataent.db.sql("update `tabOpportunity` set status = %s where name=%s",(flag,enq[0][0]))

	def update_prevdoc_status(self, flag):
		for quotation in list(set([d.prevdoc_docname for d in self.get("items")])):
			if quotation:
				doc = dataent.get_doc("Quotation", quotation)
				if doc.docstatus==2:
					dataent.throw(_("Quotation {0} is cancelled").format(quotation))
	...
	def validate_drop_ship(self):
		for d in self.get('items'):
			if d.delivered_by_supplier and not d.supplier:
				dataent.throw(_("Row #{0}: Set Supplier for item {1}").format(d.idx, d.item_code))

	def on_submit(self):
		self.check_credit_limit()
		self.update_reserved_qty()

		dataent.get_doc('Authorization Control').validate_approving_authority(self.doctype, self.company, self.base_grand_total, self)
		self.update_project()
	...
	def on_cancel(self):
		...
		if self.status == 'Closed':
			dataent.throw(_("Closed order cannot be cancelled. Unclose to cancel."))
	...
	def check_credit_limit(self):
		...
		if not cint(dataent.get_cached_value("Customer", self.customer, "bypass_credit_limit_check_at_sales_order")):
	...
	def check_nextdoc_docstatus(self):
		...
		if submit_dn:
			dataent.throw(_("Delivery Notes {0} must be cancelled before cancelling this Sales Order")
				.format(comma_and(submit_dn)))

		# Checks Sales Invoice
		submit_rv = dataent.db.sql_list("""select t1.name
			from `tabSales Invoice` t1,`tabSales Invoice Item` t2
			where t1.name = t2.parent and t2.sales_order = %s and t1.docstatus = 1""",
			self.name)

		if submit_rv:
			dataent.throw(_("Sales Invoice {0} must be cancelled before cancelling this Sales Order")
		...
		if submit_ms:
			dataent.throw(_("Maintenance Schedule {0} must be cancelled before cancelling this Sales Order")
		...
		if submit_mv:
			dataent.throw(_("Maintenance Visit {0} must be cancelled before cancelling this Sales Order")
		...
		if pro_order:
			dataent.throw(_("Work Order {0} must be cancelled before cancelling this Sales Order")
				.format(comma_and(pro_order)))

	def check_modified_date(self):
		mod_db = dataent.db.get_value("Sales Order", self.name, "modified")
		date_diff = dataent.db.sql("select TIMEDIFF('%s', '%s')" %
			( mod_db, cstr(self.modified)))
		if date_diff and date_diff[0][0]:
			dataent.throw(_("{0} {1} has been modified. Please refresh.").format(self.doctype, self.name))
	...
	def update_reserved_qty(self, so_item_rows=None):
		...
		def _valid_for_reserve(item_code, warehouse):
			if item_code and warehouse and [item_code, warehouse] not in item_wh_list \
				and dataent.get_cached_value("Item", item_code, "is_stock_item"):
	...
	def validate_supplier_after_submit(self):
		...
		for item in self.items:
			if item.supplier:
				supplier = dataent.db.get_value("Sales Order Item", {"parent": self.name, "item_code": item.item_code},
					"supplier")
	...
	def update_delivery_status(self):
		...
		for item in self.items:
			if item.delivered_by_supplier:
				item_delivered_qty  = dataent.db.sql("""select sum(qty)
		...
		for table in [self.items, self.packed_items]:
			for i in table:
				...
				if not for_raw_material_request:
					total_work_order_qty = flt(dataent.db.sql('''select sum(qty) from `tabWork Order`
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		...
		def _get_delivery_date(ref_doc_delivery_date, red_doc_transaction_date, transaction_date):
			...
			if delivery_date <= transaction_date:
				delivery_date_diff = dataent.utils.date_diff(ref_doc_delivery_date, red_doc_transaction_date)
				delivery_date = dataent.utils.add_days(transaction_date, delivery_date_diff)
		...
		for d in self.get("items"):
			reference_delivery_date = dataent.db.get_value("Sales Order Item",
		...
		for item in self.items:
			if item.ensure_delivery_based_on_produced_serial_no:
				if item.item_code in normal_items:
					dataent.throw(_("Cannot ensure delivery by Serial No as \
					Item {0} is added with and without Ensure Delivery by \
					Serial No.").format(item.item_code))
				if item.item_code not in reserved_items:
					if not dataent.get_cached_value("Item", item.item_code, "has_serial_no"):
						dataent.throw(_("Item {0} has no Serial No. Only serilialized items \
						can have delivery based on Serial No").format(item.item_code))
					if not dataent.db.exists("BOM", {"item": item.item_code, "is_active": 1}):
						dataent.throw(_("No active BOM found for item {0}. Delivery by \
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc("Sales Order", source_name, {
		...
		"Sales Order Item": {
			...
			"condition": lambda doc: not dataent.db.exists('Product Bundle', doc.item_code),
			"postprocess": update_item
		}
...
def make_delivery_note(source_name, target_doc=None):
	...
	def update_item(source, target, source_parent):
		...
		if item:
			target.cost_center = dataent.db.get_value("Project", source_parent.project, "cost_center") \
...
def make_maintenance_schedule(source_name, target_doc=None):
	maint_schedule = dataent.db.sql("""select t1.name
		from `tabMaintenance Schedule` t1, `tabMaintenance Schedule Item` t2
...
def make_maintenance_visit(source_name, target_doc=None):
	visit = dataent.db.sql("""select t1.name
		from `tabMaintenance Visit` t1, `tabMaintenance Visit Purpose` t2
...
def make_purchase_order_for_drop_shipment(source_name, for_supplier=None, target_doc=None):
	def set_missing_values(source, target):
		...
		default_price_list = dataent.get_value("Supplier", supplier, "default_price_list")
	...
	if for_supplier:
		...
		sales_order = dataent.get_doc("Sales Order", source_name)
		for item in sales_order.items:
			if item.supplier and item.supplier not in suppliers:
				suppliers.append(item.supplier)
	for supplier in suppliers:
		po =dataent.get_list("Purchase Order", filters={"sales_order":source_name, "supplier":supplier, "docstatus": ("<", "2")})
		if len(po) == 0:
			doc = get_mapped_doc("Sales Order", source_name, {
				"Sales Order": {
	...
	if suppliers:
		...
		dataent.msgprint(_("PO already created for all sales order items"))
...
def get_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	return dataent.db.sql("""select {field} from `tabSupplier`
		where docstatus < 2
			and ({key} like %(txt)s
...
def make_work_orders(items, sales_order, company, project=None):
	...
	for i in items:
		if not i.get("bom"):
			dataent.throw(_("Please select BOM against item {0}").format(i.get("item_code")))
		if not i.get("pending_qty"):
			dataent.throw(_("Please select Qty against item {0}").format(i.get("item_code")))

		work_order = dataent.get_doc(dict(
			doctype='Work Order',
...
def get_default_bom_item(item_code):
	bom = dataent.get_all('BOM', dict(item=item_code, is_active=True),
...
def make_raw_material_request(items, company, sales_order, project=None):
	...
	if not raw_materials:
		dataent.msgprint(_("Material Request not created, as quantity for Raw Materials already available."))
		return
