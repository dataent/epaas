...
dataent.ui.form.on("Customer", {
	setup: function(frm) {
		...
		frm.set_query('customer_primary_contact', function(doc) {
			return {
				query: "epaas.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					'customer': doc.name
				}
			}
		...
		frm.set_query('customer_primary_address', function(doc) {
			return {
				query: "epaas.selling.doctype.customer.customer.get_customer_primary_address",
				filters: {
					'customer': doc.name
				}
			}
	...
	refresh: function(frm) {
		if(dataent.defaults.get_default("cust_master_name")!="Naming Series") {
			frm.toggle_display("naming_series", false);
		} else {
			epaas.toggle_naming_series();
		}

		dataent.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Customer'}
		...
		if(!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Accounts Receivable'), function() {
				dataent.set_route('query-report', 'Accounts Receivable', {customer:frm.doc.name});
			});
	...
	validate: function(frm) {
		if(frm.doc.lead_name) dataent.model.clear_doc("Lead", frm.doc.lead_name);
