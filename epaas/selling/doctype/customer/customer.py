...
from dataent.utils import flt, cint, cstr, today
...
from dataent.model.rename_doc import update_linked_doctypes
...
class Customer(TransactionBase):
	...
	def autoname(self):
		cust_master_name = dataent.defaults.get_global_default('cust_master_name')
	...
	def get_customer_name(self):
		if dataent.db.get_value("Customer", self.customer_name):
			count = dataent.db.sql("""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabCustomer
	...
	def validate(self):
		...
		if dataent.db.exists('Customer', self.name):
			customer = dataent.get_doc('Customer', self.name)
			if self.loyalty_program == customer.loyalty_program and not self.loyalty_program_tier:
				self.loyalty_program_tier = customer.loyalty_program_tier

		if self.sales_team:
			if sum([member.allocated_percentage or 0 for member in self.sales_team]) != 100:
				dataent.throw(_("Total contribution percentage should be equal to 100"))

	def check_customer_group_change(self):
		dataent.flags.customer_group_changed = False

		if not self.get('__islocal'):
			if self.customer_group != dataent.db.get_value('Customer', self.name, 'customer_group'):
				dataent.flags.customer_group_changed = True
	...
	def update_customer_groups(self):
		...
		if dataent.flags.customer_group_changed:
			update_linked_doctypes('Customer', dataent.db.escape(self.name), 'Customer Group',
	...
	def update_lead_status(self):
		...
		if self.lead_name:
			dataent.db.set_value('Lead', self.lead_name, 'status', 'Converted', update_modified=False)

	def create_lead_address_contact(self):
		if self.lead_name:
			# assign lead address to customer (if already not set)
			address_names = dataent.get_all('Dynamic Link', filters={
								"parenttype":"Address",
								"link_doctype":"Lead",
			...
			lead = dataent.db.get_value("Lead", self.lead_name, ["organization_lead", "lead_name", "email_id", "phone", "mobile_no", "gender", "salutation"], as_dict=True)

			if not lead.lead_name:
				dataent.throw(_("Please mention the Lead Name in Lead {0}").format(self.lead_name))

			if lead.organization_lead:
				contact_names = dataent.get_all('Dynamic Link', filters={
									"parenttype":"Contact",
									"link_doctype":"Lead",
									"link_name":self.lead_name
								}, fields=["parent as name"])
				...
				for contact_name in contact_names:
					contact = dataent.get_doc('Contact', contact_name.get('name'))
	...
	def validate_name_with_customer_group(self):
		if dataent.db.exists("Customer Group", self.name):
			dataent.throw(_("A Customer Group exists with same name please change the Customer name or rename the Customer Group"), dataent.NameError)

	def validate_credit_limit_on_change(self):
		if self.get("__islocal") or not self.credit_limit \
			or self.credit_limit == dataent.db.get_value("Customer", self.name, "credit_limit"):
			return
		...
		for company in dataent.get_all("Company"):
			...
			if flt(self.credit_limit) < outstanding_amt:
				dataent.throw(_("""New credit limit is less than current outstanding amount for the customer. Credit limit has to be atleast {0}""").format(outstanding_amt))

	def on_trash(self):
		if self.customer_primary_contact:
			dataent.db.sql("""update `tabCustomer`
				set customer_primary_contact=null, mobile_no=null, email_id=null
				where name=%s""", self.name)

		delete_contact_and_address('Customer', self.name)
		if self.lead_name:
			dataent.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	def after_rename(self, olddn, newdn, merge=False):
		if dataent.defaults.get_global_default('cust_master_name') == 'Customer Name':
			dataent.db.set(self, "customer_name", newdn)
	...
	def set_loyalty_program(self):
		...
		if len(loyalty_program) == 1:
			...
			dataent.msgprint(_("Multiple Loyalty Program found for the Customer. Please select manually."))
...
def get_loyalty_programs(doc):
	...
	lp_details = []
	loyalty_programs = dataent.get_all("Loyalty Program",
...
def get_customer_list(doctype, txt, searchfield, start, page_len, filters=None):
	...
	return dataent.db.sql("""select %s from `tabCustomer` where docstatus < 2
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller = dataent.db.get_value('Accounts Settings', None, 'credit_controller')
		if not credit_controller or credit_controller not in dataent.get_roles():
			throw(_("Please contact to the user who have Sales Master Manager {0} role")
				.format(" / " + credit_controller if credit_controller else ""))

def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False):
	# Outstanding based on GL Entries
	outstanding_based_on_gle = dataent.db.sql("""
	...
	if not ignore_outstanding_sales_order:
		outstanding_based_on_so = dataent.db.sql("""
	...
	unmarked_delivery_note_items = dataent.db.sql("""select
	...
	for dn_item in unmarked_delivery_note_items:
		si_amount = dataent.db.sql("""select sum(amount)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit, customer_group = dataent.get_cached_value("Customer",
			customer, ["credit_limit", "customer_group"])

		if not credit_limit:
			credit_limit = dataent.get_cached_value("Customer Group", customer_group, "credit_limit")

	if not credit_limit:
		credit_limit = dataent.get_cached_value('Company',  company,  "credit_limit")
...
def get_customer_primary_address(doctype, txt, searchfield, start, page_len, filters):
	customer = dataent.db.escape(filters.get('customer'))
	return dataent.db.sql("""
