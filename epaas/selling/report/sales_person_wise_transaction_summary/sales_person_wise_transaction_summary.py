	...
	columns = [
		{
			...
			"fieldname": dataent.scrub(filters['doc_type']),
			"fieldtype": "Link",
			"width": 140
		},
		{
...
def get_conditions(filters, date_field):
	...
	if filters.get("sales_person"):
		lft, rgt = dataent.get_value("Sales Person", filters.get("sales_person"), ["lft", "rgt"])
...
def get_items(filters):
	...
	items = []
	if key:
		items = dataent.db.sql_list("""select name from tabItem where %s = %s""" %
			(key, '%s'), (filters[key]))
...
def get_item_details():
	item_details = {}
	for d in dataent.db.sql("""SELECT `name`, `item_group`, `brand` FROM `tabItem`""", as_dict=1):
