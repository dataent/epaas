...
from dataent.utils import getdate, flt, add_to_date, add_days
...
class Analytics(object):
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		self.entries = dataent.get_all(self.filters.doc_type,
	...
	def get_rows_by_group(self):
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				row[scrub(period)] = amount
				if d.parent:
					self.entity_periodic_data.setdefault(d.parent, dataent._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = dataent._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, dataent._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		self.depth_map = dataent._dict()

		self.group_entries = dataent.db.sql("""select name, lft, rgt , {parent} as parent
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = dataent._dict(dataent.db.sql(""" select name, supplier_group from `tabSupplier`"""))
