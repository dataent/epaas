...
def get_item_groups():
	item_groups = dataent._dict()
	for d in dataent.get_all("Item Group", fields=["name", "lft", "rgt"]):
		item_groups.setdefault(d.name, dataent._dict({
			"lft": d.lft,
			"rgt": d.rgt
		}))
	return item_groups
...
def get_sales_persons():
	sales_persons = dataent._dict()
	for d in dataent.get_all("Sales Person", fields=["name", "lft", "rgt"]):
		sales_persons.setdefault(d.name, dataent._dict({
			"lft": d.lft,
			"rgt": d.rgt
		}))
