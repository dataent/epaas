...
dataent.pages['point-of-sale'].on_page_load = function(wrapper) {
	...
	dataent.db.get_value('POS Settings', {name: 'POS Settings'}, 'is_online', (r) => {
		if (r && !cint(r.use_pos_in_offline_mode)) {
			...
			// offline
			dataent.flags.is_offline = true;
			dataent.set_route('pos');
		}
	});
};

dataent.pages['point-of-sale'].refresh = function(wrapper) {
	if (wrapper.pos) {
		wrapper.pos.make_new_invoice();
	}

	if (dataent.flags.is_offline) {
		dataent.set_route('pos');
	}
}

epaas.pos.PointOfSale = class PointOfSale {
	constructor(wrapper) {
		this.wrapper = $(wrapper).find('.layout-main-section');
		this.page = wrapper.page;

		const assets = [
			'assets/epaas/js/pos/clusterize.js',
			'assets/epaas/css/pos.css'
		];
	...
	get_pos_profile() {
		return dataent.xcall("epaas.stock.get_item_details.get_pos_profile",
	...
	raise_exception_for_pos_profile() {
		...
		dataent.throw(__("POS Profile is required to use Point-of-Sale"));
	}
	...
	make_cart() {
		...
		dataent.ui.form.on('Sales Invoice', 'selling_price_list', (frm) => {
		...
		this.items = new POSItems({
			...
			events: {
				update_cart: (item, field, value) => {
					if(!this.frm.doc.customer) {
						dataent.throw(__('Please select a customer'));
					}
	...
	update_item_in_cart(item_code, field='qty', value=1, batch_no) {
		...
		dataent.flags.hide_serial_batch_dialog = true;
	...
	select_batch_and_serial_no(row) {
		...
		epaas.show_serial_batch_selector(this.frm, row, () => {
				...
				this.update_item_in_frm(item, 'qty', item.qty)
					.then(() => {
						...
						dataent.run_serially([
							() => {
								if (item.qty === 0) {
									dataent.model.clear_doc(item.doctype, item.name);
								}
							},
							() => this.update_cart_data(item)
						]);
	...
	update_item_in_frm(item, field, value) {
		if (field == 'qty' && value < 0) {
			dataent.msgprint(__("Quantity must be positive"));
			value = item.qty;
		} else {
			if (in_list(["qty", "serial_no", "batch"], field)) {
				...
				return dataent.model.set_value(item.doctype, item.name, field, value);
			}
		}
	...
	set_primary_action_in_modal() {
		if (!this.frm.msgbox) {
			this.frm.msgbox = dataent.msgprint(
	...
	change_pos_profile() {
		return new Promise((resolve) => {
			const on_submit = ({ pos_profile, set_as_default }) => {
				...
				if (set_as_default) {
					dataent.call({
						method: "epaas.accounts.doctype.pos_profile.pos_profile.set_default_profile",
						args: {
			...
			dataent.prompt(this.get_prompt_fields(),
				on_submit,
	...
	setup_company() {
		return new Promise(resolve => {
			if(!this.frm.doc.company) {
				dataent.prompt({fieldname:"company", options: "Company", fieldtype:"Link",
	...
	make_sales_invoice_frm() {
		...
		function get_frm(_frm) {
			...
			const name = dataent.model.make_new_doc_and_get_name(doctype, true);
			frm.refresh(name);
			frm.doc.items = [];
			frm.doc.is_pos = 1;
	...
	prepare_menu() {
		...
		this.page.add_menu_item(__("Form View"), function () {
			dataent.model.sync(me.frm.doc);
			dataent.set_route("Form", me.frm.doc.doctype, me.frm.doc.name);
		});
		...
		this.page.add_menu_item(__('POS Settings'), function() {
			dataent.set_route('Form', 'POS Settings');
		});
		...
		this.page.add_menu_item(__('Close the POS'), function() {
			var voucher = dataent.model.get_new_doc('POS Closing Voucher');
			voucher.pos_profile = me.frm.doc.pos_profile;
			voucher.user = dataent.session.user;
			...
			dataent.set_route('Form', 'POS Closing Voucher', voucher.name);
		});
	}
...
class POSCart {
	...
	get_grand_total() {
		...
		if (!cint(dataent.sys_defaults.disable_rounded_total)) {
	...
	make_customer_field() {
		this.customer_field = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Link',
				label: 'Customer',
				fieldname: 'customer',
	...
	make_loyalty_points() {
		this.available_loyalty_points = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Int',
	...
	bind_events() {
		...
		this.wrapper.find('.additional_discount_percentage').on('change', (e) => {
			...
			dataent.model.set_value(this.frm.doctype, this.frm.docname,
...
class POSItems {
	constructor({wrapper, frm, events}) {
		...
		dataent.db.get_value("Item Group", {lft: 1, is_group: 1}, "name", (r) => {
	...
	make_fields() {
		// Search field
		const me = this;
		this.search_field = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Data',
		...
		this.item_group_field = dataent.ui.form.make_control({
			df: {
				fieldtype: 'Link',
				label: 'Item Group',
				options: 'Item Group',
				...
				get_query: () => {
					return {
						query: 'epaas.selling.page.point_of_sale.point_of_sale.item_group_query',
						filters: {
	...
	get_items({start = 0, page_length = 40, search_value='', item_group=this.parent_item_group}={}) {
		...
		return new Promise(res => {
			dataent.call({
				method: "epaas.selling.page.point_of_sale.point_of_sale.get_items",
				freeze: true,
				args: {
					start,
					page_length,
...
class Payment {
			...
			},
			{
				...
				onchange: () => {
					me.update_cur_frm_value("redeem_loyalty_points", () => {
						dataent.flags.redeem_loyalty_points = false;
			...
			},
			{
				...
				onchange: () => {
					me.update_cur_frm_value("loyalty_points", () => {
						dataent.flags.loyalty_points = false;
						me.update_loyalty_points();
					});
				}
			},
			...
			},
			{
				...
				onchange: () => {
					me.update_cur_frm_value('write_off_amount', () => {
						dataent.flags.change_amount = false;
			...
			},
			{
				...
				onchange: () => {
					me.update_cur_frm_value('change_amount', () => {
						dataent.flags.write_off_amount = false;
	...
	set_flag() {
		dataent.flags.write_off_amount = true;
		dataent.flags.change_amount = true;
		dataent.flags.loyalty_points = true;
		dataent.flags.redeem_loyalty_points = true;
		dataent.flags.payment_method = true;
	}

	update_cur_frm_value(fieldname, callback) {
		if (dataent.flags[fieldname]) {
		...
		var me = this;
			$.each(this.frm.doc.payments, function(i, data) {
				if (__(data.mode_of_payment) == __(fieldname)) {
					dataent.model.set_value('Sales Invoice Payment', data.name, 'amount', value)
						.then(() => {
							me.update_change_amount();
							me.update_write_off_amount();
						});
