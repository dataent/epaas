...
from epaas.accounts.doctype.pos_profile.pos_profile import get_item_groups
...
def search_serial_or_batch_or_barcode_number(search_value):
	# search barcode no
	barcode_data = dataent.db.get_value('Item Barcode', {'barcode': search_value}, ['barcode', 'parent as item_code'], as_dict=True)
	if barcode_data:
		return barcode_data

	# search serial no
	serial_no_data = dataent.db.get_value('Serial No', search_value, ['name as serial_no', 'item_code'], as_dict=True)
	if serial_no_data:
		return serial_no_data

	# search batch no
	batch_no_data = dataent.db.get_value('Batch', search_value, ['name as batch_no', 'item as item_code'], as_dict=True)
	if batch_no_data:
...
def get_conditions(item_code, serial_no, batch_no, barcode):
	if serial_no or batch_no or barcode:
		return dataent.db.escape(item_code), "i.name = %(item_code)s"

	condition = """(i.name like %(item_code)s
			or i.item_name like %(item_code)s)"""

	return '%%%s%%'%(dataent.db.escape(item_code)), condition
...
def item_group_query(doctype, txt, searchfield, start, page_len, filters):
	...
	return dataent.db.sql(""" select distinct name from `tabItem Group`
