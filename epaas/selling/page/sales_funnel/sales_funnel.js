...
dataent.pages['sales-funnel'].on_page_load = function(wrapper) {
	...
	wrapper.sales_funnel = new epaas.SalesFunnel(wrapper);

	dataent.breadcrumbs.add("Selling");
}

epaas.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		this.company_field = wrapper.page.add_field({"fieldtype": "Link", "fieldname": "company", "options": "Company",
			"label": __("Company"), "reqd": 1, "default": dataent.defaults.get_user_default('company'),
			change: function() {
				me.company = this.value || dataent.defaults.get_user_default('company');
				me.get_data();
			}
		}),
		...
		this.company = dataent.defaults.get_user_default('company');
		this.options = {
			from_date: dataent.datetime.add_months(dataent.datetime.get_today(), -1),
			to_date: dataent.datetime.get_today(),
			chart: 'sales_funnel'
		};
		...
		$.each(this.options, function(k, v) {
			if (['from_date', 'to_date'].includes(k)) {
				me.elements[k].val(dataent.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function() {
				if (['from_date', 'to_date'].includes(k)) {
					me.options[k] = dataent.datetime.user_to_str($(this).val()) != 'Invalid date' ? dataent.datetime.user_to_str($(this).val()) : dataent.datetime.get_today();
				} else {
	...
	get_data(btn) {
		...
		if (me.options.chart == 'sales_funnel'){
			dataent.call({
				method: "epaas.selling.page.sales_funnel.sales_funnel.get_funnel_data",
				args: {
			...
			dataent.call({
				method: "epaas.selling.page.sales_funnel.sales_funnel.get_opp_by_lead_source",
				args: {
					from_date: this.options.from_date,
					to_date: this.options.to_date,
					company: this.company
			...
			dataent.call({
				method: "epaas.selling.page.sales_funnel.sales_funnel.get_pipeline_data",
				args: {
					from_date: this.options.from_date,
					to_date: this.options.to_date,
					company: this.company
	...
	render_opp_by_lead_source() {
		let me = this;
		let currency = dataent.defaults.get_default("currency");
