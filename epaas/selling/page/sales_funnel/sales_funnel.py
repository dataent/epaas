...
from epaas.accounts.report.utils import convert
import pandas as pd

@dataent.whitelist()
def get_funnel_data(from_date, to_date, company):
	active_leads = dataent.db.sql("""select count(*) from `tabLead`
		where (date(`modified`) between %s and %s)
		and status != "Do Not Contact" and company=%s""", (from_date, to_date, company))[0][0]

	active_leads += dataent.db.sql("""select count(distinct contact.name) from `tabContact` contact
	...
	opportunities = dataent.db.sql("""select count(*) from `tabOpportunity`
		where (date(`creation`) between %s and %s)
		and status != "Lost" and company=%s""", (from_date, to_date, company))[0][0]

	quotations = dataent.db.sql("""select count(*) from `tabQuotation`
		where docstatus = 1 and (date(`creation`) between %s and %s)
		and status != "Lost" and company=%s""", (from_date, to_date, company))[0][0]

	sales_orders = dataent.db.sql("""select count(*) from `tabSales Order`
...
def get_opp_by_lead_source(from_date, to_date, company):
	opportunities = dataent.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], ['transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability', 'source'])

	if opportunities:
		default_currency = dataent.get_cached_value('Global Defaults', 'None',  'default_currency')
...
def get_pipeline_data(from_date, to_date, company):
	opportunities = dataent.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], ['transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability'])
