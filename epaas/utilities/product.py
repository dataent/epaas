...
import dataent
from dataent.utils import cint, fmt_money, flt, nowdate, getdate
from epaas.accounts.doctype.pricing_rule.pricing_rule import get_pricing_rule_for_item
...
def get_qty_in_stock(item_code, item_warehouse_field, warehouse=None):
	...
	template_item_code, is_stock_item = dataent.db.get_value("Item", item_code, ["variant_of", "is_stock_item"])

	if not warehouse:
		warehouse = dataent.db.get_value("Item", item_code, item_warehouse_field)

	if not warehouse and template_item_code and template_item_code != item_code:
		warehouse = dataent.db.get_value("Item", template_item_code, item_warehouse_field)

	if warehouse:
		stock_qty = dataent.db.sql("""
	...
	return dataent._dict({"in_stock": in_stock, "stock_qty": stock_qty, "is_stock_item": is_stock_item})


def adjust_qty_for_expired_items(item_code, stock_qty, warehouse):
	batches = dataent.get_all('Batch', filters=[{'item': item_code}], fields=['expiry_date', 'name'])
...
def get_price(item_code, price_list, customer_group, company, qty=1):
	template_item_code = dataent.db.get_value("Item", item_code, "variant_of")

	if price_list:
		price = dataent.get_all("Item Price", fields=["price_list_rate", "currency"],
		...
		if price:
			pricing_rule = get_pricing_rule_for_item(dataent._dict({
				"item_code": item_code,
				"qty": qty,
				...
				"currency": dataent.db.get_value("Price List", price_list, "currency")
			}))
			...
			if price_obj:
				...
				price_obj["currency_symbol"] = not cint(dataent.db.get_default("hide_currency_symbol")) \
					and (dataent.db.get_value("Currency", price_obj.currency, "symbol", cache=True) or price_obj.currency) \
					or ""

				uom_conversion_factor = dataent.db.sql("""select	C.conversion_factor
