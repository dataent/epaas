dataent.pages["leaderboard"].on_page_load = function (wrapper) {
	dataent.leaderboard = new dataent.Leaderboard(wrapper);
}

dataent.Leaderboard = Class.extend({
	...
	make: function () {
		...
		this.type_select = this.page.add_select(__("Type"),
			me.options.selected_filter.map(d => {
				return {"label": __(dataent.model.unscrub(d)), value: d }
			})
		);
		...
		this.$sidebar_list.on('click', 'li', function(e) {
			...
			me.options.selected_company = dataent.defaults.get_default('company');
	...
	make_request: function ($container) {
		...
		dataent.model.with_doctype(me.options.selected_doctype, function () {
	...
	get_leaderboard: function (notify, $container) {
		...
		if(!me.options.selected_company) {
			dataent.throw(__("Please select Company"));
		}
		dataent.call({
			method: "epaas.utilities.page.leaderboard.leaderboard.get_leaderboard",
			args: {
		...
		const html =
			`<div class="list-headers">
				<div class="list-item list-item--head" data-list-renderer="${"List"}">
					${
					fields.map(filter => {
							const col = dataent.model.unscrub(filter);
							return (
	...
	get_item_html: function (item) {
		...
		const currency = dataent.get_doc(":Company", company).default_currency;
