...
import dataent
import unittest
from epaas.hotels.doctype.hotel_room_reservation.hotel_room_reservation import HotelRoomPricingNotSetError, HotelRoomUnavailableError
test_dependencies = ["Hotel Room Package", "Hotel Room Pricing", "Hotel Room"]

class TestHotelRoomReservation(unittest.TestCase):
	def setUp(self):
		dataent.db.sql("delete from `tabHotel Room Reservation`")
		dataent.db.sql("delete from `tabHotel Room Reservation Item`")
