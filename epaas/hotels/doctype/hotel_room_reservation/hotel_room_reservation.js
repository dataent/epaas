...
dataent.ui.form.on('Hotel Room Reservation', {
	...
	recalculate_rates: function(frm) {
		...
		dataent.call({
			"method": "epaas.hotels.doctype.hotel_room_reservation.hotel_room_reservation.get_room_rate",
	...
	make_invoice: function(frm) {
		dataent.model.with_doc("Hotel Settings", "Hotel Settings", ()=>{
			dataent.model.with_doctype("Sales Invoice", ()=>{
				let hotel_settings = dataent.get_doc("Hotel Settings", "Hotel Settings");
				let invoice = dataent.model.get_new_doc("Sales Invoice");
				...
				for (let d of frm.doc.items){
					let invoice_item = dataent.model.add_child(invoice, "items")
				...
				dataent.set_route("Form", invoice.doctype, invoice.name);
			});
		});
	}
});

dataent.ui.form.on('Hotel Room Reservation Item', {
