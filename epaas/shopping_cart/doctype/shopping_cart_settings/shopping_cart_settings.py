...
from dataent.utils import comma_and
from dataent.model.document import Document
from dataent.utils import get_datetime, get_datetime_str, now_datetime

class ShoppingCartSetupError(dataent.ValidationError): pass

class ShoppingCartSettings(Document):
	def onload(self):
		self.get("__onload").quotation_series = dataent.get_meta("Quotation").get_options("naming_series")
	...
	def validate_exchange_rates_exist(self):
		...
		price_list_currency_map = dataent.db.get_values("Price List",
			[self.price_list],
			"currency")
		...
		for price_list, currency in price_list_currency_map.items():
			if not currency:
				dataent.throw(_("Currency is required for Price List {0}").format(price_list))
		...
		if expected_to_exist:
			...
			exists = dataent.db.sql_list("""select CONCAT(from_currency,'-',to_currency) from `tabCurrency Exchange`
	...
	def validate_tax_rule(self):
		if not dataent.db.get_value("Tax Rule", {"use_for_shopping_cart" : 1}, "name"):
			dataent.throw(dataent._("Set Tax Rule for shopping cart"), ShoppingCartSetupError)
...
def validate_cart_settings(doc, method):
	dataent.get_doc("Shopping Cart Settings", "Shopping Cart Settings").run_method("validate")

def get_shopping_cart_settings():
	if not getattr(dataent.local, "shopping_cart_settings", None):
		dataent.local.shopping_cart_settings = dataent.get_doc("Shopping Cart Settings", "Shopping Cart Settings")

	return dataent.local.shopping_cart_settings
...
def check_shopping_cart_enabled():
	if not get_shopping_cart_settings().enabled:
		dataent.throw(_("You need to enable Shopping Cart"), ShoppingCartSetupError)
