...
import unittest
import dataent
from dataent.utils import nowdate, add_months
from epaas.shopping_cart.cart import _get_cart_quotation, update_cart, get_party
...
class TestShoppingCart(unittest.TestCase):
	...
	def test_get_cart_new_user(self):
		...
		self.assertEqual(quotation.contact_person,
			dataent.db.get_value("Contact", dict(email_id="test_cart_user@example.com")))
		self.assertEqual(quotation.contact_email, dataent.session.user)
	...
	def enable_shopping_cart(self):
		settings = dataent.get_doc("Shopping Cart Settings", "Shopping Cart Settings")
		...
		# insert item price
		if not dataent.db.get_value("Item Price", {"price_list":  "_Test Price List India",
			"item_code": "_Test Item"}):
			dataent.get_doc({
				"doctype": "Item Price",
				"price_list": "_Test Price List India",
		...
		settings.save()
		dataent.local.shopping_cart_settings = None
	...
	def login_as_new_user(self):
		...
		dataent.set_user("test_cart_user@example.com")

	def login_as_customer(self):
		self.create_user_if_not_exists("test_contact_customer@example.com",
			"_Test Contact For _Test Customer")
		dataent.set_user("test_contact_customer@example.com")
	...
	def create_user_if_not_exists(self, email, first_name = None):
		if dataent.db.exists("User", email):
			return
